
kernel:     file format elf32-i386


Disassembly of section .text:

80100000 <multiboot_header>:
80100000:	02 b0 ad 1b 00 00    	add    0x1bad(%eax),%dh
80100006:	00 00                	add    %al,(%eax)
80100008:	fe 4f 52             	decb   0x52(%edi)
8010000b:	e4                   	.byte 0xe4

8010000c <entry>:

# Entering xv6 on boot processor, with paging off.
.globl entry
entry:
  # Turn on page size extension for 4Mbyte pages
  movl    %cr4, %eax
8010000c:	0f 20 e0             	mov    %cr4,%eax
  orl     $(CR4_PSE), %eax
8010000f:	83 c8 10             	or     $0x10,%eax
  movl    %eax, %cr4
80100012:	0f 22 e0             	mov    %eax,%cr4
  # Set page directory
  movl    $(V2P_WO(entrypgdir)), %eax
80100015:	b8 00 a0 10 00       	mov    $0x10a000,%eax
  movl    %eax, %cr3
8010001a:	0f 22 d8             	mov    %eax,%cr3
  # Turn on paging.
  movl    %cr0, %eax
8010001d:	0f 20 c0             	mov    %cr0,%eax
  orl     $(CR0_PG|CR0_WP), %eax
80100020:	0d 00 00 01 80       	or     $0x80010000,%eax
  movl    %eax, %cr0
80100025:	0f 22 c0             	mov    %eax,%cr0

  # Set up the stack pointer.
  movl $(stack + KSTACKSIZE), %esp
80100028:	bc f0 73 11 80       	mov    $0x801173f0,%esp

  # Jump to main(), and switch to executing at
  # high addresses. The indirect call is needed because
  # the assembler produces a PC-relative instruction
  # for a direct jump.
  mov $main, %eax
8010002d:	b8 10 31 10 80       	mov    $0x80103110,%eax
  jmp *%eax
80100032:	ff e0                	jmp    *%eax
80100034:	66 90                	xchg   %ax,%ax
80100036:	66 90                	xchg   %ax,%ax
80100038:	66 90                	xchg   %ax,%ax
8010003a:	66 90                	xchg   %ax,%ax
8010003c:	66 90                	xchg   %ax,%ax
8010003e:	66 90                	xchg   %ax,%ax

80100040 <binit>:
  struct buf head;
} bcache;

void
binit(void)
{
80100040:	55                   	push   %ebp
80100041:	89 e5                	mov    %esp,%ebp
80100043:	53                   	push   %ebx

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
  bcache.head.next = &bcache.head;
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
80100044:	bb 54 b5 10 80       	mov    $0x8010b554,%ebx
{
80100049:	83 ec 0c             	sub    $0xc,%esp
  initlock(&bcache.lock, "bcache");
8010004c:	68 00 7e 10 80       	push   $0x80107e00
80100051:	68 20 b5 10 80       	push   $0x8010b520
80100056:	e8 b5 4f 00 00       	call   80105010 <initlock>
  bcache.head.next = &bcache.head;
8010005b:	83 c4 10             	add    $0x10,%esp
8010005e:	b8 1c fc 10 80       	mov    $0x8010fc1c,%eax
  bcache.head.prev = &bcache.head;
80100063:	c7 05 6c fc 10 80 1c 	movl   $0x8010fc1c,0x8010fc6c
8010006a:	fc 10 80 
  bcache.head.next = &bcache.head;
8010006d:	c7 05 70 fc 10 80 1c 	movl   $0x8010fc1c,0x8010fc70
80100074:	fc 10 80 
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
80100077:	eb 09                	jmp    80100082 <binit+0x42>
80100079:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100080:	89 d3                	mov    %edx,%ebx
    b->next = bcache.head.next;
80100082:	89 43 54             	mov    %eax,0x54(%ebx)
    b->prev = &bcache.head;
    initsleeplock(&b->lock, "buffer");
80100085:	83 ec 08             	sub    $0x8,%esp
80100088:	8d 43 0c             	lea    0xc(%ebx),%eax
    b->prev = &bcache.head;
8010008b:	c7 43 50 1c fc 10 80 	movl   $0x8010fc1c,0x50(%ebx)
    initsleeplock(&b->lock, "buffer");
80100092:	68 07 7e 10 80       	push   $0x80107e07
80100097:	50                   	push   %eax
80100098:	e8 43 4e 00 00       	call   80104ee0 <initsleeplock>
    bcache.head.next->prev = b;
8010009d:	a1 70 fc 10 80       	mov    0x8010fc70,%eax
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
801000a2:	8d 93 5c 02 00 00    	lea    0x25c(%ebx),%edx
801000a8:	83 c4 10             	add    $0x10,%esp
    bcache.head.next->prev = b;
801000ab:	89 58 50             	mov    %ebx,0x50(%eax)
    bcache.head.next = b;
801000ae:	89 d8                	mov    %ebx,%eax
801000b0:	89 1d 70 fc 10 80    	mov    %ebx,0x8010fc70
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
801000b6:	81 fb c0 f9 10 80    	cmp    $0x8010f9c0,%ebx
801000bc:	75 c2                	jne    80100080 <binit+0x40>
  }
}
801000be:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801000c1:	c9                   	leave  
801000c2:	c3                   	ret    
801000c3:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801000ca:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

801000d0 <bread>:
}

// Return a locked buf with the contents of the indicated block.
struct buf*
bread(uint dev, uint blockno)
{
801000d0:	55                   	push   %ebp
801000d1:	89 e5                	mov    %esp,%ebp
801000d3:	57                   	push   %edi
801000d4:	56                   	push   %esi
801000d5:	53                   	push   %ebx
801000d6:	83 ec 18             	sub    $0x18,%esp
801000d9:	8b 75 08             	mov    0x8(%ebp),%esi
801000dc:	8b 7d 0c             	mov    0xc(%ebp),%edi
  acquire(&bcache.lock);
801000df:	68 20 b5 10 80       	push   $0x8010b520
801000e4:	e8 f7 50 00 00       	call   801051e0 <acquire>
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
801000e9:	8b 1d 70 fc 10 80    	mov    0x8010fc70,%ebx
801000ef:	83 c4 10             	add    $0x10,%esp
801000f2:	81 fb 1c fc 10 80    	cmp    $0x8010fc1c,%ebx
801000f8:	75 11                	jne    8010010b <bread+0x3b>
801000fa:	eb 24                	jmp    80100120 <bread+0x50>
801000fc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80100100:	8b 5b 54             	mov    0x54(%ebx),%ebx
80100103:	81 fb 1c fc 10 80    	cmp    $0x8010fc1c,%ebx
80100109:	74 15                	je     80100120 <bread+0x50>
    if(b->dev == dev && b->blockno == blockno){
8010010b:	3b 73 04             	cmp    0x4(%ebx),%esi
8010010e:	75 f0                	jne    80100100 <bread+0x30>
80100110:	3b 7b 08             	cmp    0x8(%ebx),%edi
80100113:	75 eb                	jne    80100100 <bread+0x30>
      b->refcnt++;
80100115:	83 43 4c 01          	addl   $0x1,0x4c(%ebx)
      release(&bcache.lock);
80100119:	eb 3f                	jmp    8010015a <bread+0x8a>
8010011b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010011f:	90                   	nop
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100120:	8b 1d 6c fc 10 80    	mov    0x8010fc6c,%ebx
80100126:	81 fb 1c fc 10 80    	cmp    $0x8010fc1c,%ebx
8010012c:	75 0d                	jne    8010013b <bread+0x6b>
8010012e:	eb 6e                	jmp    8010019e <bread+0xce>
80100130:	8b 5b 50             	mov    0x50(%ebx),%ebx
80100133:	81 fb 1c fc 10 80    	cmp    $0x8010fc1c,%ebx
80100139:	74 63                	je     8010019e <bread+0xce>
    if(b->refcnt == 0 && (b->flags & B_DIRTY) == 0) {
8010013b:	8b 43 4c             	mov    0x4c(%ebx),%eax
8010013e:	85 c0                	test   %eax,%eax
80100140:	75 ee                	jne    80100130 <bread+0x60>
80100142:	f6 03 04             	testb  $0x4,(%ebx)
80100145:	75 e9                	jne    80100130 <bread+0x60>
      b->dev = dev;
80100147:	89 73 04             	mov    %esi,0x4(%ebx)
      b->blockno = blockno;
8010014a:	89 7b 08             	mov    %edi,0x8(%ebx)
      b->flags = 0;
8010014d:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
      b->refcnt = 1;
80100153:	c7 43 4c 01 00 00 00 	movl   $0x1,0x4c(%ebx)
      release(&bcache.lock);
8010015a:	83 ec 0c             	sub    $0xc,%esp
8010015d:	68 20 b5 10 80       	push   $0x8010b520
80100162:	e8 19 50 00 00       	call   80105180 <release>
      acquiresleep(&b->lock);
80100167:	8d 43 0c             	lea    0xc(%ebx),%eax
8010016a:	89 04 24             	mov    %eax,(%esp)
8010016d:	e8 ae 4d 00 00       	call   80104f20 <acquiresleep>
      return b;
80100172:	83 c4 10             	add    $0x10,%esp
  struct buf *b;

  b = bget(dev, blockno);
  if((b->flags & B_VALID) == 0) {
80100175:	f6 03 02             	testb  $0x2,(%ebx)
80100178:	74 0e                	je     80100188 <bread+0xb8>
    iderw(b);
  }
  return b;
}
8010017a:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010017d:	89 d8                	mov    %ebx,%eax
8010017f:	5b                   	pop    %ebx
80100180:	5e                   	pop    %esi
80100181:	5f                   	pop    %edi
80100182:	5d                   	pop    %ebp
80100183:	c3                   	ret    
80100184:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    iderw(b);
80100188:	83 ec 0c             	sub    $0xc,%esp
8010018b:	53                   	push   %ebx
8010018c:	e8 ff 21 00 00       	call   80102390 <iderw>
80100191:	83 c4 10             	add    $0x10,%esp
}
80100194:	8d 65 f4             	lea    -0xc(%ebp),%esp
80100197:	89 d8                	mov    %ebx,%eax
80100199:	5b                   	pop    %ebx
8010019a:	5e                   	pop    %esi
8010019b:	5f                   	pop    %edi
8010019c:	5d                   	pop    %ebp
8010019d:	c3                   	ret    
  panic("bget: no buffers");
8010019e:	83 ec 0c             	sub    $0xc,%esp
801001a1:	68 0e 7e 10 80       	push   $0x80107e0e
801001a6:	e8 d5 01 00 00       	call   80100380 <panic>
801001ab:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801001af:	90                   	nop

801001b0 <bwrite>:

// Write b's contents to disk.  Must be locked.
void
bwrite(struct buf *b)
{
801001b0:	55                   	push   %ebp
801001b1:	89 e5                	mov    %esp,%ebp
801001b3:	53                   	push   %ebx
801001b4:	83 ec 10             	sub    $0x10,%esp
801001b7:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(!holdingsleep(&b->lock))
801001ba:	8d 43 0c             	lea    0xc(%ebx),%eax
801001bd:	50                   	push   %eax
801001be:	e8 fd 4d 00 00       	call   80104fc0 <holdingsleep>
801001c3:	83 c4 10             	add    $0x10,%esp
801001c6:	85 c0                	test   %eax,%eax
801001c8:	74 0f                	je     801001d9 <bwrite+0x29>
    panic("bwrite");
  b->flags |= B_DIRTY;
801001ca:	83 0b 04             	orl    $0x4,(%ebx)
  iderw(b);
801001cd:	89 5d 08             	mov    %ebx,0x8(%ebp)
}
801001d0:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801001d3:	c9                   	leave  
  iderw(b);
801001d4:	e9 b7 21 00 00       	jmp    80102390 <iderw>
    panic("bwrite");
801001d9:	83 ec 0c             	sub    $0xc,%esp
801001dc:	68 1f 7e 10 80       	push   $0x80107e1f
801001e1:	e8 9a 01 00 00       	call   80100380 <panic>
801001e6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801001ed:	8d 76 00             	lea    0x0(%esi),%esi

801001f0 <brelse>:

// Release a locked buffer.
// Move to the head of the MRU list.
void
brelse(struct buf *b)
{
801001f0:	55                   	push   %ebp
801001f1:	89 e5                	mov    %esp,%ebp
801001f3:	56                   	push   %esi
801001f4:	53                   	push   %ebx
801001f5:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(!holdingsleep(&b->lock))
801001f8:	8d 73 0c             	lea    0xc(%ebx),%esi
801001fb:	83 ec 0c             	sub    $0xc,%esp
801001fe:	56                   	push   %esi
801001ff:	e8 bc 4d 00 00       	call   80104fc0 <holdingsleep>
80100204:	83 c4 10             	add    $0x10,%esp
80100207:	85 c0                	test   %eax,%eax
80100209:	74 66                	je     80100271 <brelse+0x81>
    panic("brelse");

  releasesleep(&b->lock);
8010020b:	83 ec 0c             	sub    $0xc,%esp
8010020e:	56                   	push   %esi
8010020f:	e8 6c 4d 00 00       	call   80104f80 <releasesleep>

  acquire(&bcache.lock);
80100214:	c7 04 24 20 b5 10 80 	movl   $0x8010b520,(%esp)
8010021b:	e8 c0 4f 00 00       	call   801051e0 <acquire>
  b->refcnt--;
80100220:	8b 43 4c             	mov    0x4c(%ebx),%eax
  if (b->refcnt == 0) {
80100223:	83 c4 10             	add    $0x10,%esp
  b->refcnt--;
80100226:	83 e8 01             	sub    $0x1,%eax
80100229:	89 43 4c             	mov    %eax,0x4c(%ebx)
  if (b->refcnt == 0) {
8010022c:	85 c0                	test   %eax,%eax
8010022e:	75 2f                	jne    8010025f <brelse+0x6f>
    // no one is waiting for it.
    b->next->prev = b->prev;
80100230:	8b 43 54             	mov    0x54(%ebx),%eax
80100233:	8b 53 50             	mov    0x50(%ebx),%edx
80100236:	89 50 50             	mov    %edx,0x50(%eax)
    b->prev->next = b->next;
80100239:	8b 43 50             	mov    0x50(%ebx),%eax
8010023c:	8b 53 54             	mov    0x54(%ebx),%edx
8010023f:	89 50 54             	mov    %edx,0x54(%eax)
    b->next = bcache.head.next;
80100242:	a1 70 fc 10 80       	mov    0x8010fc70,%eax
    b->prev = &bcache.head;
80100247:	c7 43 50 1c fc 10 80 	movl   $0x8010fc1c,0x50(%ebx)
    b->next = bcache.head.next;
8010024e:	89 43 54             	mov    %eax,0x54(%ebx)
    bcache.head.next->prev = b;
80100251:	a1 70 fc 10 80       	mov    0x8010fc70,%eax
80100256:	89 58 50             	mov    %ebx,0x50(%eax)
    bcache.head.next = b;
80100259:	89 1d 70 fc 10 80    	mov    %ebx,0x8010fc70
  }
  
  release(&bcache.lock);
8010025f:	c7 45 08 20 b5 10 80 	movl   $0x8010b520,0x8(%ebp)
}
80100266:	8d 65 f8             	lea    -0x8(%ebp),%esp
80100269:	5b                   	pop    %ebx
8010026a:	5e                   	pop    %esi
8010026b:	5d                   	pop    %ebp
  release(&bcache.lock);
8010026c:	e9 0f 4f 00 00       	jmp    80105180 <release>
    panic("brelse");
80100271:	83 ec 0c             	sub    $0xc,%esp
80100274:	68 26 7e 10 80       	push   $0x80107e26
80100279:	e8 02 01 00 00       	call   80100380 <panic>
8010027e:	66 90                	xchg   %ax,%ax

80100280 <consoleread>:
  }
}

int
consoleread(struct inode *ip, char *dst, int n)
{
80100280:	55                   	push   %ebp
80100281:	89 e5                	mov    %esp,%ebp
80100283:	57                   	push   %edi
80100284:	56                   	push   %esi
80100285:	53                   	push   %ebx
80100286:	83 ec 18             	sub    $0x18,%esp
80100289:	8b 5d 10             	mov    0x10(%ebp),%ebx
8010028c:	8b 75 0c             	mov    0xc(%ebp),%esi
  uint target;
  int c;

  iunlock(ip);
8010028f:	ff 75 08             	push   0x8(%ebp)
  target = n;
80100292:	89 df                	mov    %ebx,%edi
  iunlock(ip);
80100294:	e8 77 16 00 00       	call   80101910 <iunlock>
  acquire(&cons.lock);
80100299:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801002a0:	e8 3b 4f 00 00       	call   801051e0 <acquire>
  while(n > 0){
801002a5:	83 c4 10             	add    $0x10,%esp
801002a8:	85 db                	test   %ebx,%ebx
801002aa:	0f 8e 94 00 00 00    	jle    80100344 <consoleread+0xc4>
    while(input.r == input.w){
801002b0:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
801002b5:	3b 05 04 ff 10 80    	cmp    0x8010ff04,%eax
801002bb:	74 25                	je     801002e2 <consoleread+0x62>
801002bd:	eb 59                	jmp    80100318 <consoleread+0x98>
801002bf:	90                   	nop
      if(myproc()->killed){
        release(&cons.lock);
        ilock(ip);
        return -1;
      }
      sleep(&input.r, &cons.lock);
801002c0:	83 ec 08             	sub    $0x8,%esp
801002c3:	68 20 ff 10 80       	push   $0x8010ff20
801002c8:	68 00 ff 10 80       	push   $0x8010ff00
801002cd:	e8 4e 41 00 00       	call   80104420 <sleep>
    while(input.r == input.w){
801002d2:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
801002d7:	83 c4 10             	add    $0x10,%esp
801002da:	3b 05 04 ff 10 80    	cmp    0x8010ff04,%eax
801002e0:	75 36                	jne    80100318 <consoleread+0x98>
      if(myproc()->killed){
801002e2:	e8 89 37 00 00       	call   80103a70 <myproc>
801002e7:	8b 48 24             	mov    0x24(%eax),%ecx
801002ea:	85 c9                	test   %ecx,%ecx
801002ec:	74 d2                	je     801002c0 <consoleread+0x40>
        release(&cons.lock);
801002ee:	83 ec 0c             	sub    $0xc,%esp
801002f1:	68 20 ff 10 80       	push   $0x8010ff20
801002f6:	e8 85 4e 00 00       	call   80105180 <release>
        ilock(ip);
801002fb:	5a                   	pop    %edx
801002fc:	ff 75 08             	push   0x8(%ebp)
801002ff:	e8 2c 15 00 00       	call   80101830 <ilock>
        return -1;
80100304:	83 c4 10             	add    $0x10,%esp
  }
  release(&cons.lock);
  ilock(ip);

  return target - n;
}
80100307:	8d 65 f4             	lea    -0xc(%ebp),%esp
        return -1;
8010030a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010030f:	5b                   	pop    %ebx
80100310:	5e                   	pop    %esi
80100311:	5f                   	pop    %edi
80100312:	5d                   	pop    %ebp
80100313:	c3                   	ret    
80100314:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    c = input.buf[input.r++ % INPUT_BUF];
80100318:	8d 50 01             	lea    0x1(%eax),%edx
8010031b:	89 15 00 ff 10 80    	mov    %edx,0x8010ff00
80100321:	89 c2                	mov    %eax,%edx
80100323:	83 e2 7f             	and    $0x7f,%edx
80100326:	0f be 8a 80 fe 10 80 	movsbl -0x7fef0180(%edx),%ecx
    if(c == C('D')){  // EOF
8010032d:	80 f9 04             	cmp    $0x4,%cl
80100330:	74 37                	je     80100369 <consoleread+0xe9>
    *dst++ = c;
80100332:	83 c6 01             	add    $0x1,%esi
    --n;
80100335:	83 eb 01             	sub    $0x1,%ebx
    *dst++ = c;
80100338:	88 4e ff             	mov    %cl,-0x1(%esi)
    if(c == '\n')
8010033b:	83 f9 0a             	cmp    $0xa,%ecx
8010033e:	0f 85 64 ff ff ff    	jne    801002a8 <consoleread+0x28>
  release(&cons.lock);
80100344:	83 ec 0c             	sub    $0xc,%esp
80100347:	68 20 ff 10 80       	push   $0x8010ff20
8010034c:	e8 2f 4e 00 00       	call   80105180 <release>
  ilock(ip);
80100351:	58                   	pop    %eax
80100352:	ff 75 08             	push   0x8(%ebp)
80100355:	e8 d6 14 00 00       	call   80101830 <ilock>
  return target - n;
8010035a:	89 f8                	mov    %edi,%eax
8010035c:	83 c4 10             	add    $0x10,%esp
}
8010035f:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return target - n;
80100362:	29 d8                	sub    %ebx,%eax
}
80100364:	5b                   	pop    %ebx
80100365:	5e                   	pop    %esi
80100366:	5f                   	pop    %edi
80100367:	5d                   	pop    %ebp
80100368:	c3                   	ret    
      if(n < target){
80100369:	39 fb                	cmp    %edi,%ebx
8010036b:	73 d7                	jae    80100344 <consoleread+0xc4>
        input.r--;
8010036d:	a3 00 ff 10 80       	mov    %eax,0x8010ff00
80100372:	eb d0                	jmp    80100344 <consoleread+0xc4>
80100374:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010037b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010037f:	90                   	nop

80100380 <panic>:
{
80100380:	55                   	push   %ebp
80100381:	89 e5                	mov    %esp,%ebp
80100383:	56                   	push   %esi
80100384:	53                   	push   %ebx
80100385:	83 ec 30             	sub    $0x30,%esp
}

static inline void
cli(void)
{
  asm volatile("cli");
80100388:	fa                   	cli    
  cons.locking = 0;
80100389:	c7 05 54 ff 10 80 00 	movl   $0x0,0x8010ff54
80100390:	00 00 00 
  getcallerpcs(&s, pcs);
80100393:	8d 5d d0             	lea    -0x30(%ebp),%ebx
80100396:	8d 75 f8             	lea    -0x8(%ebp),%esi
  cprintf("lapicid %d: panic: ", lapicid());
80100399:	e8 02 26 00 00       	call   801029a0 <lapicid>
8010039e:	83 ec 08             	sub    $0x8,%esp
801003a1:	50                   	push   %eax
801003a2:	68 2d 7e 10 80       	push   $0x80107e2d
801003a7:	e8 f4 02 00 00       	call   801006a0 <cprintf>
  cprintf(s);
801003ac:	58                   	pop    %eax
801003ad:	ff 75 08             	push   0x8(%ebp)
801003b0:	e8 eb 02 00 00       	call   801006a0 <cprintf>
  cprintf("\n");
801003b5:	c7 04 24 4c 84 10 80 	movl   $0x8010844c,(%esp)
801003bc:	e8 df 02 00 00       	call   801006a0 <cprintf>
  getcallerpcs(&s, pcs);
801003c1:	8d 45 08             	lea    0x8(%ebp),%eax
801003c4:	5a                   	pop    %edx
801003c5:	59                   	pop    %ecx
801003c6:	53                   	push   %ebx
801003c7:	50                   	push   %eax
801003c8:	e8 63 4c 00 00       	call   80105030 <getcallerpcs>
  for(i=0; i<10; i++)
801003cd:	83 c4 10             	add    $0x10,%esp
    cprintf(" %p", pcs[i]);
801003d0:	83 ec 08             	sub    $0x8,%esp
801003d3:	ff 33                	push   (%ebx)
  for(i=0; i<10; i++)
801003d5:	83 c3 04             	add    $0x4,%ebx
    cprintf(" %p", pcs[i]);
801003d8:	68 41 7e 10 80       	push   $0x80107e41
801003dd:	e8 be 02 00 00       	call   801006a0 <cprintf>
  for(i=0; i<10; i++)
801003e2:	83 c4 10             	add    $0x10,%esp
801003e5:	39 f3                	cmp    %esi,%ebx
801003e7:	75 e7                	jne    801003d0 <panic+0x50>
  panicked = 1; // freeze other CPU
801003e9:	c7 05 58 ff 10 80 01 	movl   $0x1,0x8010ff58
801003f0:	00 00 00 
  for(;;)
801003f3:	eb fe                	jmp    801003f3 <panic+0x73>
801003f5:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801003fc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80100400 <consputc.part.0>:
consputc(int c)
80100400:	55                   	push   %ebp
80100401:	89 e5                	mov    %esp,%ebp
80100403:	57                   	push   %edi
80100404:	56                   	push   %esi
80100405:	53                   	push   %ebx
80100406:	89 c3                	mov    %eax,%ebx
80100408:	83 ec 1c             	sub    $0x1c,%esp
  if(c == BACKSPACE){
8010040b:	3d 00 01 00 00       	cmp    $0x100,%eax
80100410:	0f 84 ea 00 00 00    	je     80100500 <consputc.part.0+0x100>
    uartputc(c);
80100416:	83 ec 0c             	sub    $0xc,%esp
80100419:	50                   	push   %eax
8010041a:	e8 f1 64 00 00       	call   80106910 <uartputc>
8010041f:	83 c4 10             	add    $0x10,%esp
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80100422:	bf d4 03 00 00       	mov    $0x3d4,%edi
80100427:	b8 0e 00 00 00       	mov    $0xe,%eax
8010042c:	89 fa                	mov    %edi,%edx
8010042e:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
8010042f:	be d5 03 00 00       	mov    $0x3d5,%esi
80100434:	89 f2                	mov    %esi,%edx
80100436:	ec                   	in     (%dx),%al
  pos = inb(CRTPORT+1) << 8;
80100437:	0f b6 c8             	movzbl %al,%ecx
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
8010043a:	89 fa                	mov    %edi,%edx
8010043c:	b8 0f 00 00 00       	mov    $0xf,%eax
80100441:	c1 e1 08             	shl    $0x8,%ecx
80100444:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80100445:	89 f2                	mov    %esi,%edx
80100447:	ec                   	in     (%dx),%al
  pos |= inb(CRTPORT+1);
80100448:	0f b6 c0             	movzbl %al,%eax
8010044b:	09 c8                	or     %ecx,%eax
  if(c == '\n')
8010044d:	83 fb 0a             	cmp    $0xa,%ebx
80100450:	0f 84 92 00 00 00    	je     801004e8 <consputc.part.0+0xe8>
  else if(c == BACKSPACE){
80100456:	81 fb 00 01 00 00    	cmp    $0x100,%ebx
8010045c:	74 72                	je     801004d0 <consputc.part.0+0xd0>
    crt[pos++] = (c&0xff) | 0x0700;  // black on white
8010045e:	0f b6 db             	movzbl %bl,%ebx
80100461:	8d 70 01             	lea    0x1(%eax),%esi
80100464:	80 cf 07             	or     $0x7,%bh
80100467:	66 89 9c 00 00 80 0b 	mov    %bx,-0x7ff48000(%eax,%eax,1)
8010046e:	80 
  if(pos < 0 || pos > 25*80)
8010046f:	81 fe d0 07 00 00    	cmp    $0x7d0,%esi
80100475:	0f 8f fb 00 00 00    	jg     80100576 <consputc.part.0+0x176>
  if((pos/80) >= 24){  // Scroll up.
8010047b:	81 fe 7f 07 00 00    	cmp    $0x77f,%esi
80100481:	0f 8f a9 00 00 00    	jg     80100530 <consputc.part.0+0x130>
  outb(CRTPORT+1, pos>>8);
80100487:	89 f0                	mov    %esi,%eax
  crt[pos] = ' ' | 0x0700;
80100489:	8d b4 36 00 80 0b 80 	lea    -0x7ff48000(%esi,%esi,1),%esi
  outb(CRTPORT+1, pos);
80100490:	88 45 e7             	mov    %al,-0x19(%ebp)
  outb(CRTPORT+1, pos>>8);
80100493:	0f b6 fc             	movzbl %ah,%edi
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80100496:	bb d4 03 00 00       	mov    $0x3d4,%ebx
8010049b:	b8 0e 00 00 00       	mov    $0xe,%eax
801004a0:	89 da                	mov    %ebx,%edx
801004a2:	ee                   	out    %al,(%dx)
801004a3:	b9 d5 03 00 00       	mov    $0x3d5,%ecx
801004a8:	89 f8                	mov    %edi,%eax
801004aa:	89 ca                	mov    %ecx,%edx
801004ac:	ee                   	out    %al,(%dx)
801004ad:	b8 0f 00 00 00       	mov    $0xf,%eax
801004b2:	89 da                	mov    %ebx,%edx
801004b4:	ee                   	out    %al,(%dx)
801004b5:	0f b6 45 e7          	movzbl -0x19(%ebp),%eax
801004b9:	89 ca                	mov    %ecx,%edx
801004bb:	ee                   	out    %al,(%dx)
  crt[pos] = ' ' | 0x0700;
801004bc:	b8 20 07 00 00       	mov    $0x720,%eax
801004c1:	66 89 06             	mov    %ax,(%esi)
}
801004c4:	8d 65 f4             	lea    -0xc(%ebp),%esp
801004c7:	5b                   	pop    %ebx
801004c8:	5e                   	pop    %esi
801004c9:	5f                   	pop    %edi
801004ca:	5d                   	pop    %ebp
801004cb:	c3                   	ret    
801004cc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    if(pos > 0) --pos;
801004d0:	8d 70 ff             	lea    -0x1(%eax),%esi
801004d3:	85 c0                	test   %eax,%eax
801004d5:	75 98                	jne    8010046f <consputc.part.0+0x6f>
801004d7:	c6 45 e7 00          	movb   $0x0,-0x19(%ebp)
801004db:	be 00 80 0b 80       	mov    $0x800b8000,%esi
801004e0:	31 ff                	xor    %edi,%edi
801004e2:	eb b2                	jmp    80100496 <consputc.part.0+0x96>
801004e4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    pos += 80 - pos%80;
801004e8:	ba cd cc cc cc       	mov    $0xcccccccd,%edx
801004ed:	f7 e2                	mul    %edx
801004ef:	c1 ea 06             	shr    $0x6,%edx
801004f2:	8d 04 92             	lea    (%edx,%edx,4),%eax
801004f5:	c1 e0 04             	shl    $0x4,%eax
801004f8:	8d 70 50             	lea    0x50(%eax),%esi
801004fb:	e9 6f ff ff ff       	jmp    8010046f <consputc.part.0+0x6f>
    uartputc('\b'); uartputc(' '); uartputc('\b');
80100500:	83 ec 0c             	sub    $0xc,%esp
80100503:	6a 08                	push   $0x8
80100505:	e8 06 64 00 00       	call   80106910 <uartputc>
8010050a:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80100511:	e8 fa 63 00 00       	call   80106910 <uartputc>
80100516:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
8010051d:	e8 ee 63 00 00       	call   80106910 <uartputc>
80100522:	83 c4 10             	add    $0x10,%esp
80100525:	e9 f8 fe ff ff       	jmp    80100422 <consputc.part.0+0x22>
8010052a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    memmove(crt, crt+80, sizeof(crt[0])*23*80);
80100530:	83 ec 04             	sub    $0x4,%esp
    pos -= 80;
80100533:	8d 5e b0             	lea    -0x50(%esi),%ebx
    memset(crt+pos, 0, sizeof(crt[0])*(24*80 - pos));
80100536:	8d b4 36 60 7f 0b 80 	lea    -0x7ff480a0(%esi,%esi,1),%esi
  outb(CRTPORT+1, pos);
8010053d:	bf 07 00 00 00       	mov    $0x7,%edi
    memmove(crt, crt+80, sizeof(crt[0])*23*80);
80100542:	68 60 0e 00 00       	push   $0xe60
80100547:	68 a0 80 0b 80       	push   $0x800b80a0
8010054c:	68 00 80 0b 80       	push   $0x800b8000
80100551:	e8 ea 4d 00 00       	call   80105340 <memmove>
    memset(crt+pos, 0, sizeof(crt[0])*(24*80 - pos));
80100556:	b8 80 07 00 00       	mov    $0x780,%eax
8010055b:	83 c4 0c             	add    $0xc,%esp
8010055e:	29 d8                	sub    %ebx,%eax
80100560:	01 c0                	add    %eax,%eax
80100562:	50                   	push   %eax
80100563:	6a 00                	push   $0x0
80100565:	56                   	push   %esi
80100566:	e8 35 4d 00 00       	call   801052a0 <memset>
  outb(CRTPORT+1, pos);
8010056b:	88 5d e7             	mov    %bl,-0x19(%ebp)
8010056e:	83 c4 10             	add    $0x10,%esp
80100571:	e9 20 ff ff ff       	jmp    80100496 <consputc.part.0+0x96>
    panic("pos under/overflow");
80100576:	83 ec 0c             	sub    $0xc,%esp
80100579:	68 45 7e 10 80       	push   $0x80107e45
8010057e:	e8 fd fd ff ff       	call   80100380 <panic>
80100583:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010058a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80100590 <consolewrite>:

int
consolewrite(struct inode *ip, char *buf, int n)
{
80100590:	55                   	push   %ebp
80100591:	89 e5                	mov    %esp,%ebp
80100593:	57                   	push   %edi
80100594:	56                   	push   %esi
80100595:	53                   	push   %ebx
80100596:	83 ec 18             	sub    $0x18,%esp
  int i;

  iunlock(ip);
80100599:	ff 75 08             	push   0x8(%ebp)
{
8010059c:	8b 75 10             	mov    0x10(%ebp),%esi
  iunlock(ip);
8010059f:	e8 6c 13 00 00       	call   80101910 <iunlock>
  acquire(&cons.lock);
801005a4:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801005ab:	e8 30 4c 00 00       	call   801051e0 <acquire>
  for(i = 0; i < n; i++)
801005b0:	83 c4 10             	add    $0x10,%esp
801005b3:	85 f6                	test   %esi,%esi
801005b5:	7e 25                	jle    801005dc <consolewrite+0x4c>
801005b7:	8b 5d 0c             	mov    0xc(%ebp),%ebx
801005ba:	8d 3c 33             	lea    (%ebx,%esi,1),%edi
  if(panicked){
801005bd:	8b 15 58 ff 10 80    	mov    0x8010ff58,%edx
    consputc(buf[i] & 0xff);
801005c3:	0f b6 03             	movzbl (%ebx),%eax
  if(panicked){
801005c6:	85 d2                	test   %edx,%edx
801005c8:	74 06                	je     801005d0 <consolewrite+0x40>
  asm volatile("cli");
801005ca:	fa                   	cli    
    for(;;)
801005cb:	eb fe                	jmp    801005cb <consolewrite+0x3b>
801005cd:	8d 76 00             	lea    0x0(%esi),%esi
801005d0:	e8 2b fe ff ff       	call   80100400 <consputc.part.0>
  for(i = 0; i < n; i++)
801005d5:	83 c3 01             	add    $0x1,%ebx
801005d8:	39 df                	cmp    %ebx,%edi
801005da:	75 e1                	jne    801005bd <consolewrite+0x2d>
  release(&cons.lock);
801005dc:	83 ec 0c             	sub    $0xc,%esp
801005df:	68 20 ff 10 80       	push   $0x8010ff20
801005e4:	e8 97 4b 00 00       	call   80105180 <release>
  ilock(ip);
801005e9:	58                   	pop    %eax
801005ea:	ff 75 08             	push   0x8(%ebp)
801005ed:	e8 3e 12 00 00       	call   80101830 <ilock>

  return n;
}
801005f2:	8d 65 f4             	lea    -0xc(%ebp),%esp
801005f5:	89 f0                	mov    %esi,%eax
801005f7:	5b                   	pop    %ebx
801005f8:	5e                   	pop    %esi
801005f9:	5f                   	pop    %edi
801005fa:	5d                   	pop    %ebp
801005fb:	c3                   	ret    
801005fc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80100600 <printint>:
{
80100600:	55                   	push   %ebp
80100601:	89 e5                	mov    %esp,%ebp
80100603:	57                   	push   %edi
80100604:	56                   	push   %esi
80100605:	53                   	push   %ebx
80100606:	83 ec 2c             	sub    $0x2c,%esp
80100609:	89 55 d4             	mov    %edx,-0x2c(%ebp)
8010060c:	89 4d d0             	mov    %ecx,-0x30(%ebp)
  if(sign && (sign = xx < 0))
8010060f:	85 c9                	test   %ecx,%ecx
80100611:	74 04                	je     80100617 <printint+0x17>
80100613:	85 c0                	test   %eax,%eax
80100615:	78 6d                	js     80100684 <printint+0x84>
    x = xx;
80100617:	c7 45 d0 00 00 00 00 	movl   $0x0,-0x30(%ebp)
8010061e:	89 c1                	mov    %eax,%ecx
  i = 0;
80100620:	31 db                	xor    %ebx,%ebx
80100622:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    buf[i++] = digits[x % base];
80100628:	89 c8                	mov    %ecx,%eax
8010062a:	31 d2                	xor    %edx,%edx
8010062c:	89 de                	mov    %ebx,%esi
8010062e:	89 cf                	mov    %ecx,%edi
80100630:	f7 75 d4             	divl   -0x2c(%ebp)
80100633:	8d 5b 01             	lea    0x1(%ebx),%ebx
80100636:	0f b6 92 70 7e 10 80 	movzbl -0x7fef8190(%edx),%edx
  }while((x /= base) != 0);
8010063d:	89 c1                	mov    %eax,%ecx
    buf[i++] = digits[x % base];
8010063f:	88 54 1d d7          	mov    %dl,-0x29(%ebp,%ebx,1)
  }while((x /= base) != 0);
80100643:	3b 7d d4             	cmp    -0x2c(%ebp),%edi
80100646:	73 e0                	jae    80100628 <printint+0x28>
  if(sign)
80100648:	8b 4d d0             	mov    -0x30(%ebp),%ecx
8010064b:	85 c9                	test   %ecx,%ecx
8010064d:	74 0c                	je     8010065b <printint+0x5b>
    buf[i++] = '-';
8010064f:	c6 44 1d d8 2d       	movb   $0x2d,-0x28(%ebp,%ebx,1)
    buf[i++] = digits[x % base];
80100654:	89 de                	mov    %ebx,%esi
    buf[i++] = '-';
80100656:	ba 2d 00 00 00       	mov    $0x2d,%edx
  while(--i >= 0)
8010065b:	8d 5c 35 d7          	lea    -0x29(%ebp,%esi,1),%ebx
8010065f:	0f be c2             	movsbl %dl,%eax
  if(panicked){
80100662:	8b 15 58 ff 10 80    	mov    0x8010ff58,%edx
80100668:	85 d2                	test   %edx,%edx
8010066a:	74 04                	je     80100670 <printint+0x70>
8010066c:	fa                   	cli    
    for(;;)
8010066d:	eb fe                	jmp    8010066d <printint+0x6d>
8010066f:	90                   	nop
80100670:	e8 8b fd ff ff       	call   80100400 <consputc.part.0>
  while(--i >= 0)
80100675:	8d 45 d7             	lea    -0x29(%ebp),%eax
80100678:	39 c3                	cmp    %eax,%ebx
8010067a:	74 0e                	je     8010068a <printint+0x8a>
    consputc(buf[i]);
8010067c:	0f be 03             	movsbl (%ebx),%eax
8010067f:	83 eb 01             	sub    $0x1,%ebx
80100682:	eb de                	jmp    80100662 <printint+0x62>
    x = -xx;
80100684:	f7 d8                	neg    %eax
80100686:	89 c1                	mov    %eax,%ecx
80100688:	eb 96                	jmp    80100620 <printint+0x20>
}
8010068a:	83 c4 2c             	add    $0x2c,%esp
8010068d:	5b                   	pop    %ebx
8010068e:	5e                   	pop    %esi
8010068f:	5f                   	pop    %edi
80100690:	5d                   	pop    %ebp
80100691:	c3                   	ret    
80100692:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100699:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

801006a0 <cprintf>:
{
801006a0:	55                   	push   %ebp
801006a1:	89 e5                	mov    %esp,%ebp
801006a3:	57                   	push   %edi
801006a4:	56                   	push   %esi
801006a5:	53                   	push   %ebx
801006a6:	83 ec 1c             	sub    $0x1c,%esp
  locking = cons.locking;
801006a9:	a1 54 ff 10 80       	mov    0x8010ff54,%eax
801006ae:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  if(locking)
801006b1:	85 c0                	test   %eax,%eax
801006b3:	0f 85 27 01 00 00    	jne    801007e0 <cprintf+0x140>
  if (fmt == 0)
801006b9:	8b 75 08             	mov    0x8(%ebp),%esi
801006bc:	85 f6                	test   %esi,%esi
801006be:	0f 84 ac 01 00 00    	je     80100870 <cprintf+0x1d0>
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
801006c4:	0f b6 06             	movzbl (%esi),%eax
  argp = (uint*)(void*)(&fmt + 1);
801006c7:	8d 7d 0c             	lea    0xc(%ebp),%edi
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
801006ca:	31 db                	xor    %ebx,%ebx
801006cc:	85 c0                	test   %eax,%eax
801006ce:	74 56                	je     80100726 <cprintf+0x86>
    if(c != '%'){
801006d0:	83 f8 25             	cmp    $0x25,%eax
801006d3:	0f 85 cf 00 00 00    	jne    801007a8 <cprintf+0x108>
    c = fmt[++i] & 0xff;
801006d9:	83 c3 01             	add    $0x1,%ebx
801006dc:	0f b6 14 1e          	movzbl (%esi,%ebx,1),%edx
    if(c == 0)
801006e0:	85 d2                	test   %edx,%edx
801006e2:	74 42                	je     80100726 <cprintf+0x86>
    switch(c){
801006e4:	83 fa 70             	cmp    $0x70,%edx
801006e7:	0f 84 90 00 00 00    	je     8010077d <cprintf+0xdd>
801006ed:	7f 51                	jg     80100740 <cprintf+0xa0>
801006ef:	83 fa 25             	cmp    $0x25,%edx
801006f2:	0f 84 c0 00 00 00    	je     801007b8 <cprintf+0x118>
801006f8:	83 fa 64             	cmp    $0x64,%edx
801006fb:	0f 85 f4 00 00 00    	jne    801007f5 <cprintf+0x155>
      printint(*argp++, 10, 1);
80100701:	8d 47 04             	lea    0x4(%edi),%eax
80100704:	b9 01 00 00 00       	mov    $0x1,%ecx
80100709:	ba 0a 00 00 00       	mov    $0xa,%edx
8010070e:	89 45 e0             	mov    %eax,-0x20(%ebp)
80100711:	8b 07                	mov    (%edi),%eax
80100713:	e8 e8 fe ff ff       	call   80100600 <printint>
80100718:	8b 7d e0             	mov    -0x20(%ebp),%edi
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
8010071b:	83 c3 01             	add    $0x1,%ebx
8010071e:	0f b6 04 1e          	movzbl (%esi,%ebx,1),%eax
80100722:	85 c0                	test   %eax,%eax
80100724:	75 aa                	jne    801006d0 <cprintf+0x30>
  if(locking)
80100726:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100729:	85 c0                	test   %eax,%eax
8010072b:	0f 85 22 01 00 00    	jne    80100853 <cprintf+0x1b3>
}
80100731:	8d 65 f4             	lea    -0xc(%ebp),%esp
80100734:	5b                   	pop    %ebx
80100735:	5e                   	pop    %esi
80100736:	5f                   	pop    %edi
80100737:	5d                   	pop    %ebp
80100738:	c3                   	ret    
80100739:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    switch(c){
80100740:	83 fa 73             	cmp    $0x73,%edx
80100743:	75 33                	jne    80100778 <cprintf+0xd8>
      if((s = (char*)*argp++) == 0)
80100745:	8d 47 04             	lea    0x4(%edi),%eax
80100748:	8b 3f                	mov    (%edi),%edi
8010074a:	89 45 e0             	mov    %eax,-0x20(%ebp)
8010074d:	85 ff                	test   %edi,%edi
8010074f:	0f 84 e3 00 00 00    	je     80100838 <cprintf+0x198>
      for(; *s; s++)
80100755:	0f be 07             	movsbl (%edi),%eax
80100758:	84 c0                	test   %al,%al
8010075a:	0f 84 08 01 00 00    	je     80100868 <cprintf+0x1c8>
  if(panicked){
80100760:	8b 15 58 ff 10 80    	mov    0x8010ff58,%edx
80100766:	85 d2                	test   %edx,%edx
80100768:	0f 84 b2 00 00 00    	je     80100820 <cprintf+0x180>
8010076e:	fa                   	cli    
    for(;;)
8010076f:	eb fe                	jmp    8010076f <cprintf+0xcf>
80100771:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    switch(c){
80100778:	83 fa 78             	cmp    $0x78,%edx
8010077b:	75 78                	jne    801007f5 <cprintf+0x155>
      printint(*argp++, 16, 0);
8010077d:	8d 47 04             	lea    0x4(%edi),%eax
80100780:	31 c9                	xor    %ecx,%ecx
80100782:	ba 10 00 00 00       	mov    $0x10,%edx
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
80100787:	83 c3 01             	add    $0x1,%ebx
      printint(*argp++, 16, 0);
8010078a:	89 45 e0             	mov    %eax,-0x20(%ebp)
8010078d:	8b 07                	mov    (%edi),%eax
8010078f:	e8 6c fe ff ff       	call   80100600 <printint>
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
80100794:	0f b6 04 1e          	movzbl (%esi,%ebx,1),%eax
      printint(*argp++, 16, 0);
80100798:	8b 7d e0             	mov    -0x20(%ebp),%edi
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
8010079b:	85 c0                	test   %eax,%eax
8010079d:	0f 85 2d ff ff ff    	jne    801006d0 <cprintf+0x30>
801007a3:	eb 81                	jmp    80100726 <cprintf+0x86>
801007a5:	8d 76 00             	lea    0x0(%esi),%esi
  if(panicked){
801007a8:	8b 0d 58 ff 10 80    	mov    0x8010ff58,%ecx
801007ae:	85 c9                	test   %ecx,%ecx
801007b0:	74 14                	je     801007c6 <cprintf+0x126>
801007b2:	fa                   	cli    
    for(;;)
801007b3:	eb fe                	jmp    801007b3 <cprintf+0x113>
801007b5:	8d 76 00             	lea    0x0(%esi),%esi
  if(panicked){
801007b8:	a1 58 ff 10 80       	mov    0x8010ff58,%eax
801007bd:	85 c0                	test   %eax,%eax
801007bf:	75 6c                	jne    8010082d <cprintf+0x18d>
801007c1:	b8 25 00 00 00       	mov    $0x25,%eax
801007c6:	e8 35 fc ff ff       	call   80100400 <consputc.part.0>
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
801007cb:	83 c3 01             	add    $0x1,%ebx
801007ce:	0f b6 04 1e          	movzbl (%esi,%ebx,1),%eax
801007d2:	85 c0                	test   %eax,%eax
801007d4:	0f 85 f6 fe ff ff    	jne    801006d0 <cprintf+0x30>
801007da:	e9 47 ff ff ff       	jmp    80100726 <cprintf+0x86>
801007df:	90                   	nop
    acquire(&cons.lock);
801007e0:	83 ec 0c             	sub    $0xc,%esp
801007e3:	68 20 ff 10 80       	push   $0x8010ff20
801007e8:	e8 f3 49 00 00       	call   801051e0 <acquire>
801007ed:	83 c4 10             	add    $0x10,%esp
801007f0:	e9 c4 fe ff ff       	jmp    801006b9 <cprintf+0x19>
  if(panicked){
801007f5:	8b 0d 58 ff 10 80    	mov    0x8010ff58,%ecx
801007fb:	85 c9                	test   %ecx,%ecx
801007fd:	75 31                	jne    80100830 <cprintf+0x190>
801007ff:	b8 25 00 00 00       	mov    $0x25,%eax
80100804:	89 55 e0             	mov    %edx,-0x20(%ebp)
80100807:	e8 f4 fb ff ff       	call   80100400 <consputc.part.0>
8010080c:	8b 15 58 ff 10 80    	mov    0x8010ff58,%edx
80100812:	85 d2                	test   %edx,%edx
80100814:	8b 55 e0             	mov    -0x20(%ebp),%edx
80100817:	74 2e                	je     80100847 <cprintf+0x1a7>
80100819:	fa                   	cli    
    for(;;)
8010081a:	eb fe                	jmp    8010081a <cprintf+0x17a>
8010081c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80100820:	e8 db fb ff ff       	call   80100400 <consputc.part.0>
      for(; *s; s++)
80100825:	83 c7 01             	add    $0x1,%edi
80100828:	e9 28 ff ff ff       	jmp    80100755 <cprintf+0xb5>
8010082d:	fa                   	cli    
    for(;;)
8010082e:	eb fe                	jmp    8010082e <cprintf+0x18e>
80100830:	fa                   	cli    
80100831:	eb fe                	jmp    80100831 <cprintf+0x191>
80100833:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80100837:	90                   	nop
        s = "(null)";
80100838:	bf 58 7e 10 80       	mov    $0x80107e58,%edi
      for(; *s; s++)
8010083d:	b8 28 00 00 00       	mov    $0x28,%eax
80100842:	e9 19 ff ff ff       	jmp    80100760 <cprintf+0xc0>
80100847:	89 d0                	mov    %edx,%eax
80100849:	e8 b2 fb ff ff       	call   80100400 <consputc.part.0>
8010084e:	e9 c8 fe ff ff       	jmp    8010071b <cprintf+0x7b>
    release(&cons.lock);
80100853:	83 ec 0c             	sub    $0xc,%esp
80100856:	68 20 ff 10 80       	push   $0x8010ff20
8010085b:	e8 20 49 00 00       	call   80105180 <release>
80100860:	83 c4 10             	add    $0x10,%esp
}
80100863:	e9 c9 fe ff ff       	jmp    80100731 <cprintf+0x91>
      if((s = (char*)*argp++) == 0)
80100868:	8b 7d e0             	mov    -0x20(%ebp),%edi
8010086b:	e9 ab fe ff ff       	jmp    8010071b <cprintf+0x7b>
    panic("null fmt");
80100870:	83 ec 0c             	sub    $0xc,%esp
80100873:	68 5f 7e 10 80       	push   $0x80107e5f
80100878:	e8 03 fb ff ff       	call   80100380 <panic>
8010087d:	8d 76 00             	lea    0x0(%esi),%esi

80100880 <consoleintr>:
{
80100880:	55                   	push   %ebp
80100881:	89 e5                	mov    %esp,%ebp
80100883:	57                   	push   %edi
80100884:	56                   	push   %esi
  int c, doprocdump = 0;
80100885:	31 f6                	xor    %esi,%esi
{
80100887:	53                   	push   %ebx
80100888:	83 ec 18             	sub    $0x18,%esp
8010088b:	8b 7d 08             	mov    0x8(%ebp),%edi
  acquire(&cons.lock);
8010088e:	68 20 ff 10 80       	push   $0x8010ff20
80100893:	e8 48 49 00 00       	call   801051e0 <acquire>
  while((c = getc()) >= 0){
80100898:	83 c4 10             	add    $0x10,%esp
8010089b:	eb 1a                	jmp    801008b7 <consoleintr+0x37>
8010089d:	8d 76 00             	lea    0x0(%esi),%esi
    switch(c){
801008a0:	83 fb 08             	cmp    $0x8,%ebx
801008a3:	0f 84 d7 00 00 00    	je     80100980 <consoleintr+0x100>
801008a9:	83 fb 10             	cmp    $0x10,%ebx
801008ac:	0f 85 32 01 00 00    	jne    801009e4 <consoleintr+0x164>
801008b2:	be 01 00 00 00       	mov    $0x1,%esi
  while((c = getc()) >= 0){
801008b7:	ff d7                	call   *%edi
801008b9:	89 c3                	mov    %eax,%ebx
801008bb:	85 c0                	test   %eax,%eax
801008bd:	0f 88 05 01 00 00    	js     801009c8 <consoleintr+0x148>
    switch(c){
801008c3:	83 fb 15             	cmp    $0x15,%ebx
801008c6:	74 78                	je     80100940 <consoleintr+0xc0>
801008c8:	7e d6                	jle    801008a0 <consoleintr+0x20>
801008ca:	83 fb 7f             	cmp    $0x7f,%ebx
801008cd:	0f 84 ad 00 00 00    	je     80100980 <consoleintr+0x100>
      if(c != 0 && input.e-input.r < INPUT_BUF){
801008d3:	a1 08 ff 10 80       	mov    0x8010ff08,%eax
801008d8:	89 c2                	mov    %eax,%edx
801008da:	2b 15 00 ff 10 80    	sub    0x8010ff00,%edx
801008e0:	83 fa 7f             	cmp    $0x7f,%edx
801008e3:	77 d2                	ja     801008b7 <consoleintr+0x37>
        input.buf[input.e++ % INPUT_BUF] = c;
801008e5:	8d 48 01             	lea    0x1(%eax),%ecx
  if(panicked){
801008e8:	8b 15 58 ff 10 80    	mov    0x8010ff58,%edx
        input.buf[input.e++ % INPUT_BUF] = c;
801008ee:	83 e0 7f             	and    $0x7f,%eax
801008f1:	89 0d 08 ff 10 80    	mov    %ecx,0x8010ff08
        c = (c == '\r') ? '\n' : c;
801008f7:	83 fb 0d             	cmp    $0xd,%ebx
801008fa:	0f 84 13 01 00 00    	je     80100a13 <consoleintr+0x193>
        input.buf[input.e++ % INPUT_BUF] = c;
80100900:	88 98 80 fe 10 80    	mov    %bl,-0x7fef0180(%eax)
  if(panicked){
80100906:	85 d2                	test   %edx,%edx
80100908:	0f 85 10 01 00 00    	jne    80100a1e <consoleintr+0x19e>
8010090e:	89 d8                	mov    %ebx,%eax
80100910:	e8 eb fa ff ff       	call   80100400 <consputc.part.0>
        if(c == '\n' || c == C('D') || input.e == input.r+INPUT_BUF){
80100915:	83 fb 0a             	cmp    $0xa,%ebx
80100918:	0f 84 14 01 00 00    	je     80100a32 <consoleintr+0x1b2>
8010091e:	83 fb 04             	cmp    $0x4,%ebx
80100921:	0f 84 0b 01 00 00    	je     80100a32 <consoleintr+0x1b2>
80100927:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
8010092c:	83 e8 80             	sub    $0xffffff80,%eax
8010092f:	39 05 08 ff 10 80    	cmp    %eax,0x8010ff08
80100935:	75 80                	jne    801008b7 <consoleintr+0x37>
80100937:	e9 fb 00 00 00       	jmp    80100a37 <consoleintr+0x1b7>
8010093c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
      while(input.e != input.w &&
80100940:	a1 08 ff 10 80       	mov    0x8010ff08,%eax
80100945:	39 05 04 ff 10 80    	cmp    %eax,0x8010ff04
8010094b:	0f 84 66 ff ff ff    	je     801008b7 <consoleintr+0x37>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
80100951:	83 e8 01             	sub    $0x1,%eax
80100954:	89 c2                	mov    %eax,%edx
80100956:	83 e2 7f             	and    $0x7f,%edx
      while(input.e != input.w &&
80100959:	80 ba 80 fe 10 80 0a 	cmpb   $0xa,-0x7fef0180(%edx)
80100960:	0f 84 51 ff ff ff    	je     801008b7 <consoleintr+0x37>
  if(panicked){
80100966:	8b 15 58 ff 10 80    	mov    0x8010ff58,%edx
        input.e--;
8010096c:	a3 08 ff 10 80       	mov    %eax,0x8010ff08
  if(panicked){
80100971:	85 d2                	test   %edx,%edx
80100973:	74 33                	je     801009a8 <consoleintr+0x128>
80100975:	fa                   	cli    
    for(;;)
80100976:	eb fe                	jmp    80100976 <consoleintr+0xf6>
80100978:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010097f:	90                   	nop
      if(input.e != input.w){
80100980:	a1 08 ff 10 80       	mov    0x8010ff08,%eax
80100985:	3b 05 04 ff 10 80    	cmp    0x8010ff04,%eax
8010098b:	0f 84 26 ff ff ff    	je     801008b7 <consoleintr+0x37>
        input.e--;
80100991:	83 e8 01             	sub    $0x1,%eax
80100994:	a3 08 ff 10 80       	mov    %eax,0x8010ff08
  if(panicked){
80100999:	a1 58 ff 10 80       	mov    0x8010ff58,%eax
8010099e:	85 c0                	test   %eax,%eax
801009a0:	74 56                	je     801009f8 <consoleintr+0x178>
801009a2:	fa                   	cli    
    for(;;)
801009a3:	eb fe                	jmp    801009a3 <consoleintr+0x123>
801009a5:	8d 76 00             	lea    0x0(%esi),%esi
801009a8:	b8 00 01 00 00       	mov    $0x100,%eax
801009ad:	e8 4e fa ff ff       	call   80100400 <consputc.part.0>
      while(input.e != input.w &&
801009b2:	a1 08 ff 10 80       	mov    0x8010ff08,%eax
801009b7:	3b 05 04 ff 10 80    	cmp    0x8010ff04,%eax
801009bd:	75 92                	jne    80100951 <consoleintr+0xd1>
801009bf:	e9 f3 fe ff ff       	jmp    801008b7 <consoleintr+0x37>
801009c4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  release(&cons.lock);
801009c8:	83 ec 0c             	sub    $0xc,%esp
801009cb:	68 20 ff 10 80       	push   $0x8010ff20
801009d0:	e8 ab 47 00 00       	call   80105180 <release>
  if(doprocdump) {
801009d5:	83 c4 10             	add    $0x10,%esp
801009d8:	85 f6                	test   %esi,%esi
801009da:	75 2b                	jne    80100a07 <consoleintr+0x187>
}
801009dc:	8d 65 f4             	lea    -0xc(%ebp),%esp
801009df:	5b                   	pop    %ebx
801009e0:	5e                   	pop    %esi
801009e1:	5f                   	pop    %edi
801009e2:	5d                   	pop    %ebp
801009e3:	c3                   	ret    
      if(c != 0 && input.e-input.r < INPUT_BUF){
801009e4:	85 db                	test   %ebx,%ebx
801009e6:	0f 84 cb fe ff ff    	je     801008b7 <consoleintr+0x37>
801009ec:	e9 e2 fe ff ff       	jmp    801008d3 <consoleintr+0x53>
801009f1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801009f8:	b8 00 01 00 00       	mov    $0x100,%eax
801009fd:	e8 fe f9 ff ff       	call   80100400 <consputc.part.0>
80100a02:	e9 b0 fe ff ff       	jmp    801008b7 <consoleintr+0x37>
}
80100a07:	8d 65 f4             	lea    -0xc(%ebp),%esp
80100a0a:	5b                   	pop    %ebx
80100a0b:	5e                   	pop    %esi
80100a0c:	5f                   	pop    %edi
80100a0d:	5d                   	pop    %ebp
    procdump();  // now call procdump() wo. cons.lock held
80100a0e:	e9 0d 3c 00 00       	jmp    80104620 <procdump>
        input.buf[input.e++ % INPUT_BUF] = c;
80100a13:	c6 80 80 fe 10 80 0a 	movb   $0xa,-0x7fef0180(%eax)
  if(panicked){
80100a1a:	85 d2                	test   %edx,%edx
80100a1c:	74 0a                	je     80100a28 <consoleintr+0x1a8>
80100a1e:	fa                   	cli    
    for(;;)
80100a1f:	eb fe                	jmp    80100a1f <consoleintr+0x19f>
80100a21:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100a28:	b8 0a 00 00 00       	mov    $0xa,%eax
80100a2d:	e8 ce f9 ff ff       	call   80100400 <consputc.part.0>
          input.w = input.e;
80100a32:	a1 08 ff 10 80       	mov    0x8010ff08,%eax
          wakeup(&input.r);
80100a37:	83 ec 0c             	sub    $0xc,%esp
          input.w = input.e;
80100a3a:	a3 04 ff 10 80       	mov    %eax,0x8010ff04
          wakeup(&input.r);
80100a3f:	68 00 ff 10 80       	push   $0x8010ff00
80100a44:	e8 97 3a 00 00       	call   801044e0 <wakeup>
80100a49:	83 c4 10             	add    $0x10,%esp
80100a4c:	e9 66 fe ff ff       	jmp    801008b7 <consoleintr+0x37>
80100a51:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100a58:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100a5f:	90                   	nop

80100a60 <consoleinit>:

void
consoleinit(void)
{
80100a60:	55                   	push   %ebp
80100a61:	89 e5                	mov    %esp,%ebp
80100a63:	83 ec 10             	sub    $0x10,%esp
  initlock(&cons.lock, "console");
80100a66:	68 68 7e 10 80       	push   $0x80107e68
80100a6b:	68 20 ff 10 80       	push   $0x8010ff20
80100a70:	e8 9b 45 00 00       	call   80105010 <initlock>

  devsw[CONSOLE].write = consolewrite;
  devsw[CONSOLE].read = consoleread;
  cons.locking = 1;

  ioapicenable(IRQ_KBD, 0);
80100a75:	58                   	pop    %eax
80100a76:	5a                   	pop    %edx
80100a77:	6a 00                	push   $0x0
80100a79:	6a 01                	push   $0x1
  devsw[CONSOLE].write = consolewrite;
80100a7b:	c7 05 0c 09 11 80 90 	movl   $0x80100590,0x8011090c
80100a82:	05 10 80 
  devsw[CONSOLE].read = consoleread;
80100a85:	c7 05 08 09 11 80 80 	movl   $0x80100280,0x80110908
80100a8c:	02 10 80 
  cons.locking = 1;
80100a8f:	c7 05 54 ff 10 80 01 	movl   $0x1,0x8010ff54
80100a96:	00 00 00 
  ioapicenable(IRQ_KBD, 0);
80100a99:	e8 92 1a 00 00       	call   80102530 <ioapicenable>
}
80100a9e:	83 c4 10             	add    $0x10,%esp
80100aa1:	c9                   	leave  
80100aa2:	c3                   	ret    
80100aa3:	66 90                	xchg   %ax,%ax
80100aa5:	66 90                	xchg   %ax,%ax
80100aa7:	66 90                	xchg   %ax,%ax
80100aa9:	66 90                	xchg   %ax,%ax
80100aab:	66 90                	xchg   %ax,%ax
80100aad:	66 90                	xchg   %ax,%ax
80100aaf:	90                   	nop

80100ab0 <exec>:
#include "x86.h"
#include "elf.h"

int
exec(char *path, char **argv)
{
80100ab0:	55                   	push   %ebp
80100ab1:	89 e5                	mov    %esp,%ebp
80100ab3:	57                   	push   %edi
80100ab4:	56                   	push   %esi
80100ab5:	53                   	push   %ebx
80100ab6:	81 ec 0c 01 00 00    	sub    $0x10c,%esp
  uint argc, sz, sp, ustack[3+MAXARG+1];
  struct elfhdr elf;
  struct inode *ip;
  struct proghdr ph;
  pde_t *pgdir, *oldpgdir;
  struct proc *curproc = myproc();
80100abc:	e8 af 2f 00 00       	call   80103a70 <myproc>
80100ac1:	89 85 ec fe ff ff    	mov    %eax,-0x114(%ebp)

  begin_op();
80100ac7:	e8 44 23 00 00       	call   80102e10 <begin_op>

  if((ip = namei(path)) == 0){
80100acc:	83 ec 0c             	sub    $0xc,%esp
80100acf:	ff 75 08             	push   0x8(%ebp)
80100ad2:	e8 79 16 00 00       	call   80102150 <namei>
80100ad7:	83 c4 10             	add    $0x10,%esp
80100ada:	85 c0                	test   %eax,%eax
80100adc:	0f 84 76 03 00 00    	je     80100e58 <exec+0x3a8>
    end_op();
    cprintf("exec: fail\n");
    return -1;
  }
  ilock(ip);
80100ae2:	83 ec 0c             	sub    $0xc,%esp
80100ae5:	89 c3                	mov    %eax,%ebx
80100ae7:	50                   	push   %eax
80100ae8:	e8 43 0d 00 00       	call   80101830 <ilock>
  pgdir = 0;

  // Check ELF header
  if(readi(ip, (char*)&elf, 0, sizeof(elf)) != sizeof(elf))
80100aed:	8d 85 24 ff ff ff    	lea    -0xdc(%ebp),%eax
80100af3:	6a 34                	push   $0x34
80100af5:	6a 00                	push   $0x0
80100af7:	50                   	push   %eax
80100af8:	53                   	push   %ebx
80100af9:	e8 42 10 00 00       	call   80101b40 <readi>
80100afe:	83 c4 20             	add    $0x20,%esp
80100b01:	83 f8 34             	cmp    $0x34,%eax
80100b04:	74 22                	je     80100b28 <exec+0x78>

 bad:
  if(pgdir)
    freevm(pgdir);
  if(ip){
    iunlockput(ip);
80100b06:	83 ec 0c             	sub    $0xc,%esp
80100b09:	53                   	push   %ebx
80100b0a:	e8 b1 0f 00 00       	call   80101ac0 <iunlockput>
    end_op();
80100b0f:	e8 6c 23 00 00       	call   80102e80 <end_op>
80100b14:	83 c4 10             	add    $0x10,%esp
  }
  return -1;
80100b17:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80100b1c:	8d 65 f4             	lea    -0xc(%ebp),%esp
80100b1f:	5b                   	pop    %ebx
80100b20:	5e                   	pop    %esi
80100b21:	5f                   	pop    %edi
80100b22:	5d                   	pop    %ebp
80100b23:	c3                   	ret    
80100b24:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  if(elf.magic != ELF_MAGIC)
80100b28:	81 bd 24 ff ff ff 7f 	cmpl   $0x464c457f,-0xdc(%ebp)
80100b2f:	45 4c 46 
80100b32:	75 d2                	jne    80100b06 <exec+0x56>
  if((pgdir = setupkvm()) == 0)
80100b34:	e8 67 6f 00 00       	call   80107aa0 <setupkvm>
80100b39:	89 85 f4 fe ff ff    	mov    %eax,-0x10c(%ebp)
80100b3f:	85 c0                	test   %eax,%eax
80100b41:	74 c3                	je     80100b06 <exec+0x56>
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100b43:	66 83 bd 50 ff ff ff 	cmpw   $0x0,-0xb0(%ebp)
80100b4a:	00 
80100b4b:	8b b5 40 ff ff ff    	mov    -0xc0(%ebp),%esi
80100b51:	0f 84 20 03 00 00    	je     80100e77 <exec+0x3c7>
  sz = 0;
80100b57:	c7 85 f0 fe ff ff 00 	movl   $0x0,-0x110(%ebp)
80100b5e:	00 00 00 
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100b61:	31 ff                	xor    %edi,%edi
80100b63:	e9 8e 00 00 00       	jmp    80100bf6 <exec+0x146>
80100b68:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100b6f:	90                   	nop
    if(ph.type != ELF_PROG_LOAD)
80100b70:	83 bd 04 ff ff ff 01 	cmpl   $0x1,-0xfc(%ebp)
80100b77:	75 6c                	jne    80100be5 <exec+0x135>
    if(ph.memsz < ph.filesz)
80100b79:	8b 85 18 ff ff ff    	mov    -0xe8(%ebp),%eax
80100b7f:	3b 85 14 ff ff ff    	cmp    -0xec(%ebp),%eax
80100b85:	0f 82 87 00 00 00    	jb     80100c12 <exec+0x162>
    if(ph.vaddr + ph.memsz < ph.vaddr)
80100b8b:	03 85 0c ff ff ff    	add    -0xf4(%ebp),%eax
80100b91:	72 7f                	jb     80100c12 <exec+0x162>
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
80100b93:	83 ec 04             	sub    $0x4,%esp
80100b96:	50                   	push   %eax
80100b97:	ff b5 f0 fe ff ff    	push   -0x110(%ebp)
80100b9d:	ff b5 f4 fe ff ff    	push   -0x10c(%ebp)
80100ba3:	e8 18 6d 00 00       	call   801078c0 <allocuvm>
80100ba8:	83 c4 10             	add    $0x10,%esp
80100bab:	89 85 f0 fe ff ff    	mov    %eax,-0x110(%ebp)
80100bb1:	85 c0                	test   %eax,%eax
80100bb3:	74 5d                	je     80100c12 <exec+0x162>
    if(ph.vaddr % PGSIZE != 0)
80100bb5:	8b 85 0c ff ff ff    	mov    -0xf4(%ebp),%eax
80100bbb:	a9 ff 0f 00 00       	test   $0xfff,%eax
80100bc0:	75 50                	jne    80100c12 <exec+0x162>
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
80100bc2:	83 ec 0c             	sub    $0xc,%esp
80100bc5:	ff b5 14 ff ff ff    	push   -0xec(%ebp)
80100bcb:	ff b5 08 ff ff ff    	push   -0xf8(%ebp)
80100bd1:	53                   	push   %ebx
80100bd2:	50                   	push   %eax
80100bd3:	ff b5 f4 fe ff ff    	push   -0x10c(%ebp)
80100bd9:	e8 f2 6b 00 00       	call   801077d0 <loaduvm>
80100bde:	83 c4 20             	add    $0x20,%esp
80100be1:	85 c0                	test   %eax,%eax
80100be3:	78 2d                	js     80100c12 <exec+0x162>
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100be5:	0f b7 85 50 ff ff ff 	movzwl -0xb0(%ebp),%eax
80100bec:	83 c7 01             	add    $0x1,%edi
80100bef:	83 c6 20             	add    $0x20,%esi
80100bf2:	39 f8                	cmp    %edi,%eax
80100bf4:	7e 3a                	jle    80100c30 <exec+0x180>
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
80100bf6:	8d 85 04 ff ff ff    	lea    -0xfc(%ebp),%eax
80100bfc:	6a 20                	push   $0x20
80100bfe:	56                   	push   %esi
80100bff:	50                   	push   %eax
80100c00:	53                   	push   %ebx
80100c01:	e8 3a 0f 00 00       	call   80101b40 <readi>
80100c06:	83 c4 10             	add    $0x10,%esp
80100c09:	83 f8 20             	cmp    $0x20,%eax
80100c0c:	0f 84 5e ff ff ff    	je     80100b70 <exec+0xc0>
    freevm(pgdir);
80100c12:	83 ec 0c             	sub    $0xc,%esp
80100c15:	ff b5 f4 fe ff ff    	push   -0x10c(%ebp)
80100c1b:	e8 00 6e 00 00       	call   80107a20 <freevm>
  if(ip){
80100c20:	83 c4 10             	add    $0x10,%esp
80100c23:	e9 de fe ff ff       	jmp    80100b06 <exec+0x56>
80100c28:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100c2f:	90                   	nop
  sz = PGROUNDUP(sz);
80100c30:	8b bd f0 fe ff ff    	mov    -0x110(%ebp),%edi
80100c36:	81 c7 ff 0f 00 00    	add    $0xfff,%edi
80100c3c:	81 e7 00 f0 ff ff    	and    $0xfffff000,%edi
  if((sz = allocuvm(pgdir, sz, sz + 2*PGSIZE)) == 0)
80100c42:	8d b7 00 20 00 00    	lea    0x2000(%edi),%esi
  iunlockput(ip);
80100c48:	83 ec 0c             	sub    $0xc,%esp
80100c4b:	53                   	push   %ebx
80100c4c:	e8 6f 0e 00 00       	call   80101ac0 <iunlockput>
  end_op();
80100c51:	e8 2a 22 00 00       	call   80102e80 <end_op>
  if((sz = allocuvm(pgdir, sz, sz + 2*PGSIZE)) == 0)
80100c56:	83 c4 0c             	add    $0xc,%esp
80100c59:	56                   	push   %esi
80100c5a:	57                   	push   %edi
80100c5b:	8b bd f4 fe ff ff    	mov    -0x10c(%ebp),%edi
80100c61:	57                   	push   %edi
80100c62:	e8 59 6c 00 00       	call   801078c0 <allocuvm>
80100c67:	83 c4 10             	add    $0x10,%esp
80100c6a:	89 c6                	mov    %eax,%esi
80100c6c:	85 c0                	test   %eax,%eax
80100c6e:	0f 84 94 00 00 00    	je     80100d08 <exec+0x258>
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
80100c74:	83 ec 08             	sub    $0x8,%esp
80100c77:	8d 80 00 e0 ff ff    	lea    -0x2000(%eax),%eax
  for(argc = 0; argv[argc]; argc++) {
80100c7d:	89 f3                	mov    %esi,%ebx
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
80100c7f:	50                   	push   %eax
80100c80:	57                   	push   %edi
  for(argc = 0; argv[argc]; argc++) {
80100c81:	31 ff                	xor    %edi,%edi
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
80100c83:	e8 b8 6e 00 00       	call   80107b40 <clearpteu>
  for(argc = 0; argv[argc]; argc++) {
80100c88:	8b 45 0c             	mov    0xc(%ebp),%eax
80100c8b:	83 c4 10             	add    $0x10,%esp
80100c8e:	8d 95 58 ff ff ff    	lea    -0xa8(%ebp),%edx
80100c94:	8b 00                	mov    (%eax),%eax
80100c96:	85 c0                	test   %eax,%eax
80100c98:	0f 84 8b 00 00 00    	je     80100d29 <exec+0x279>
80100c9e:	89 b5 f0 fe ff ff    	mov    %esi,-0x110(%ebp)
80100ca4:	8b b5 f4 fe ff ff    	mov    -0x10c(%ebp),%esi
80100caa:	eb 23                	jmp    80100ccf <exec+0x21f>
80100cac:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80100cb0:	8b 45 0c             	mov    0xc(%ebp),%eax
    ustack[3+argc] = sp;
80100cb3:	89 9c bd 64 ff ff ff 	mov    %ebx,-0x9c(%ebp,%edi,4)
  for(argc = 0; argv[argc]; argc++) {
80100cba:	83 c7 01             	add    $0x1,%edi
    ustack[3+argc] = sp;
80100cbd:	8d 95 58 ff ff ff    	lea    -0xa8(%ebp),%edx
  for(argc = 0; argv[argc]; argc++) {
80100cc3:	8b 04 b8             	mov    (%eax,%edi,4),%eax
80100cc6:	85 c0                	test   %eax,%eax
80100cc8:	74 59                	je     80100d23 <exec+0x273>
    if(argc >= MAXARG)
80100cca:	83 ff 20             	cmp    $0x20,%edi
80100ccd:	74 39                	je     80100d08 <exec+0x258>
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
80100ccf:	83 ec 0c             	sub    $0xc,%esp
80100cd2:	50                   	push   %eax
80100cd3:	e8 c8 47 00 00       	call   801054a0 <strlen>
80100cd8:	29 c3                	sub    %eax,%ebx
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
80100cda:	58                   	pop    %eax
80100cdb:	8b 45 0c             	mov    0xc(%ebp),%eax
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
80100cde:	83 eb 01             	sub    $0x1,%ebx
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
80100ce1:	ff 34 b8             	push   (%eax,%edi,4)
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
80100ce4:	83 e3 fc             	and    $0xfffffffc,%ebx
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
80100ce7:	e8 b4 47 00 00       	call   801054a0 <strlen>
80100cec:	83 c0 01             	add    $0x1,%eax
80100cef:	50                   	push   %eax
80100cf0:	8b 45 0c             	mov    0xc(%ebp),%eax
80100cf3:	ff 34 b8             	push   (%eax,%edi,4)
80100cf6:	53                   	push   %ebx
80100cf7:	56                   	push   %esi
80100cf8:	e8 13 70 00 00       	call   80107d10 <copyout>
80100cfd:	83 c4 20             	add    $0x20,%esp
80100d00:	85 c0                	test   %eax,%eax
80100d02:	79 ac                	jns    80100cb0 <exec+0x200>
80100d04:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    freevm(pgdir);
80100d08:	83 ec 0c             	sub    $0xc,%esp
80100d0b:	ff b5 f4 fe ff ff    	push   -0x10c(%ebp)
80100d11:	e8 0a 6d 00 00       	call   80107a20 <freevm>
80100d16:	83 c4 10             	add    $0x10,%esp
  return -1;
80100d19:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80100d1e:	e9 f9 fd ff ff       	jmp    80100b1c <exec+0x6c>
80100d23:	8b b5 f0 fe ff ff    	mov    -0x110(%ebp),%esi
  ustack[2] = sp - (argc+1)*4;  // argv pointer
80100d29:	8d 04 bd 04 00 00 00 	lea    0x4(,%edi,4),%eax
80100d30:	89 d9                	mov    %ebx,%ecx
  ustack[1] = argc;
80100d32:	89 bd 5c ff ff ff    	mov    %edi,-0xa4(%ebp)
  ustack[3+argc] = 0;
80100d38:	c7 84 bd 64 ff ff ff 	movl   $0x0,-0x9c(%ebp,%edi,4)
80100d3f:	00 00 00 00 
  ustack[2] = sp - (argc+1)*4;  // argv pointer
80100d43:	29 c1                	sub    %eax,%ecx
  sp -= (3+argc+1) * 4;
80100d45:	89 df                	mov    %ebx,%edi
80100d47:	83 c0 0c             	add    $0xc,%eax
80100d4a:	29 c7                	sub    %eax,%edi
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
80100d4c:	50                   	push   %eax
80100d4d:	52                   	push   %edx
80100d4e:	57                   	push   %edi
80100d4f:	ff b5 f4 fe ff ff    	push   -0x10c(%ebp)
  ustack[0] = 0xffffffff;  // fake return PC
80100d55:	c7 85 58 ff ff ff ff 	movl   $0xffffffff,-0xa8(%ebp)
80100d5c:	ff ff ff 
  ustack[2] = sp - (argc+1)*4;  // argv pointer
80100d5f:	89 8d 60 ff ff ff    	mov    %ecx,-0xa0(%ebp)
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
80100d65:	e8 a6 6f 00 00       	call   80107d10 <copyout>
80100d6a:	83 c4 10             	add    $0x10,%esp
80100d6d:	85 c0                	test   %eax,%eax
80100d6f:	78 97                	js     80100d08 <exec+0x258>
  for(last=s=path; *s; s++)
80100d71:	8b 45 08             	mov    0x8(%ebp),%eax
80100d74:	8b 55 08             	mov    0x8(%ebp),%edx
80100d77:	0f b6 00             	movzbl (%eax),%eax
80100d7a:	84 c0                	test   %al,%al
80100d7c:	74 11                	je     80100d8f <exec+0x2df>
80100d7e:	89 d1                	mov    %edx,%ecx
      last = s+1;
80100d80:	83 c1 01             	add    $0x1,%ecx
80100d83:	3c 2f                	cmp    $0x2f,%al
  for(last=s=path; *s; s++)
80100d85:	0f b6 01             	movzbl (%ecx),%eax
      last = s+1;
80100d88:	0f 44 d1             	cmove  %ecx,%edx
  for(last=s=path; *s; s++)
80100d8b:	84 c0                	test   %al,%al
80100d8d:	75 f1                	jne    80100d80 <exec+0x2d0>
  safestrcpy(curproc->name, last, sizeof(curproc->name));
80100d8f:	8b 8d ec fe ff ff    	mov    -0x114(%ebp),%ecx
80100d95:	83 ec 04             	sub    $0x4,%esp
80100d98:	6a 10                	push   $0x10
80100d9a:	8d 59 6c             	lea    0x6c(%ecx),%ebx
80100d9d:	52                   	push   %edx
80100d9e:	53                   	push   %ebx
80100d9f:	e8 bc 46 00 00       	call   80105460 <safestrcpy>
  oldpgdir = curproc->pgdir;
80100da4:	8b 8d ec fe ff ff    	mov    -0x114(%ebp),%ecx
80100daa:	8b 41 04             	mov    0x4(%ecx),%eax
  curproc->tf->eip = elf.entry;  // main
80100dad:	8b 51 18             	mov    0x18(%ecx),%edx
  curproc->sz = sz;
80100db0:	89 31                	mov    %esi,(%ecx)
  oldpgdir = curproc->pgdir;
80100db2:	89 85 f0 fe ff ff    	mov    %eax,-0x110(%ebp)
  curproc->pgdir = pgdir;
80100db8:	8b 85 f4 fe ff ff    	mov    -0x10c(%ebp),%eax
80100dbe:	89 41 04             	mov    %eax,0x4(%ecx)
  curproc->tf->eip = elf.entry;  // main
80100dc1:	89 c8                	mov    %ecx,%eax
80100dc3:	8b 8d 3c ff ff ff    	mov    -0xc4(%ebp),%ecx
80100dc9:	89 4a 38             	mov    %ecx,0x38(%edx)
  curproc->tf->esp = sp;
80100dcc:	8b 50 18             	mov    0x18(%eax),%edx
80100dcf:	89 7a 44             	mov    %edi,0x44(%edx)
  switchuvm(curproc);
80100dd2:	89 04 24             	mov    %eax,(%esp)
80100dd5:	e8 66 68 00 00       	call   80107640 <switchuvm>
  freevm(oldpgdir);
80100dda:	8b 85 f0 fe ff ff    	mov    -0x110(%ebp),%eax
80100de0:	89 04 24             	mov    %eax,(%esp)
80100de3:	e8 38 6c 00 00       	call   80107a20 <freevm>
  if(strncmp(curproc->name,"sh",3) != 0  && strncmp(curproc->name ,"init",5) != 0 && curproc->queue != CLASS1){
80100de8:	83 c4 0c             	add    $0xc,%esp
80100deb:	6a 03                	push   $0x3
80100ded:	68 8d 7e 10 80       	push   $0x80107e8d
80100df2:	53                   	push   %ebx
80100df3:	e8 b8 45 00 00       	call   801053b0 <strncmp>
80100df8:	83 c4 10             	add    $0x10,%esp
80100dfb:	85 c0                	test   %eax,%eax
80100dfd:	75 07                	jne    80100e06 <exec+0x356>
  return 0;
80100dff:	31 c0                	xor    %eax,%eax
80100e01:	e9 16 fd ff ff       	jmp    80100b1c <exec+0x6c>
  if(strncmp(curproc->name,"sh",3) != 0  && strncmp(curproc->name ,"init",5) != 0 && curproc->queue != CLASS1){
80100e06:	83 ec 04             	sub    $0x4,%esp
80100e09:	6a 05                	push   $0x5
80100e0b:	68 90 7e 10 80       	push   $0x80107e90
80100e10:	53                   	push   %ebx
80100e11:	e8 9a 45 00 00       	call   801053b0 <strncmp>
80100e16:	83 c4 10             	add    $0x10,%esp
80100e19:	85 c0                	test   %eax,%eax
80100e1b:	74 e2                	je     80100dff <exec+0x34f>
80100e1d:	8b 85 ec fe ff ff    	mov    -0x114(%ebp),%eax
80100e23:	83 78 7c 00          	cmpl   $0x0,0x7c(%eax)
80100e27:	74 d6                	je     80100dff <exec+0x34f>
    if(curproc->state == RUNNABLE) {
80100e29:	83 78 0c 03          	cmpl   $0x3,0xc(%eax)
80100e2d:	74 54                	je     80100e83 <exec+0x3d3>
      curproc->queue = CLASS2_FCFS ;
80100e2f:	8b 85 ec fe ff ff    	mov    -0x114(%ebp),%eax
      cprintf("gaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaav");
80100e35:	83 ec 0c             	sub    $0xc,%esp
      curproc->queue = CLASS2_FCFS ;
80100e38:	c7 40 7c 02 00 00 00 	movl   $0x2,0x7c(%eax)
      cprintf("gaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaav");
80100e3f:	68 b4 7e 10 80       	push   $0x80107eb4
80100e44:	e8 57 f8 ff ff       	call   801006a0 <cprintf>
80100e49:	83 c4 10             	add    $0x10,%esp
  return 0;
80100e4c:	31 c0                	xor    %eax,%eax
80100e4e:	e9 c9 fc ff ff       	jmp    80100b1c <exec+0x6c>
80100e53:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80100e57:	90                   	nop
    end_op();
80100e58:	e8 23 20 00 00       	call   80102e80 <end_op>
    cprintf("exec: fail\n");
80100e5d:	83 ec 0c             	sub    $0xc,%esp
80100e60:	68 81 7e 10 80       	push   $0x80107e81
80100e65:	e8 36 f8 ff ff       	call   801006a0 <cprintf>
    return -1;
80100e6a:	83 c4 10             	add    $0x10,%esp
80100e6d:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80100e72:	e9 a5 fc ff ff       	jmp    80100b1c <exec+0x6c>
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100e77:	be 00 20 00 00       	mov    $0x2000,%esi
80100e7c:	31 ff                	xor    %edi,%edi
80100e7e:	e9 c5 fd ff ff       	jmp    80100c48 <exec+0x198>
      change_num(curproc, -1);
80100e83:	52                   	push   %edx
80100e84:	89 c6                	mov    %eax,%esi
80100e86:	52                   	push   %edx
80100e87:	6a ff                	push   $0xffffffff
80100e89:	50                   	push   %eax
80100e8a:	e8 01 2b 00 00       	call   80103990 <change_num>
      curproc->queue = CLASS2_FCFS ;
80100e8f:	c7 46 7c 02 00 00 00 	movl   $0x2,0x7c(%esi)
      change_num(curproc, 1);
80100e96:	59                   	pop    %ecx
80100e97:	5b                   	pop    %ebx
80100e98:	6a 01                	push   $0x1
80100e9a:	56                   	push   %esi
80100e9b:	e8 f0 2a 00 00       	call   80103990 <change_num>
      cprintf("kllllllllllllllllllllllllllf");
80100ea0:	c7 04 24 95 7e 10 80 	movl   $0x80107e95,(%esp)
80100ea7:	e8 f4 f7 ff ff       	call   801006a0 <cprintf>
80100eac:	83 c4 10             	add    $0x10,%esp
  return 0;
80100eaf:	31 c0                	xor    %eax,%eax
80100eb1:	e9 66 fc ff ff       	jmp    80100b1c <exec+0x6c>
80100eb6:	66 90                	xchg   %ax,%ax
80100eb8:	66 90                	xchg   %ax,%ax
80100eba:	66 90                	xchg   %ax,%ax
80100ebc:	66 90                	xchg   %ax,%ax
80100ebe:	66 90                	xchg   %ax,%ax

80100ec0 <fileinit>:
  struct file file[NFILE];
} ftable;

void
fileinit(void)
{
80100ec0:	55                   	push   %ebp
80100ec1:	89 e5                	mov    %esp,%ebp
80100ec3:	83 ec 10             	sub    $0x10,%esp
  initlock(&ftable.lock, "ftable");
80100ec6:	68 d7 7e 10 80       	push   $0x80107ed7
80100ecb:	68 60 ff 10 80       	push   $0x8010ff60
80100ed0:	e8 3b 41 00 00       	call   80105010 <initlock>
}
80100ed5:	83 c4 10             	add    $0x10,%esp
80100ed8:	c9                   	leave  
80100ed9:	c3                   	ret    
80100eda:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80100ee0 <filealloc>:

// Allocate a file structure.
struct file*
filealloc(void)
{
80100ee0:	55                   	push   %ebp
80100ee1:	89 e5                	mov    %esp,%ebp
80100ee3:	53                   	push   %ebx
  struct file *f;

  acquire(&ftable.lock);
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80100ee4:	bb 94 ff 10 80       	mov    $0x8010ff94,%ebx
{
80100ee9:	83 ec 10             	sub    $0x10,%esp
  acquire(&ftable.lock);
80100eec:	68 60 ff 10 80       	push   $0x8010ff60
80100ef1:	e8 ea 42 00 00       	call   801051e0 <acquire>
80100ef6:	83 c4 10             	add    $0x10,%esp
80100ef9:	eb 10                	jmp    80100f0b <filealloc+0x2b>
80100efb:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80100eff:	90                   	nop
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80100f00:	83 c3 18             	add    $0x18,%ebx
80100f03:	81 fb f4 08 11 80    	cmp    $0x801108f4,%ebx
80100f09:	74 25                	je     80100f30 <filealloc+0x50>
    if(f->ref == 0){
80100f0b:	8b 43 04             	mov    0x4(%ebx),%eax
80100f0e:	85 c0                	test   %eax,%eax
80100f10:	75 ee                	jne    80100f00 <filealloc+0x20>
      f->ref = 1;
      release(&ftable.lock);
80100f12:	83 ec 0c             	sub    $0xc,%esp
      f->ref = 1;
80100f15:	c7 43 04 01 00 00 00 	movl   $0x1,0x4(%ebx)
      release(&ftable.lock);
80100f1c:	68 60 ff 10 80       	push   $0x8010ff60
80100f21:	e8 5a 42 00 00       	call   80105180 <release>
      return f;
    }
  }
  release(&ftable.lock);
  return 0;
}
80100f26:	89 d8                	mov    %ebx,%eax
      return f;
80100f28:	83 c4 10             	add    $0x10,%esp
}
80100f2b:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80100f2e:	c9                   	leave  
80100f2f:	c3                   	ret    
  release(&ftable.lock);
80100f30:	83 ec 0c             	sub    $0xc,%esp
  return 0;
80100f33:	31 db                	xor    %ebx,%ebx
  release(&ftable.lock);
80100f35:	68 60 ff 10 80       	push   $0x8010ff60
80100f3a:	e8 41 42 00 00       	call   80105180 <release>
}
80100f3f:	89 d8                	mov    %ebx,%eax
  return 0;
80100f41:	83 c4 10             	add    $0x10,%esp
}
80100f44:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80100f47:	c9                   	leave  
80100f48:	c3                   	ret    
80100f49:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80100f50 <filedup>:

// Increment ref count for file f.
struct file*
filedup(struct file *f)
{
80100f50:	55                   	push   %ebp
80100f51:	89 e5                	mov    %esp,%ebp
80100f53:	53                   	push   %ebx
80100f54:	83 ec 10             	sub    $0x10,%esp
80100f57:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquire(&ftable.lock);
80100f5a:	68 60 ff 10 80       	push   $0x8010ff60
80100f5f:	e8 7c 42 00 00       	call   801051e0 <acquire>
  if(f->ref < 1)
80100f64:	8b 43 04             	mov    0x4(%ebx),%eax
80100f67:	83 c4 10             	add    $0x10,%esp
80100f6a:	85 c0                	test   %eax,%eax
80100f6c:	7e 1a                	jle    80100f88 <filedup+0x38>
    panic("filedup");
  f->ref++;
80100f6e:	83 c0 01             	add    $0x1,%eax
  release(&ftable.lock);
80100f71:	83 ec 0c             	sub    $0xc,%esp
  f->ref++;
80100f74:	89 43 04             	mov    %eax,0x4(%ebx)
  release(&ftable.lock);
80100f77:	68 60 ff 10 80       	push   $0x8010ff60
80100f7c:	e8 ff 41 00 00       	call   80105180 <release>
  return f;
}
80100f81:	89 d8                	mov    %ebx,%eax
80100f83:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80100f86:	c9                   	leave  
80100f87:	c3                   	ret    
    panic("filedup");
80100f88:	83 ec 0c             	sub    $0xc,%esp
80100f8b:	68 de 7e 10 80       	push   $0x80107ede
80100f90:	e8 eb f3 ff ff       	call   80100380 <panic>
80100f95:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100f9c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80100fa0 <fileclose>:

// Close file f.  (Decrement ref count, close when reaches 0.)
void
fileclose(struct file *f)
{
80100fa0:	55                   	push   %ebp
80100fa1:	89 e5                	mov    %esp,%ebp
80100fa3:	57                   	push   %edi
80100fa4:	56                   	push   %esi
80100fa5:	53                   	push   %ebx
80100fa6:	83 ec 28             	sub    $0x28,%esp
80100fa9:	8b 5d 08             	mov    0x8(%ebp),%ebx
  struct file ff;

  acquire(&ftable.lock);
80100fac:	68 60 ff 10 80       	push   $0x8010ff60
80100fb1:	e8 2a 42 00 00       	call   801051e0 <acquire>
  if(f->ref < 1)
80100fb6:	8b 53 04             	mov    0x4(%ebx),%edx
80100fb9:	83 c4 10             	add    $0x10,%esp
80100fbc:	85 d2                	test   %edx,%edx
80100fbe:	0f 8e a5 00 00 00    	jle    80101069 <fileclose+0xc9>
    panic("fileclose");
  if(--f->ref > 0){
80100fc4:	83 ea 01             	sub    $0x1,%edx
80100fc7:	89 53 04             	mov    %edx,0x4(%ebx)
80100fca:	75 44                	jne    80101010 <fileclose+0x70>
    release(&ftable.lock);
    return;
  }
  ff = *f;
80100fcc:	0f b6 43 09          	movzbl 0x9(%ebx),%eax
  f->ref = 0;
  f->type = FD_NONE;
  release(&ftable.lock);
80100fd0:	83 ec 0c             	sub    $0xc,%esp
  ff = *f;
80100fd3:	8b 3b                	mov    (%ebx),%edi
  f->type = FD_NONE;
80100fd5:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
  ff = *f;
80100fdb:	8b 73 0c             	mov    0xc(%ebx),%esi
80100fde:	88 45 e7             	mov    %al,-0x19(%ebp)
80100fe1:	8b 43 10             	mov    0x10(%ebx),%eax
  release(&ftable.lock);
80100fe4:	68 60 ff 10 80       	push   $0x8010ff60
  ff = *f;
80100fe9:	89 45 e0             	mov    %eax,-0x20(%ebp)
  release(&ftable.lock);
80100fec:	e8 8f 41 00 00       	call   80105180 <release>

  if(ff.type == FD_PIPE)
80100ff1:	83 c4 10             	add    $0x10,%esp
80100ff4:	83 ff 01             	cmp    $0x1,%edi
80100ff7:	74 57                	je     80101050 <fileclose+0xb0>
    pipeclose(ff.pipe, ff.writable);
  else if(ff.type == FD_INODE){
80100ff9:	83 ff 02             	cmp    $0x2,%edi
80100ffc:	74 2a                	je     80101028 <fileclose+0x88>
    begin_op();
    iput(ff.ip);
    end_op();
  }
}
80100ffe:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101001:	5b                   	pop    %ebx
80101002:	5e                   	pop    %esi
80101003:	5f                   	pop    %edi
80101004:	5d                   	pop    %ebp
80101005:	c3                   	ret    
80101006:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010100d:	8d 76 00             	lea    0x0(%esi),%esi
    release(&ftable.lock);
80101010:	c7 45 08 60 ff 10 80 	movl   $0x8010ff60,0x8(%ebp)
}
80101017:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010101a:	5b                   	pop    %ebx
8010101b:	5e                   	pop    %esi
8010101c:	5f                   	pop    %edi
8010101d:	5d                   	pop    %ebp
    release(&ftable.lock);
8010101e:	e9 5d 41 00 00       	jmp    80105180 <release>
80101023:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80101027:	90                   	nop
    begin_op();
80101028:	e8 e3 1d 00 00       	call   80102e10 <begin_op>
    iput(ff.ip);
8010102d:	83 ec 0c             	sub    $0xc,%esp
80101030:	ff 75 e0             	push   -0x20(%ebp)
80101033:	e8 28 09 00 00       	call   80101960 <iput>
    end_op();
80101038:	83 c4 10             	add    $0x10,%esp
}
8010103b:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010103e:	5b                   	pop    %ebx
8010103f:	5e                   	pop    %esi
80101040:	5f                   	pop    %edi
80101041:	5d                   	pop    %ebp
    end_op();
80101042:	e9 39 1e 00 00       	jmp    80102e80 <end_op>
80101047:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010104e:	66 90                	xchg   %ax,%ax
    pipeclose(ff.pipe, ff.writable);
80101050:	0f be 5d e7          	movsbl -0x19(%ebp),%ebx
80101054:	83 ec 08             	sub    $0x8,%esp
80101057:	53                   	push   %ebx
80101058:	56                   	push   %esi
80101059:	e8 82 25 00 00       	call   801035e0 <pipeclose>
8010105e:	83 c4 10             	add    $0x10,%esp
}
80101061:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101064:	5b                   	pop    %ebx
80101065:	5e                   	pop    %esi
80101066:	5f                   	pop    %edi
80101067:	5d                   	pop    %ebp
80101068:	c3                   	ret    
    panic("fileclose");
80101069:	83 ec 0c             	sub    $0xc,%esp
8010106c:	68 e6 7e 10 80       	push   $0x80107ee6
80101071:	e8 0a f3 ff ff       	call   80100380 <panic>
80101076:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010107d:	8d 76 00             	lea    0x0(%esi),%esi

80101080 <filestat>:

// Get metadata about file f.
int
filestat(struct file *f, struct stat *st)
{
80101080:	55                   	push   %ebp
80101081:	89 e5                	mov    %esp,%ebp
80101083:	53                   	push   %ebx
80101084:	83 ec 04             	sub    $0x4,%esp
80101087:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(f->type == FD_INODE){
8010108a:	83 3b 02             	cmpl   $0x2,(%ebx)
8010108d:	75 31                	jne    801010c0 <filestat+0x40>
    ilock(f->ip);
8010108f:	83 ec 0c             	sub    $0xc,%esp
80101092:	ff 73 10             	push   0x10(%ebx)
80101095:	e8 96 07 00 00       	call   80101830 <ilock>
    stati(f->ip, st);
8010109a:	58                   	pop    %eax
8010109b:	5a                   	pop    %edx
8010109c:	ff 75 0c             	push   0xc(%ebp)
8010109f:	ff 73 10             	push   0x10(%ebx)
801010a2:	e8 69 0a 00 00       	call   80101b10 <stati>
    iunlock(f->ip);
801010a7:	59                   	pop    %ecx
801010a8:	ff 73 10             	push   0x10(%ebx)
801010ab:	e8 60 08 00 00       	call   80101910 <iunlock>
    return 0;
  }
  return -1;
}
801010b0:	8b 5d fc             	mov    -0x4(%ebp),%ebx
    return 0;
801010b3:	83 c4 10             	add    $0x10,%esp
801010b6:	31 c0                	xor    %eax,%eax
}
801010b8:	c9                   	leave  
801010b9:	c3                   	ret    
801010ba:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
801010c0:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  return -1;
801010c3:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801010c8:	c9                   	leave  
801010c9:	c3                   	ret    
801010ca:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

801010d0 <fileread>:

// Read from file f.
int
fileread(struct file *f, char *addr, int n)
{
801010d0:	55                   	push   %ebp
801010d1:	89 e5                	mov    %esp,%ebp
801010d3:	57                   	push   %edi
801010d4:	56                   	push   %esi
801010d5:	53                   	push   %ebx
801010d6:	83 ec 0c             	sub    $0xc,%esp
801010d9:	8b 5d 08             	mov    0x8(%ebp),%ebx
801010dc:	8b 75 0c             	mov    0xc(%ebp),%esi
801010df:	8b 7d 10             	mov    0x10(%ebp),%edi
  int r;

  if(f->readable == 0)
801010e2:	80 7b 08 00          	cmpb   $0x0,0x8(%ebx)
801010e6:	74 60                	je     80101148 <fileread+0x78>
    return -1;
  if(f->type == FD_PIPE)
801010e8:	8b 03                	mov    (%ebx),%eax
801010ea:	83 f8 01             	cmp    $0x1,%eax
801010ed:	74 41                	je     80101130 <fileread+0x60>
    return piperead(f->pipe, addr, n);
  if(f->type == FD_INODE){
801010ef:	83 f8 02             	cmp    $0x2,%eax
801010f2:	75 5b                	jne    8010114f <fileread+0x7f>
    ilock(f->ip);
801010f4:	83 ec 0c             	sub    $0xc,%esp
801010f7:	ff 73 10             	push   0x10(%ebx)
801010fa:	e8 31 07 00 00       	call   80101830 <ilock>
    if((r = readi(f->ip, addr, f->off, n)) > 0)
801010ff:	57                   	push   %edi
80101100:	ff 73 14             	push   0x14(%ebx)
80101103:	56                   	push   %esi
80101104:	ff 73 10             	push   0x10(%ebx)
80101107:	e8 34 0a 00 00       	call   80101b40 <readi>
8010110c:	83 c4 20             	add    $0x20,%esp
8010110f:	89 c6                	mov    %eax,%esi
80101111:	85 c0                	test   %eax,%eax
80101113:	7e 03                	jle    80101118 <fileread+0x48>
      f->off += r;
80101115:	01 43 14             	add    %eax,0x14(%ebx)
    iunlock(f->ip);
80101118:	83 ec 0c             	sub    $0xc,%esp
8010111b:	ff 73 10             	push   0x10(%ebx)
8010111e:	e8 ed 07 00 00       	call   80101910 <iunlock>
    return r;
80101123:	83 c4 10             	add    $0x10,%esp
  }
  panic("fileread");
}
80101126:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101129:	89 f0                	mov    %esi,%eax
8010112b:	5b                   	pop    %ebx
8010112c:	5e                   	pop    %esi
8010112d:	5f                   	pop    %edi
8010112e:	5d                   	pop    %ebp
8010112f:	c3                   	ret    
    return piperead(f->pipe, addr, n);
80101130:	8b 43 0c             	mov    0xc(%ebx),%eax
80101133:	89 45 08             	mov    %eax,0x8(%ebp)
}
80101136:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101139:	5b                   	pop    %ebx
8010113a:	5e                   	pop    %esi
8010113b:	5f                   	pop    %edi
8010113c:	5d                   	pop    %ebp
    return piperead(f->pipe, addr, n);
8010113d:	e9 3e 26 00 00       	jmp    80103780 <piperead>
80101142:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    return -1;
80101148:	be ff ff ff ff       	mov    $0xffffffff,%esi
8010114d:	eb d7                	jmp    80101126 <fileread+0x56>
  panic("fileread");
8010114f:	83 ec 0c             	sub    $0xc,%esp
80101152:	68 f0 7e 10 80       	push   $0x80107ef0
80101157:	e8 24 f2 ff ff       	call   80100380 <panic>
8010115c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80101160 <filewrite>:

//PAGEBREAK!
// Write to file f.
int
filewrite(struct file *f, char *addr, int n)
{
80101160:	55                   	push   %ebp
80101161:	89 e5                	mov    %esp,%ebp
80101163:	57                   	push   %edi
80101164:	56                   	push   %esi
80101165:	53                   	push   %ebx
80101166:	83 ec 1c             	sub    $0x1c,%esp
80101169:	8b 45 0c             	mov    0xc(%ebp),%eax
8010116c:	8b 5d 08             	mov    0x8(%ebp),%ebx
8010116f:	89 45 dc             	mov    %eax,-0x24(%ebp)
80101172:	8b 45 10             	mov    0x10(%ebp),%eax
  int r;

  if(f->writable == 0)
80101175:	80 7b 09 00          	cmpb   $0x0,0x9(%ebx)
{
80101179:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  if(f->writable == 0)
8010117c:	0f 84 bd 00 00 00    	je     8010123f <filewrite+0xdf>
    return -1;
  if(f->type == FD_PIPE)
80101182:	8b 03                	mov    (%ebx),%eax
80101184:	83 f8 01             	cmp    $0x1,%eax
80101187:	0f 84 bf 00 00 00    	je     8010124c <filewrite+0xec>
    return pipewrite(f->pipe, addr, n);
  if(f->type == FD_INODE){
8010118d:	83 f8 02             	cmp    $0x2,%eax
80101190:	0f 85 c8 00 00 00    	jne    8010125e <filewrite+0xfe>
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((MAXOPBLOCKS-1-1-2) / 2) * 512;
    int i = 0;
    while(i < n){
80101196:	8b 45 e4             	mov    -0x1c(%ebp),%eax
    int i = 0;
80101199:	31 f6                	xor    %esi,%esi
    while(i < n){
8010119b:	85 c0                	test   %eax,%eax
8010119d:	7f 30                	jg     801011cf <filewrite+0x6f>
8010119f:	e9 94 00 00 00       	jmp    80101238 <filewrite+0xd8>
801011a4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
        n1 = max;

      begin_op();
      ilock(f->ip);
      if ((r = writei(f->ip, addr + i, f->off, n1)) > 0)
        f->off += r;
801011a8:	01 43 14             	add    %eax,0x14(%ebx)
      iunlock(f->ip);
801011ab:	83 ec 0c             	sub    $0xc,%esp
801011ae:	ff 73 10             	push   0x10(%ebx)
        f->off += r;
801011b1:	89 45 e0             	mov    %eax,-0x20(%ebp)
      iunlock(f->ip);
801011b4:	e8 57 07 00 00       	call   80101910 <iunlock>
      end_op();
801011b9:	e8 c2 1c 00 00       	call   80102e80 <end_op>

      if(r < 0)
        break;
      if(r != n1)
801011be:	8b 45 e0             	mov    -0x20(%ebp),%eax
801011c1:	83 c4 10             	add    $0x10,%esp
801011c4:	39 c7                	cmp    %eax,%edi
801011c6:	75 5c                	jne    80101224 <filewrite+0xc4>
        panic("short filewrite");
      i += r;
801011c8:	01 fe                	add    %edi,%esi
    while(i < n){
801011ca:	39 75 e4             	cmp    %esi,-0x1c(%ebp)
801011cd:	7e 69                	jle    80101238 <filewrite+0xd8>
      int n1 = n - i;
801011cf:	8b 7d e4             	mov    -0x1c(%ebp),%edi
801011d2:	b8 00 06 00 00       	mov    $0x600,%eax
801011d7:	29 f7                	sub    %esi,%edi
801011d9:	39 c7                	cmp    %eax,%edi
801011db:	0f 4f f8             	cmovg  %eax,%edi
      begin_op();
801011de:	e8 2d 1c 00 00       	call   80102e10 <begin_op>
      ilock(f->ip);
801011e3:	83 ec 0c             	sub    $0xc,%esp
801011e6:	ff 73 10             	push   0x10(%ebx)
801011e9:	e8 42 06 00 00       	call   80101830 <ilock>
      if ((r = writei(f->ip, addr + i, f->off, n1)) > 0)
801011ee:	8b 45 dc             	mov    -0x24(%ebp),%eax
801011f1:	57                   	push   %edi
801011f2:	ff 73 14             	push   0x14(%ebx)
801011f5:	01 f0                	add    %esi,%eax
801011f7:	50                   	push   %eax
801011f8:	ff 73 10             	push   0x10(%ebx)
801011fb:	e8 40 0a 00 00       	call   80101c40 <writei>
80101200:	83 c4 20             	add    $0x20,%esp
80101203:	85 c0                	test   %eax,%eax
80101205:	7f a1                	jg     801011a8 <filewrite+0x48>
      iunlock(f->ip);
80101207:	83 ec 0c             	sub    $0xc,%esp
8010120a:	ff 73 10             	push   0x10(%ebx)
8010120d:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80101210:	e8 fb 06 00 00       	call   80101910 <iunlock>
      end_op();
80101215:	e8 66 1c 00 00       	call   80102e80 <end_op>
      if(r < 0)
8010121a:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010121d:	83 c4 10             	add    $0x10,%esp
80101220:	85 c0                	test   %eax,%eax
80101222:	75 1b                	jne    8010123f <filewrite+0xdf>
        panic("short filewrite");
80101224:	83 ec 0c             	sub    $0xc,%esp
80101227:	68 f9 7e 10 80       	push   $0x80107ef9
8010122c:	e8 4f f1 ff ff       	call   80100380 <panic>
80101231:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    }
    return i == n ? n : -1;
80101238:	89 f0                	mov    %esi,%eax
8010123a:	3b 75 e4             	cmp    -0x1c(%ebp),%esi
8010123d:	74 05                	je     80101244 <filewrite+0xe4>
8010123f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
  }
  panic("filewrite");
}
80101244:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101247:	5b                   	pop    %ebx
80101248:	5e                   	pop    %esi
80101249:	5f                   	pop    %edi
8010124a:	5d                   	pop    %ebp
8010124b:	c3                   	ret    
    return pipewrite(f->pipe, addr, n);
8010124c:	8b 43 0c             	mov    0xc(%ebx),%eax
8010124f:	89 45 08             	mov    %eax,0x8(%ebp)
}
80101252:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101255:	5b                   	pop    %ebx
80101256:	5e                   	pop    %esi
80101257:	5f                   	pop    %edi
80101258:	5d                   	pop    %ebp
    return pipewrite(f->pipe, addr, n);
80101259:	e9 22 24 00 00       	jmp    80103680 <pipewrite>
  panic("filewrite");
8010125e:	83 ec 0c             	sub    $0xc,%esp
80101261:	68 ff 7e 10 80       	push   $0x80107eff
80101266:	e8 15 f1 ff ff       	call   80100380 <panic>
8010126b:	66 90                	xchg   %ax,%ax
8010126d:	66 90                	xchg   %ax,%ax
8010126f:	90                   	nop

80101270 <bfree>:
}

// Free a disk block.
static void
bfree(int dev, uint b)
{
80101270:	55                   	push   %ebp
80101271:	89 c1                	mov    %eax,%ecx
  struct buf *bp;
  int bi, m;

  bp = bread(dev, BBLOCK(b, sb));
80101273:	89 d0                	mov    %edx,%eax
80101275:	c1 e8 0c             	shr    $0xc,%eax
80101278:	03 05 cc 25 11 80    	add    0x801125cc,%eax
{
8010127e:	89 e5                	mov    %esp,%ebp
80101280:	56                   	push   %esi
80101281:	53                   	push   %ebx
80101282:	89 d3                	mov    %edx,%ebx
  bp = bread(dev, BBLOCK(b, sb));
80101284:	83 ec 08             	sub    $0x8,%esp
80101287:	50                   	push   %eax
80101288:	51                   	push   %ecx
80101289:	e8 42 ee ff ff       	call   801000d0 <bread>
  bi = b % BPB;
  m = 1 << (bi % 8);
8010128e:	89 d9                	mov    %ebx,%ecx
  if((bp->data[bi/8] & m) == 0)
80101290:	c1 fb 03             	sar    $0x3,%ebx
80101293:	83 c4 10             	add    $0x10,%esp
  bp = bread(dev, BBLOCK(b, sb));
80101296:	89 c6                	mov    %eax,%esi
  m = 1 << (bi % 8);
80101298:	83 e1 07             	and    $0x7,%ecx
8010129b:	b8 01 00 00 00       	mov    $0x1,%eax
  if((bp->data[bi/8] & m) == 0)
801012a0:	81 e3 ff 01 00 00    	and    $0x1ff,%ebx
  m = 1 << (bi % 8);
801012a6:	d3 e0                	shl    %cl,%eax
  if((bp->data[bi/8] & m) == 0)
801012a8:	0f b6 4c 1e 5c       	movzbl 0x5c(%esi,%ebx,1),%ecx
801012ad:	85 c1                	test   %eax,%ecx
801012af:	74 23                	je     801012d4 <bfree+0x64>
    panic("freeing free block");
  bp->data[bi/8] &= ~m;
801012b1:	f7 d0                	not    %eax
  log_write(bp);
801012b3:	83 ec 0c             	sub    $0xc,%esp
  bp->data[bi/8] &= ~m;
801012b6:	21 c8                	and    %ecx,%eax
801012b8:	88 44 1e 5c          	mov    %al,0x5c(%esi,%ebx,1)
  log_write(bp);
801012bc:	56                   	push   %esi
801012bd:	e8 2e 1d 00 00       	call   80102ff0 <log_write>
  brelse(bp);
801012c2:	89 34 24             	mov    %esi,(%esp)
801012c5:	e8 26 ef ff ff       	call   801001f0 <brelse>
}
801012ca:	83 c4 10             	add    $0x10,%esp
801012cd:	8d 65 f8             	lea    -0x8(%ebp),%esp
801012d0:	5b                   	pop    %ebx
801012d1:	5e                   	pop    %esi
801012d2:	5d                   	pop    %ebp
801012d3:	c3                   	ret    
    panic("freeing free block");
801012d4:	83 ec 0c             	sub    $0xc,%esp
801012d7:	68 09 7f 10 80       	push   $0x80107f09
801012dc:	e8 9f f0 ff ff       	call   80100380 <panic>
801012e1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801012e8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801012ef:	90                   	nop

801012f0 <balloc>:
{
801012f0:	55                   	push   %ebp
801012f1:	89 e5                	mov    %esp,%ebp
801012f3:	57                   	push   %edi
801012f4:	56                   	push   %esi
801012f5:	53                   	push   %ebx
801012f6:	83 ec 1c             	sub    $0x1c,%esp
  for(b = 0; b < sb.size; b += BPB){
801012f9:	8b 0d b4 25 11 80    	mov    0x801125b4,%ecx
{
801012ff:	89 45 d8             	mov    %eax,-0x28(%ebp)
  for(b = 0; b < sb.size; b += BPB){
80101302:	85 c9                	test   %ecx,%ecx
80101304:	0f 84 87 00 00 00    	je     80101391 <balloc+0xa1>
8010130a:	c7 45 dc 00 00 00 00 	movl   $0x0,-0x24(%ebp)
    bp = bread(dev, BBLOCK(b, sb));
80101311:	8b 75 dc             	mov    -0x24(%ebp),%esi
80101314:	83 ec 08             	sub    $0x8,%esp
80101317:	89 f0                	mov    %esi,%eax
80101319:	c1 f8 0c             	sar    $0xc,%eax
8010131c:	03 05 cc 25 11 80    	add    0x801125cc,%eax
80101322:	50                   	push   %eax
80101323:	ff 75 d8             	push   -0x28(%ebp)
80101326:	e8 a5 ed ff ff       	call   801000d0 <bread>
8010132b:	83 c4 10             	add    $0x10,%esp
8010132e:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
80101331:	a1 b4 25 11 80       	mov    0x801125b4,%eax
80101336:	89 45 e0             	mov    %eax,-0x20(%ebp)
80101339:	31 c0                	xor    %eax,%eax
8010133b:	eb 2f                	jmp    8010136c <balloc+0x7c>
8010133d:	8d 76 00             	lea    0x0(%esi),%esi
      m = 1 << (bi % 8);
80101340:	89 c1                	mov    %eax,%ecx
80101342:	bb 01 00 00 00       	mov    $0x1,%ebx
      if((bp->data[bi/8] & m) == 0){  // Is block free?
80101347:	8b 55 e4             	mov    -0x1c(%ebp),%edx
      m = 1 << (bi % 8);
8010134a:	83 e1 07             	and    $0x7,%ecx
8010134d:	d3 e3                	shl    %cl,%ebx
      if((bp->data[bi/8] & m) == 0){  // Is block free?
8010134f:	89 c1                	mov    %eax,%ecx
80101351:	c1 f9 03             	sar    $0x3,%ecx
80101354:	0f b6 7c 0a 5c       	movzbl 0x5c(%edx,%ecx,1),%edi
80101359:	89 fa                	mov    %edi,%edx
8010135b:	85 df                	test   %ebx,%edi
8010135d:	74 41                	je     801013a0 <balloc+0xb0>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
8010135f:	83 c0 01             	add    $0x1,%eax
80101362:	83 c6 01             	add    $0x1,%esi
80101365:	3d 00 10 00 00       	cmp    $0x1000,%eax
8010136a:	74 05                	je     80101371 <balloc+0x81>
8010136c:	39 75 e0             	cmp    %esi,-0x20(%ebp)
8010136f:	77 cf                	ja     80101340 <balloc+0x50>
    brelse(bp);
80101371:	83 ec 0c             	sub    $0xc,%esp
80101374:	ff 75 e4             	push   -0x1c(%ebp)
80101377:	e8 74 ee ff ff       	call   801001f0 <brelse>
  for(b = 0; b < sb.size; b += BPB){
8010137c:	81 45 dc 00 10 00 00 	addl   $0x1000,-0x24(%ebp)
80101383:	83 c4 10             	add    $0x10,%esp
80101386:	8b 45 dc             	mov    -0x24(%ebp),%eax
80101389:	39 05 b4 25 11 80    	cmp    %eax,0x801125b4
8010138f:	77 80                	ja     80101311 <balloc+0x21>
  panic("balloc: out of blocks");
80101391:	83 ec 0c             	sub    $0xc,%esp
80101394:	68 1c 7f 10 80       	push   $0x80107f1c
80101399:	e8 e2 ef ff ff       	call   80100380 <panic>
8010139e:	66 90                	xchg   %ax,%ax
        bp->data[bi/8] |= m;  // Mark block in use.
801013a0:	8b 7d e4             	mov    -0x1c(%ebp),%edi
        log_write(bp);
801013a3:	83 ec 0c             	sub    $0xc,%esp
        bp->data[bi/8] |= m;  // Mark block in use.
801013a6:	09 da                	or     %ebx,%edx
801013a8:	88 54 0f 5c          	mov    %dl,0x5c(%edi,%ecx,1)
        log_write(bp);
801013ac:	57                   	push   %edi
801013ad:	e8 3e 1c 00 00       	call   80102ff0 <log_write>
        brelse(bp);
801013b2:	89 3c 24             	mov    %edi,(%esp)
801013b5:	e8 36 ee ff ff       	call   801001f0 <brelse>
  bp = bread(dev, bno);
801013ba:	58                   	pop    %eax
801013bb:	5a                   	pop    %edx
801013bc:	56                   	push   %esi
801013bd:	ff 75 d8             	push   -0x28(%ebp)
801013c0:	e8 0b ed ff ff       	call   801000d0 <bread>
  memset(bp->data, 0, BSIZE);
801013c5:	83 c4 0c             	add    $0xc,%esp
  bp = bread(dev, bno);
801013c8:	89 c3                	mov    %eax,%ebx
  memset(bp->data, 0, BSIZE);
801013ca:	8d 40 5c             	lea    0x5c(%eax),%eax
801013cd:	68 00 02 00 00       	push   $0x200
801013d2:	6a 00                	push   $0x0
801013d4:	50                   	push   %eax
801013d5:	e8 c6 3e 00 00       	call   801052a0 <memset>
  log_write(bp);
801013da:	89 1c 24             	mov    %ebx,(%esp)
801013dd:	e8 0e 1c 00 00       	call   80102ff0 <log_write>
  brelse(bp);
801013e2:	89 1c 24             	mov    %ebx,(%esp)
801013e5:	e8 06 ee ff ff       	call   801001f0 <brelse>
}
801013ea:	8d 65 f4             	lea    -0xc(%ebp),%esp
801013ed:	89 f0                	mov    %esi,%eax
801013ef:	5b                   	pop    %ebx
801013f0:	5e                   	pop    %esi
801013f1:	5f                   	pop    %edi
801013f2:	5d                   	pop    %ebp
801013f3:	c3                   	ret    
801013f4:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801013fb:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801013ff:	90                   	nop

80101400 <iget>:
// Find the inode with number inum on device dev
// and return the in-memory copy. Does not lock
// the inode and does not read it from disk.
static struct inode*
iget(uint dev, uint inum)
{
80101400:	55                   	push   %ebp
80101401:	89 e5                	mov    %esp,%ebp
80101403:	57                   	push   %edi
80101404:	89 c7                	mov    %eax,%edi
80101406:	56                   	push   %esi
  struct inode *ip, *empty;

  acquire(&icache.lock);

  // Is the inode already cached?
  empty = 0;
80101407:	31 f6                	xor    %esi,%esi
{
80101409:	53                   	push   %ebx
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
8010140a:	bb 94 09 11 80       	mov    $0x80110994,%ebx
{
8010140f:	83 ec 28             	sub    $0x28,%esp
80101412:	89 55 e4             	mov    %edx,-0x1c(%ebp)
  acquire(&icache.lock);
80101415:	68 60 09 11 80       	push   $0x80110960
8010141a:	e8 c1 3d 00 00       	call   801051e0 <acquire>
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
8010141f:	8b 55 e4             	mov    -0x1c(%ebp),%edx
  acquire(&icache.lock);
80101422:	83 c4 10             	add    $0x10,%esp
80101425:	eb 1b                	jmp    80101442 <iget+0x42>
80101427:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010142e:	66 90                	xchg   %ax,%ax
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
80101430:	39 3b                	cmp    %edi,(%ebx)
80101432:	74 6c                	je     801014a0 <iget+0xa0>
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
80101434:	81 c3 90 00 00 00    	add    $0x90,%ebx
8010143a:	81 fb b4 25 11 80    	cmp    $0x801125b4,%ebx
80101440:	73 26                	jae    80101468 <iget+0x68>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
80101442:	8b 43 08             	mov    0x8(%ebx),%eax
80101445:	85 c0                	test   %eax,%eax
80101447:	7f e7                	jg     80101430 <iget+0x30>
      ip->ref++;
      release(&icache.lock);
      return ip;
    }
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
80101449:	85 f6                	test   %esi,%esi
8010144b:	75 e7                	jne    80101434 <iget+0x34>
8010144d:	85 c0                	test   %eax,%eax
8010144f:	75 76                	jne    801014c7 <iget+0xc7>
80101451:	89 de                	mov    %ebx,%esi
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
80101453:	81 c3 90 00 00 00    	add    $0x90,%ebx
80101459:	81 fb b4 25 11 80    	cmp    $0x801125b4,%ebx
8010145f:	72 e1                	jb     80101442 <iget+0x42>
80101461:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
      empty = ip;
  }

  // Recycle an inode cache entry.
  if(empty == 0)
80101468:	85 f6                	test   %esi,%esi
8010146a:	74 79                	je     801014e5 <iget+0xe5>
  ip = empty;
  ip->dev = dev;
  ip->inum = inum;
  ip->ref = 1;
  ip->valid = 0;
  release(&icache.lock);
8010146c:	83 ec 0c             	sub    $0xc,%esp
  ip->dev = dev;
8010146f:	89 3e                	mov    %edi,(%esi)
  ip->inum = inum;
80101471:	89 56 04             	mov    %edx,0x4(%esi)
  ip->ref = 1;
80101474:	c7 46 08 01 00 00 00 	movl   $0x1,0x8(%esi)
  ip->valid = 0;
8010147b:	c7 46 4c 00 00 00 00 	movl   $0x0,0x4c(%esi)
  release(&icache.lock);
80101482:	68 60 09 11 80       	push   $0x80110960
80101487:	e8 f4 3c 00 00       	call   80105180 <release>

  return ip;
8010148c:	83 c4 10             	add    $0x10,%esp
}
8010148f:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101492:	89 f0                	mov    %esi,%eax
80101494:	5b                   	pop    %ebx
80101495:	5e                   	pop    %esi
80101496:	5f                   	pop    %edi
80101497:	5d                   	pop    %ebp
80101498:	c3                   	ret    
80101499:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
801014a0:	39 53 04             	cmp    %edx,0x4(%ebx)
801014a3:	75 8f                	jne    80101434 <iget+0x34>
      release(&icache.lock);
801014a5:	83 ec 0c             	sub    $0xc,%esp
      ip->ref++;
801014a8:	83 c0 01             	add    $0x1,%eax
      return ip;
801014ab:	89 de                	mov    %ebx,%esi
      release(&icache.lock);
801014ad:	68 60 09 11 80       	push   $0x80110960
      ip->ref++;
801014b2:	89 43 08             	mov    %eax,0x8(%ebx)
      release(&icache.lock);
801014b5:	e8 c6 3c 00 00       	call   80105180 <release>
      return ip;
801014ba:	83 c4 10             	add    $0x10,%esp
}
801014bd:	8d 65 f4             	lea    -0xc(%ebp),%esp
801014c0:	89 f0                	mov    %esi,%eax
801014c2:	5b                   	pop    %ebx
801014c3:	5e                   	pop    %esi
801014c4:	5f                   	pop    %edi
801014c5:	5d                   	pop    %ebp
801014c6:	c3                   	ret    
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
801014c7:	81 c3 90 00 00 00    	add    $0x90,%ebx
801014cd:	81 fb b4 25 11 80    	cmp    $0x801125b4,%ebx
801014d3:	73 10                	jae    801014e5 <iget+0xe5>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
801014d5:	8b 43 08             	mov    0x8(%ebx),%eax
801014d8:	85 c0                	test   %eax,%eax
801014da:	0f 8f 50 ff ff ff    	jg     80101430 <iget+0x30>
801014e0:	e9 68 ff ff ff       	jmp    8010144d <iget+0x4d>
    panic("iget: no inodes");
801014e5:	83 ec 0c             	sub    $0xc,%esp
801014e8:	68 32 7f 10 80       	push   $0x80107f32
801014ed:	e8 8e ee ff ff       	call   80100380 <panic>
801014f2:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801014f9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80101500 <bmap>:

// Return the disk block address of the nth block in inode ip.
// If there is no such block, bmap allocates one.
static uint
bmap(struct inode *ip, uint bn)
{
80101500:	55                   	push   %ebp
80101501:	89 e5                	mov    %esp,%ebp
80101503:	57                   	push   %edi
80101504:	56                   	push   %esi
80101505:	89 c6                	mov    %eax,%esi
80101507:	53                   	push   %ebx
80101508:	83 ec 1c             	sub    $0x1c,%esp
  uint addr, *a;
  struct buf *bp;

  if(bn < NDIRECT){
8010150b:	83 fa 0b             	cmp    $0xb,%edx
8010150e:	0f 86 8c 00 00 00    	jbe    801015a0 <bmap+0xa0>
    if((addr = ip->addrs[bn]) == 0)
      ip->addrs[bn] = addr = balloc(ip->dev);
    return addr;
  }
  bn -= NDIRECT;
80101514:	8d 5a f4             	lea    -0xc(%edx),%ebx

  if(bn < NINDIRECT){
80101517:	83 fb 7f             	cmp    $0x7f,%ebx
8010151a:	0f 87 a2 00 00 00    	ja     801015c2 <bmap+0xc2>
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0)
80101520:	8b 80 8c 00 00 00    	mov    0x8c(%eax),%eax
80101526:	85 c0                	test   %eax,%eax
80101528:	74 5e                	je     80101588 <bmap+0x88>
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
    bp = bread(ip->dev, addr);
8010152a:	83 ec 08             	sub    $0x8,%esp
8010152d:	50                   	push   %eax
8010152e:	ff 36                	push   (%esi)
80101530:	e8 9b eb ff ff       	call   801000d0 <bread>
    a = (uint*)bp->data;
    if((addr = a[bn]) == 0){
80101535:	83 c4 10             	add    $0x10,%esp
80101538:	8d 5c 98 5c          	lea    0x5c(%eax,%ebx,4),%ebx
    bp = bread(ip->dev, addr);
8010153c:	89 c2                	mov    %eax,%edx
    if((addr = a[bn]) == 0){
8010153e:	8b 3b                	mov    (%ebx),%edi
80101540:	85 ff                	test   %edi,%edi
80101542:	74 1c                	je     80101560 <bmap+0x60>
      a[bn] = addr = balloc(ip->dev);
      log_write(bp);
    }
    brelse(bp);
80101544:	83 ec 0c             	sub    $0xc,%esp
80101547:	52                   	push   %edx
80101548:	e8 a3 ec ff ff       	call   801001f0 <brelse>
8010154d:	83 c4 10             	add    $0x10,%esp
    return addr;
  }

  panic("bmap: out of range");
}
80101550:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101553:	89 f8                	mov    %edi,%eax
80101555:	5b                   	pop    %ebx
80101556:	5e                   	pop    %esi
80101557:	5f                   	pop    %edi
80101558:	5d                   	pop    %ebp
80101559:	c3                   	ret    
8010155a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80101560:	89 45 e4             	mov    %eax,-0x1c(%ebp)
      a[bn] = addr = balloc(ip->dev);
80101563:	8b 06                	mov    (%esi),%eax
80101565:	e8 86 fd ff ff       	call   801012f0 <balloc>
      log_write(bp);
8010156a:	8b 55 e4             	mov    -0x1c(%ebp),%edx
8010156d:	83 ec 0c             	sub    $0xc,%esp
      a[bn] = addr = balloc(ip->dev);
80101570:	89 03                	mov    %eax,(%ebx)
80101572:	89 c7                	mov    %eax,%edi
      log_write(bp);
80101574:	52                   	push   %edx
80101575:	e8 76 1a 00 00       	call   80102ff0 <log_write>
8010157a:	8b 55 e4             	mov    -0x1c(%ebp),%edx
8010157d:	83 c4 10             	add    $0x10,%esp
80101580:	eb c2                	jmp    80101544 <bmap+0x44>
80101582:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
80101588:	8b 06                	mov    (%esi),%eax
8010158a:	e8 61 fd ff ff       	call   801012f0 <balloc>
8010158f:	89 86 8c 00 00 00    	mov    %eax,0x8c(%esi)
80101595:	eb 93                	jmp    8010152a <bmap+0x2a>
80101597:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010159e:	66 90                	xchg   %ax,%ax
    if((addr = ip->addrs[bn]) == 0)
801015a0:	8d 5a 14             	lea    0x14(%edx),%ebx
801015a3:	8b 7c 98 0c          	mov    0xc(%eax,%ebx,4),%edi
801015a7:	85 ff                	test   %edi,%edi
801015a9:	75 a5                	jne    80101550 <bmap+0x50>
      ip->addrs[bn] = addr = balloc(ip->dev);
801015ab:	8b 00                	mov    (%eax),%eax
801015ad:	e8 3e fd ff ff       	call   801012f0 <balloc>
801015b2:	89 44 9e 0c          	mov    %eax,0xc(%esi,%ebx,4)
801015b6:	89 c7                	mov    %eax,%edi
}
801015b8:	8d 65 f4             	lea    -0xc(%ebp),%esp
801015bb:	5b                   	pop    %ebx
801015bc:	89 f8                	mov    %edi,%eax
801015be:	5e                   	pop    %esi
801015bf:	5f                   	pop    %edi
801015c0:	5d                   	pop    %ebp
801015c1:	c3                   	ret    
  panic("bmap: out of range");
801015c2:	83 ec 0c             	sub    $0xc,%esp
801015c5:	68 42 7f 10 80       	push   $0x80107f42
801015ca:	e8 b1 ed ff ff       	call   80100380 <panic>
801015cf:	90                   	nop

801015d0 <readsb>:
{
801015d0:	55                   	push   %ebp
801015d1:	89 e5                	mov    %esp,%ebp
801015d3:	56                   	push   %esi
801015d4:	53                   	push   %ebx
801015d5:	8b 75 0c             	mov    0xc(%ebp),%esi
  bp = bread(dev, 1);
801015d8:	83 ec 08             	sub    $0x8,%esp
801015db:	6a 01                	push   $0x1
801015dd:	ff 75 08             	push   0x8(%ebp)
801015e0:	e8 eb ea ff ff       	call   801000d0 <bread>
  memmove(sb, bp->data, sizeof(*sb));
801015e5:	83 c4 0c             	add    $0xc,%esp
  bp = bread(dev, 1);
801015e8:	89 c3                	mov    %eax,%ebx
  memmove(sb, bp->data, sizeof(*sb));
801015ea:	8d 40 5c             	lea    0x5c(%eax),%eax
801015ed:	6a 1c                	push   $0x1c
801015ef:	50                   	push   %eax
801015f0:	56                   	push   %esi
801015f1:	e8 4a 3d 00 00       	call   80105340 <memmove>
  brelse(bp);
801015f6:	89 5d 08             	mov    %ebx,0x8(%ebp)
801015f9:	83 c4 10             	add    $0x10,%esp
}
801015fc:	8d 65 f8             	lea    -0x8(%ebp),%esp
801015ff:	5b                   	pop    %ebx
80101600:	5e                   	pop    %esi
80101601:	5d                   	pop    %ebp
  brelse(bp);
80101602:	e9 e9 eb ff ff       	jmp    801001f0 <brelse>
80101607:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010160e:	66 90                	xchg   %ax,%ax

80101610 <iinit>:
{
80101610:	55                   	push   %ebp
80101611:	89 e5                	mov    %esp,%ebp
80101613:	53                   	push   %ebx
80101614:	bb a0 09 11 80       	mov    $0x801109a0,%ebx
80101619:	83 ec 0c             	sub    $0xc,%esp
  initlock(&icache.lock, "icache");
8010161c:	68 55 7f 10 80       	push   $0x80107f55
80101621:	68 60 09 11 80       	push   $0x80110960
80101626:	e8 e5 39 00 00       	call   80105010 <initlock>
  for(i = 0; i < NINODE; i++) {
8010162b:	83 c4 10             	add    $0x10,%esp
8010162e:	66 90                	xchg   %ax,%ax
    initsleeplock(&icache.inode[i].lock, "inode");
80101630:	83 ec 08             	sub    $0x8,%esp
80101633:	68 5c 7f 10 80       	push   $0x80107f5c
80101638:	53                   	push   %ebx
  for(i = 0; i < NINODE; i++) {
80101639:	81 c3 90 00 00 00    	add    $0x90,%ebx
    initsleeplock(&icache.inode[i].lock, "inode");
8010163f:	e8 9c 38 00 00       	call   80104ee0 <initsleeplock>
  for(i = 0; i < NINODE; i++) {
80101644:	83 c4 10             	add    $0x10,%esp
80101647:	81 fb c0 25 11 80    	cmp    $0x801125c0,%ebx
8010164d:	75 e1                	jne    80101630 <iinit+0x20>
  bp = bread(dev, 1);
8010164f:	83 ec 08             	sub    $0x8,%esp
80101652:	6a 01                	push   $0x1
80101654:	ff 75 08             	push   0x8(%ebp)
80101657:	e8 74 ea ff ff       	call   801000d0 <bread>
  memmove(sb, bp->data, sizeof(*sb));
8010165c:	83 c4 0c             	add    $0xc,%esp
  bp = bread(dev, 1);
8010165f:	89 c3                	mov    %eax,%ebx
  memmove(sb, bp->data, sizeof(*sb));
80101661:	8d 40 5c             	lea    0x5c(%eax),%eax
80101664:	6a 1c                	push   $0x1c
80101666:	50                   	push   %eax
80101667:	68 b4 25 11 80       	push   $0x801125b4
8010166c:	e8 cf 3c 00 00       	call   80105340 <memmove>
  brelse(bp);
80101671:	89 1c 24             	mov    %ebx,(%esp)
80101674:	e8 77 eb ff ff       	call   801001f0 <brelse>
  cprintf("sb: size %d nblocks %d ninodes %d nlog %d logstart %d\
80101679:	ff 35 cc 25 11 80    	push   0x801125cc
8010167f:	ff 35 c8 25 11 80    	push   0x801125c8
80101685:	ff 35 c4 25 11 80    	push   0x801125c4
8010168b:	ff 35 c0 25 11 80    	push   0x801125c0
80101691:	ff 35 bc 25 11 80    	push   0x801125bc
80101697:	ff 35 b8 25 11 80    	push   0x801125b8
8010169d:	ff 35 b4 25 11 80    	push   0x801125b4
801016a3:	68 c0 7f 10 80       	push   $0x80107fc0
801016a8:	e8 f3 ef ff ff       	call   801006a0 <cprintf>
}
801016ad:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801016b0:	83 c4 30             	add    $0x30,%esp
801016b3:	c9                   	leave  
801016b4:	c3                   	ret    
801016b5:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801016bc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

801016c0 <ialloc>:
{
801016c0:	55                   	push   %ebp
801016c1:	89 e5                	mov    %esp,%ebp
801016c3:	57                   	push   %edi
801016c4:	56                   	push   %esi
801016c5:	53                   	push   %ebx
801016c6:	83 ec 1c             	sub    $0x1c,%esp
801016c9:	8b 45 0c             	mov    0xc(%ebp),%eax
  for(inum = 1; inum < sb.ninodes; inum++){
801016cc:	83 3d bc 25 11 80 01 	cmpl   $0x1,0x801125bc
{
801016d3:	8b 75 08             	mov    0x8(%ebp),%esi
801016d6:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  for(inum = 1; inum < sb.ninodes; inum++){
801016d9:	0f 86 91 00 00 00    	jbe    80101770 <ialloc+0xb0>
801016df:	bf 01 00 00 00       	mov    $0x1,%edi
801016e4:	eb 21                	jmp    80101707 <ialloc+0x47>
801016e6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801016ed:	8d 76 00             	lea    0x0(%esi),%esi
    brelse(bp);
801016f0:	83 ec 0c             	sub    $0xc,%esp
  for(inum = 1; inum < sb.ninodes; inum++){
801016f3:	83 c7 01             	add    $0x1,%edi
    brelse(bp);
801016f6:	53                   	push   %ebx
801016f7:	e8 f4 ea ff ff       	call   801001f0 <brelse>
  for(inum = 1; inum < sb.ninodes; inum++){
801016fc:	83 c4 10             	add    $0x10,%esp
801016ff:	3b 3d bc 25 11 80    	cmp    0x801125bc,%edi
80101705:	73 69                	jae    80101770 <ialloc+0xb0>
    bp = bread(dev, IBLOCK(inum, sb));
80101707:	89 f8                	mov    %edi,%eax
80101709:	83 ec 08             	sub    $0x8,%esp
8010170c:	c1 e8 03             	shr    $0x3,%eax
8010170f:	03 05 c8 25 11 80    	add    0x801125c8,%eax
80101715:	50                   	push   %eax
80101716:	56                   	push   %esi
80101717:	e8 b4 e9 ff ff       	call   801000d0 <bread>
    if(dip->type == 0){  // a free inode
8010171c:	83 c4 10             	add    $0x10,%esp
    bp = bread(dev, IBLOCK(inum, sb));
8010171f:	89 c3                	mov    %eax,%ebx
    dip = (struct dinode*)bp->data + inum%IPB;
80101721:	89 f8                	mov    %edi,%eax
80101723:	83 e0 07             	and    $0x7,%eax
80101726:	c1 e0 06             	shl    $0x6,%eax
80101729:	8d 4c 03 5c          	lea    0x5c(%ebx,%eax,1),%ecx
    if(dip->type == 0){  // a free inode
8010172d:	66 83 39 00          	cmpw   $0x0,(%ecx)
80101731:	75 bd                	jne    801016f0 <ialloc+0x30>
      memset(dip, 0, sizeof(*dip));
80101733:	83 ec 04             	sub    $0x4,%esp
80101736:	89 4d e0             	mov    %ecx,-0x20(%ebp)
80101739:	6a 40                	push   $0x40
8010173b:	6a 00                	push   $0x0
8010173d:	51                   	push   %ecx
8010173e:	e8 5d 3b 00 00       	call   801052a0 <memset>
      dip->type = type;
80101743:	0f b7 45 e4          	movzwl -0x1c(%ebp),%eax
80101747:	8b 4d e0             	mov    -0x20(%ebp),%ecx
8010174a:	66 89 01             	mov    %ax,(%ecx)
      log_write(bp);   // mark it allocated on the disk
8010174d:	89 1c 24             	mov    %ebx,(%esp)
80101750:	e8 9b 18 00 00       	call   80102ff0 <log_write>
      brelse(bp);
80101755:	89 1c 24             	mov    %ebx,(%esp)
80101758:	e8 93 ea ff ff       	call   801001f0 <brelse>
      return iget(dev, inum);
8010175d:	83 c4 10             	add    $0x10,%esp
}
80101760:	8d 65 f4             	lea    -0xc(%ebp),%esp
      return iget(dev, inum);
80101763:	89 fa                	mov    %edi,%edx
}
80101765:	5b                   	pop    %ebx
      return iget(dev, inum);
80101766:	89 f0                	mov    %esi,%eax
}
80101768:	5e                   	pop    %esi
80101769:	5f                   	pop    %edi
8010176a:	5d                   	pop    %ebp
      return iget(dev, inum);
8010176b:	e9 90 fc ff ff       	jmp    80101400 <iget>
  panic("ialloc: no inodes");
80101770:	83 ec 0c             	sub    $0xc,%esp
80101773:	68 62 7f 10 80       	push   $0x80107f62
80101778:	e8 03 ec ff ff       	call   80100380 <panic>
8010177d:	8d 76 00             	lea    0x0(%esi),%esi

80101780 <iupdate>:
{
80101780:	55                   	push   %ebp
80101781:	89 e5                	mov    %esp,%ebp
80101783:	56                   	push   %esi
80101784:	53                   	push   %ebx
80101785:	8b 5d 08             	mov    0x8(%ebp),%ebx
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
80101788:	8b 43 04             	mov    0x4(%ebx),%eax
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
8010178b:	83 c3 5c             	add    $0x5c,%ebx
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
8010178e:	83 ec 08             	sub    $0x8,%esp
80101791:	c1 e8 03             	shr    $0x3,%eax
80101794:	03 05 c8 25 11 80    	add    0x801125c8,%eax
8010179a:	50                   	push   %eax
8010179b:	ff 73 a4             	push   -0x5c(%ebx)
8010179e:	e8 2d e9 ff ff       	call   801000d0 <bread>
  dip->type = ip->type;
801017a3:	0f b7 53 f4          	movzwl -0xc(%ebx),%edx
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
801017a7:	83 c4 0c             	add    $0xc,%esp
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
801017aa:	89 c6                	mov    %eax,%esi
  dip = (struct dinode*)bp->data + ip->inum%IPB;
801017ac:	8b 43 a8             	mov    -0x58(%ebx),%eax
801017af:	83 e0 07             	and    $0x7,%eax
801017b2:	c1 e0 06             	shl    $0x6,%eax
801017b5:	8d 44 06 5c          	lea    0x5c(%esi,%eax,1),%eax
  dip->type = ip->type;
801017b9:	66 89 10             	mov    %dx,(%eax)
  dip->major = ip->major;
801017bc:	0f b7 53 f6          	movzwl -0xa(%ebx),%edx
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
801017c0:	83 c0 0c             	add    $0xc,%eax
  dip->major = ip->major;
801017c3:	66 89 50 f6          	mov    %dx,-0xa(%eax)
  dip->minor = ip->minor;
801017c7:	0f b7 53 f8          	movzwl -0x8(%ebx),%edx
801017cb:	66 89 50 f8          	mov    %dx,-0x8(%eax)
  dip->nlink = ip->nlink;
801017cf:	0f b7 53 fa          	movzwl -0x6(%ebx),%edx
801017d3:	66 89 50 fa          	mov    %dx,-0x6(%eax)
  dip->size = ip->size;
801017d7:	8b 53 fc             	mov    -0x4(%ebx),%edx
801017da:	89 50 fc             	mov    %edx,-0x4(%eax)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
801017dd:	6a 34                	push   $0x34
801017df:	53                   	push   %ebx
801017e0:	50                   	push   %eax
801017e1:	e8 5a 3b 00 00       	call   80105340 <memmove>
  log_write(bp);
801017e6:	89 34 24             	mov    %esi,(%esp)
801017e9:	e8 02 18 00 00       	call   80102ff0 <log_write>
  brelse(bp);
801017ee:	89 75 08             	mov    %esi,0x8(%ebp)
801017f1:	83 c4 10             	add    $0x10,%esp
}
801017f4:	8d 65 f8             	lea    -0x8(%ebp),%esp
801017f7:	5b                   	pop    %ebx
801017f8:	5e                   	pop    %esi
801017f9:	5d                   	pop    %ebp
  brelse(bp);
801017fa:	e9 f1 e9 ff ff       	jmp    801001f0 <brelse>
801017ff:	90                   	nop

80101800 <idup>:
{
80101800:	55                   	push   %ebp
80101801:	89 e5                	mov    %esp,%ebp
80101803:	53                   	push   %ebx
80101804:	83 ec 10             	sub    $0x10,%esp
80101807:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquire(&icache.lock);
8010180a:	68 60 09 11 80       	push   $0x80110960
8010180f:	e8 cc 39 00 00       	call   801051e0 <acquire>
  ip->ref++;
80101814:	83 43 08 01          	addl   $0x1,0x8(%ebx)
  release(&icache.lock);
80101818:	c7 04 24 60 09 11 80 	movl   $0x80110960,(%esp)
8010181f:	e8 5c 39 00 00       	call   80105180 <release>
}
80101824:	89 d8                	mov    %ebx,%eax
80101826:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80101829:	c9                   	leave  
8010182a:	c3                   	ret    
8010182b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010182f:	90                   	nop

80101830 <ilock>:
{
80101830:	55                   	push   %ebp
80101831:	89 e5                	mov    %esp,%ebp
80101833:	56                   	push   %esi
80101834:	53                   	push   %ebx
80101835:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(ip == 0 || ip->ref < 1)
80101838:	85 db                	test   %ebx,%ebx
8010183a:	0f 84 b7 00 00 00    	je     801018f7 <ilock+0xc7>
80101840:	8b 53 08             	mov    0x8(%ebx),%edx
80101843:	85 d2                	test   %edx,%edx
80101845:	0f 8e ac 00 00 00    	jle    801018f7 <ilock+0xc7>
  acquiresleep(&ip->lock);
8010184b:	83 ec 0c             	sub    $0xc,%esp
8010184e:	8d 43 0c             	lea    0xc(%ebx),%eax
80101851:	50                   	push   %eax
80101852:	e8 c9 36 00 00       	call   80104f20 <acquiresleep>
  if(ip->valid == 0){
80101857:	8b 43 4c             	mov    0x4c(%ebx),%eax
8010185a:	83 c4 10             	add    $0x10,%esp
8010185d:	85 c0                	test   %eax,%eax
8010185f:	74 0f                	je     80101870 <ilock+0x40>
}
80101861:	8d 65 f8             	lea    -0x8(%ebp),%esp
80101864:	5b                   	pop    %ebx
80101865:	5e                   	pop    %esi
80101866:	5d                   	pop    %ebp
80101867:	c3                   	ret    
80101868:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010186f:	90                   	nop
    bp = bread(ip->dev, IBLOCK(ip->inum, sb));
80101870:	8b 43 04             	mov    0x4(%ebx),%eax
80101873:	83 ec 08             	sub    $0x8,%esp
80101876:	c1 e8 03             	shr    $0x3,%eax
80101879:	03 05 c8 25 11 80    	add    0x801125c8,%eax
8010187f:	50                   	push   %eax
80101880:	ff 33                	push   (%ebx)
80101882:	e8 49 e8 ff ff       	call   801000d0 <bread>
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
80101887:	83 c4 0c             	add    $0xc,%esp
    bp = bread(ip->dev, IBLOCK(ip->inum, sb));
8010188a:	89 c6                	mov    %eax,%esi
    dip = (struct dinode*)bp->data + ip->inum%IPB;
8010188c:	8b 43 04             	mov    0x4(%ebx),%eax
8010188f:	83 e0 07             	and    $0x7,%eax
80101892:	c1 e0 06             	shl    $0x6,%eax
80101895:	8d 44 06 5c          	lea    0x5c(%esi,%eax,1),%eax
    ip->type = dip->type;
80101899:	0f b7 10             	movzwl (%eax),%edx
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
8010189c:	83 c0 0c             	add    $0xc,%eax
    ip->type = dip->type;
8010189f:	66 89 53 50          	mov    %dx,0x50(%ebx)
    ip->major = dip->major;
801018a3:	0f b7 50 f6          	movzwl -0xa(%eax),%edx
801018a7:	66 89 53 52          	mov    %dx,0x52(%ebx)
    ip->minor = dip->minor;
801018ab:	0f b7 50 f8          	movzwl -0x8(%eax),%edx
801018af:	66 89 53 54          	mov    %dx,0x54(%ebx)
    ip->nlink = dip->nlink;
801018b3:	0f b7 50 fa          	movzwl -0x6(%eax),%edx
801018b7:	66 89 53 56          	mov    %dx,0x56(%ebx)
    ip->size = dip->size;
801018bb:	8b 50 fc             	mov    -0x4(%eax),%edx
801018be:	89 53 58             	mov    %edx,0x58(%ebx)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
801018c1:	6a 34                	push   $0x34
801018c3:	50                   	push   %eax
801018c4:	8d 43 5c             	lea    0x5c(%ebx),%eax
801018c7:	50                   	push   %eax
801018c8:	e8 73 3a 00 00       	call   80105340 <memmove>
    brelse(bp);
801018cd:	89 34 24             	mov    %esi,(%esp)
801018d0:	e8 1b e9 ff ff       	call   801001f0 <brelse>
    if(ip->type == 0)
801018d5:	83 c4 10             	add    $0x10,%esp
801018d8:	66 83 7b 50 00       	cmpw   $0x0,0x50(%ebx)
    ip->valid = 1;
801018dd:	c7 43 4c 01 00 00 00 	movl   $0x1,0x4c(%ebx)
    if(ip->type == 0)
801018e4:	0f 85 77 ff ff ff    	jne    80101861 <ilock+0x31>
      panic("ilock: no type");
801018ea:	83 ec 0c             	sub    $0xc,%esp
801018ed:	68 7a 7f 10 80       	push   $0x80107f7a
801018f2:	e8 89 ea ff ff       	call   80100380 <panic>
    panic("ilock");
801018f7:	83 ec 0c             	sub    $0xc,%esp
801018fa:	68 74 7f 10 80       	push   $0x80107f74
801018ff:	e8 7c ea ff ff       	call   80100380 <panic>
80101904:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010190b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010190f:	90                   	nop

80101910 <iunlock>:
{
80101910:	55                   	push   %ebp
80101911:	89 e5                	mov    %esp,%ebp
80101913:	56                   	push   %esi
80101914:	53                   	push   %ebx
80101915:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(ip == 0 || !holdingsleep(&ip->lock) || ip->ref < 1)
80101918:	85 db                	test   %ebx,%ebx
8010191a:	74 28                	je     80101944 <iunlock+0x34>
8010191c:	83 ec 0c             	sub    $0xc,%esp
8010191f:	8d 73 0c             	lea    0xc(%ebx),%esi
80101922:	56                   	push   %esi
80101923:	e8 98 36 00 00       	call   80104fc0 <holdingsleep>
80101928:	83 c4 10             	add    $0x10,%esp
8010192b:	85 c0                	test   %eax,%eax
8010192d:	74 15                	je     80101944 <iunlock+0x34>
8010192f:	8b 43 08             	mov    0x8(%ebx),%eax
80101932:	85 c0                	test   %eax,%eax
80101934:	7e 0e                	jle    80101944 <iunlock+0x34>
  releasesleep(&ip->lock);
80101936:	89 75 08             	mov    %esi,0x8(%ebp)
}
80101939:	8d 65 f8             	lea    -0x8(%ebp),%esp
8010193c:	5b                   	pop    %ebx
8010193d:	5e                   	pop    %esi
8010193e:	5d                   	pop    %ebp
  releasesleep(&ip->lock);
8010193f:	e9 3c 36 00 00       	jmp    80104f80 <releasesleep>
    panic("iunlock");
80101944:	83 ec 0c             	sub    $0xc,%esp
80101947:	68 89 7f 10 80       	push   $0x80107f89
8010194c:	e8 2f ea ff ff       	call   80100380 <panic>
80101951:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80101958:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010195f:	90                   	nop

80101960 <iput>:
{
80101960:	55                   	push   %ebp
80101961:	89 e5                	mov    %esp,%ebp
80101963:	57                   	push   %edi
80101964:	56                   	push   %esi
80101965:	53                   	push   %ebx
80101966:	83 ec 28             	sub    $0x28,%esp
80101969:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquiresleep(&ip->lock);
8010196c:	8d 7b 0c             	lea    0xc(%ebx),%edi
8010196f:	57                   	push   %edi
80101970:	e8 ab 35 00 00       	call   80104f20 <acquiresleep>
  if(ip->valid && ip->nlink == 0){
80101975:	8b 53 4c             	mov    0x4c(%ebx),%edx
80101978:	83 c4 10             	add    $0x10,%esp
8010197b:	85 d2                	test   %edx,%edx
8010197d:	74 07                	je     80101986 <iput+0x26>
8010197f:	66 83 7b 56 00       	cmpw   $0x0,0x56(%ebx)
80101984:	74 32                	je     801019b8 <iput+0x58>
  releasesleep(&ip->lock);
80101986:	83 ec 0c             	sub    $0xc,%esp
80101989:	57                   	push   %edi
8010198a:	e8 f1 35 00 00       	call   80104f80 <releasesleep>
  acquire(&icache.lock);
8010198f:	c7 04 24 60 09 11 80 	movl   $0x80110960,(%esp)
80101996:	e8 45 38 00 00       	call   801051e0 <acquire>
  ip->ref--;
8010199b:	83 6b 08 01          	subl   $0x1,0x8(%ebx)
  release(&icache.lock);
8010199f:	83 c4 10             	add    $0x10,%esp
801019a2:	c7 45 08 60 09 11 80 	movl   $0x80110960,0x8(%ebp)
}
801019a9:	8d 65 f4             	lea    -0xc(%ebp),%esp
801019ac:	5b                   	pop    %ebx
801019ad:	5e                   	pop    %esi
801019ae:	5f                   	pop    %edi
801019af:	5d                   	pop    %ebp
  release(&icache.lock);
801019b0:	e9 cb 37 00 00       	jmp    80105180 <release>
801019b5:	8d 76 00             	lea    0x0(%esi),%esi
    acquire(&icache.lock);
801019b8:	83 ec 0c             	sub    $0xc,%esp
801019bb:	68 60 09 11 80       	push   $0x80110960
801019c0:	e8 1b 38 00 00       	call   801051e0 <acquire>
    int r = ip->ref;
801019c5:	8b 73 08             	mov    0x8(%ebx),%esi
    release(&icache.lock);
801019c8:	c7 04 24 60 09 11 80 	movl   $0x80110960,(%esp)
801019cf:	e8 ac 37 00 00       	call   80105180 <release>
    if(r == 1){
801019d4:	83 c4 10             	add    $0x10,%esp
801019d7:	83 fe 01             	cmp    $0x1,%esi
801019da:	75 aa                	jne    80101986 <iput+0x26>
801019dc:	8d 8b 8c 00 00 00    	lea    0x8c(%ebx),%ecx
801019e2:	89 7d e4             	mov    %edi,-0x1c(%ebp)
801019e5:	8d 73 5c             	lea    0x5c(%ebx),%esi
801019e8:	89 cf                	mov    %ecx,%edi
801019ea:	eb 0b                	jmp    801019f7 <iput+0x97>
801019ec:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
{
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
801019f0:	83 c6 04             	add    $0x4,%esi
801019f3:	39 fe                	cmp    %edi,%esi
801019f5:	74 19                	je     80101a10 <iput+0xb0>
    if(ip->addrs[i]){
801019f7:	8b 16                	mov    (%esi),%edx
801019f9:	85 d2                	test   %edx,%edx
801019fb:	74 f3                	je     801019f0 <iput+0x90>
      bfree(ip->dev, ip->addrs[i]);
801019fd:	8b 03                	mov    (%ebx),%eax
801019ff:	e8 6c f8 ff ff       	call   80101270 <bfree>
      ip->addrs[i] = 0;
80101a04:	c7 06 00 00 00 00    	movl   $0x0,(%esi)
80101a0a:	eb e4                	jmp    801019f0 <iput+0x90>
80101a0c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    }
  }

  if(ip->addrs[NDIRECT]){
80101a10:	8b 83 8c 00 00 00    	mov    0x8c(%ebx),%eax
80101a16:	8b 7d e4             	mov    -0x1c(%ebp),%edi
80101a19:	85 c0                	test   %eax,%eax
80101a1b:	75 2d                	jne    80101a4a <iput+0xea>
    bfree(ip->dev, ip->addrs[NDIRECT]);
    ip->addrs[NDIRECT] = 0;
  }

  ip->size = 0;
  iupdate(ip);
80101a1d:	83 ec 0c             	sub    $0xc,%esp
  ip->size = 0;
80101a20:	c7 43 58 00 00 00 00 	movl   $0x0,0x58(%ebx)
  iupdate(ip);
80101a27:	53                   	push   %ebx
80101a28:	e8 53 fd ff ff       	call   80101780 <iupdate>
      ip->type = 0;
80101a2d:	31 c0                	xor    %eax,%eax
80101a2f:	66 89 43 50          	mov    %ax,0x50(%ebx)
      iupdate(ip);
80101a33:	89 1c 24             	mov    %ebx,(%esp)
80101a36:	e8 45 fd ff ff       	call   80101780 <iupdate>
      ip->valid = 0;
80101a3b:	c7 43 4c 00 00 00 00 	movl   $0x0,0x4c(%ebx)
80101a42:	83 c4 10             	add    $0x10,%esp
80101a45:	e9 3c ff ff ff       	jmp    80101986 <iput+0x26>
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
80101a4a:	83 ec 08             	sub    $0x8,%esp
80101a4d:	50                   	push   %eax
80101a4e:	ff 33                	push   (%ebx)
80101a50:	e8 7b e6 ff ff       	call   801000d0 <bread>
80101a55:	89 7d e0             	mov    %edi,-0x20(%ebp)
80101a58:	83 c4 10             	add    $0x10,%esp
80101a5b:	8d 88 5c 02 00 00    	lea    0x25c(%eax),%ecx
80101a61:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    for(j = 0; j < NINDIRECT; j++){
80101a64:	8d 70 5c             	lea    0x5c(%eax),%esi
80101a67:	89 cf                	mov    %ecx,%edi
80101a69:	eb 0c                	jmp    80101a77 <iput+0x117>
80101a6b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80101a6f:	90                   	nop
80101a70:	83 c6 04             	add    $0x4,%esi
80101a73:	39 f7                	cmp    %esi,%edi
80101a75:	74 0f                	je     80101a86 <iput+0x126>
      if(a[j])
80101a77:	8b 16                	mov    (%esi),%edx
80101a79:	85 d2                	test   %edx,%edx
80101a7b:	74 f3                	je     80101a70 <iput+0x110>
        bfree(ip->dev, a[j]);
80101a7d:	8b 03                	mov    (%ebx),%eax
80101a7f:	e8 ec f7 ff ff       	call   80101270 <bfree>
80101a84:	eb ea                	jmp    80101a70 <iput+0x110>
    brelse(bp);
80101a86:	83 ec 0c             	sub    $0xc,%esp
80101a89:	ff 75 e4             	push   -0x1c(%ebp)
80101a8c:	8b 7d e0             	mov    -0x20(%ebp),%edi
80101a8f:	e8 5c e7 ff ff       	call   801001f0 <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
80101a94:	8b 93 8c 00 00 00    	mov    0x8c(%ebx),%edx
80101a9a:	8b 03                	mov    (%ebx),%eax
80101a9c:	e8 cf f7 ff ff       	call   80101270 <bfree>
    ip->addrs[NDIRECT] = 0;
80101aa1:	83 c4 10             	add    $0x10,%esp
80101aa4:	c7 83 8c 00 00 00 00 	movl   $0x0,0x8c(%ebx)
80101aab:	00 00 00 
80101aae:	e9 6a ff ff ff       	jmp    80101a1d <iput+0xbd>
80101ab3:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80101aba:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80101ac0 <iunlockput>:
{
80101ac0:	55                   	push   %ebp
80101ac1:	89 e5                	mov    %esp,%ebp
80101ac3:	56                   	push   %esi
80101ac4:	53                   	push   %ebx
80101ac5:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(ip == 0 || !holdingsleep(&ip->lock) || ip->ref < 1)
80101ac8:	85 db                	test   %ebx,%ebx
80101aca:	74 34                	je     80101b00 <iunlockput+0x40>
80101acc:	83 ec 0c             	sub    $0xc,%esp
80101acf:	8d 73 0c             	lea    0xc(%ebx),%esi
80101ad2:	56                   	push   %esi
80101ad3:	e8 e8 34 00 00       	call   80104fc0 <holdingsleep>
80101ad8:	83 c4 10             	add    $0x10,%esp
80101adb:	85 c0                	test   %eax,%eax
80101add:	74 21                	je     80101b00 <iunlockput+0x40>
80101adf:	8b 43 08             	mov    0x8(%ebx),%eax
80101ae2:	85 c0                	test   %eax,%eax
80101ae4:	7e 1a                	jle    80101b00 <iunlockput+0x40>
  releasesleep(&ip->lock);
80101ae6:	83 ec 0c             	sub    $0xc,%esp
80101ae9:	56                   	push   %esi
80101aea:	e8 91 34 00 00       	call   80104f80 <releasesleep>
  iput(ip);
80101aef:	89 5d 08             	mov    %ebx,0x8(%ebp)
80101af2:	83 c4 10             	add    $0x10,%esp
}
80101af5:	8d 65 f8             	lea    -0x8(%ebp),%esp
80101af8:	5b                   	pop    %ebx
80101af9:	5e                   	pop    %esi
80101afa:	5d                   	pop    %ebp
  iput(ip);
80101afb:	e9 60 fe ff ff       	jmp    80101960 <iput>
    panic("iunlock");
80101b00:	83 ec 0c             	sub    $0xc,%esp
80101b03:	68 89 7f 10 80       	push   $0x80107f89
80101b08:	e8 73 e8 ff ff       	call   80100380 <panic>
80101b0d:	8d 76 00             	lea    0x0(%esi),%esi

80101b10 <stati>:

// Copy stat information from inode.
// Caller must hold ip->lock.
void
stati(struct inode *ip, struct stat *st)
{
80101b10:	55                   	push   %ebp
80101b11:	89 e5                	mov    %esp,%ebp
80101b13:	8b 55 08             	mov    0x8(%ebp),%edx
80101b16:	8b 45 0c             	mov    0xc(%ebp),%eax
  st->dev = ip->dev;
80101b19:	8b 0a                	mov    (%edx),%ecx
80101b1b:	89 48 04             	mov    %ecx,0x4(%eax)
  st->ino = ip->inum;
80101b1e:	8b 4a 04             	mov    0x4(%edx),%ecx
80101b21:	89 48 08             	mov    %ecx,0x8(%eax)
  st->type = ip->type;
80101b24:	0f b7 4a 50          	movzwl 0x50(%edx),%ecx
80101b28:	66 89 08             	mov    %cx,(%eax)
  st->nlink = ip->nlink;
80101b2b:	0f b7 4a 56          	movzwl 0x56(%edx),%ecx
80101b2f:	66 89 48 0c          	mov    %cx,0xc(%eax)
  st->size = ip->size;
80101b33:	8b 52 58             	mov    0x58(%edx),%edx
80101b36:	89 50 10             	mov    %edx,0x10(%eax)
}
80101b39:	5d                   	pop    %ebp
80101b3a:	c3                   	ret    
80101b3b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80101b3f:	90                   	nop

80101b40 <readi>:
//PAGEBREAK!
// Read data from inode.
// Caller must hold ip->lock.
int
readi(struct inode *ip, char *dst, uint off, uint n)
{
80101b40:	55                   	push   %ebp
80101b41:	89 e5                	mov    %esp,%ebp
80101b43:	57                   	push   %edi
80101b44:	56                   	push   %esi
80101b45:	53                   	push   %ebx
80101b46:	83 ec 1c             	sub    $0x1c,%esp
80101b49:	8b 7d 0c             	mov    0xc(%ebp),%edi
80101b4c:	8b 45 08             	mov    0x8(%ebp),%eax
80101b4f:	8b 75 10             	mov    0x10(%ebp),%esi
80101b52:	89 7d e0             	mov    %edi,-0x20(%ebp)
80101b55:	8b 7d 14             	mov    0x14(%ebp),%edi
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101b58:	66 83 78 50 03       	cmpw   $0x3,0x50(%eax)
{
80101b5d:	89 45 d8             	mov    %eax,-0x28(%ebp)
80101b60:	89 7d e4             	mov    %edi,-0x1c(%ebp)
  if(ip->type == T_DEV){
80101b63:	0f 84 a7 00 00 00    	je     80101c10 <readi+0xd0>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].read)
      return -1;
    return devsw[ip->major].read(ip, dst, n);
  }

  if(off > ip->size || off + n < off)
80101b69:	8b 45 d8             	mov    -0x28(%ebp),%eax
80101b6c:	8b 40 58             	mov    0x58(%eax),%eax
80101b6f:	39 c6                	cmp    %eax,%esi
80101b71:	0f 87 ba 00 00 00    	ja     80101c31 <readi+0xf1>
80101b77:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
80101b7a:	31 c9                	xor    %ecx,%ecx
80101b7c:	89 da                	mov    %ebx,%edx
80101b7e:	01 f2                	add    %esi,%edx
80101b80:	0f 92 c1             	setb   %cl
80101b83:	89 cf                	mov    %ecx,%edi
80101b85:	0f 82 a6 00 00 00    	jb     80101c31 <readi+0xf1>
    return -1;
  if(off + n > ip->size)
    n = ip->size - off;
80101b8b:	89 c1                	mov    %eax,%ecx
80101b8d:	29 f1                	sub    %esi,%ecx
80101b8f:	39 d0                	cmp    %edx,%eax
80101b91:	0f 43 cb             	cmovae %ebx,%ecx
80101b94:	89 4d e4             	mov    %ecx,-0x1c(%ebp)

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101b97:	85 c9                	test   %ecx,%ecx
80101b99:	74 67                	je     80101c02 <readi+0xc2>
80101b9b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80101b9f:	90                   	nop
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101ba0:	8b 5d d8             	mov    -0x28(%ebp),%ebx
80101ba3:	89 f2                	mov    %esi,%edx
80101ba5:	c1 ea 09             	shr    $0x9,%edx
80101ba8:	89 d8                	mov    %ebx,%eax
80101baa:	e8 51 f9 ff ff       	call   80101500 <bmap>
80101baf:	83 ec 08             	sub    $0x8,%esp
80101bb2:	50                   	push   %eax
80101bb3:	ff 33                	push   (%ebx)
80101bb5:	e8 16 e5 ff ff       	call   801000d0 <bread>
    m = min(n - tot, BSIZE - off%BSIZE);
80101bba:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
80101bbd:	b9 00 02 00 00       	mov    $0x200,%ecx
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101bc2:	89 c2                	mov    %eax,%edx
    m = min(n - tot, BSIZE - off%BSIZE);
80101bc4:	89 f0                	mov    %esi,%eax
80101bc6:	25 ff 01 00 00       	and    $0x1ff,%eax
80101bcb:	29 fb                	sub    %edi,%ebx
    memmove(dst, bp->data + off%BSIZE, m);
80101bcd:	89 55 dc             	mov    %edx,-0x24(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
80101bd0:	29 c1                	sub    %eax,%ecx
    memmove(dst, bp->data + off%BSIZE, m);
80101bd2:	8d 44 02 5c          	lea    0x5c(%edx,%eax,1),%eax
    m = min(n - tot, BSIZE - off%BSIZE);
80101bd6:	39 d9                	cmp    %ebx,%ecx
80101bd8:	0f 46 d9             	cmovbe %ecx,%ebx
    memmove(dst, bp->data + off%BSIZE, m);
80101bdb:	83 c4 0c             	add    $0xc,%esp
80101bde:	53                   	push   %ebx
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101bdf:	01 df                	add    %ebx,%edi
80101be1:	01 de                	add    %ebx,%esi
    memmove(dst, bp->data + off%BSIZE, m);
80101be3:	50                   	push   %eax
80101be4:	ff 75 e0             	push   -0x20(%ebp)
80101be7:	e8 54 37 00 00       	call   80105340 <memmove>
    brelse(bp);
80101bec:	8b 55 dc             	mov    -0x24(%ebp),%edx
80101bef:	89 14 24             	mov    %edx,(%esp)
80101bf2:	e8 f9 e5 ff ff       	call   801001f0 <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101bf7:	01 5d e0             	add    %ebx,-0x20(%ebp)
80101bfa:	83 c4 10             	add    $0x10,%esp
80101bfd:	39 7d e4             	cmp    %edi,-0x1c(%ebp)
80101c00:	77 9e                	ja     80101ba0 <readi+0x60>
  }
  return n;
80101c02:	8b 45 e4             	mov    -0x1c(%ebp),%eax
}
80101c05:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101c08:	5b                   	pop    %ebx
80101c09:	5e                   	pop    %esi
80101c0a:	5f                   	pop    %edi
80101c0b:	5d                   	pop    %ebp
80101c0c:	c3                   	ret    
80101c0d:	8d 76 00             	lea    0x0(%esi),%esi
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].read)
80101c10:	0f bf 40 52          	movswl 0x52(%eax),%eax
80101c14:	66 83 f8 09          	cmp    $0x9,%ax
80101c18:	77 17                	ja     80101c31 <readi+0xf1>
80101c1a:	8b 04 c5 00 09 11 80 	mov    -0x7feef700(,%eax,8),%eax
80101c21:	85 c0                	test   %eax,%eax
80101c23:	74 0c                	je     80101c31 <readi+0xf1>
    return devsw[ip->major].read(ip, dst, n);
80101c25:	89 7d 10             	mov    %edi,0x10(%ebp)
}
80101c28:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101c2b:	5b                   	pop    %ebx
80101c2c:	5e                   	pop    %esi
80101c2d:	5f                   	pop    %edi
80101c2e:	5d                   	pop    %ebp
    return devsw[ip->major].read(ip, dst, n);
80101c2f:	ff e0                	jmp    *%eax
      return -1;
80101c31:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101c36:	eb cd                	jmp    80101c05 <readi+0xc5>
80101c38:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80101c3f:	90                   	nop

80101c40 <writei>:
// PAGEBREAK!
// Write data to inode.
// Caller must hold ip->lock.
int
writei(struct inode *ip, char *src, uint off, uint n)
{
80101c40:	55                   	push   %ebp
80101c41:	89 e5                	mov    %esp,%ebp
80101c43:	57                   	push   %edi
80101c44:	56                   	push   %esi
80101c45:	53                   	push   %ebx
80101c46:	83 ec 1c             	sub    $0x1c,%esp
80101c49:	8b 45 08             	mov    0x8(%ebp),%eax
80101c4c:	8b 75 0c             	mov    0xc(%ebp),%esi
80101c4f:	8b 55 14             	mov    0x14(%ebp),%edx
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101c52:	66 83 78 50 03       	cmpw   $0x3,0x50(%eax)
{
80101c57:	89 75 dc             	mov    %esi,-0x24(%ebp)
80101c5a:	89 45 d8             	mov    %eax,-0x28(%ebp)
80101c5d:	8b 75 10             	mov    0x10(%ebp),%esi
80101c60:	89 55 e0             	mov    %edx,-0x20(%ebp)
  if(ip->type == T_DEV){
80101c63:	0f 84 b7 00 00 00    	je     80101d20 <writei+0xe0>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].write)
      return -1;
    return devsw[ip->major].write(ip, src, n);
  }

  if(off > ip->size || off + n < off)
80101c69:	8b 45 d8             	mov    -0x28(%ebp),%eax
80101c6c:	3b 70 58             	cmp    0x58(%eax),%esi
80101c6f:	0f 87 e7 00 00 00    	ja     80101d5c <writei+0x11c>
80101c75:	8b 7d e0             	mov    -0x20(%ebp),%edi
80101c78:	31 d2                	xor    %edx,%edx
80101c7a:	89 f8                	mov    %edi,%eax
80101c7c:	01 f0                	add    %esi,%eax
80101c7e:	0f 92 c2             	setb   %dl
    return -1;
  if(off + n > MAXFILE*BSIZE)
80101c81:	3d 00 18 01 00       	cmp    $0x11800,%eax
80101c86:	0f 87 d0 00 00 00    	ja     80101d5c <writei+0x11c>
80101c8c:	85 d2                	test   %edx,%edx
80101c8e:	0f 85 c8 00 00 00    	jne    80101d5c <writei+0x11c>
    return -1;

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80101c94:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
80101c9b:	85 ff                	test   %edi,%edi
80101c9d:	74 72                	je     80101d11 <writei+0xd1>
80101c9f:	90                   	nop
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101ca0:	8b 7d d8             	mov    -0x28(%ebp),%edi
80101ca3:	89 f2                	mov    %esi,%edx
80101ca5:	c1 ea 09             	shr    $0x9,%edx
80101ca8:	89 f8                	mov    %edi,%eax
80101caa:	e8 51 f8 ff ff       	call   80101500 <bmap>
80101caf:	83 ec 08             	sub    $0x8,%esp
80101cb2:	50                   	push   %eax
80101cb3:	ff 37                	push   (%edi)
80101cb5:	e8 16 e4 ff ff       	call   801000d0 <bread>
    m = min(n - tot, BSIZE - off%BSIZE);
80101cba:	b9 00 02 00 00       	mov    $0x200,%ecx
80101cbf:	8b 5d e0             	mov    -0x20(%ebp),%ebx
80101cc2:	2b 5d e4             	sub    -0x1c(%ebp),%ebx
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101cc5:	89 c7                	mov    %eax,%edi
    m = min(n - tot, BSIZE - off%BSIZE);
80101cc7:	89 f0                	mov    %esi,%eax
80101cc9:	25 ff 01 00 00       	and    $0x1ff,%eax
80101cce:	29 c1                	sub    %eax,%ecx
    memmove(bp->data + off%BSIZE, src, m);
80101cd0:	8d 44 07 5c          	lea    0x5c(%edi,%eax,1),%eax
    m = min(n - tot, BSIZE - off%BSIZE);
80101cd4:	39 d9                	cmp    %ebx,%ecx
80101cd6:	0f 46 d9             	cmovbe %ecx,%ebx
    memmove(bp->data + off%BSIZE, src, m);
80101cd9:	83 c4 0c             	add    $0xc,%esp
80101cdc:	53                   	push   %ebx
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80101cdd:	01 de                	add    %ebx,%esi
    memmove(bp->data + off%BSIZE, src, m);
80101cdf:	ff 75 dc             	push   -0x24(%ebp)
80101ce2:	50                   	push   %eax
80101ce3:	e8 58 36 00 00       	call   80105340 <memmove>
    log_write(bp);
80101ce8:	89 3c 24             	mov    %edi,(%esp)
80101ceb:	e8 00 13 00 00       	call   80102ff0 <log_write>
    brelse(bp);
80101cf0:	89 3c 24             	mov    %edi,(%esp)
80101cf3:	e8 f8 e4 ff ff       	call   801001f0 <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80101cf8:	01 5d e4             	add    %ebx,-0x1c(%ebp)
80101cfb:	83 c4 10             	add    $0x10,%esp
80101cfe:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80101d01:	01 5d dc             	add    %ebx,-0x24(%ebp)
80101d04:	39 45 e0             	cmp    %eax,-0x20(%ebp)
80101d07:	77 97                	ja     80101ca0 <writei+0x60>
  }

  if(n > 0 && off > ip->size){
80101d09:	8b 45 d8             	mov    -0x28(%ebp),%eax
80101d0c:	3b 70 58             	cmp    0x58(%eax),%esi
80101d0f:	77 37                	ja     80101d48 <writei+0x108>
    ip->size = off;
    iupdate(ip);
  }
  return n;
80101d11:	8b 45 e0             	mov    -0x20(%ebp),%eax
}
80101d14:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101d17:	5b                   	pop    %ebx
80101d18:	5e                   	pop    %esi
80101d19:	5f                   	pop    %edi
80101d1a:	5d                   	pop    %ebp
80101d1b:	c3                   	ret    
80101d1c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].write)
80101d20:	0f bf 40 52          	movswl 0x52(%eax),%eax
80101d24:	66 83 f8 09          	cmp    $0x9,%ax
80101d28:	77 32                	ja     80101d5c <writei+0x11c>
80101d2a:	8b 04 c5 04 09 11 80 	mov    -0x7feef6fc(,%eax,8),%eax
80101d31:	85 c0                	test   %eax,%eax
80101d33:	74 27                	je     80101d5c <writei+0x11c>
    return devsw[ip->major].write(ip, src, n);
80101d35:	89 55 10             	mov    %edx,0x10(%ebp)
}
80101d38:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101d3b:	5b                   	pop    %ebx
80101d3c:	5e                   	pop    %esi
80101d3d:	5f                   	pop    %edi
80101d3e:	5d                   	pop    %ebp
    return devsw[ip->major].write(ip, src, n);
80101d3f:	ff e0                	jmp    *%eax
80101d41:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    ip->size = off;
80101d48:	8b 45 d8             	mov    -0x28(%ebp),%eax
    iupdate(ip);
80101d4b:	83 ec 0c             	sub    $0xc,%esp
    ip->size = off;
80101d4e:	89 70 58             	mov    %esi,0x58(%eax)
    iupdate(ip);
80101d51:	50                   	push   %eax
80101d52:	e8 29 fa ff ff       	call   80101780 <iupdate>
80101d57:	83 c4 10             	add    $0x10,%esp
80101d5a:	eb b5                	jmp    80101d11 <writei+0xd1>
      return -1;
80101d5c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101d61:	eb b1                	jmp    80101d14 <writei+0xd4>
80101d63:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80101d6a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80101d70 <namecmp>:
//PAGEBREAK!
// Directories

int
namecmp(const char *s, const char *t)
{
80101d70:	55                   	push   %ebp
80101d71:	89 e5                	mov    %esp,%ebp
80101d73:	83 ec 0c             	sub    $0xc,%esp
  return strncmp(s, t, DIRSIZ);
80101d76:	6a 0e                	push   $0xe
80101d78:	ff 75 0c             	push   0xc(%ebp)
80101d7b:	ff 75 08             	push   0x8(%ebp)
80101d7e:	e8 2d 36 00 00       	call   801053b0 <strncmp>
}
80101d83:	c9                   	leave  
80101d84:	c3                   	ret    
80101d85:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80101d8c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80101d90 <dirlookup>:

// Look for a directory entry in a directory.
// If found, set *poff to byte offset of entry.
struct inode*
dirlookup(struct inode *dp, char *name, uint *poff)
{
80101d90:	55                   	push   %ebp
80101d91:	89 e5                	mov    %esp,%ebp
80101d93:	57                   	push   %edi
80101d94:	56                   	push   %esi
80101d95:	53                   	push   %ebx
80101d96:	83 ec 1c             	sub    $0x1c,%esp
80101d99:	8b 5d 08             	mov    0x8(%ebp),%ebx
  uint off, inum;
  struct dirent de;

  if(dp->type != T_DIR)
80101d9c:	66 83 7b 50 01       	cmpw   $0x1,0x50(%ebx)
80101da1:	0f 85 85 00 00 00    	jne    80101e2c <dirlookup+0x9c>
    panic("dirlookup not DIR");

  for(off = 0; off < dp->size; off += sizeof(de)){
80101da7:	8b 53 58             	mov    0x58(%ebx),%edx
80101daa:	31 ff                	xor    %edi,%edi
80101dac:	8d 75 d8             	lea    -0x28(%ebp),%esi
80101daf:	85 d2                	test   %edx,%edx
80101db1:	74 3e                	je     80101df1 <dirlookup+0x61>
80101db3:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80101db7:	90                   	nop
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80101db8:	6a 10                	push   $0x10
80101dba:	57                   	push   %edi
80101dbb:	56                   	push   %esi
80101dbc:	53                   	push   %ebx
80101dbd:	e8 7e fd ff ff       	call   80101b40 <readi>
80101dc2:	83 c4 10             	add    $0x10,%esp
80101dc5:	83 f8 10             	cmp    $0x10,%eax
80101dc8:	75 55                	jne    80101e1f <dirlookup+0x8f>
      panic("dirlookup read");
    if(de.inum == 0)
80101dca:	66 83 7d d8 00       	cmpw   $0x0,-0x28(%ebp)
80101dcf:	74 18                	je     80101de9 <dirlookup+0x59>
  return strncmp(s, t, DIRSIZ);
80101dd1:	83 ec 04             	sub    $0x4,%esp
80101dd4:	8d 45 da             	lea    -0x26(%ebp),%eax
80101dd7:	6a 0e                	push   $0xe
80101dd9:	50                   	push   %eax
80101dda:	ff 75 0c             	push   0xc(%ebp)
80101ddd:	e8 ce 35 00 00       	call   801053b0 <strncmp>
      continue;
    if(namecmp(name, de.name) == 0){
80101de2:	83 c4 10             	add    $0x10,%esp
80101de5:	85 c0                	test   %eax,%eax
80101de7:	74 17                	je     80101e00 <dirlookup+0x70>
  for(off = 0; off < dp->size; off += sizeof(de)){
80101de9:	83 c7 10             	add    $0x10,%edi
80101dec:	3b 7b 58             	cmp    0x58(%ebx),%edi
80101def:	72 c7                	jb     80101db8 <dirlookup+0x28>
      return iget(dp->dev, inum);
    }
  }

  return 0;
}
80101df1:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
80101df4:	31 c0                	xor    %eax,%eax
}
80101df6:	5b                   	pop    %ebx
80101df7:	5e                   	pop    %esi
80101df8:	5f                   	pop    %edi
80101df9:	5d                   	pop    %ebp
80101dfa:	c3                   	ret    
80101dfb:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80101dff:	90                   	nop
      if(poff)
80101e00:	8b 45 10             	mov    0x10(%ebp),%eax
80101e03:	85 c0                	test   %eax,%eax
80101e05:	74 05                	je     80101e0c <dirlookup+0x7c>
        *poff = off;
80101e07:	8b 45 10             	mov    0x10(%ebp),%eax
80101e0a:	89 38                	mov    %edi,(%eax)
      inum = de.inum;
80101e0c:	0f b7 55 d8          	movzwl -0x28(%ebp),%edx
      return iget(dp->dev, inum);
80101e10:	8b 03                	mov    (%ebx),%eax
80101e12:	e8 e9 f5 ff ff       	call   80101400 <iget>
}
80101e17:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101e1a:	5b                   	pop    %ebx
80101e1b:	5e                   	pop    %esi
80101e1c:	5f                   	pop    %edi
80101e1d:	5d                   	pop    %ebp
80101e1e:	c3                   	ret    
      panic("dirlookup read");
80101e1f:	83 ec 0c             	sub    $0xc,%esp
80101e22:	68 a3 7f 10 80       	push   $0x80107fa3
80101e27:	e8 54 e5 ff ff       	call   80100380 <panic>
    panic("dirlookup not DIR");
80101e2c:	83 ec 0c             	sub    $0xc,%esp
80101e2f:	68 91 7f 10 80       	push   $0x80107f91
80101e34:	e8 47 e5 ff ff       	call   80100380 <panic>
80101e39:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80101e40 <namex>:
// If parent != 0, return the inode for the parent and copy the final
// path element into name, which must have room for DIRSIZ bytes.
// Must be called inside a transaction since it calls iput().
static struct inode*
namex(char *path, int nameiparent, char *name)
{
80101e40:	55                   	push   %ebp
80101e41:	89 e5                	mov    %esp,%ebp
80101e43:	57                   	push   %edi
80101e44:	56                   	push   %esi
80101e45:	53                   	push   %ebx
80101e46:	89 c3                	mov    %eax,%ebx
80101e48:	83 ec 1c             	sub    $0x1c,%esp
  struct inode *ip, *next;

  if(*path == '/')
80101e4b:	80 38 2f             	cmpb   $0x2f,(%eax)
{
80101e4e:	89 55 dc             	mov    %edx,-0x24(%ebp)
80101e51:	89 4d e4             	mov    %ecx,-0x1c(%ebp)
  if(*path == '/')
80101e54:	0f 84 64 01 00 00    	je     80101fbe <namex+0x17e>
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(myproc()->cwd);
80101e5a:	e8 11 1c 00 00       	call   80103a70 <myproc>
  acquire(&icache.lock);
80101e5f:	83 ec 0c             	sub    $0xc,%esp
    ip = idup(myproc()->cwd);
80101e62:	8b 70 68             	mov    0x68(%eax),%esi
  acquire(&icache.lock);
80101e65:	68 60 09 11 80       	push   $0x80110960
80101e6a:	e8 71 33 00 00       	call   801051e0 <acquire>
  ip->ref++;
80101e6f:	83 46 08 01          	addl   $0x1,0x8(%esi)
  release(&icache.lock);
80101e73:	c7 04 24 60 09 11 80 	movl   $0x80110960,(%esp)
80101e7a:	e8 01 33 00 00       	call   80105180 <release>
80101e7f:	83 c4 10             	add    $0x10,%esp
80101e82:	eb 07                	jmp    80101e8b <namex+0x4b>
80101e84:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    path++;
80101e88:	83 c3 01             	add    $0x1,%ebx
  while(*path == '/')
80101e8b:	0f b6 03             	movzbl (%ebx),%eax
80101e8e:	3c 2f                	cmp    $0x2f,%al
80101e90:	74 f6                	je     80101e88 <namex+0x48>
  if(*path == 0)
80101e92:	84 c0                	test   %al,%al
80101e94:	0f 84 06 01 00 00    	je     80101fa0 <namex+0x160>
  while(*path != '/' && *path != 0)
80101e9a:	0f b6 03             	movzbl (%ebx),%eax
80101e9d:	84 c0                	test   %al,%al
80101e9f:	0f 84 10 01 00 00    	je     80101fb5 <namex+0x175>
80101ea5:	89 df                	mov    %ebx,%edi
80101ea7:	3c 2f                	cmp    $0x2f,%al
80101ea9:	0f 84 06 01 00 00    	je     80101fb5 <namex+0x175>
80101eaf:	90                   	nop
80101eb0:	0f b6 47 01          	movzbl 0x1(%edi),%eax
    path++;
80101eb4:	83 c7 01             	add    $0x1,%edi
  while(*path != '/' && *path != 0)
80101eb7:	3c 2f                	cmp    $0x2f,%al
80101eb9:	74 04                	je     80101ebf <namex+0x7f>
80101ebb:	84 c0                	test   %al,%al
80101ebd:	75 f1                	jne    80101eb0 <namex+0x70>
  len = path - s;
80101ebf:	89 f8                	mov    %edi,%eax
80101ec1:	29 d8                	sub    %ebx,%eax
  if(len >= DIRSIZ)
80101ec3:	83 f8 0d             	cmp    $0xd,%eax
80101ec6:	0f 8e ac 00 00 00    	jle    80101f78 <namex+0x138>
    memmove(name, s, DIRSIZ);
80101ecc:	83 ec 04             	sub    $0x4,%esp
80101ecf:	6a 0e                	push   $0xe
80101ed1:	53                   	push   %ebx
    path++;
80101ed2:	89 fb                	mov    %edi,%ebx
    memmove(name, s, DIRSIZ);
80101ed4:	ff 75 e4             	push   -0x1c(%ebp)
80101ed7:	e8 64 34 00 00       	call   80105340 <memmove>
80101edc:	83 c4 10             	add    $0x10,%esp
  while(*path == '/')
80101edf:	80 3f 2f             	cmpb   $0x2f,(%edi)
80101ee2:	75 0c                	jne    80101ef0 <namex+0xb0>
80101ee4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    path++;
80101ee8:	83 c3 01             	add    $0x1,%ebx
  while(*path == '/')
80101eeb:	80 3b 2f             	cmpb   $0x2f,(%ebx)
80101eee:	74 f8                	je     80101ee8 <namex+0xa8>

  while((path = skipelem(path, name)) != 0){
    ilock(ip);
80101ef0:	83 ec 0c             	sub    $0xc,%esp
80101ef3:	56                   	push   %esi
80101ef4:	e8 37 f9 ff ff       	call   80101830 <ilock>
    if(ip->type != T_DIR){
80101ef9:	83 c4 10             	add    $0x10,%esp
80101efc:	66 83 7e 50 01       	cmpw   $0x1,0x50(%esi)
80101f01:	0f 85 cd 00 00 00    	jne    80101fd4 <namex+0x194>
      iunlockput(ip);
      return 0;
    }
    if(nameiparent && *path == '\0'){
80101f07:	8b 45 dc             	mov    -0x24(%ebp),%eax
80101f0a:	85 c0                	test   %eax,%eax
80101f0c:	74 09                	je     80101f17 <namex+0xd7>
80101f0e:	80 3b 00             	cmpb   $0x0,(%ebx)
80101f11:	0f 84 22 01 00 00    	je     80102039 <namex+0x1f9>
      // Stop one level early.
      iunlock(ip);
      return ip;
    }
    if((next = dirlookup(ip, name, 0)) == 0){
80101f17:	83 ec 04             	sub    $0x4,%esp
80101f1a:	6a 00                	push   $0x0
80101f1c:	ff 75 e4             	push   -0x1c(%ebp)
80101f1f:	56                   	push   %esi
80101f20:	e8 6b fe ff ff       	call   80101d90 <dirlookup>
  if(ip == 0 || !holdingsleep(&ip->lock) || ip->ref < 1)
80101f25:	8d 56 0c             	lea    0xc(%esi),%edx
    if((next = dirlookup(ip, name, 0)) == 0){
80101f28:	83 c4 10             	add    $0x10,%esp
80101f2b:	89 c7                	mov    %eax,%edi
80101f2d:	85 c0                	test   %eax,%eax
80101f2f:	0f 84 e1 00 00 00    	je     80102016 <namex+0x1d6>
  if(ip == 0 || !holdingsleep(&ip->lock) || ip->ref < 1)
80101f35:	83 ec 0c             	sub    $0xc,%esp
80101f38:	89 55 e0             	mov    %edx,-0x20(%ebp)
80101f3b:	52                   	push   %edx
80101f3c:	e8 7f 30 00 00       	call   80104fc0 <holdingsleep>
80101f41:	83 c4 10             	add    $0x10,%esp
80101f44:	85 c0                	test   %eax,%eax
80101f46:	0f 84 30 01 00 00    	je     8010207c <namex+0x23c>
80101f4c:	8b 56 08             	mov    0x8(%esi),%edx
80101f4f:	85 d2                	test   %edx,%edx
80101f51:	0f 8e 25 01 00 00    	jle    8010207c <namex+0x23c>
  releasesleep(&ip->lock);
80101f57:	8b 55 e0             	mov    -0x20(%ebp),%edx
80101f5a:	83 ec 0c             	sub    $0xc,%esp
80101f5d:	52                   	push   %edx
80101f5e:	e8 1d 30 00 00       	call   80104f80 <releasesleep>
  iput(ip);
80101f63:	89 34 24             	mov    %esi,(%esp)
80101f66:	89 fe                	mov    %edi,%esi
80101f68:	e8 f3 f9 ff ff       	call   80101960 <iput>
80101f6d:	83 c4 10             	add    $0x10,%esp
80101f70:	e9 16 ff ff ff       	jmp    80101e8b <namex+0x4b>
80101f75:	8d 76 00             	lea    0x0(%esi),%esi
    name[len] = 0;
80101f78:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
80101f7b:	8d 14 01             	lea    (%ecx,%eax,1),%edx
    memmove(name, s, len);
80101f7e:	83 ec 04             	sub    $0x4,%esp
80101f81:	89 55 e0             	mov    %edx,-0x20(%ebp)
80101f84:	50                   	push   %eax
80101f85:	53                   	push   %ebx
    name[len] = 0;
80101f86:	89 fb                	mov    %edi,%ebx
    memmove(name, s, len);
80101f88:	ff 75 e4             	push   -0x1c(%ebp)
80101f8b:	e8 b0 33 00 00       	call   80105340 <memmove>
    name[len] = 0;
80101f90:	8b 55 e0             	mov    -0x20(%ebp),%edx
80101f93:	83 c4 10             	add    $0x10,%esp
80101f96:	c6 02 00             	movb   $0x0,(%edx)
80101f99:	e9 41 ff ff ff       	jmp    80101edf <namex+0x9f>
80101f9e:	66 90                	xchg   %ax,%ax
      return 0;
    }
    iunlockput(ip);
    ip = next;
  }
  if(nameiparent){
80101fa0:	8b 45 dc             	mov    -0x24(%ebp),%eax
80101fa3:	85 c0                	test   %eax,%eax
80101fa5:	0f 85 be 00 00 00    	jne    80102069 <namex+0x229>
    iput(ip);
    return 0;
  }
  return ip;
}
80101fab:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101fae:	89 f0                	mov    %esi,%eax
80101fb0:	5b                   	pop    %ebx
80101fb1:	5e                   	pop    %esi
80101fb2:	5f                   	pop    %edi
80101fb3:	5d                   	pop    %ebp
80101fb4:	c3                   	ret    
  while(*path != '/' && *path != 0)
80101fb5:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80101fb8:	89 df                	mov    %ebx,%edi
80101fba:	31 c0                	xor    %eax,%eax
80101fbc:	eb c0                	jmp    80101f7e <namex+0x13e>
    ip = iget(ROOTDEV, ROOTINO);
80101fbe:	ba 01 00 00 00       	mov    $0x1,%edx
80101fc3:	b8 01 00 00 00       	mov    $0x1,%eax
80101fc8:	e8 33 f4 ff ff       	call   80101400 <iget>
80101fcd:	89 c6                	mov    %eax,%esi
80101fcf:	e9 b7 fe ff ff       	jmp    80101e8b <namex+0x4b>
  if(ip == 0 || !holdingsleep(&ip->lock) || ip->ref < 1)
80101fd4:	83 ec 0c             	sub    $0xc,%esp
80101fd7:	8d 5e 0c             	lea    0xc(%esi),%ebx
80101fda:	53                   	push   %ebx
80101fdb:	e8 e0 2f 00 00       	call   80104fc0 <holdingsleep>
80101fe0:	83 c4 10             	add    $0x10,%esp
80101fe3:	85 c0                	test   %eax,%eax
80101fe5:	0f 84 91 00 00 00    	je     8010207c <namex+0x23c>
80101feb:	8b 46 08             	mov    0x8(%esi),%eax
80101fee:	85 c0                	test   %eax,%eax
80101ff0:	0f 8e 86 00 00 00    	jle    8010207c <namex+0x23c>
  releasesleep(&ip->lock);
80101ff6:	83 ec 0c             	sub    $0xc,%esp
80101ff9:	53                   	push   %ebx
80101ffa:	e8 81 2f 00 00       	call   80104f80 <releasesleep>
  iput(ip);
80101fff:	89 34 24             	mov    %esi,(%esp)
      return 0;
80102002:	31 f6                	xor    %esi,%esi
  iput(ip);
80102004:	e8 57 f9 ff ff       	call   80101960 <iput>
      return 0;
80102009:	83 c4 10             	add    $0x10,%esp
}
8010200c:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010200f:	89 f0                	mov    %esi,%eax
80102011:	5b                   	pop    %ebx
80102012:	5e                   	pop    %esi
80102013:	5f                   	pop    %edi
80102014:	5d                   	pop    %ebp
80102015:	c3                   	ret    
  if(ip == 0 || !holdingsleep(&ip->lock) || ip->ref < 1)
80102016:	83 ec 0c             	sub    $0xc,%esp
80102019:	89 55 e4             	mov    %edx,-0x1c(%ebp)
8010201c:	52                   	push   %edx
8010201d:	e8 9e 2f 00 00       	call   80104fc0 <holdingsleep>
80102022:	83 c4 10             	add    $0x10,%esp
80102025:	85 c0                	test   %eax,%eax
80102027:	74 53                	je     8010207c <namex+0x23c>
80102029:	8b 4e 08             	mov    0x8(%esi),%ecx
8010202c:	85 c9                	test   %ecx,%ecx
8010202e:	7e 4c                	jle    8010207c <namex+0x23c>
  releasesleep(&ip->lock);
80102030:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80102033:	83 ec 0c             	sub    $0xc,%esp
80102036:	52                   	push   %edx
80102037:	eb c1                	jmp    80101ffa <namex+0x1ba>
  if(ip == 0 || !holdingsleep(&ip->lock) || ip->ref < 1)
80102039:	83 ec 0c             	sub    $0xc,%esp
8010203c:	8d 5e 0c             	lea    0xc(%esi),%ebx
8010203f:	53                   	push   %ebx
80102040:	e8 7b 2f 00 00       	call   80104fc0 <holdingsleep>
80102045:	83 c4 10             	add    $0x10,%esp
80102048:	85 c0                	test   %eax,%eax
8010204a:	74 30                	je     8010207c <namex+0x23c>
8010204c:	8b 7e 08             	mov    0x8(%esi),%edi
8010204f:	85 ff                	test   %edi,%edi
80102051:	7e 29                	jle    8010207c <namex+0x23c>
  releasesleep(&ip->lock);
80102053:	83 ec 0c             	sub    $0xc,%esp
80102056:	53                   	push   %ebx
80102057:	e8 24 2f 00 00       	call   80104f80 <releasesleep>
}
8010205c:	83 c4 10             	add    $0x10,%esp
}
8010205f:	8d 65 f4             	lea    -0xc(%ebp),%esp
80102062:	89 f0                	mov    %esi,%eax
80102064:	5b                   	pop    %ebx
80102065:	5e                   	pop    %esi
80102066:	5f                   	pop    %edi
80102067:	5d                   	pop    %ebp
80102068:	c3                   	ret    
    iput(ip);
80102069:	83 ec 0c             	sub    $0xc,%esp
8010206c:	56                   	push   %esi
    return 0;
8010206d:	31 f6                	xor    %esi,%esi
    iput(ip);
8010206f:	e8 ec f8 ff ff       	call   80101960 <iput>
    return 0;
80102074:	83 c4 10             	add    $0x10,%esp
80102077:	e9 2f ff ff ff       	jmp    80101fab <namex+0x16b>
    panic("iunlock");
8010207c:	83 ec 0c             	sub    $0xc,%esp
8010207f:	68 89 7f 10 80       	push   $0x80107f89
80102084:	e8 f7 e2 ff ff       	call   80100380 <panic>
80102089:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80102090 <dirlink>:
{
80102090:	55                   	push   %ebp
80102091:	89 e5                	mov    %esp,%ebp
80102093:	57                   	push   %edi
80102094:	56                   	push   %esi
80102095:	53                   	push   %ebx
80102096:	83 ec 20             	sub    $0x20,%esp
80102099:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if((ip = dirlookup(dp, name, 0)) != 0){
8010209c:	6a 00                	push   $0x0
8010209e:	ff 75 0c             	push   0xc(%ebp)
801020a1:	53                   	push   %ebx
801020a2:	e8 e9 fc ff ff       	call   80101d90 <dirlookup>
801020a7:	83 c4 10             	add    $0x10,%esp
801020aa:	85 c0                	test   %eax,%eax
801020ac:	75 67                	jne    80102115 <dirlink+0x85>
  for(off = 0; off < dp->size; off += sizeof(de)){
801020ae:	8b 7b 58             	mov    0x58(%ebx),%edi
801020b1:	8d 75 d8             	lea    -0x28(%ebp),%esi
801020b4:	85 ff                	test   %edi,%edi
801020b6:	74 29                	je     801020e1 <dirlink+0x51>
801020b8:	31 ff                	xor    %edi,%edi
801020ba:	8d 75 d8             	lea    -0x28(%ebp),%esi
801020bd:	eb 09                	jmp    801020c8 <dirlink+0x38>
801020bf:	90                   	nop
801020c0:	83 c7 10             	add    $0x10,%edi
801020c3:	3b 7b 58             	cmp    0x58(%ebx),%edi
801020c6:	73 19                	jae    801020e1 <dirlink+0x51>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801020c8:	6a 10                	push   $0x10
801020ca:	57                   	push   %edi
801020cb:	56                   	push   %esi
801020cc:	53                   	push   %ebx
801020cd:	e8 6e fa ff ff       	call   80101b40 <readi>
801020d2:	83 c4 10             	add    $0x10,%esp
801020d5:	83 f8 10             	cmp    $0x10,%eax
801020d8:	75 4e                	jne    80102128 <dirlink+0x98>
    if(de.inum == 0)
801020da:	66 83 7d d8 00       	cmpw   $0x0,-0x28(%ebp)
801020df:	75 df                	jne    801020c0 <dirlink+0x30>
  strncpy(de.name, name, DIRSIZ);
801020e1:	83 ec 04             	sub    $0x4,%esp
801020e4:	8d 45 da             	lea    -0x26(%ebp),%eax
801020e7:	6a 0e                	push   $0xe
801020e9:	ff 75 0c             	push   0xc(%ebp)
801020ec:	50                   	push   %eax
801020ed:	e8 0e 33 00 00       	call   80105400 <strncpy>
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801020f2:	6a 10                	push   $0x10
  de.inum = inum;
801020f4:	8b 45 10             	mov    0x10(%ebp),%eax
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801020f7:	57                   	push   %edi
801020f8:	56                   	push   %esi
801020f9:	53                   	push   %ebx
  de.inum = inum;
801020fa:	66 89 45 d8          	mov    %ax,-0x28(%ebp)
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801020fe:	e8 3d fb ff ff       	call   80101c40 <writei>
80102103:	83 c4 20             	add    $0x20,%esp
80102106:	83 f8 10             	cmp    $0x10,%eax
80102109:	75 2a                	jne    80102135 <dirlink+0xa5>
  return 0;
8010210b:	31 c0                	xor    %eax,%eax
}
8010210d:	8d 65 f4             	lea    -0xc(%ebp),%esp
80102110:	5b                   	pop    %ebx
80102111:	5e                   	pop    %esi
80102112:	5f                   	pop    %edi
80102113:	5d                   	pop    %ebp
80102114:	c3                   	ret    
    iput(ip);
80102115:	83 ec 0c             	sub    $0xc,%esp
80102118:	50                   	push   %eax
80102119:	e8 42 f8 ff ff       	call   80101960 <iput>
    return -1;
8010211e:	83 c4 10             	add    $0x10,%esp
80102121:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80102126:	eb e5                	jmp    8010210d <dirlink+0x7d>
      panic("dirlink read");
80102128:	83 ec 0c             	sub    $0xc,%esp
8010212b:	68 b2 7f 10 80       	push   $0x80107fb2
80102130:	e8 4b e2 ff ff       	call   80100380 <panic>
    panic("dirlink");
80102135:	83 ec 0c             	sub    $0xc,%esp
80102138:	68 8a 87 10 80       	push   $0x8010878a
8010213d:	e8 3e e2 ff ff       	call   80100380 <panic>
80102142:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102149:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80102150 <namei>:

struct inode*
namei(char *path)
{
80102150:	55                   	push   %ebp
  char name[DIRSIZ];
  return namex(path, 0, name);
80102151:	31 d2                	xor    %edx,%edx
{
80102153:	89 e5                	mov    %esp,%ebp
80102155:	83 ec 18             	sub    $0x18,%esp
  return namex(path, 0, name);
80102158:	8b 45 08             	mov    0x8(%ebp),%eax
8010215b:	8d 4d ea             	lea    -0x16(%ebp),%ecx
8010215e:	e8 dd fc ff ff       	call   80101e40 <namex>
}
80102163:	c9                   	leave  
80102164:	c3                   	ret    
80102165:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010216c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80102170 <nameiparent>:

struct inode*
nameiparent(char *path, char *name)
{
80102170:	55                   	push   %ebp
  return namex(path, 1, name);
80102171:	ba 01 00 00 00       	mov    $0x1,%edx
{
80102176:	89 e5                	mov    %esp,%ebp
  return namex(path, 1, name);
80102178:	8b 4d 0c             	mov    0xc(%ebp),%ecx
8010217b:	8b 45 08             	mov    0x8(%ebp),%eax
}
8010217e:	5d                   	pop    %ebp
  return namex(path, 1, name);
8010217f:	e9 bc fc ff ff       	jmp    80101e40 <namex>
80102184:	66 90                	xchg   %ax,%ax
80102186:	66 90                	xchg   %ax,%ax
80102188:	66 90                	xchg   %ax,%ax
8010218a:	66 90                	xchg   %ax,%ax
8010218c:	66 90                	xchg   %ax,%ax
8010218e:	66 90                	xchg   %ax,%ax

80102190 <idestart>:
}

// Start the request for b.  Caller must hold idelock.
static void
idestart(struct buf *b)
{
80102190:	55                   	push   %ebp
80102191:	89 e5                	mov    %esp,%ebp
80102193:	57                   	push   %edi
80102194:	56                   	push   %esi
80102195:	53                   	push   %ebx
80102196:	83 ec 0c             	sub    $0xc,%esp
  if(b == 0)
80102199:	85 c0                	test   %eax,%eax
8010219b:	0f 84 b4 00 00 00    	je     80102255 <idestart+0xc5>
    panic("idestart");
  if(b->blockno >= FSSIZE)
801021a1:	8b 70 08             	mov    0x8(%eax),%esi
801021a4:	89 c3                	mov    %eax,%ebx
801021a6:	81 fe e7 03 00 00    	cmp    $0x3e7,%esi
801021ac:	0f 87 96 00 00 00    	ja     80102248 <idestart+0xb8>
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801021b2:	b9 f7 01 00 00       	mov    $0x1f7,%ecx
801021b7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801021be:	66 90                	xchg   %ax,%ax
801021c0:	89 ca                	mov    %ecx,%edx
801021c2:	ec                   	in     (%dx),%al
  while(((r = inb(0x1f7)) & (IDE_BSY|IDE_DRDY)) != IDE_DRDY)
801021c3:	83 e0 c0             	and    $0xffffffc0,%eax
801021c6:	3c 40                	cmp    $0x40,%al
801021c8:	75 f6                	jne    801021c0 <idestart+0x30>
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801021ca:	31 ff                	xor    %edi,%edi
801021cc:	ba f6 03 00 00       	mov    $0x3f6,%edx
801021d1:	89 f8                	mov    %edi,%eax
801021d3:	ee                   	out    %al,(%dx)
801021d4:	b8 01 00 00 00       	mov    $0x1,%eax
801021d9:	ba f2 01 00 00       	mov    $0x1f2,%edx
801021de:	ee                   	out    %al,(%dx)
801021df:	ba f3 01 00 00       	mov    $0x1f3,%edx
801021e4:	89 f0                	mov    %esi,%eax
801021e6:	ee                   	out    %al,(%dx)

  idewait(0);
  outb(0x3f6, 0);  // generate interrupt
  outb(0x1f2, sector_per_block);  // number of sectors
  outb(0x1f3, sector & 0xff);
  outb(0x1f4, (sector >> 8) & 0xff);
801021e7:	89 f0                	mov    %esi,%eax
801021e9:	ba f4 01 00 00       	mov    $0x1f4,%edx
801021ee:	c1 f8 08             	sar    $0x8,%eax
801021f1:	ee                   	out    %al,(%dx)
801021f2:	ba f5 01 00 00       	mov    $0x1f5,%edx
801021f7:	89 f8                	mov    %edi,%eax
801021f9:	ee                   	out    %al,(%dx)
  outb(0x1f5, (sector >> 16) & 0xff);
  outb(0x1f6, 0xe0 | ((b->dev&1)<<4) | ((sector>>24)&0x0f));
801021fa:	0f b6 43 04          	movzbl 0x4(%ebx),%eax
801021fe:	ba f6 01 00 00       	mov    $0x1f6,%edx
80102203:	c1 e0 04             	shl    $0x4,%eax
80102206:	83 e0 10             	and    $0x10,%eax
80102209:	83 c8 e0             	or     $0xffffffe0,%eax
8010220c:	ee                   	out    %al,(%dx)
  if(b->flags & B_DIRTY){
8010220d:	f6 03 04             	testb  $0x4,(%ebx)
80102210:	75 16                	jne    80102228 <idestart+0x98>
80102212:	b8 20 00 00 00       	mov    $0x20,%eax
80102217:	89 ca                	mov    %ecx,%edx
80102219:	ee                   	out    %al,(%dx)
    outb(0x1f7, write_cmd);
    outsl(0x1f0, b->data, BSIZE/4);
  } else {
    outb(0x1f7, read_cmd);
  }
}
8010221a:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010221d:	5b                   	pop    %ebx
8010221e:	5e                   	pop    %esi
8010221f:	5f                   	pop    %edi
80102220:	5d                   	pop    %ebp
80102221:	c3                   	ret    
80102222:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80102228:	b8 30 00 00 00       	mov    $0x30,%eax
8010222d:	89 ca                	mov    %ecx,%edx
8010222f:	ee                   	out    %al,(%dx)
  asm volatile("cld; rep outsl" :
80102230:	b9 80 00 00 00       	mov    $0x80,%ecx
    outsl(0x1f0, b->data, BSIZE/4);
80102235:	8d 73 5c             	lea    0x5c(%ebx),%esi
80102238:	ba f0 01 00 00       	mov    $0x1f0,%edx
8010223d:	fc                   	cld    
8010223e:	f3 6f                	rep outsl %ds:(%esi),(%dx)
}
80102240:	8d 65 f4             	lea    -0xc(%ebp),%esp
80102243:	5b                   	pop    %ebx
80102244:	5e                   	pop    %esi
80102245:	5f                   	pop    %edi
80102246:	5d                   	pop    %ebp
80102247:	c3                   	ret    
    panic("incorrect blockno");
80102248:	83 ec 0c             	sub    $0xc,%esp
8010224b:	68 1c 80 10 80       	push   $0x8010801c
80102250:	e8 2b e1 ff ff       	call   80100380 <panic>
    panic("idestart");
80102255:	83 ec 0c             	sub    $0xc,%esp
80102258:	68 13 80 10 80       	push   $0x80108013
8010225d:	e8 1e e1 ff ff       	call   80100380 <panic>
80102262:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102269:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80102270 <ideinit>:
{
80102270:	55                   	push   %ebp
80102271:	89 e5                	mov    %esp,%ebp
80102273:	83 ec 10             	sub    $0x10,%esp
  initlock(&idelock, "ide");
80102276:	68 2e 80 10 80       	push   $0x8010802e
8010227b:	68 00 26 11 80       	push   $0x80112600
80102280:	e8 8b 2d 00 00       	call   80105010 <initlock>
  ioapicenable(IRQ_IDE, ncpu - 1);
80102285:	58                   	pop    %eax
80102286:	a1 84 27 11 80       	mov    0x80112784,%eax
8010228b:	5a                   	pop    %edx
8010228c:	83 e8 01             	sub    $0x1,%eax
8010228f:	50                   	push   %eax
80102290:	6a 0e                	push   $0xe
80102292:	e8 99 02 00 00       	call   80102530 <ioapicenable>
  while(((r = inb(0x1f7)) & (IDE_BSY|IDE_DRDY)) != IDE_DRDY)
80102297:	83 c4 10             	add    $0x10,%esp
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
8010229a:	ba f7 01 00 00       	mov    $0x1f7,%edx
8010229f:	90                   	nop
801022a0:	ec                   	in     (%dx),%al
801022a1:	83 e0 c0             	and    $0xffffffc0,%eax
801022a4:	3c 40                	cmp    $0x40,%al
801022a6:	75 f8                	jne    801022a0 <ideinit+0x30>
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801022a8:	b8 f0 ff ff ff       	mov    $0xfffffff0,%eax
801022ad:	ba f6 01 00 00       	mov    $0x1f6,%edx
801022b2:	ee                   	out    %al,(%dx)
801022b3:	b9 e8 03 00 00       	mov    $0x3e8,%ecx
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801022b8:	ba f7 01 00 00       	mov    $0x1f7,%edx
801022bd:	eb 06                	jmp    801022c5 <ideinit+0x55>
801022bf:	90                   	nop
  for(i=0; i<1000; i++){
801022c0:	83 e9 01             	sub    $0x1,%ecx
801022c3:	74 0f                	je     801022d4 <ideinit+0x64>
801022c5:	ec                   	in     (%dx),%al
    if(inb(0x1f7) != 0){
801022c6:	84 c0                	test   %al,%al
801022c8:	74 f6                	je     801022c0 <ideinit+0x50>
      havedisk1 = 1;
801022ca:	c7 05 e0 25 11 80 01 	movl   $0x1,0x801125e0
801022d1:	00 00 00 
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801022d4:	b8 e0 ff ff ff       	mov    $0xffffffe0,%eax
801022d9:	ba f6 01 00 00       	mov    $0x1f6,%edx
801022de:	ee                   	out    %al,(%dx)
}
801022df:	c9                   	leave  
801022e0:	c3                   	ret    
801022e1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801022e8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801022ef:	90                   	nop

801022f0 <ideintr>:

// Interrupt handler.
void
ideintr(void)
{
801022f0:	55                   	push   %ebp
801022f1:	89 e5                	mov    %esp,%ebp
801022f3:	57                   	push   %edi
801022f4:	56                   	push   %esi
801022f5:	53                   	push   %ebx
801022f6:	83 ec 18             	sub    $0x18,%esp
  struct buf *b;

  // First queued buffer is the active request.
  acquire(&idelock);
801022f9:	68 00 26 11 80       	push   $0x80112600
801022fe:	e8 dd 2e 00 00       	call   801051e0 <acquire>

  if((b = idequeue) == 0){
80102303:	8b 1d e4 25 11 80    	mov    0x801125e4,%ebx
80102309:	83 c4 10             	add    $0x10,%esp
8010230c:	85 db                	test   %ebx,%ebx
8010230e:	74 63                	je     80102373 <ideintr+0x83>
    release(&idelock);
    return;
  }
  idequeue = b->qnext;
80102310:	8b 43 58             	mov    0x58(%ebx),%eax
80102313:	a3 e4 25 11 80       	mov    %eax,0x801125e4

  // Read data if needed.
  if(!(b->flags & B_DIRTY) && idewait(1) >= 0)
80102318:	8b 33                	mov    (%ebx),%esi
8010231a:	f7 c6 04 00 00 00    	test   $0x4,%esi
80102320:	75 2f                	jne    80102351 <ideintr+0x61>
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102322:	ba f7 01 00 00       	mov    $0x1f7,%edx
80102327:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010232e:	66 90                	xchg   %ax,%ax
80102330:	ec                   	in     (%dx),%al
  while(((r = inb(0x1f7)) & (IDE_BSY|IDE_DRDY)) != IDE_DRDY)
80102331:	89 c1                	mov    %eax,%ecx
80102333:	83 e1 c0             	and    $0xffffffc0,%ecx
80102336:	80 f9 40             	cmp    $0x40,%cl
80102339:	75 f5                	jne    80102330 <ideintr+0x40>
  if(checkerr && (r & (IDE_DF|IDE_ERR)) != 0)
8010233b:	a8 21                	test   $0x21,%al
8010233d:	75 12                	jne    80102351 <ideintr+0x61>
    insl(0x1f0, b->data, BSIZE/4);
8010233f:	8d 7b 5c             	lea    0x5c(%ebx),%edi
  asm volatile("cld; rep insl" :
80102342:	b9 80 00 00 00       	mov    $0x80,%ecx
80102347:	ba f0 01 00 00       	mov    $0x1f0,%edx
8010234c:	fc                   	cld    
8010234d:	f3 6d                	rep insl (%dx),%es:(%edi)

  // Wake process waiting for this buf.
  b->flags |= B_VALID;
8010234f:	8b 33                	mov    (%ebx),%esi
  b->flags &= ~B_DIRTY;
80102351:	83 e6 fb             	and    $0xfffffffb,%esi
  wakeup(b);
80102354:	83 ec 0c             	sub    $0xc,%esp
  b->flags &= ~B_DIRTY;
80102357:	83 ce 02             	or     $0x2,%esi
8010235a:	89 33                	mov    %esi,(%ebx)
  wakeup(b);
8010235c:	53                   	push   %ebx
8010235d:	e8 7e 21 00 00       	call   801044e0 <wakeup>

  // Start disk on next buf in queue.
  if(idequeue != 0)
80102362:	a1 e4 25 11 80       	mov    0x801125e4,%eax
80102367:	83 c4 10             	add    $0x10,%esp
8010236a:	85 c0                	test   %eax,%eax
8010236c:	74 05                	je     80102373 <ideintr+0x83>
    idestart(idequeue);
8010236e:	e8 1d fe ff ff       	call   80102190 <idestart>
    release(&idelock);
80102373:	83 ec 0c             	sub    $0xc,%esp
80102376:	68 00 26 11 80       	push   $0x80112600
8010237b:	e8 00 2e 00 00       	call   80105180 <release>

  release(&idelock);
}
80102380:	8d 65 f4             	lea    -0xc(%ebp),%esp
80102383:	5b                   	pop    %ebx
80102384:	5e                   	pop    %esi
80102385:	5f                   	pop    %edi
80102386:	5d                   	pop    %ebp
80102387:	c3                   	ret    
80102388:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010238f:	90                   	nop

80102390 <iderw>:
// Sync buf with disk.
// If B_DIRTY is set, write buf to disk, clear B_DIRTY, set B_VALID.
// Else if B_VALID is not set, read buf from disk, set B_VALID.
void
iderw(struct buf *b)
{
80102390:	55                   	push   %ebp
80102391:	89 e5                	mov    %esp,%ebp
80102393:	53                   	push   %ebx
80102394:	83 ec 10             	sub    $0x10,%esp
80102397:	8b 5d 08             	mov    0x8(%ebp),%ebx
  struct buf **pp;

  if(!holdingsleep(&b->lock))
8010239a:	8d 43 0c             	lea    0xc(%ebx),%eax
8010239d:	50                   	push   %eax
8010239e:	e8 1d 2c 00 00       	call   80104fc0 <holdingsleep>
801023a3:	83 c4 10             	add    $0x10,%esp
801023a6:	85 c0                	test   %eax,%eax
801023a8:	0f 84 c3 00 00 00    	je     80102471 <iderw+0xe1>
    panic("iderw: buf not locked");
  if((b->flags & (B_VALID|B_DIRTY)) == B_VALID)
801023ae:	8b 03                	mov    (%ebx),%eax
801023b0:	83 e0 06             	and    $0x6,%eax
801023b3:	83 f8 02             	cmp    $0x2,%eax
801023b6:	0f 84 a8 00 00 00    	je     80102464 <iderw+0xd4>
    panic("iderw: nothing to do");
  if(b->dev != 0 && !havedisk1)
801023bc:	8b 53 04             	mov    0x4(%ebx),%edx
801023bf:	85 d2                	test   %edx,%edx
801023c1:	74 0d                	je     801023d0 <iderw+0x40>
801023c3:	a1 e0 25 11 80       	mov    0x801125e0,%eax
801023c8:	85 c0                	test   %eax,%eax
801023ca:	0f 84 87 00 00 00    	je     80102457 <iderw+0xc7>
    panic("iderw: ide disk 1 not present");

  acquire(&idelock);  //DOC:acquire-lock
801023d0:	83 ec 0c             	sub    $0xc,%esp
801023d3:	68 00 26 11 80       	push   $0x80112600
801023d8:	e8 03 2e 00 00       	call   801051e0 <acquire>

  // Append b to idequeue.
  b->qnext = 0;
  for(pp=&idequeue; *pp; pp=&(*pp)->qnext)  //DOC:insert-queue
801023dd:	a1 e4 25 11 80       	mov    0x801125e4,%eax
  b->qnext = 0;
801023e2:	c7 43 58 00 00 00 00 	movl   $0x0,0x58(%ebx)
  for(pp=&idequeue; *pp; pp=&(*pp)->qnext)  //DOC:insert-queue
801023e9:	83 c4 10             	add    $0x10,%esp
801023ec:	85 c0                	test   %eax,%eax
801023ee:	74 60                	je     80102450 <iderw+0xc0>
801023f0:	89 c2                	mov    %eax,%edx
801023f2:	8b 40 58             	mov    0x58(%eax),%eax
801023f5:	85 c0                	test   %eax,%eax
801023f7:	75 f7                	jne    801023f0 <iderw+0x60>
801023f9:	83 c2 58             	add    $0x58,%edx
    ;
  *pp = b;
801023fc:	89 1a                	mov    %ebx,(%edx)

  // Start disk if necessary.
  if(idequeue == b)
801023fe:	39 1d e4 25 11 80    	cmp    %ebx,0x801125e4
80102404:	74 3a                	je     80102440 <iderw+0xb0>
    idestart(b);

  // Wait for request to finish.
  while((b->flags & (B_VALID|B_DIRTY)) != B_VALID){
80102406:	8b 03                	mov    (%ebx),%eax
80102408:	83 e0 06             	and    $0x6,%eax
8010240b:	83 f8 02             	cmp    $0x2,%eax
8010240e:	74 1b                	je     8010242b <iderw+0x9b>
    sleep(b, &idelock);
80102410:	83 ec 08             	sub    $0x8,%esp
80102413:	68 00 26 11 80       	push   $0x80112600
80102418:	53                   	push   %ebx
80102419:	e8 02 20 00 00       	call   80104420 <sleep>
  while((b->flags & (B_VALID|B_DIRTY)) != B_VALID){
8010241e:	8b 03                	mov    (%ebx),%eax
80102420:	83 c4 10             	add    $0x10,%esp
80102423:	83 e0 06             	and    $0x6,%eax
80102426:	83 f8 02             	cmp    $0x2,%eax
80102429:	75 e5                	jne    80102410 <iderw+0x80>
  }


  release(&idelock);
8010242b:	c7 45 08 00 26 11 80 	movl   $0x80112600,0x8(%ebp)
}
80102432:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80102435:	c9                   	leave  
  release(&idelock);
80102436:	e9 45 2d 00 00       	jmp    80105180 <release>
8010243b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010243f:	90                   	nop
    idestart(b);
80102440:	89 d8                	mov    %ebx,%eax
80102442:	e8 49 fd ff ff       	call   80102190 <idestart>
80102447:	eb bd                	jmp    80102406 <iderw+0x76>
80102449:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  for(pp=&idequeue; *pp; pp=&(*pp)->qnext)  //DOC:insert-queue
80102450:	ba e4 25 11 80       	mov    $0x801125e4,%edx
80102455:	eb a5                	jmp    801023fc <iderw+0x6c>
    panic("iderw: ide disk 1 not present");
80102457:	83 ec 0c             	sub    $0xc,%esp
8010245a:	68 5d 80 10 80       	push   $0x8010805d
8010245f:	e8 1c df ff ff       	call   80100380 <panic>
    panic("iderw: nothing to do");
80102464:	83 ec 0c             	sub    $0xc,%esp
80102467:	68 48 80 10 80       	push   $0x80108048
8010246c:	e8 0f df ff ff       	call   80100380 <panic>
    panic("iderw: buf not locked");
80102471:	83 ec 0c             	sub    $0xc,%esp
80102474:	68 32 80 10 80       	push   $0x80108032
80102479:	e8 02 df ff ff       	call   80100380 <panic>
8010247e:	66 90                	xchg   %ax,%ax

80102480 <ioapicinit>:
  ioapic->data = data;
}

void
ioapicinit(void)
{
80102480:	55                   	push   %ebp
  int i, id, maxintr;

  ioapic = (volatile struct ioapic*)IOAPIC;
80102481:	c7 05 34 26 11 80 00 	movl   $0xfec00000,0x80112634
80102488:	00 c0 fe 
{
8010248b:	89 e5                	mov    %esp,%ebp
8010248d:	56                   	push   %esi
8010248e:	53                   	push   %ebx
  ioapic->reg = reg;
8010248f:	c7 05 00 00 c0 fe 01 	movl   $0x1,0xfec00000
80102496:	00 00 00 
  return ioapic->data;
80102499:	8b 15 34 26 11 80    	mov    0x80112634,%edx
8010249f:	8b 72 10             	mov    0x10(%edx),%esi
  ioapic->reg = reg;
801024a2:	c7 02 00 00 00 00    	movl   $0x0,(%edx)
  return ioapic->data;
801024a8:	8b 0d 34 26 11 80    	mov    0x80112634,%ecx
  maxintr = (ioapicread(REG_VER) >> 16) & 0xFF;
  id = ioapicread(REG_ID) >> 24;
  if(id != ioapicid)
801024ae:	0f b6 15 80 27 11 80 	movzbl 0x80112780,%edx
  maxintr = (ioapicread(REG_VER) >> 16) & 0xFF;
801024b5:	c1 ee 10             	shr    $0x10,%esi
801024b8:	89 f0                	mov    %esi,%eax
801024ba:	0f b6 f0             	movzbl %al,%esi
  return ioapic->data;
801024bd:	8b 41 10             	mov    0x10(%ecx),%eax
  id = ioapicread(REG_ID) >> 24;
801024c0:	c1 e8 18             	shr    $0x18,%eax
  if(id != ioapicid)
801024c3:	39 c2                	cmp    %eax,%edx
801024c5:	74 16                	je     801024dd <ioapicinit+0x5d>
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");
801024c7:	83 ec 0c             	sub    $0xc,%esp
801024ca:	68 7c 80 10 80       	push   $0x8010807c
801024cf:	e8 cc e1 ff ff       	call   801006a0 <cprintf>
  ioapic->reg = reg;
801024d4:	8b 0d 34 26 11 80    	mov    0x80112634,%ecx
801024da:	83 c4 10             	add    $0x10,%esp
801024dd:	83 c6 21             	add    $0x21,%esi
{
801024e0:	ba 10 00 00 00       	mov    $0x10,%edx
801024e5:	b8 20 00 00 00       	mov    $0x20,%eax
801024ea:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  ioapic->reg = reg;
801024f0:	89 11                	mov    %edx,(%ecx)

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
801024f2:	89 c3                	mov    %eax,%ebx
  ioapic->data = data;
801024f4:	8b 0d 34 26 11 80    	mov    0x80112634,%ecx
  for(i = 0; i <= maxintr; i++){
801024fa:	83 c0 01             	add    $0x1,%eax
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
801024fd:	81 cb 00 00 01 00    	or     $0x10000,%ebx
  ioapic->data = data;
80102503:	89 59 10             	mov    %ebx,0x10(%ecx)
  ioapic->reg = reg;
80102506:	8d 5a 01             	lea    0x1(%edx),%ebx
  for(i = 0; i <= maxintr; i++){
80102509:	83 c2 02             	add    $0x2,%edx
  ioapic->reg = reg;
8010250c:	89 19                	mov    %ebx,(%ecx)
  ioapic->data = data;
8010250e:	8b 0d 34 26 11 80    	mov    0x80112634,%ecx
80102514:	c7 41 10 00 00 00 00 	movl   $0x0,0x10(%ecx)
  for(i = 0; i <= maxintr; i++){
8010251b:	39 f0                	cmp    %esi,%eax
8010251d:	75 d1                	jne    801024f0 <ioapicinit+0x70>
    ioapicwrite(REG_TABLE+2*i+1, 0);
  }
}
8010251f:	8d 65 f8             	lea    -0x8(%ebp),%esp
80102522:	5b                   	pop    %ebx
80102523:	5e                   	pop    %esi
80102524:	5d                   	pop    %ebp
80102525:	c3                   	ret    
80102526:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010252d:	8d 76 00             	lea    0x0(%esi),%esi

80102530 <ioapicenable>:

void
ioapicenable(int irq, int cpunum)
{
80102530:	55                   	push   %ebp
  ioapic->reg = reg;
80102531:	8b 0d 34 26 11 80    	mov    0x80112634,%ecx
{
80102537:	89 e5                	mov    %esp,%ebp
80102539:	8b 45 08             	mov    0x8(%ebp),%eax
  // Mark interrupt edge-triggered, active high,
  // enabled, and routed to the given cpunum,
  // which happens to be that cpu's APIC ID.
  ioapicwrite(REG_TABLE+2*irq, T_IRQ0 + irq);
8010253c:	8d 50 20             	lea    0x20(%eax),%edx
8010253f:	8d 44 00 10          	lea    0x10(%eax,%eax,1),%eax
  ioapic->reg = reg;
80102543:	89 01                	mov    %eax,(%ecx)
  ioapic->data = data;
80102545:	8b 0d 34 26 11 80    	mov    0x80112634,%ecx
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
8010254b:	83 c0 01             	add    $0x1,%eax
  ioapic->data = data;
8010254e:	89 51 10             	mov    %edx,0x10(%ecx)
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
80102551:	8b 55 0c             	mov    0xc(%ebp),%edx
  ioapic->reg = reg;
80102554:	89 01                	mov    %eax,(%ecx)
  ioapic->data = data;
80102556:	a1 34 26 11 80       	mov    0x80112634,%eax
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
8010255b:	c1 e2 18             	shl    $0x18,%edx
  ioapic->data = data;
8010255e:	89 50 10             	mov    %edx,0x10(%eax)
}
80102561:	5d                   	pop    %ebp
80102562:	c3                   	ret    
80102563:	66 90                	xchg   %ax,%ax
80102565:	66 90                	xchg   %ax,%ax
80102567:	66 90                	xchg   %ax,%ax
80102569:	66 90                	xchg   %ax,%ax
8010256b:	66 90                	xchg   %ax,%ax
8010256d:	66 90                	xchg   %ax,%ax
8010256f:	90                   	nop

80102570 <kfree>:
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(char *v)
{
80102570:	55                   	push   %ebp
80102571:	89 e5                	mov    %esp,%ebp
80102573:	53                   	push   %ebx
80102574:	83 ec 04             	sub    $0x4,%esp
80102577:	8b 5d 08             	mov    0x8(%ebp),%ebx
  struct run *r;

  if((uint)v % PGSIZE || v < end || V2P(v) >= PHYSTOP)
8010257a:	f7 c3 ff 0f 00 00    	test   $0xfff,%ebx
80102580:	75 76                	jne    801025f8 <kfree+0x88>
80102582:	81 fb f0 73 11 80    	cmp    $0x801173f0,%ebx
80102588:	72 6e                	jb     801025f8 <kfree+0x88>
8010258a:	8d 83 00 00 00 80    	lea    -0x80000000(%ebx),%eax
80102590:	3d ff ff ff 0d       	cmp    $0xdffffff,%eax
80102595:	77 61                	ja     801025f8 <kfree+0x88>
    panic("kfree");

  // Fill with junk to catch dangling refs.
  memset(v, 1, PGSIZE);
80102597:	83 ec 04             	sub    $0x4,%esp
8010259a:	68 00 10 00 00       	push   $0x1000
8010259f:	6a 01                	push   $0x1
801025a1:	53                   	push   %ebx
801025a2:	e8 f9 2c 00 00       	call   801052a0 <memset>

  if(kmem.use_lock)
801025a7:	8b 15 74 26 11 80    	mov    0x80112674,%edx
801025ad:	83 c4 10             	add    $0x10,%esp
801025b0:	85 d2                	test   %edx,%edx
801025b2:	75 1c                	jne    801025d0 <kfree+0x60>
    acquire(&kmem.lock);
  r = (struct run*)v;
  r->next = kmem.freelist;
801025b4:	a1 78 26 11 80       	mov    0x80112678,%eax
801025b9:	89 03                	mov    %eax,(%ebx)
  kmem.freelist = r;
  if(kmem.use_lock)
801025bb:	a1 74 26 11 80       	mov    0x80112674,%eax
  kmem.freelist = r;
801025c0:	89 1d 78 26 11 80    	mov    %ebx,0x80112678
  if(kmem.use_lock)
801025c6:	85 c0                	test   %eax,%eax
801025c8:	75 1e                	jne    801025e8 <kfree+0x78>
    release(&kmem.lock);
}
801025ca:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801025cd:	c9                   	leave  
801025ce:	c3                   	ret    
801025cf:	90                   	nop
    acquire(&kmem.lock);
801025d0:	83 ec 0c             	sub    $0xc,%esp
801025d3:	68 40 26 11 80       	push   $0x80112640
801025d8:	e8 03 2c 00 00       	call   801051e0 <acquire>
801025dd:	83 c4 10             	add    $0x10,%esp
801025e0:	eb d2                	jmp    801025b4 <kfree+0x44>
801025e2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    release(&kmem.lock);
801025e8:	c7 45 08 40 26 11 80 	movl   $0x80112640,0x8(%ebp)
}
801025ef:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801025f2:	c9                   	leave  
    release(&kmem.lock);
801025f3:	e9 88 2b 00 00       	jmp    80105180 <release>
    panic("kfree");
801025f8:	83 ec 0c             	sub    $0xc,%esp
801025fb:	68 ae 80 10 80       	push   $0x801080ae
80102600:	e8 7b dd ff ff       	call   80100380 <panic>
80102605:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010260c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80102610 <freerange>:
{
80102610:	55                   	push   %ebp
80102611:	89 e5                	mov    %esp,%ebp
80102613:	56                   	push   %esi
  p = (char*)PGROUNDUP((uint)vstart);
80102614:	8b 45 08             	mov    0x8(%ebp),%eax
{
80102617:	8b 75 0c             	mov    0xc(%ebp),%esi
8010261a:	53                   	push   %ebx
  p = (char*)PGROUNDUP((uint)vstart);
8010261b:	8d 98 ff 0f 00 00    	lea    0xfff(%eax),%ebx
80102621:	81 e3 00 f0 ff ff    	and    $0xfffff000,%ebx
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102627:	81 c3 00 10 00 00    	add    $0x1000,%ebx
8010262d:	39 de                	cmp    %ebx,%esi
8010262f:	72 23                	jb     80102654 <freerange+0x44>
80102631:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    kfree(p);
80102638:	83 ec 0c             	sub    $0xc,%esp
8010263b:	8d 83 00 f0 ff ff    	lea    -0x1000(%ebx),%eax
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102641:	81 c3 00 10 00 00    	add    $0x1000,%ebx
    kfree(p);
80102647:	50                   	push   %eax
80102648:	e8 23 ff ff ff       	call   80102570 <kfree>
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
8010264d:	83 c4 10             	add    $0x10,%esp
80102650:	39 f3                	cmp    %esi,%ebx
80102652:	76 e4                	jbe    80102638 <freerange+0x28>
}
80102654:	8d 65 f8             	lea    -0x8(%ebp),%esp
80102657:	5b                   	pop    %ebx
80102658:	5e                   	pop    %esi
80102659:	5d                   	pop    %ebp
8010265a:	c3                   	ret    
8010265b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010265f:	90                   	nop

80102660 <kinit2>:
{
80102660:	55                   	push   %ebp
80102661:	89 e5                	mov    %esp,%ebp
80102663:	56                   	push   %esi
  p = (char*)PGROUNDUP((uint)vstart);
80102664:	8b 45 08             	mov    0x8(%ebp),%eax
{
80102667:	8b 75 0c             	mov    0xc(%ebp),%esi
8010266a:	53                   	push   %ebx
  p = (char*)PGROUNDUP((uint)vstart);
8010266b:	8d 98 ff 0f 00 00    	lea    0xfff(%eax),%ebx
80102671:	81 e3 00 f0 ff ff    	and    $0xfffff000,%ebx
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102677:	81 c3 00 10 00 00    	add    $0x1000,%ebx
8010267d:	39 de                	cmp    %ebx,%esi
8010267f:	72 23                	jb     801026a4 <kinit2+0x44>
80102681:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    kfree(p);
80102688:	83 ec 0c             	sub    $0xc,%esp
8010268b:	8d 83 00 f0 ff ff    	lea    -0x1000(%ebx),%eax
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102691:	81 c3 00 10 00 00    	add    $0x1000,%ebx
    kfree(p);
80102697:	50                   	push   %eax
80102698:	e8 d3 fe ff ff       	call   80102570 <kfree>
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
8010269d:	83 c4 10             	add    $0x10,%esp
801026a0:	39 de                	cmp    %ebx,%esi
801026a2:	73 e4                	jae    80102688 <kinit2+0x28>
  kmem.use_lock = 1;
801026a4:	c7 05 74 26 11 80 01 	movl   $0x1,0x80112674
801026ab:	00 00 00 
}
801026ae:	8d 65 f8             	lea    -0x8(%ebp),%esp
801026b1:	5b                   	pop    %ebx
801026b2:	5e                   	pop    %esi
801026b3:	5d                   	pop    %ebp
801026b4:	c3                   	ret    
801026b5:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801026bc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

801026c0 <kinit1>:
{
801026c0:	55                   	push   %ebp
801026c1:	89 e5                	mov    %esp,%ebp
801026c3:	56                   	push   %esi
801026c4:	53                   	push   %ebx
801026c5:	8b 75 0c             	mov    0xc(%ebp),%esi
  initlock(&kmem.lock, "kmem");
801026c8:	83 ec 08             	sub    $0x8,%esp
801026cb:	68 b4 80 10 80       	push   $0x801080b4
801026d0:	68 40 26 11 80       	push   $0x80112640
801026d5:	e8 36 29 00 00       	call   80105010 <initlock>
  p = (char*)PGROUNDUP((uint)vstart);
801026da:	8b 45 08             	mov    0x8(%ebp),%eax
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
801026dd:	83 c4 10             	add    $0x10,%esp
  kmem.use_lock = 0;
801026e0:	c7 05 74 26 11 80 00 	movl   $0x0,0x80112674
801026e7:	00 00 00 
  p = (char*)PGROUNDUP((uint)vstart);
801026ea:	8d 98 ff 0f 00 00    	lea    0xfff(%eax),%ebx
801026f0:	81 e3 00 f0 ff ff    	and    $0xfffff000,%ebx
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
801026f6:	81 c3 00 10 00 00    	add    $0x1000,%ebx
801026fc:	39 de                	cmp    %ebx,%esi
801026fe:	72 1c                	jb     8010271c <kinit1+0x5c>
    kfree(p);
80102700:	83 ec 0c             	sub    $0xc,%esp
80102703:	8d 83 00 f0 ff ff    	lea    -0x1000(%ebx),%eax
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102709:	81 c3 00 10 00 00    	add    $0x1000,%ebx
    kfree(p);
8010270f:	50                   	push   %eax
80102710:	e8 5b fe ff ff       	call   80102570 <kfree>
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102715:	83 c4 10             	add    $0x10,%esp
80102718:	39 de                	cmp    %ebx,%esi
8010271a:	73 e4                	jae    80102700 <kinit1+0x40>
}
8010271c:	8d 65 f8             	lea    -0x8(%ebp),%esp
8010271f:	5b                   	pop    %ebx
80102720:	5e                   	pop    %esi
80102721:	5d                   	pop    %ebp
80102722:	c3                   	ret    
80102723:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010272a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80102730 <kalloc>:
char*
kalloc(void)
{
  struct run *r;

  if(kmem.use_lock)
80102730:	a1 74 26 11 80       	mov    0x80112674,%eax
80102735:	85 c0                	test   %eax,%eax
80102737:	75 1f                	jne    80102758 <kalloc+0x28>
    acquire(&kmem.lock);
  r = kmem.freelist;
80102739:	a1 78 26 11 80       	mov    0x80112678,%eax
  if(r)
8010273e:	85 c0                	test   %eax,%eax
80102740:	74 0e                	je     80102750 <kalloc+0x20>
    kmem.freelist = r->next;
80102742:	8b 10                	mov    (%eax),%edx
80102744:	89 15 78 26 11 80    	mov    %edx,0x80112678
  if(kmem.use_lock)
8010274a:	c3                   	ret    
8010274b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010274f:	90                   	nop
    release(&kmem.lock);
  return (char*)r;
}
80102750:	c3                   	ret    
80102751:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
{
80102758:	55                   	push   %ebp
80102759:	89 e5                	mov    %esp,%ebp
8010275b:	83 ec 24             	sub    $0x24,%esp
    acquire(&kmem.lock);
8010275e:	68 40 26 11 80       	push   $0x80112640
80102763:	e8 78 2a 00 00       	call   801051e0 <acquire>
  r = kmem.freelist;
80102768:	a1 78 26 11 80       	mov    0x80112678,%eax
  if(kmem.use_lock)
8010276d:	8b 15 74 26 11 80    	mov    0x80112674,%edx
  if(r)
80102773:	83 c4 10             	add    $0x10,%esp
80102776:	85 c0                	test   %eax,%eax
80102778:	74 08                	je     80102782 <kalloc+0x52>
    kmem.freelist = r->next;
8010277a:	8b 08                	mov    (%eax),%ecx
8010277c:	89 0d 78 26 11 80    	mov    %ecx,0x80112678
  if(kmem.use_lock)
80102782:	85 d2                	test   %edx,%edx
80102784:	74 16                	je     8010279c <kalloc+0x6c>
    release(&kmem.lock);
80102786:	83 ec 0c             	sub    $0xc,%esp
80102789:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010278c:	68 40 26 11 80       	push   $0x80112640
80102791:	e8 ea 29 00 00       	call   80105180 <release>
  return (char*)r;
80102796:	8b 45 f4             	mov    -0xc(%ebp),%eax
    release(&kmem.lock);
80102799:	83 c4 10             	add    $0x10,%esp
}
8010279c:	c9                   	leave  
8010279d:	c3                   	ret    
8010279e:	66 90                	xchg   %ax,%ax

801027a0 <kbdgetc>:
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801027a0:	ba 64 00 00 00       	mov    $0x64,%edx
801027a5:	ec                   	in     (%dx),%al
    normalmap, shiftmap, ctlmap, ctlmap
  };
  uint st, data, c;

  st = inb(KBSTATP);
  if((st & KBS_DIB) == 0)
801027a6:	a8 01                	test   $0x1,%al
801027a8:	0f 84 c2 00 00 00    	je     80102870 <kbdgetc+0xd0>
{
801027ae:	55                   	push   %ebp
801027af:	ba 60 00 00 00       	mov    $0x60,%edx
801027b4:	89 e5                	mov    %esp,%ebp
801027b6:	53                   	push   %ebx
801027b7:	ec                   	in     (%dx),%al
    return -1;
  data = inb(KBDATAP);

  if(data == 0xE0){
    shift |= E0ESC;
801027b8:	8b 1d 7c 26 11 80    	mov    0x8011267c,%ebx
  data = inb(KBDATAP);
801027be:	0f b6 c8             	movzbl %al,%ecx
  if(data == 0xE0){
801027c1:	3c e0                	cmp    $0xe0,%al
801027c3:	74 5b                	je     80102820 <kbdgetc+0x80>
    return 0;
  } else if(data & 0x80){
    // Key released
    data = (shift & E0ESC ? data : data & 0x7F);
801027c5:	89 da                	mov    %ebx,%edx
801027c7:	83 e2 40             	and    $0x40,%edx
  } else if(data & 0x80){
801027ca:	84 c0                	test   %al,%al
801027cc:	78 62                	js     80102830 <kbdgetc+0x90>
    shift &= ~(shiftcode[data] | E0ESC);
    return 0;
  } else if(shift & E0ESC){
801027ce:	85 d2                	test   %edx,%edx
801027d0:	74 09                	je     801027db <kbdgetc+0x3b>
    // Last character was an E0 escape; or with 0x80
    data |= 0x80;
801027d2:	83 c8 80             	or     $0xffffff80,%eax
    shift &= ~E0ESC;
801027d5:	83 e3 bf             	and    $0xffffffbf,%ebx
    data |= 0x80;
801027d8:	0f b6 c8             	movzbl %al,%ecx
  }

  shift |= shiftcode[data];
801027db:	0f b6 91 e0 81 10 80 	movzbl -0x7fef7e20(%ecx),%edx
  shift ^= togglecode[data];
801027e2:	0f b6 81 e0 80 10 80 	movzbl -0x7fef7f20(%ecx),%eax
  shift |= shiftcode[data];
801027e9:	09 da                	or     %ebx,%edx
  shift ^= togglecode[data];
801027eb:	31 c2                	xor    %eax,%edx
  c = charcode[shift & (CTL | SHIFT)][data];
801027ed:	89 d0                	mov    %edx,%eax
  shift ^= togglecode[data];
801027ef:	89 15 7c 26 11 80    	mov    %edx,0x8011267c
  c = charcode[shift & (CTL | SHIFT)][data];
801027f5:	83 e0 03             	and    $0x3,%eax
  if(shift & CAPSLOCK){
801027f8:	83 e2 08             	and    $0x8,%edx
  c = charcode[shift & (CTL | SHIFT)][data];
801027fb:	8b 04 85 c0 80 10 80 	mov    -0x7fef7f40(,%eax,4),%eax
80102802:	0f b6 04 08          	movzbl (%eax,%ecx,1),%eax
  if(shift & CAPSLOCK){
80102806:	74 0b                	je     80102813 <kbdgetc+0x73>
    if('a' <= c && c <= 'z')
80102808:	8d 50 9f             	lea    -0x61(%eax),%edx
8010280b:	83 fa 19             	cmp    $0x19,%edx
8010280e:	77 48                	ja     80102858 <kbdgetc+0xb8>
      c += 'A' - 'a';
80102810:	83 e8 20             	sub    $0x20,%eax
    else if('A' <= c && c <= 'Z')
      c += 'a' - 'A';
  }
  return c;
}
80102813:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80102816:	c9                   	leave  
80102817:	c3                   	ret    
80102818:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010281f:	90                   	nop
    shift |= E0ESC;
80102820:	83 cb 40             	or     $0x40,%ebx
    return 0;
80102823:	31 c0                	xor    %eax,%eax
    shift |= E0ESC;
80102825:	89 1d 7c 26 11 80    	mov    %ebx,0x8011267c
}
8010282b:	8b 5d fc             	mov    -0x4(%ebp),%ebx
8010282e:	c9                   	leave  
8010282f:	c3                   	ret    
    data = (shift & E0ESC ? data : data & 0x7F);
80102830:	83 e0 7f             	and    $0x7f,%eax
80102833:	85 d2                	test   %edx,%edx
80102835:	0f 44 c8             	cmove  %eax,%ecx
    shift &= ~(shiftcode[data] | E0ESC);
80102838:	0f b6 81 e0 81 10 80 	movzbl -0x7fef7e20(%ecx),%eax
8010283f:	83 c8 40             	or     $0x40,%eax
80102842:	0f b6 c0             	movzbl %al,%eax
80102845:	f7 d0                	not    %eax
80102847:	21 d8                	and    %ebx,%eax
}
80102849:	8b 5d fc             	mov    -0x4(%ebp),%ebx
    shift &= ~(shiftcode[data] | E0ESC);
8010284c:	a3 7c 26 11 80       	mov    %eax,0x8011267c
    return 0;
80102851:	31 c0                	xor    %eax,%eax
}
80102853:	c9                   	leave  
80102854:	c3                   	ret    
80102855:	8d 76 00             	lea    0x0(%esi),%esi
    else if('A' <= c && c <= 'Z')
80102858:	8d 48 bf             	lea    -0x41(%eax),%ecx
      c += 'a' - 'A';
8010285b:	8d 50 20             	lea    0x20(%eax),%edx
}
8010285e:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80102861:	c9                   	leave  
      c += 'a' - 'A';
80102862:	83 f9 1a             	cmp    $0x1a,%ecx
80102865:	0f 42 c2             	cmovb  %edx,%eax
}
80102868:	c3                   	ret    
80102869:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    return -1;
80102870:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80102875:	c3                   	ret    
80102876:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010287d:	8d 76 00             	lea    0x0(%esi),%esi

80102880 <kbdintr>:

void
kbdintr(void)
{
80102880:	55                   	push   %ebp
80102881:	89 e5                	mov    %esp,%ebp
80102883:	83 ec 14             	sub    $0x14,%esp
  consoleintr(kbdgetc);
80102886:	68 a0 27 10 80       	push   $0x801027a0
8010288b:	e8 f0 df ff ff       	call   80100880 <consoleintr>
}
80102890:	83 c4 10             	add    $0x10,%esp
80102893:	c9                   	leave  
80102894:	c3                   	ret    
80102895:	66 90                	xchg   %ax,%ax
80102897:	66 90                	xchg   %ax,%ax
80102899:	66 90                	xchg   %ax,%ax
8010289b:	66 90                	xchg   %ax,%ax
8010289d:	66 90                	xchg   %ax,%ax
8010289f:	90                   	nop

801028a0 <lapicinit>:
}

void
lapicinit(void)
{
  if(!lapic)
801028a0:	a1 80 26 11 80       	mov    0x80112680,%eax
801028a5:	85 c0                	test   %eax,%eax
801028a7:	0f 84 cb 00 00 00    	je     80102978 <lapicinit+0xd8>
  lapic[index] = value;
801028ad:	c7 80 f0 00 00 00 3f 	movl   $0x13f,0xf0(%eax)
801028b4:	01 00 00 
  lapic[ID];  // wait for write to finish, by reading
801028b7:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
801028ba:	c7 80 e0 03 00 00 0b 	movl   $0xb,0x3e0(%eax)
801028c1:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
801028c4:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
801028c7:	c7 80 20 03 00 00 20 	movl   $0x20020,0x320(%eax)
801028ce:	00 02 00 
  lapic[ID];  // wait for write to finish, by reading
801028d1:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
801028d4:	c7 80 80 03 00 00 80 	movl   $0x989680,0x380(%eax)
801028db:	96 98 00 
  lapic[ID];  // wait for write to finish, by reading
801028de:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
801028e1:	c7 80 50 03 00 00 00 	movl   $0x10000,0x350(%eax)
801028e8:	00 01 00 
  lapic[ID];  // wait for write to finish, by reading
801028eb:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
801028ee:	c7 80 60 03 00 00 00 	movl   $0x10000,0x360(%eax)
801028f5:	00 01 00 
  lapic[ID];  // wait for write to finish, by reading
801028f8:	8b 50 20             	mov    0x20(%eax),%edx
  lapicw(LINT0, MASKED);
  lapicw(LINT1, MASKED);

  // Disable performance counter overflow interrupts
  // on machines that provide that interrupt entry.
  if(((lapic[VER]>>16) & 0xFF) >= 4)
801028fb:	8b 50 30             	mov    0x30(%eax),%edx
801028fe:	c1 ea 10             	shr    $0x10,%edx
80102901:	81 e2 fc 00 00 00    	and    $0xfc,%edx
80102907:	75 77                	jne    80102980 <lapicinit+0xe0>
  lapic[index] = value;
80102909:	c7 80 70 03 00 00 33 	movl   $0x33,0x370(%eax)
80102910:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102913:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102916:	c7 80 80 02 00 00 00 	movl   $0x0,0x280(%eax)
8010291d:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102920:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102923:	c7 80 80 02 00 00 00 	movl   $0x0,0x280(%eax)
8010292a:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
8010292d:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102930:	c7 80 b0 00 00 00 00 	movl   $0x0,0xb0(%eax)
80102937:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
8010293a:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
8010293d:	c7 80 10 03 00 00 00 	movl   $0x0,0x310(%eax)
80102944:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102947:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
8010294a:	c7 80 00 03 00 00 00 	movl   $0x88500,0x300(%eax)
80102951:	85 08 00 
  lapic[ID];  // wait for write to finish, by reading
80102954:	8b 50 20             	mov    0x20(%eax),%edx
80102957:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010295e:	66 90                	xchg   %ax,%ax
  lapicw(EOI, 0);

  // Send an Init Level De-Assert to synchronise arbitration ID's.
  lapicw(ICRHI, 0);
  lapicw(ICRLO, BCAST | INIT | LEVEL);
  while(lapic[ICRLO] & DELIVS)
80102960:	8b 90 00 03 00 00    	mov    0x300(%eax),%edx
80102966:	80 e6 10             	and    $0x10,%dh
80102969:	75 f5                	jne    80102960 <lapicinit+0xc0>
  lapic[index] = value;
8010296b:	c7 80 80 00 00 00 00 	movl   $0x0,0x80(%eax)
80102972:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102975:	8b 40 20             	mov    0x20(%eax),%eax
    ;

  // Enable interrupts on the APIC (but not on the processor).
  lapicw(TPR, 0);
}
80102978:	c3                   	ret    
80102979:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  lapic[index] = value;
80102980:	c7 80 40 03 00 00 00 	movl   $0x10000,0x340(%eax)
80102987:	00 01 00 
  lapic[ID];  // wait for write to finish, by reading
8010298a:	8b 50 20             	mov    0x20(%eax),%edx
}
8010298d:	e9 77 ff ff ff       	jmp    80102909 <lapicinit+0x69>
80102992:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102999:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

801029a0 <lapicid>:

int
lapicid(void)
{
  if (!lapic)
801029a0:	a1 80 26 11 80       	mov    0x80112680,%eax
801029a5:	85 c0                	test   %eax,%eax
801029a7:	74 07                	je     801029b0 <lapicid+0x10>
    return 0;
  return lapic[ID] >> 24;
801029a9:	8b 40 20             	mov    0x20(%eax),%eax
801029ac:	c1 e8 18             	shr    $0x18,%eax
801029af:	c3                   	ret    
    return 0;
801029b0:	31 c0                	xor    %eax,%eax
}
801029b2:	c3                   	ret    
801029b3:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801029ba:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

801029c0 <lapiceoi>:

// Acknowledge interrupt.
void
lapiceoi(void)
{
  if(lapic)
801029c0:	a1 80 26 11 80       	mov    0x80112680,%eax
801029c5:	85 c0                	test   %eax,%eax
801029c7:	74 0d                	je     801029d6 <lapiceoi+0x16>
  lapic[index] = value;
801029c9:	c7 80 b0 00 00 00 00 	movl   $0x0,0xb0(%eax)
801029d0:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
801029d3:	8b 40 20             	mov    0x20(%eax),%eax
    lapicw(EOI, 0);
}
801029d6:	c3                   	ret    
801029d7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801029de:	66 90                	xchg   %ax,%ax

801029e0 <microdelay>:
// Spin for a given number of microseconds.
// On real hardware would want to tune this dynamically.
void
microdelay(int us)
{
}
801029e0:	c3                   	ret    
801029e1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801029e8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801029ef:	90                   	nop

801029f0 <lapicstartap>:

// Start additional processor running entry code at addr.
// See Appendix B of MultiProcessor Specification.
void
lapicstartap(uchar apicid, uint addr)
{
801029f0:	55                   	push   %ebp
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801029f1:	b8 0f 00 00 00       	mov    $0xf,%eax
801029f6:	ba 70 00 00 00       	mov    $0x70,%edx
801029fb:	89 e5                	mov    %esp,%ebp
801029fd:	53                   	push   %ebx
801029fe:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80102a01:	8b 5d 08             	mov    0x8(%ebp),%ebx
80102a04:	ee                   	out    %al,(%dx)
80102a05:	b8 0a 00 00 00       	mov    $0xa,%eax
80102a0a:	ba 71 00 00 00       	mov    $0x71,%edx
80102a0f:	ee                   	out    %al,(%dx)
  // and the warm reset vector (DWORD based at 40:67) to point at
  // the AP startup code prior to the [universal startup algorithm]."
  outb(CMOS_PORT, 0xF);  // offset 0xF is shutdown code
  outb(CMOS_PORT+1, 0x0A);
  wrv = (ushort*)P2V((0x40<<4 | 0x67));  // Warm reset vector
  wrv[0] = 0;
80102a10:	31 c0                	xor    %eax,%eax
  wrv[1] = addr >> 4;

  // "Universal startup algorithm."
  // Send INIT (level-triggered) interrupt to reset other CPU.
  lapicw(ICRHI, apicid<<24);
80102a12:	c1 e3 18             	shl    $0x18,%ebx
  wrv[0] = 0;
80102a15:	66 a3 67 04 00 80    	mov    %ax,0x80000467
  wrv[1] = addr >> 4;
80102a1b:	89 c8                	mov    %ecx,%eax
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
    lapicw(ICRHI, apicid<<24);
    lapicw(ICRLO, STARTUP | (addr>>12));
80102a1d:	c1 e9 0c             	shr    $0xc,%ecx
  lapicw(ICRHI, apicid<<24);
80102a20:	89 da                	mov    %ebx,%edx
  wrv[1] = addr >> 4;
80102a22:	c1 e8 04             	shr    $0x4,%eax
    lapicw(ICRLO, STARTUP | (addr>>12));
80102a25:	80 cd 06             	or     $0x6,%ch
  wrv[1] = addr >> 4;
80102a28:	66 a3 69 04 00 80    	mov    %ax,0x80000469
  lapic[index] = value;
80102a2e:	a1 80 26 11 80       	mov    0x80112680,%eax
80102a33:	89 98 10 03 00 00    	mov    %ebx,0x310(%eax)
  lapic[ID];  // wait for write to finish, by reading
80102a39:	8b 58 20             	mov    0x20(%eax),%ebx
  lapic[index] = value;
80102a3c:	c7 80 00 03 00 00 00 	movl   $0xc500,0x300(%eax)
80102a43:	c5 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102a46:	8b 58 20             	mov    0x20(%eax),%ebx
  lapic[index] = value;
80102a49:	c7 80 00 03 00 00 00 	movl   $0x8500,0x300(%eax)
80102a50:	85 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102a53:	8b 58 20             	mov    0x20(%eax),%ebx
  lapic[index] = value;
80102a56:	89 90 10 03 00 00    	mov    %edx,0x310(%eax)
  lapic[ID];  // wait for write to finish, by reading
80102a5c:	8b 58 20             	mov    0x20(%eax),%ebx
  lapic[index] = value;
80102a5f:	89 88 00 03 00 00    	mov    %ecx,0x300(%eax)
  lapic[ID];  // wait for write to finish, by reading
80102a65:	8b 58 20             	mov    0x20(%eax),%ebx
  lapic[index] = value;
80102a68:	89 90 10 03 00 00    	mov    %edx,0x310(%eax)
  lapic[ID];  // wait for write to finish, by reading
80102a6e:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102a71:	89 88 00 03 00 00    	mov    %ecx,0x300(%eax)
  lapic[ID];  // wait for write to finish, by reading
80102a77:	8b 40 20             	mov    0x20(%eax),%eax
    microdelay(200);
  }
}
80102a7a:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80102a7d:	c9                   	leave  
80102a7e:	c3                   	ret    
80102a7f:	90                   	nop

80102a80 <cmostime>:
}

// qemu seems to use 24-hour GWT and the values are BCD encoded
void
cmostime(struct rtcdate *r)
{
80102a80:	55                   	push   %ebp
80102a81:	b8 0b 00 00 00       	mov    $0xb,%eax
80102a86:	ba 70 00 00 00       	mov    $0x70,%edx
80102a8b:	89 e5                	mov    %esp,%ebp
80102a8d:	57                   	push   %edi
80102a8e:	56                   	push   %esi
80102a8f:	53                   	push   %ebx
80102a90:	83 ec 4c             	sub    $0x4c,%esp
80102a93:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102a94:	ba 71 00 00 00       	mov    $0x71,%edx
80102a99:	ec                   	in     (%dx),%al
  struct rtcdate t1, t2;
  int sb, bcd;

  sb = cmos_read(CMOS_STATB);

  bcd = (sb & (1 << 2)) == 0;
80102a9a:	83 e0 04             	and    $0x4,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102a9d:	bb 70 00 00 00       	mov    $0x70,%ebx
80102aa2:	88 45 b3             	mov    %al,-0x4d(%ebp)
80102aa5:	8d 76 00             	lea    0x0(%esi),%esi
80102aa8:	31 c0                	xor    %eax,%eax
80102aaa:	89 da                	mov    %ebx,%edx
80102aac:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102aad:	b9 71 00 00 00       	mov    $0x71,%ecx
80102ab2:	89 ca                	mov    %ecx,%edx
80102ab4:	ec                   	in     (%dx),%al
80102ab5:	88 45 b7             	mov    %al,-0x49(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102ab8:	89 da                	mov    %ebx,%edx
80102aba:	b8 02 00 00 00       	mov    $0x2,%eax
80102abf:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102ac0:	89 ca                	mov    %ecx,%edx
80102ac2:	ec                   	in     (%dx),%al
80102ac3:	88 45 b6             	mov    %al,-0x4a(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102ac6:	89 da                	mov    %ebx,%edx
80102ac8:	b8 04 00 00 00       	mov    $0x4,%eax
80102acd:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102ace:	89 ca                	mov    %ecx,%edx
80102ad0:	ec                   	in     (%dx),%al
80102ad1:	88 45 b5             	mov    %al,-0x4b(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102ad4:	89 da                	mov    %ebx,%edx
80102ad6:	b8 07 00 00 00       	mov    $0x7,%eax
80102adb:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102adc:	89 ca                	mov    %ecx,%edx
80102ade:	ec                   	in     (%dx),%al
80102adf:	88 45 b4             	mov    %al,-0x4c(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102ae2:	89 da                	mov    %ebx,%edx
80102ae4:	b8 08 00 00 00       	mov    $0x8,%eax
80102ae9:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102aea:	89 ca                	mov    %ecx,%edx
80102aec:	ec                   	in     (%dx),%al
80102aed:	89 c7                	mov    %eax,%edi
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102aef:	89 da                	mov    %ebx,%edx
80102af1:	b8 09 00 00 00       	mov    $0x9,%eax
80102af6:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102af7:	89 ca                	mov    %ecx,%edx
80102af9:	ec                   	in     (%dx),%al
80102afa:	89 c6                	mov    %eax,%esi
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102afc:	89 da                	mov    %ebx,%edx
80102afe:	b8 0a 00 00 00       	mov    $0xa,%eax
80102b03:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102b04:	89 ca                	mov    %ecx,%edx
80102b06:	ec                   	in     (%dx),%al

  // make sure CMOS doesn't modify time while we read it
  for(;;) {
    fill_rtcdate(&t1);
    if(cmos_read(CMOS_STATA) & CMOS_UIP)
80102b07:	84 c0                	test   %al,%al
80102b09:	78 9d                	js     80102aa8 <cmostime+0x28>
  return inb(CMOS_RETURN);
80102b0b:	0f b6 45 b7          	movzbl -0x49(%ebp),%eax
80102b0f:	89 fa                	mov    %edi,%edx
80102b11:	0f b6 fa             	movzbl %dl,%edi
80102b14:	89 f2                	mov    %esi,%edx
80102b16:	89 45 b8             	mov    %eax,-0x48(%ebp)
80102b19:	0f b6 45 b6          	movzbl -0x4a(%ebp),%eax
80102b1d:	0f b6 f2             	movzbl %dl,%esi
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102b20:	89 da                	mov    %ebx,%edx
80102b22:	89 7d c8             	mov    %edi,-0x38(%ebp)
80102b25:	89 45 bc             	mov    %eax,-0x44(%ebp)
80102b28:	0f b6 45 b5          	movzbl -0x4b(%ebp),%eax
80102b2c:	89 75 cc             	mov    %esi,-0x34(%ebp)
80102b2f:	89 45 c0             	mov    %eax,-0x40(%ebp)
80102b32:	0f b6 45 b4          	movzbl -0x4c(%ebp),%eax
80102b36:	89 45 c4             	mov    %eax,-0x3c(%ebp)
80102b39:	31 c0                	xor    %eax,%eax
80102b3b:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102b3c:	89 ca                	mov    %ecx,%edx
80102b3e:	ec                   	in     (%dx),%al
80102b3f:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102b42:	89 da                	mov    %ebx,%edx
80102b44:	89 45 d0             	mov    %eax,-0x30(%ebp)
80102b47:	b8 02 00 00 00       	mov    $0x2,%eax
80102b4c:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102b4d:	89 ca                	mov    %ecx,%edx
80102b4f:	ec                   	in     (%dx),%al
80102b50:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102b53:	89 da                	mov    %ebx,%edx
80102b55:	89 45 d4             	mov    %eax,-0x2c(%ebp)
80102b58:	b8 04 00 00 00       	mov    $0x4,%eax
80102b5d:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102b5e:	89 ca                	mov    %ecx,%edx
80102b60:	ec                   	in     (%dx),%al
80102b61:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102b64:	89 da                	mov    %ebx,%edx
80102b66:	89 45 d8             	mov    %eax,-0x28(%ebp)
80102b69:	b8 07 00 00 00       	mov    $0x7,%eax
80102b6e:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102b6f:	89 ca                	mov    %ecx,%edx
80102b71:	ec                   	in     (%dx),%al
80102b72:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102b75:	89 da                	mov    %ebx,%edx
80102b77:	89 45 dc             	mov    %eax,-0x24(%ebp)
80102b7a:	b8 08 00 00 00       	mov    $0x8,%eax
80102b7f:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102b80:	89 ca                	mov    %ecx,%edx
80102b82:	ec                   	in     (%dx),%al
80102b83:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102b86:	89 da                	mov    %ebx,%edx
80102b88:	89 45 e0             	mov    %eax,-0x20(%ebp)
80102b8b:	b8 09 00 00 00       	mov    $0x9,%eax
80102b90:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102b91:	89 ca                	mov    %ecx,%edx
80102b93:	ec                   	in     (%dx),%al
80102b94:	0f b6 c0             	movzbl %al,%eax
        continue;
    fill_rtcdate(&t2);
    if(memcmp(&t1, &t2, sizeof(t1)) == 0)
80102b97:	83 ec 04             	sub    $0x4,%esp
  return inb(CMOS_RETURN);
80102b9a:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if(memcmp(&t1, &t2, sizeof(t1)) == 0)
80102b9d:	8d 45 d0             	lea    -0x30(%ebp),%eax
80102ba0:	6a 18                	push   $0x18
80102ba2:	50                   	push   %eax
80102ba3:	8d 45 b8             	lea    -0x48(%ebp),%eax
80102ba6:	50                   	push   %eax
80102ba7:	e8 44 27 00 00       	call   801052f0 <memcmp>
80102bac:	83 c4 10             	add    $0x10,%esp
80102baf:	85 c0                	test   %eax,%eax
80102bb1:	0f 85 f1 fe ff ff    	jne    80102aa8 <cmostime+0x28>
      break;
  }

  // convert
  if(bcd) {
80102bb7:	80 7d b3 00          	cmpb   $0x0,-0x4d(%ebp)
80102bbb:	75 78                	jne    80102c35 <cmostime+0x1b5>
#define    CONV(x)     (t1.x = ((t1.x >> 4) * 10) + (t1.x & 0xf))
    CONV(second);
80102bbd:	8b 45 b8             	mov    -0x48(%ebp),%eax
80102bc0:	89 c2                	mov    %eax,%edx
80102bc2:	83 e0 0f             	and    $0xf,%eax
80102bc5:	c1 ea 04             	shr    $0x4,%edx
80102bc8:	8d 14 92             	lea    (%edx,%edx,4),%edx
80102bcb:	8d 04 50             	lea    (%eax,%edx,2),%eax
80102bce:	89 45 b8             	mov    %eax,-0x48(%ebp)
    CONV(minute);
80102bd1:	8b 45 bc             	mov    -0x44(%ebp),%eax
80102bd4:	89 c2                	mov    %eax,%edx
80102bd6:	83 e0 0f             	and    $0xf,%eax
80102bd9:	c1 ea 04             	shr    $0x4,%edx
80102bdc:	8d 14 92             	lea    (%edx,%edx,4),%edx
80102bdf:	8d 04 50             	lea    (%eax,%edx,2),%eax
80102be2:	89 45 bc             	mov    %eax,-0x44(%ebp)
    CONV(hour  );
80102be5:	8b 45 c0             	mov    -0x40(%ebp),%eax
80102be8:	89 c2                	mov    %eax,%edx
80102bea:	83 e0 0f             	and    $0xf,%eax
80102bed:	c1 ea 04             	shr    $0x4,%edx
80102bf0:	8d 14 92             	lea    (%edx,%edx,4),%edx
80102bf3:	8d 04 50             	lea    (%eax,%edx,2),%eax
80102bf6:	89 45 c0             	mov    %eax,-0x40(%ebp)
    CONV(day   );
80102bf9:	8b 45 c4             	mov    -0x3c(%ebp),%eax
80102bfc:	89 c2                	mov    %eax,%edx
80102bfe:	83 e0 0f             	and    $0xf,%eax
80102c01:	c1 ea 04             	shr    $0x4,%edx
80102c04:	8d 14 92             	lea    (%edx,%edx,4),%edx
80102c07:	8d 04 50             	lea    (%eax,%edx,2),%eax
80102c0a:	89 45 c4             	mov    %eax,-0x3c(%ebp)
    CONV(month );
80102c0d:	8b 45 c8             	mov    -0x38(%ebp),%eax
80102c10:	89 c2                	mov    %eax,%edx
80102c12:	83 e0 0f             	and    $0xf,%eax
80102c15:	c1 ea 04             	shr    $0x4,%edx
80102c18:	8d 14 92             	lea    (%edx,%edx,4),%edx
80102c1b:	8d 04 50             	lea    (%eax,%edx,2),%eax
80102c1e:	89 45 c8             	mov    %eax,-0x38(%ebp)
    CONV(year  );
80102c21:	8b 45 cc             	mov    -0x34(%ebp),%eax
80102c24:	89 c2                	mov    %eax,%edx
80102c26:	83 e0 0f             	and    $0xf,%eax
80102c29:	c1 ea 04             	shr    $0x4,%edx
80102c2c:	8d 14 92             	lea    (%edx,%edx,4),%edx
80102c2f:	8d 04 50             	lea    (%eax,%edx,2),%eax
80102c32:	89 45 cc             	mov    %eax,-0x34(%ebp)
#undef     CONV
  }

  *r = t1;
80102c35:	8b 75 08             	mov    0x8(%ebp),%esi
80102c38:	8b 45 b8             	mov    -0x48(%ebp),%eax
80102c3b:	89 06                	mov    %eax,(%esi)
80102c3d:	8b 45 bc             	mov    -0x44(%ebp),%eax
80102c40:	89 46 04             	mov    %eax,0x4(%esi)
80102c43:	8b 45 c0             	mov    -0x40(%ebp),%eax
80102c46:	89 46 08             	mov    %eax,0x8(%esi)
80102c49:	8b 45 c4             	mov    -0x3c(%ebp),%eax
80102c4c:	89 46 0c             	mov    %eax,0xc(%esi)
80102c4f:	8b 45 c8             	mov    -0x38(%ebp),%eax
80102c52:	89 46 10             	mov    %eax,0x10(%esi)
80102c55:	8b 45 cc             	mov    -0x34(%ebp),%eax
80102c58:	89 46 14             	mov    %eax,0x14(%esi)
  r->year += 2000;
80102c5b:	81 46 14 d0 07 00 00 	addl   $0x7d0,0x14(%esi)
}
80102c62:	8d 65 f4             	lea    -0xc(%ebp),%esp
80102c65:	5b                   	pop    %ebx
80102c66:	5e                   	pop    %esi
80102c67:	5f                   	pop    %edi
80102c68:	5d                   	pop    %ebp
80102c69:	c3                   	ret    
80102c6a:	66 90                	xchg   %ax,%ax
80102c6c:	66 90                	xchg   %ax,%ax
80102c6e:	66 90                	xchg   %ax,%ax

80102c70 <install_trans>:
static void
install_trans(void)
{
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
80102c70:	8b 0d e8 26 11 80    	mov    0x801126e8,%ecx
80102c76:	85 c9                	test   %ecx,%ecx
80102c78:	0f 8e 8a 00 00 00    	jle    80102d08 <install_trans+0x98>
{
80102c7e:	55                   	push   %ebp
80102c7f:	89 e5                	mov    %esp,%ebp
80102c81:	57                   	push   %edi
  for (tail = 0; tail < log.lh.n; tail++) {
80102c82:	31 ff                	xor    %edi,%edi
{
80102c84:	56                   	push   %esi
80102c85:	53                   	push   %ebx
80102c86:	83 ec 0c             	sub    $0xc,%esp
80102c89:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
80102c90:	a1 d4 26 11 80       	mov    0x801126d4,%eax
80102c95:	83 ec 08             	sub    $0x8,%esp
80102c98:	01 f8                	add    %edi,%eax
80102c9a:	83 c0 01             	add    $0x1,%eax
80102c9d:	50                   	push   %eax
80102c9e:	ff 35 e4 26 11 80    	push   0x801126e4
80102ca4:	e8 27 d4 ff ff       	call   801000d0 <bread>
80102ca9:	89 c6                	mov    %eax,%esi
    struct buf *dbuf = bread(log.dev, log.lh.block[tail]); // read dst
80102cab:	58                   	pop    %eax
80102cac:	5a                   	pop    %edx
80102cad:	ff 34 bd ec 26 11 80 	push   -0x7feed914(,%edi,4)
80102cb4:	ff 35 e4 26 11 80    	push   0x801126e4
  for (tail = 0; tail < log.lh.n; tail++) {
80102cba:	83 c7 01             	add    $0x1,%edi
    struct buf *dbuf = bread(log.dev, log.lh.block[tail]); // read dst
80102cbd:	e8 0e d4 ff ff       	call   801000d0 <bread>
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
80102cc2:	83 c4 0c             	add    $0xc,%esp
    struct buf *dbuf = bread(log.dev, log.lh.block[tail]); // read dst
80102cc5:	89 c3                	mov    %eax,%ebx
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
80102cc7:	8d 46 5c             	lea    0x5c(%esi),%eax
80102cca:	68 00 02 00 00       	push   $0x200
80102ccf:	50                   	push   %eax
80102cd0:	8d 43 5c             	lea    0x5c(%ebx),%eax
80102cd3:	50                   	push   %eax
80102cd4:	e8 67 26 00 00       	call   80105340 <memmove>
    bwrite(dbuf);  // write dst to disk
80102cd9:	89 1c 24             	mov    %ebx,(%esp)
80102cdc:	e8 cf d4 ff ff       	call   801001b0 <bwrite>
    brelse(lbuf);
80102ce1:	89 34 24             	mov    %esi,(%esp)
80102ce4:	e8 07 d5 ff ff       	call   801001f0 <brelse>
    brelse(dbuf);
80102ce9:	89 1c 24             	mov    %ebx,(%esp)
80102cec:	e8 ff d4 ff ff       	call   801001f0 <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
80102cf1:	83 c4 10             	add    $0x10,%esp
80102cf4:	39 3d e8 26 11 80    	cmp    %edi,0x801126e8
80102cfa:	7f 94                	jg     80102c90 <install_trans+0x20>
  }
}
80102cfc:	8d 65 f4             	lea    -0xc(%ebp),%esp
80102cff:	5b                   	pop    %ebx
80102d00:	5e                   	pop    %esi
80102d01:	5f                   	pop    %edi
80102d02:	5d                   	pop    %ebp
80102d03:	c3                   	ret    
80102d04:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80102d08:	c3                   	ret    
80102d09:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80102d10 <write_head>:
// Write in-memory log header to disk.
// This is the true point at which the
// current transaction commits.
static void
write_head(void)
{
80102d10:	55                   	push   %ebp
80102d11:	89 e5                	mov    %esp,%ebp
80102d13:	53                   	push   %ebx
80102d14:	83 ec 0c             	sub    $0xc,%esp
  struct buf *buf = bread(log.dev, log.start);
80102d17:	ff 35 d4 26 11 80    	push   0x801126d4
80102d1d:	ff 35 e4 26 11 80    	push   0x801126e4
80102d23:	e8 a8 d3 ff ff       	call   801000d0 <bread>
  struct logheader *hb = (struct logheader *) (buf->data);
  int i;
  hb->n = log.lh.n;
  for (i = 0; i < log.lh.n; i++) {
80102d28:	83 c4 10             	add    $0x10,%esp
  struct buf *buf = bread(log.dev, log.start);
80102d2b:	89 c3                	mov    %eax,%ebx
  hb->n = log.lh.n;
80102d2d:	a1 e8 26 11 80       	mov    0x801126e8,%eax
80102d32:	89 43 5c             	mov    %eax,0x5c(%ebx)
  for (i = 0; i < log.lh.n; i++) {
80102d35:	85 c0                	test   %eax,%eax
80102d37:	7e 19                	jle    80102d52 <write_head+0x42>
80102d39:	31 d2                	xor    %edx,%edx
80102d3b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80102d3f:	90                   	nop
    hb->block[i] = log.lh.block[i];
80102d40:	8b 0c 95 ec 26 11 80 	mov    -0x7feed914(,%edx,4),%ecx
80102d47:	89 4c 93 60          	mov    %ecx,0x60(%ebx,%edx,4)
  for (i = 0; i < log.lh.n; i++) {
80102d4b:	83 c2 01             	add    $0x1,%edx
80102d4e:	39 d0                	cmp    %edx,%eax
80102d50:	75 ee                	jne    80102d40 <write_head+0x30>
  }
  bwrite(buf);
80102d52:	83 ec 0c             	sub    $0xc,%esp
80102d55:	53                   	push   %ebx
80102d56:	e8 55 d4 ff ff       	call   801001b0 <bwrite>
  brelse(buf);
80102d5b:	89 1c 24             	mov    %ebx,(%esp)
80102d5e:	e8 8d d4 ff ff       	call   801001f0 <brelse>
}
80102d63:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80102d66:	83 c4 10             	add    $0x10,%esp
80102d69:	c9                   	leave  
80102d6a:	c3                   	ret    
80102d6b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80102d6f:	90                   	nop

80102d70 <initlog>:
{
80102d70:	55                   	push   %ebp
80102d71:	89 e5                	mov    %esp,%ebp
80102d73:	53                   	push   %ebx
80102d74:	83 ec 2c             	sub    $0x2c,%esp
80102d77:	8b 5d 08             	mov    0x8(%ebp),%ebx
  initlock(&log.lock, "log");
80102d7a:	68 e0 82 10 80       	push   $0x801082e0
80102d7f:	68 a0 26 11 80       	push   $0x801126a0
80102d84:	e8 87 22 00 00       	call   80105010 <initlock>
  readsb(dev, &sb);
80102d89:	58                   	pop    %eax
80102d8a:	8d 45 dc             	lea    -0x24(%ebp),%eax
80102d8d:	5a                   	pop    %edx
80102d8e:	50                   	push   %eax
80102d8f:	53                   	push   %ebx
80102d90:	e8 3b e8 ff ff       	call   801015d0 <readsb>
  log.start = sb.logstart;
80102d95:	8b 45 ec             	mov    -0x14(%ebp),%eax
  struct buf *buf = bread(log.dev, log.start);
80102d98:	59                   	pop    %ecx
  log.dev = dev;
80102d99:	89 1d e4 26 11 80    	mov    %ebx,0x801126e4
  log.size = sb.nlog;
80102d9f:	8b 55 e8             	mov    -0x18(%ebp),%edx
  log.start = sb.logstart;
80102da2:	a3 d4 26 11 80       	mov    %eax,0x801126d4
  log.size = sb.nlog;
80102da7:	89 15 d8 26 11 80    	mov    %edx,0x801126d8
  struct buf *buf = bread(log.dev, log.start);
80102dad:	5a                   	pop    %edx
80102dae:	50                   	push   %eax
80102daf:	53                   	push   %ebx
80102db0:	e8 1b d3 ff ff       	call   801000d0 <bread>
  for (i = 0; i < log.lh.n; i++) {
80102db5:	83 c4 10             	add    $0x10,%esp
  log.lh.n = lh->n;
80102db8:	8b 58 5c             	mov    0x5c(%eax),%ebx
80102dbb:	89 1d e8 26 11 80    	mov    %ebx,0x801126e8
  for (i = 0; i < log.lh.n; i++) {
80102dc1:	85 db                	test   %ebx,%ebx
80102dc3:	7e 1d                	jle    80102de2 <initlog+0x72>
80102dc5:	31 d2                	xor    %edx,%edx
80102dc7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102dce:	66 90                	xchg   %ax,%ax
    log.lh.block[i] = lh->block[i];
80102dd0:	8b 4c 90 60          	mov    0x60(%eax,%edx,4),%ecx
80102dd4:	89 0c 95 ec 26 11 80 	mov    %ecx,-0x7feed914(,%edx,4)
  for (i = 0; i < log.lh.n; i++) {
80102ddb:	83 c2 01             	add    $0x1,%edx
80102dde:	39 d3                	cmp    %edx,%ebx
80102de0:	75 ee                	jne    80102dd0 <initlog+0x60>
  brelse(buf);
80102de2:	83 ec 0c             	sub    $0xc,%esp
80102de5:	50                   	push   %eax
80102de6:	e8 05 d4 ff ff       	call   801001f0 <brelse>

static void
recover_from_log(void)
{
  read_head();
  install_trans(); // if committed, copy from log to disk
80102deb:	e8 80 fe ff ff       	call   80102c70 <install_trans>
  log.lh.n = 0;
80102df0:	c7 05 e8 26 11 80 00 	movl   $0x0,0x801126e8
80102df7:	00 00 00 
  write_head(); // clear the log
80102dfa:	e8 11 ff ff ff       	call   80102d10 <write_head>
}
80102dff:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80102e02:	83 c4 10             	add    $0x10,%esp
80102e05:	c9                   	leave  
80102e06:	c3                   	ret    
80102e07:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102e0e:	66 90                	xchg   %ax,%ax

80102e10 <begin_op>:
}

// called at the start of each FS system call.
void
begin_op(void)
{
80102e10:	55                   	push   %ebp
80102e11:	89 e5                	mov    %esp,%ebp
80102e13:	83 ec 14             	sub    $0x14,%esp
  acquire(&log.lock);
80102e16:	68 a0 26 11 80       	push   $0x801126a0
80102e1b:	e8 c0 23 00 00       	call   801051e0 <acquire>
80102e20:	83 c4 10             	add    $0x10,%esp
80102e23:	eb 18                	jmp    80102e3d <begin_op+0x2d>
80102e25:	8d 76 00             	lea    0x0(%esi),%esi
  while(1){
    if(log.committing){
      sleep(&log, &log.lock);
80102e28:	83 ec 08             	sub    $0x8,%esp
80102e2b:	68 a0 26 11 80       	push   $0x801126a0
80102e30:	68 a0 26 11 80       	push   $0x801126a0
80102e35:	e8 e6 15 00 00       	call   80104420 <sleep>
80102e3a:	83 c4 10             	add    $0x10,%esp
    if(log.committing){
80102e3d:	a1 e0 26 11 80       	mov    0x801126e0,%eax
80102e42:	85 c0                	test   %eax,%eax
80102e44:	75 e2                	jne    80102e28 <begin_op+0x18>
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGSIZE){
80102e46:	a1 dc 26 11 80       	mov    0x801126dc,%eax
80102e4b:	8b 15 e8 26 11 80    	mov    0x801126e8,%edx
80102e51:	83 c0 01             	add    $0x1,%eax
80102e54:	8d 0c 80             	lea    (%eax,%eax,4),%ecx
80102e57:	8d 14 4a             	lea    (%edx,%ecx,2),%edx
80102e5a:	83 fa 1e             	cmp    $0x1e,%edx
80102e5d:	7f c9                	jg     80102e28 <begin_op+0x18>
      // this op might exhaust log space; wait for commit.
      sleep(&log, &log.lock);
    } else {
      log.outstanding += 1;
      release(&log.lock);
80102e5f:	83 ec 0c             	sub    $0xc,%esp
      log.outstanding += 1;
80102e62:	a3 dc 26 11 80       	mov    %eax,0x801126dc
      release(&log.lock);
80102e67:	68 a0 26 11 80       	push   $0x801126a0
80102e6c:	e8 0f 23 00 00       	call   80105180 <release>
      break;
    }
  }
}
80102e71:	83 c4 10             	add    $0x10,%esp
80102e74:	c9                   	leave  
80102e75:	c3                   	ret    
80102e76:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102e7d:	8d 76 00             	lea    0x0(%esi),%esi

80102e80 <end_op>:

// called at the end of each FS system call.
// commits if this was the last outstanding operation.
void
end_op(void)
{
80102e80:	55                   	push   %ebp
80102e81:	89 e5                	mov    %esp,%ebp
80102e83:	57                   	push   %edi
80102e84:	56                   	push   %esi
80102e85:	53                   	push   %ebx
80102e86:	83 ec 18             	sub    $0x18,%esp
  int do_commit = 0;

  acquire(&log.lock);
80102e89:	68 a0 26 11 80       	push   $0x801126a0
80102e8e:	e8 4d 23 00 00       	call   801051e0 <acquire>
  log.outstanding -= 1;
80102e93:	a1 dc 26 11 80       	mov    0x801126dc,%eax
  if(log.committing)
80102e98:	8b 35 e0 26 11 80    	mov    0x801126e0,%esi
80102e9e:	83 c4 10             	add    $0x10,%esp
  log.outstanding -= 1;
80102ea1:	8d 58 ff             	lea    -0x1(%eax),%ebx
80102ea4:	89 1d dc 26 11 80    	mov    %ebx,0x801126dc
  if(log.committing)
80102eaa:	85 f6                	test   %esi,%esi
80102eac:	0f 85 22 01 00 00    	jne    80102fd4 <end_op+0x154>
    panic("log.committing");
  if(log.outstanding == 0){
80102eb2:	85 db                	test   %ebx,%ebx
80102eb4:	0f 85 f6 00 00 00    	jne    80102fb0 <end_op+0x130>
    do_commit = 1;
    log.committing = 1;
80102eba:	c7 05 e0 26 11 80 01 	movl   $0x1,0x801126e0
80102ec1:	00 00 00 
    // begin_op() may be waiting for log space,
    // and decrementing log.outstanding has decreased
    // the amount of reserved space.
    wakeup(&log);
  }
  release(&log.lock);
80102ec4:	83 ec 0c             	sub    $0xc,%esp
80102ec7:	68 a0 26 11 80       	push   $0x801126a0
80102ecc:	e8 af 22 00 00       	call   80105180 <release>
}

static void
commit()
{
  if (log.lh.n > 0) {
80102ed1:	8b 0d e8 26 11 80    	mov    0x801126e8,%ecx
80102ed7:	83 c4 10             	add    $0x10,%esp
80102eda:	85 c9                	test   %ecx,%ecx
80102edc:	7f 42                	jg     80102f20 <end_op+0xa0>
    acquire(&log.lock);
80102ede:	83 ec 0c             	sub    $0xc,%esp
80102ee1:	68 a0 26 11 80       	push   $0x801126a0
80102ee6:	e8 f5 22 00 00       	call   801051e0 <acquire>
    wakeup(&log);
80102eeb:	c7 04 24 a0 26 11 80 	movl   $0x801126a0,(%esp)
    log.committing = 0;
80102ef2:	c7 05 e0 26 11 80 00 	movl   $0x0,0x801126e0
80102ef9:	00 00 00 
    wakeup(&log);
80102efc:	e8 df 15 00 00       	call   801044e0 <wakeup>
    release(&log.lock);
80102f01:	c7 04 24 a0 26 11 80 	movl   $0x801126a0,(%esp)
80102f08:	e8 73 22 00 00       	call   80105180 <release>
80102f0d:	83 c4 10             	add    $0x10,%esp
}
80102f10:	8d 65 f4             	lea    -0xc(%ebp),%esp
80102f13:	5b                   	pop    %ebx
80102f14:	5e                   	pop    %esi
80102f15:	5f                   	pop    %edi
80102f16:	5d                   	pop    %ebp
80102f17:	c3                   	ret    
80102f18:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102f1f:	90                   	nop
    struct buf *to = bread(log.dev, log.start+tail+1); // log block
80102f20:	a1 d4 26 11 80       	mov    0x801126d4,%eax
80102f25:	83 ec 08             	sub    $0x8,%esp
80102f28:	01 d8                	add    %ebx,%eax
80102f2a:	83 c0 01             	add    $0x1,%eax
80102f2d:	50                   	push   %eax
80102f2e:	ff 35 e4 26 11 80    	push   0x801126e4
80102f34:	e8 97 d1 ff ff       	call   801000d0 <bread>
80102f39:	89 c6                	mov    %eax,%esi
    struct buf *from = bread(log.dev, log.lh.block[tail]); // cache block
80102f3b:	58                   	pop    %eax
80102f3c:	5a                   	pop    %edx
80102f3d:	ff 34 9d ec 26 11 80 	push   -0x7feed914(,%ebx,4)
80102f44:	ff 35 e4 26 11 80    	push   0x801126e4
  for (tail = 0; tail < log.lh.n; tail++) {
80102f4a:	83 c3 01             	add    $0x1,%ebx
    struct buf *from = bread(log.dev, log.lh.block[tail]); // cache block
80102f4d:	e8 7e d1 ff ff       	call   801000d0 <bread>
    memmove(to->data, from->data, BSIZE);
80102f52:	83 c4 0c             	add    $0xc,%esp
    struct buf *from = bread(log.dev, log.lh.block[tail]); // cache block
80102f55:	89 c7                	mov    %eax,%edi
    memmove(to->data, from->data, BSIZE);
80102f57:	8d 40 5c             	lea    0x5c(%eax),%eax
80102f5a:	68 00 02 00 00       	push   $0x200
80102f5f:	50                   	push   %eax
80102f60:	8d 46 5c             	lea    0x5c(%esi),%eax
80102f63:	50                   	push   %eax
80102f64:	e8 d7 23 00 00       	call   80105340 <memmove>
    bwrite(to);  // write the log
80102f69:	89 34 24             	mov    %esi,(%esp)
80102f6c:	e8 3f d2 ff ff       	call   801001b0 <bwrite>
    brelse(from);
80102f71:	89 3c 24             	mov    %edi,(%esp)
80102f74:	e8 77 d2 ff ff       	call   801001f0 <brelse>
    brelse(to);
80102f79:	89 34 24             	mov    %esi,(%esp)
80102f7c:	e8 6f d2 ff ff       	call   801001f0 <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
80102f81:	83 c4 10             	add    $0x10,%esp
80102f84:	3b 1d e8 26 11 80    	cmp    0x801126e8,%ebx
80102f8a:	7c 94                	jl     80102f20 <end_op+0xa0>
    write_log();     // Write modified blocks from cache to log
    write_head();    // Write header to disk -- the real commit
80102f8c:	e8 7f fd ff ff       	call   80102d10 <write_head>
    install_trans(); // Now install writes to home locations
80102f91:	e8 da fc ff ff       	call   80102c70 <install_trans>
    log.lh.n = 0;
80102f96:	c7 05 e8 26 11 80 00 	movl   $0x0,0x801126e8
80102f9d:	00 00 00 
    write_head();    // Erase the transaction from the log
80102fa0:	e8 6b fd ff ff       	call   80102d10 <write_head>
80102fa5:	e9 34 ff ff ff       	jmp    80102ede <end_op+0x5e>
80102faa:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    wakeup(&log);
80102fb0:	83 ec 0c             	sub    $0xc,%esp
80102fb3:	68 a0 26 11 80       	push   $0x801126a0
80102fb8:	e8 23 15 00 00       	call   801044e0 <wakeup>
  release(&log.lock);
80102fbd:	c7 04 24 a0 26 11 80 	movl   $0x801126a0,(%esp)
80102fc4:	e8 b7 21 00 00       	call   80105180 <release>
80102fc9:	83 c4 10             	add    $0x10,%esp
}
80102fcc:	8d 65 f4             	lea    -0xc(%ebp),%esp
80102fcf:	5b                   	pop    %ebx
80102fd0:	5e                   	pop    %esi
80102fd1:	5f                   	pop    %edi
80102fd2:	5d                   	pop    %ebp
80102fd3:	c3                   	ret    
    panic("log.committing");
80102fd4:	83 ec 0c             	sub    $0xc,%esp
80102fd7:	68 e4 82 10 80       	push   $0x801082e4
80102fdc:	e8 9f d3 ff ff       	call   80100380 <panic>
80102fe1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102fe8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102fef:	90                   	nop

80102ff0 <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
80102ff0:	55                   	push   %ebp
80102ff1:	89 e5                	mov    %esp,%ebp
80102ff3:	53                   	push   %ebx
80102ff4:	83 ec 04             	sub    $0x4,%esp
  int i;

  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
80102ff7:	8b 15 e8 26 11 80    	mov    0x801126e8,%edx
{
80102ffd:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
80103000:	83 fa 1d             	cmp    $0x1d,%edx
80103003:	0f 8f 85 00 00 00    	jg     8010308e <log_write+0x9e>
80103009:	a1 d8 26 11 80       	mov    0x801126d8,%eax
8010300e:	83 e8 01             	sub    $0x1,%eax
80103011:	39 c2                	cmp    %eax,%edx
80103013:	7d 79                	jge    8010308e <log_write+0x9e>
    panic("too big a transaction");
  if (log.outstanding < 1)
80103015:	a1 dc 26 11 80       	mov    0x801126dc,%eax
8010301a:	85 c0                	test   %eax,%eax
8010301c:	7e 7d                	jle    8010309b <log_write+0xab>
    panic("log_write outside of trans");

  acquire(&log.lock);
8010301e:	83 ec 0c             	sub    $0xc,%esp
80103021:	68 a0 26 11 80       	push   $0x801126a0
80103026:	e8 b5 21 00 00       	call   801051e0 <acquire>
  for (i = 0; i < log.lh.n; i++) {
8010302b:	8b 15 e8 26 11 80    	mov    0x801126e8,%edx
80103031:	83 c4 10             	add    $0x10,%esp
80103034:	85 d2                	test   %edx,%edx
80103036:	7e 4a                	jle    80103082 <log_write+0x92>
    if (log.lh.block[i] == b->blockno)   // log absorbtion
80103038:	8b 4b 08             	mov    0x8(%ebx),%ecx
  for (i = 0; i < log.lh.n; i++) {
8010303b:	31 c0                	xor    %eax,%eax
8010303d:	eb 08                	jmp    80103047 <log_write+0x57>
8010303f:	90                   	nop
80103040:	83 c0 01             	add    $0x1,%eax
80103043:	39 c2                	cmp    %eax,%edx
80103045:	74 29                	je     80103070 <log_write+0x80>
    if (log.lh.block[i] == b->blockno)   // log absorbtion
80103047:	39 0c 85 ec 26 11 80 	cmp    %ecx,-0x7feed914(,%eax,4)
8010304e:	75 f0                	jne    80103040 <log_write+0x50>
      break;
  }
  log.lh.block[i] = b->blockno;
80103050:	89 0c 85 ec 26 11 80 	mov    %ecx,-0x7feed914(,%eax,4)
  if (i == log.lh.n)
    log.lh.n++;
  b->flags |= B_DIRTY; // prevent eviction
80103057:	83 0b 04             	orl    $0x4,(%ebx)
  release(&log.lock);
}
8010305a:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  release(&log.lock);
8010305d:	c7 45 08 a0 26 11 80 	movl   $0x801126a0,0x8(%ebp)
}
80103064:	c9                   	leave  
  release(&log.lock);
80103065:	e9 16 21 00 00       	jmp    80105180 <release>
8010306a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  log.lh.block[i] = b->blockno;
80103070:	89 0c 95 ec 26 11 80 	mov    %ecx,-0x7feed914(,%edx,4)
    log.lh.n++;
80103077:	83 c2 01             	add    $0x1,%edx
8010307a:	89 15 e8 26 11 80    	mov    %edx,0x801126e8
80103080:	eb d5                	jmp    80103057 <log_write+0x67>
  log.lh.block[i] = b->blockno;
80103082:	8b 43 08             	mov    0x8(%ebx),%eax
80103085:	a3 ec 26 11 80       	mov    %eax,0x801126ec
  if (i == log.lh.n)
8010308a:	75 cb                	jne    80103057 <log_write+0x67>
8010308c:	eb e9                	jmp    80103077 <log_write+0x87>
    panic("too big a transaction");
8010308e:	83 ec 0c             	sub    $0xc,%esp
80103091:	68 f3 82 10 80       	push   $0x801082f3
80103096:	e8 e5 d2 ff ff       	call   80100380 <panic>
    panic("log_write outside of trans");
8010309b:	83 ec 0c             	sub    $0xc,%esp
8010309e:	68 09 83 10 80       	push   $0x80108309
801030a3:	e8 d8 d2 ff ff       	call   80100380 <panic>
801030a8:	66 90                	xchg   %ax,%ax
801030aa:	66 90                	xchg   %ax,%ax
801030ac:	66 90                	xchg   %ax,%ax
801030ae:	66 90                	xchg   %ax,%ax

801030b0 <mpmain>:
}

// Common CPU setup code.
static void
mpmain(void)
{
801030b0:	55                   	push   %ebp
801030b1:	89 e5                	mov    %esp,%ebp
801030b3:	53                   	push   %ebx
801030b4:	83 ec 04             	sub    $0x4,%esp
  cprintf("cpu%d: starting %d\n", cpuid(), cpuid());
801030b7:	e8 94 09 00 00       	call   80103a50 <cpuid>
801030bc:	89 c3                	mov    %eax,%ebx
801030be:	e8 8d 09 00 00       	call   80103a50 <cpuid>
801030c3:	83 ec 04             	sub    $0x4,%esp
801030c6:	53                   	push   %ebx
801030c7:	50                   	push   %eax
801030c8:	68 24 83 10 80       	push   $0x80108324
801030cd:	e8 ce d5 ff ff       	call   801006a0 <cprintf>
  idtinit();       // load idt register
801030d2:	e8 69 34 00 00       	call   80106540 <idtinit>
  xchg(&(mycpu()->started), 1); // tell startothers() we're up
801030d7:	e8 14 09 00 00       	call   801039f0 <mycpu>
801030dc:	89 c2                	mov    %eax,%edx
xchg(volatile uint *addr, uint newval)
{
  uint result;

  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
801030de:	b8 01 00 00 00       	mov    $0x1,%eax
801030e3:	f0 87 82 a0 00 00 00 	lock xchg %eax,0xa0(%edx)
  scheduler();     // start running processes
801030ea:	e8 f1 0c 00 00       	call   80103de0 <scheduler>
801030ef:	90                   	nop

801030f0 <mpenter>:
{
801030f0:	55                   	push   %ebp
801030f1:	89 e5                	mov    %esp,%ebp
801030f3:	83 ec 08             	sub    $0x8,%esp
  switchkvm();
801030f6:	e8 35 45 00 00       	call   80107630 <switchkvm>
  seginit();
801030fb:	e8 a0 44 00 00       	call   801075a0 <seginit>
  lapicinit();
80103100:	e8 9b f7 ff ff       	call   801028a0 <lapicinit>
  mpmain();
80103105:	e8 a6 ff ff ff       	call   801030b0 <mpmain>
8010310a:	66 90                	xchg   %ax,%ax
8010310c:	66 90                	xchg   %ax,%ax
8010310e:	66 90                	xchg   %ax,%ax

80103110 <main>:
{
80103110:	8d 4c 24 04          	lea    0x4(%esp),%ecx
80103114:	83 e4 f0             	and    $0xfffffff0,%esp
80103117:	ff 71 fc             	push   -0x4(%ecx)
8010311a:	55                   	push   %ebp
8010311b:	89 e5                	mov    %esp,%ebp
8010311d:	53                   	push   %ebx
8010311e:	51                   	push   %ecx
  kinit1(end, P2V(4*1024*1024)); // phys page allocator
8010311f:	83 ec 08             	sub    $0x8,%esp
80103122:	68 00 00 40 80       	push   $0x80400000
80103127:	68 f0 73 11 80       	push   $0x801173f0
8010312c:	e8 8f f5 ff ff       	call   801026c0 <kinit1>
  kvmalloc();      // kernel page table
80103131:	e8 ea 49 00 00       	call   80107b20 <kvmalloc>
  mpinit();        // detect other processors
80103136:	e8 85 01 00 00       	call   801032c0 <mpinit>
  lapicinit();     // interrupt controller
8010313b:	e8 60 f7 ff ff       	call   801028a0 <lapicinit>
  seginit();       // segment descriptors
80103140:	e8 5b 44 00 00       	call   801075a0 <seginit>
  picinit();       // disable pic
80103145:	e8 76 03 00 00       	call   801034c0 <picinit>
  ioapicinit();    // another interrupt controller
8010314a:	e8 31 f3 ff ff       	call   80102480 <ioapicinit>
  consoleinit();   // console hardware
8010314f:	e8 0c d9 ff ff       	call   80100a60 <consoleinit>
  uartinit();      // serial port
80103154:	e8 d7 36 00 00       	call   80106830 <uartinit>
  pinit();         // process table
80103159:	e8 72 08 00 00       	call   801039d0 <pinit>
  tvinit();        // trap vectors
8010315e:	e8 5d 33 00 00       	call   801064c0 <tvinit>
  binit();         // buffer cache
80103163:	e8 d8 ce ff ff       	call   80100040 <binit>
  fileinit();      // file table
80103168:	e8 53 dd ff ff       	call   80100ec0 <fileinit>
  ideinit();       // disk 
8010316d:	e8 fe f0 ff ff       	call   80102270 <ideinit>

  // Write entry code to unused memory at 0x7000.
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = P2V(0x7000);
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);
80103172:	83 c4 0c             	add    $0xc,%esp
80103175:	68 8a 00 00 00       	push   $0x8a
8010317a:	68 8c b4 10 80       	push   $0x8010b48c
8010317f:	68 00 70 00 80       	push   $0x80007000
80103184:	e8 b7 21 00 00       	call   80105340 <memmove>

  for(c = cpus; c < cpus+ncpu; c++){
80103189:	83 c4 10             	add    $0x10,%esp
8010318c:	69 05 84 27 11 80 b0 	imul   $0xb0,0x80112784,%eax
80103193:	00 00 00 
80103196:	05 a0 27 11 80       	add    $0x801127a0,%eax
8010319b:	3d a0 27 11 80       	cmp    $0x801127a0,%eax
801031a0:	76 7e                	jbe    80103220 <main+0x110>
801031a2:	bb a0 27 11 80       	mov    $0x801127a0,%ebx
801031a7:	eb 20                	jmp    801031c9 <main+0xb9>
801031a9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801031b0:	69 05 84 27 11 80 b0 	imul   $0xb0,0x80112784,%eax
801031b7:	00 00 00 
801031ba:	81 c3 b0 00 00 00    	add    $0xb0,%ebx
801031c0:	05 a0 27 11 80       	add    $0x801127a0,%eax
801031c5:	39 c3                	cmp    %eax,%ebx
801031c7:	73 57                	jae    80103220 <main+0x110>
    if(c == mycpu())  // We've started already.
801031c9:	e8 22 08 00 00       	call   801039f0 <mycpu>
801031ce:	39 c3                	cmp    %eax,%ebx
801031d0:	74 de                	je     801031b0 <main+0xa0>
      continue;

    // Tell entryother.S what stack to use, where to enter, and what
    // pgdir to use. We cannot use kpgdir yet, because the AP processor
    // is running in low  memory, so we use entrypgdir for the APs too.
    stack = kalloc();
801031d2:	e8 59 f5 ff ff       	call   80102730 <kalloc>
    *(void**)(code-4) = stack + KSTACKSIZE;
    *(void(**)(void))(code-8) = mpenter;
    *(int**)(code-12) = (void *) V2P(entrypgdir);

    lapicstartap(c->apicid, V2P(code));
801031d7:	83 ec 08             	sub    $0x8,%esp
    *(void(**)(void))(code-8) = mpenter;
801031da:	c7 05 f8 6f 00 80 f0 	movl   $0x801030f0,0x80006ff8
801031e1:	30 10 80 
    *(int**)(code-12) = (void *) V2P(entrypgdir);
801031e4:	c7 05 f4 6f 00 80 00 	movl   $0x10a000,0x80006ff4
801031eb:	a0 10 00 
    *(void**)(code-4) = stack + KSTACKSIZE;
801031ee:	05 00 10 00 00       	add    $0x1000,%eax
801031f3:	a3 fc 6f 00 80       	mov    %eax,0x80006ffc
    lapicstartap(c->apicid, V2P(code));
801031f8:	0f b6 03             	movzbl (%ebx),%eax
801031fb:	68 00 70 00 00       	push   $0x7000
80103200:	50                   	push   %eax
80103201:	e8 ea f7 ff ff       	call   801029f0 <lapicstartap>

    // wait for cpu to finish mpmain()
    while(c->started == 0)
80103206:	83 c4 10             	add    $0x10,%esp
80103209:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80103210:	8b 83 a0 00 00 00    	mov    0xa0(%ebx),%eax
80103216:	85 c0                	test   %eax,%eax
80103218:	74 f6                	je     80103210 <main+0x100>
8010321a:	eb 94                	jmp    801031b0 <main+0xa0>
8010321c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  kinit2(P2V(4*1024*1024), P2V(PHYSTOP)); // must come after startothers()
80103220:	83 ec 08             	sub    $0x8,%esp
80103223:	68 00 00 00 8e       	push   $0x8e000000
80103228:	68 00 00 40 80       	push   $0x80400000
8010322d:	e8 2e f4 ff ff       	call   80102660 <kinit2>
  userinit();      // first user process
80103232:	e8 69 08 00 00       	call   80103aa0 <userinit>
  mpmain();        // finish this processor's setup
80103237:	e8 74 fe ff ff       	call   801030b0 <mpmain>
8010323c:	66 90                	xchg   %ax,%ax
8010323e:	66 90                	xchg   %ax,%ax

80103240 <mpsearch1>:
}

// Look for an MP structure in the len bytes at addr.
static struct mp*
mpsearch1(uint a, int len)
{
80103240:	55                   	push   %ebp
80103241:	89 e5                	mov    %esp,%ebp
80103243:	57                   	push   %edi
80103244:	56                   	push   %esi
  uchar *e, *p, *addr;

  addr = P2V(a);
80103245:	8d b0 00 00 00 80    	lea    -0x80000000(%eax),%esi
{
8010324b:	53                   	push   %ebx
  e = addr+len;
8010324c:	8d 1c 16             	lea    (%esi,%edx,1),%ebx
{
8010324f:	83 ec 0c             	sub    $0xc,%esp
  for(p = addr; p < e; p += sizeof(struct mp))
80103252:	39 de                	cmp    %ebx,%esi
80103254:	72 10                	jb     80103266 <mpsearch1+0x26>
80103256:	eb 50                	jmp    801032a8 <mpsearch1+0x68>
80103258:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010325f:	90                   	nop
80103260:	89 fe                	mov    %edi,%esi
80103262:	39 fb                	cmp    %edi,%ebx
80103264:	76 42                	jbe    801032a8 <mpsearch1+0x68>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
80103266:	83 ec 04             	sub    $0x4,%esp
80103269:	8d 7e 10             	lea    0x10(%esi),%edi
8010326c:	6a 04                	push   $0x4
8010326e:	68 38 83 10 80       	push   $0x80108338
80103273:	56                   	push   %esi
80103274:	e8 77 20 00 00       	call   801052f0 <memcmp>
80103279:	83 c4 10             	add    $0x10,%esp
8010327c:	85 c0                	test   %eax,%eax
8010327e:	75 e0                	jne    80103260 <mpsearch1+0x20>
80103280:	89 f2                	mov    %esi,%edx
80103282:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    sum += addr[i];
80103288:	0f b6 0a             	movzbl (%edx),%ecx
  for(i=0; i<len; i++)
8010328b:	83 c2 01             	add    $0x1,%edx
    sum += addr[i];
8010328e:	01 c8                	add    %ecx,%eax
  for(i=0; i<len; i++)
80103290:	39 fa                	cmp    %edi,%edx
80103292:	75 f4                	jne    80103288 <mpsearch1+0x48>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
80103294:	84 c0                	test   %al,%al
80103296:	75 c8                	jne    80103260 <mpsearch1+0x20>
      return (struct mp*)p;
  return 0;
}
80103298:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010329b:	89 f0                	mov    %esi,%eax
8010329d:	5b                   	pop    %ebx
8010329e:	5e                   	pop    %esi
8010329f:	5f                   	pop    %edi
801032a0:	5d                   	pop    %ebp
801032a1:	c3                   	ret    
801032a2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
801032a8:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
801032ab:	31 f6                	xor    %esi,%esi
}
801032ad:	5b                   	pop    %ebx
801032ae:	89 f0                	mov    %esi,%eax
801032b0:	5e                   	pop    %esi
801032b1:	5f                   	pop    %edi
801032b2:	5d                   	pop    %ebp
801032b3:	c3                   	ret    
801032b4:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801032bb:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801032bf:	90                   	nop

801032c0 <mpinit>:
  return conf;
}

void
mpinit(void)
{
801032c0:	55                   	push   %ebp
801032c1:	89 e5                	mov    %esp,%ebp
801032c3:	57                   	push   %edi
801032c4:	56                   	push   %esi
801032c5:	53                   	push   %ebx
801032c6:	83 ec 1c             	sub    $0x1c,%esp
  if((p = ((bda[0x0F]<<8)| bda[0x0E]) << 4)){
801032c9:	0f b6 05 0f 04 00 80 	movzbl 0x8000040f,%eax
801032d0:	0f b6 15 0e 04 00 80 	movzbl 0x8000040e,%edx
801032d7:	c1 e0 08             	shl    $0x8,%eax
801032da:	09 d0                	or     %edx,%eax
801032dc:	c1 e0 04             	shl    $0x4,%eax
801032df:	75 1b                	jne    801032fc <mpinit+0x3c>
    p = ((bda[0x14]<<8)|bda[0x13])*1024;
801032e1:	0f b6 05 14 04 00 80 	movzbl 0x80000414,%eax
801032e8:	0f b6 15 13 04 00 80 	movzbl 0x80000413,%edx
801032ef:	c1 e0 08             	shl    $0x8,%eax
801032f2:	09 d0                	or     %edx,%eax
801032f4:	c1 e0 0a             	shl    $0xa,%eax
    if((mp = mpsearch1(p-1024, 1024)))
801032f7:	2d 00 04 00 00       	sub    $0x400,%eax
    if((mp = mpsearch1(p, 1024)))
801032fc:	ba 00 04 00 00       	mov    $0x400,%edx
80103301:	e8 3a ff ff ff       	call   80103240 <mpsearch1>
80103306:	89 c3                	mov    %eax,%ebx
80103308:	85 c0                	test   %eax,%eax
8010330a:	0f 84 40 01 00 00    	je     80103450 <mpinit+0x190>
  if((mp = mpsearch()) == 0 || mp->physaddr == 0)
80103310:	8b 73 04             	mov    0x4(%ebx),%esi
80103313:	85 f6                	test   %esi,%esi
80103315:	0f 84 25 01 00 00    	je     80103440 <mpinit+0x180>
  if(memcmp(conf, "PCMP", 4) != 0)
8010331b:	83 ec 04             	sub    $0x4,%esp
  conf = (struct mpconf*) P2V((uint) mp->physaddr);
8010331e:	8d 86 00 00 00 80    	lea    -0x80000000(%esi),%eax
  if(memcmp(conf, "PCMP", 4) != 0)
80103324:	6a 04                	push   $0x4
80103326:	68 3d 83 10 80       	push   $0x8010833d
8010332b:	50                   	push   %eax
  conf = (struct mpconf*) P2V((uint) mp->physaddr);
8010332c:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  if(memcmp(conf, "PCMP", 4) != 0)
8010332f:	e8 bc 1f 00 00       	call   801052f0 <memcmp>
80103334:	83 c4 10             	add    $0x10,%esp
80103337:	85 c0                	test   %eax,%eax
80103339:	0f 85 01 01 00 00    	jne    80103440 <mpinit+0x180>
  if(conf->version != 1 && conf->version != 4)
8010333f:	0f b6 86 06 00 00 80 	movzbl -0x7ffffffa(%esi),%eax
80103346:	3c 01                	cmp    $0x1,%al
80103348:	74 08                	je     80103352 <mpinit+0x92>
8010334a:	3c 04                	cmp    $0x4,%al
8010334c:	0f 85 ee 00 00 00    	jne    80103440 <mpinit+0x180>
  if(sum((uchar*)conf, conf->length) != 0)
80103352:	0f b7 96 04 00 00 80 	movzwl -0x7ffffffc(%esi),%edx
  for(i=0; i<len; i++)
80103359:	66 85 d2             	test   %dx,%dx
8010335c:	74 22                	je     80103380 <mpinit+0xc0>
8010335e:	8d 3c 32             	lea    (%edx,%esi,1),%edi
80103361:	89 f0                	mov    %esi,%eax
  sum = 0;
80103363:	31 d2                	xor    %edx,%edx
80103365:	8d 76 00             	lea    0x0(%esi),%esi
    sum += addr[i];
80103368:	0f b6 88 00 00 00 80 	movzbl -0x80000000(%eax),%ecx
  for(i=0; i<len; i++)
8010336f:	83 c0 01             	add    $0x1,%eax
    sum += addr[i];
80103372:	01 ca                	add    %ecx,%edx
  for(i=0; i<len; i++)
80103374:	39 c7                	cmp    %eax,%edi
80103376:	75 f0                	jne    80103368 <mpinit+0xa8>
  if(sum((uchar*)conf, conf->length) != 0)
80103378:	84 d2                	test   %dl,%dl
8010337a:	0f 85 c0 00 00 00    	jne    80103440 <mpinit+0x180>
  struct mpioapic *ioapic;

  if((conf = mpconfig(&mp)) == 0)
    panic("Expect to run on an SMP");
  ismp = 1;
  lapic = (uint*)conf->lapicaddr;
80103380:	8b 86 24 00 00 80    	mov    -0x7fffffdc(%esi),%eax
80103386:	a3 80 26 11 80       	mov    %eax,0x80112680
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
8010338b:	0f b7 96 04 00 00 80 	movzwl -0x7ffffffc(%esi),%edx
80103392:	8d 86 2c 00 00 80    	lea    -0x7fffffd4(%esi),%eax
  ismp = 1;
80103398:	be 01 00 00 00       	mov    $0x1,%esi
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
8010339d:	03 55 e4             	add    -0x1c(%ebp),%edx
801033a0:	89 5d e4             	mov    %ebx,-0x1c(%ebp)
801033a3:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801033a7:	90                   	nop
801033a8:	39 d0                	cmp    %edx,%eax
801033aa:	73 15                	jae    801033c1 <mpinit+0x101>
    switch(*p){
801033ac:	0f b6 08             	movzbl (%eax),%ecx
801033af:	80 f9 02             	cmp    $0x2,%cl
801033b2:	74 4c                	je     80103400 <mpinit+0x140>
801033b4:	77 3a                	ja     801033f0 <mpinit+0x130>
801033b6:	84 c9                	test   %cl,%cl
801033b8:	74 56                	je     80103410 <mpinit+0x150>
      p += sizeof(struct mpioapic);
      continue;
    case MPBUS:
    case MPIOINTR:
    case MPLINTR:
      p += 8;
801033ba:	83 c0 08             	add    $0x8,%eax
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
801033bd:	39 d0                	cmp    %edx,%eax
801033bf:	72 eb                	jb     801033ac <mpinit+0xec>
    default:
      ismp = 0;
      break;
    }
  }
  if(!ismp)
801033c1:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
801033c4:	85 f6                	test   %esi,%esi
801033c6:	0f 84 d9 00 00 00    	je     801034a5 <mpinit+0x1e5>
    panic("Didn't find a suitable machine");

  if(mp->imcrp){
801033cc:	80 7b 0c 00          	cmpb   $0x0,0xc(%ebx)
801033d0:	74 15                	je     801033e7 <mpinit+0x127>
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801033d2:	b8 70 00 00 00       	mov    $0x70,%eax
801033d7:	ba 22 00 00 00       	mov    $0x22,%edx
801033dc:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801033dd:	ba 23 00 00 00       	mov    $0x23,%edx
801033e2:	ec                   	in     (%dx),%al
    // Bochs doesn't support IMCR, so this doesn't run on Bochs.
    // But it would on real hardware.
    outb(0x22, 0x70);   // Select IMCR
    outb(0x23, inb(0x23) | 1);  // Mask external interrupts.
801033e3:	83 c8 01             	or     $0x1,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801033e6:	ee                   	out    %al,(%dx)
  }
}
801033e7:	8d 65 f4             	lea    -0xc(%ebp),%esp
801033ea:	5b                   	pop    %ebx
801033eb:	5e                   	pop    %esi
801033ec:	5f                   	pop    %edi
801033ed:	5d                   	pop    %ebp
801033ee:	c3                   	ret    
801033ef:	90                   	nop
    switch(*p){
801033f0:	83 e9 03             	sub    $0x3,%ecx
801033f3:	80 f9 01             	cmp    $0x1,%cl
801033f6:	76 c2                	jbe    801033ba <mpinit+0xfa>
801033f8:	31 f6                	xor    %esi,%esi
801033fa:	eb ac                	jmp    801033a8 <mpinit+0xe8>
801033fc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
      ioapicid = ioapic->apicno;
80103400:	0f b6 48 01          	movzbl 0x1(%eax),%ecx
      p += sizeof(struct mpioapic);
80103404:	83 c0 08             	add    $0x8,%eax
      ioapicid = ioapic->apicno;
80103407:	88 0d 80 27 11 80    	mov    %cl,0x80112780
      continue;
8010340d:	eb 99                	jmp    801033a8 <mpinit+0xe8>
8010340f:	90                   	nop
      if(ncpu < NCPU) {
80103410:	8b 0d 84 27 11 80    	mov    0x80112784,%ecx
80103416:	83 f9 07             	cmp    $0x7,%ecx
80103419:	7f 19                	jg     80103434 <mpinit+0x174>
        cpus[ncpu].apicid = proc->apicid;  // apicid may differ from ncpu
8010341b:	69 f9 b0 00 00 00    	imul   $0xb0,%ecx,%edi
80103421:	0f b6 58 01          	movzbl 0x1(%eax),%ebx
        ncpu++;
80103425:	83 c1 01             	add    $0x1,%ecx
80103428:	89 0d 84 27 11 80    	mov    %ecx,0x80112784
        cpus[ncpu].apicid = proc->apicid;  // apicid may differ from ncpu
8010342e:	88 9f a0 27 11 80    	mov    %bl,-0x7feed860(%edi)
      p += sizeof(struct mpproc);
80103434:	83 c0 14             	add    $0x14,%eax
      continue;
80103437:	e9 6c ff ff ff       	jmp    801033a8 <mpinit+0xe8>
8010343c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    panic("Expect to run on an SMP");
80103440:	83 ec 0c             	sub    $0xc,%esp
80103443:	68 42 83 10 80       	push   $0x80108342
80103448:	e8 33 cf ff ff       	call   80100380 <panic>
8010344d:	8d 76 00             	lea    0x0(%esi),%esi
{
80103450:	bb 00 00 0f 80       	mov    $0x800f0000,%ebx
80103455:	eb 13                	jmp    8010346a <mpinit+0x1aa>
80103457:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010345e:	66 90                	xchg   %ax,%ax
  for(p = addr; p < e; p += sizeof(struct mp))
80103460:	89 f3                	mov    %esi,%ebx
80103462:	81 fe 00 00 10 80    	cmp    $0x80100000,%esi
80103468:	74 d6                	je     80103440 <mpinit+0x180>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
8010346a:	83 ec 04             	sub    $0x4,%esp
8010346d:	8d 73 10             	lea    0x10(%ebx),%esi
80103470:	6a 04                	push   $0x4
80103472:	68 38 83 10 80       	push   $0x80108338
80103477:	53                   	push   %ebx
80103478:	e8 73 1e 00 00       	call   801052f0 <memcmp>
8010347d:	83 c4 10             	add    $0x10,%esp
80103480:	85 c0                	test   %eax,%eax
80103482:	75 dc                	jne    80103460 <mpinit+0x1a0>
80103484:	89 da                	mov    %ebx,%edx
80103486:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010348d:	8d 76 00             	lea    0x0(%esi),%esi
    sum += addr[i];
80103490:	0f b6 0a             	movzbl (%edx),%ecx
  for(i=0; i<len; i++)
80103493:	83 c2 01             	add    $0x1,%edx
    sum += addr[i];
80103496:	01 c8                	add    %ecx,%eax
  for(i=0; i<len; i++)
80103498:	39 d6                	cmp    %edx,%esi
8010349a:	75 f4                	jne    80103490 <mpinit+0x1d0>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
8010349c:	84 c0                	test   %al,%al
8010349e:	75 c0                	jne    80103460 <mpinit+0x1a0>
801034a0:	e9 6b fe ff ff       	jmp    80103310 <mpinit+0x50>
    panic("Didn't find a suitable machine");
801034a5:	83 ec 0c             	sub    $0xc,%esp
801034a8:	68 5c 83 10 80       	push   $0x8010835c
801034ad:	e8 ce ce ff ff       	call   80100380 <panic>
801034b2:	66 90                	xchg   %ax,%ax
801034b4:	66 90                	xchg   %ax,%ax
801034b6:	66 90                	xchg   %ax,%ax
801034b8:	66 90                	xchg   %ax,%ax
801034ba:	66 90                	xchg   %ax,%ax
801034bc:	66 90                	xchg   %ax,%ax
801034be:	66 90                	xchg   %ax,%ax

801034c0 <picinit>:
801034c0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801034c5:	ba 21 00 00 00       	mov    $0x21,%edx
801034ca:	ee                   	out    %al,(%dx)
801034cb:	ba a1 00 00 00       	mov    $0xa1,%edx
801034d0:	ee                   	out    %al,(%dx)
picinit(void)
{
  // mask all interrupts
  outb(IO_PIC1+1, 0xFF);
  outb(IO_PIC2+1, 0xFF);
}
801034d1:	c3                   	ret    
801034d2:	66 90                	xchg   %ax,%ax
801034d4:	66 90                	xchg   %ax,%ax
801034d6:	66 90                	xchg   %ax,%ax
801034d8:	66 90                	xchg   %ax,%ax
801034da:	66 90                	xchg   %ax,%ax
801034dc:	66 90                	xchg   %ax,%ax
801034de:	66 90                	xchg   %ax,%ax

801034e0 <pipealloc>:
  int writeopen;  // write fd is still open
};

int
pipealloc(struct file **f0, struct file **f1)
{
801034e0:	55                   	push   %ebp
801034e1:	89 e5                	mov    %esp,%ebp
801034e3:	57                   	push   %edi
801034e4:	56                   	push   %esi
801034e5:	53                   	push   %ebx
801034e6:	83 ec 0c             	sub    $0xc,%esp
801034e9:	8b 5d 08             	mov    0x8(%ebp),%ebx
801034ec:	8b 75 0c             	mov    0xc(%ebp),%esi
  struct pipe *p;

  p = 0;
  *f0 = *f1 = 0;
801034ef:	c7 06 00 00 00 00    	movl   $0x0,(%esi)
801034f5:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
801034fb:	e8 e0 d9 ff ff       	call   80100ee0 <filealloc>
80103500:	89 03                	mov    %eax,(%ebx)
80103502:	85 c0                	test   %eax,%eax
80103504:	0f 84 a8 00 00 00    	je     801035b2 <pipealloc+0xd2>
8010350a:	e8 d1 d9 ff ff       	call   80100ee0 <filealloc>
8010350f:	89 06                	mov    %eax,(%esi)
80103511:	85 c0                	test   %eax,%eax
80103513:	0f 84 87 00 00 00    	je     801035a0 <pipealloc+0xc0>
    goto bad;
  if((p = (struct pipe*)kalloc()) == 0)
80103519:	e8 12 f2 ff ff       	call   80102730 <kalloc>
8010351e:	89 c7                	mov    %eax,%edi
80103520:	85 c0                	test   %eax,%eax
80103522:	0f 84 b0 00 00 00    	je     801035d8 <pipealloc+0xf8>
    goto bad;
  p->readopen = 1;
80103528:	c7 80 3c 02 00 00 01 	movl   $0x1,0x23c(%eax)
8010352f:	00 00 00 
  p->writeopen = 1;
  p->nwrite = 0;
  p->nread = 0;
  initlock(&p->lock, "pipe");
80103532:	83 ec 08             	sub    $0x8,%esp
  p->writeopen = 1;
80103535:	c7 80 40 02 00 00 01 	movl   $0x1,0x240(%eax)
8010353c:	00 00 00 
  p->nwrite = 0;
8010353f:	c7 80 38 02 00 00 00 	movl   $0x0,0x238(%eax)
80103546:	00 00 00 
  p->nread = 0;
80103549:	c7 80 34 02 00 00 00 	movl   $0x0,0x234(%eax)
80103550:	00 00 00 
  initlock(&p->lock, "pipe");
80103553:	68 7b 83 10 80       	push   $0x8010837b
80103558:	50                   	push   %eax
80103559:	e8 b2 1a 00 00       	call   80105010 <initlock>
  (*f0)->type = FD_PIPE;
8010355e:	8b 03                	mov    (%ebx),%eax
  (*f0)->pipe = p;
  (*f1)->type = FD_PIPE;
  (*f1)->readable = 0;
  (*f1)->writable = 1;
  (*f1)->pipe = p;
  return 0;
80103560:	83 c4 10             	add    $0x10,%esp
  (*f0)->type = FD_PIPE;
80103563:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f0)->readable = 1;
80103569:	8b 03                	mov    (%ebx),%eax
8010356b:	c6 40 08 01          	movb   $0x1,0x8(%eax)
  (*f0)->writable = 0;
8010356f:	8b 03                	mov    (%ebx),%eax
80103571:	c6 40 09 00          	movb   $0x0,0x9(%eax)
  (*f0)->pipe = p;
80103575:	8b 03                	mov    (%ebx),%eax
80103577:	89 78 0c             	mov    %edi,0xc(%eax)
  (*f1)->type = FD_PIPE;
8010357a:	8b 06                	mov    (%esi),%eax
8010357c:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f1)->readable = 0;
80103582:	8b 06                	mov    (%esi),%eax
80103584:	c6 40 08 00          	movb   $0x0,0x8(%eax)
  (*f1)->writable = 1;
80103588:	8b 06                	mov    (%esi),%eax
8010358a:	c6 40 09 01          	movb   $0x1,0x9(%eax)
  (*f1)->pipe = p;
8010358e:	8b 06                	mov    (%esi),%eax
80103590:	89 78 0c             	mov    %edi,0xc(%eax)
  if(*f0)
    fileclose(*f0);
  if(*f1)
    fileclose(*f1);
  return -1;
}
80103593:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
80103596:	31 c0                	xor    %eax,%eax
}
80103598:	5b                   	pop    %ebx
80103599:	5e                   	pop    %esi
8010359a:	5f                   	pop    %edi
8010359b:	5d                   	pop    %ebp
8010359c:	c3                   	ret    
8010359d:	8d 76 00             	lea    0x0(%esi),%esi
  if(*f0)
801035a0:	8b 03                	mov    (%ebx),%eax
801035a2:	85 c0                	test   %eax,%eax
801035a4:	74 1e                	je     801035c4 <pipealloc+0xe4>
    fileclose(*f0);
801035a6:	83 ec 0c             	sub    $0xc,%esp
801035a9:	50                   	push   %eax
801035aa:	e8 f1 d9 ff ff       	call   80100fa0 <fileclose>
801035af:	83 c4 10             	add    $0x10,%esp
  if(*f1)
801035b2:	8b 06                	mov    (%esi),%eax
801035b4:	85 c0                	test   %eax,%eax
801035b6:	74 0c                	je     801035c4 <pipealloc+0xe4>
    fileclose(*f1);
801035b8:	83 ec 0c             	sub    $0xc,%esp
801035bb:	50                   	push   %eax
801035bc:	e8 df d9 ff ff       	call   80100fa0 <fileclose>
801035c1:	83 c4 10             	add    $0x10,%esp
}
801035c4:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return -1;
801035c7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801035cc:	5b                   	pop    %ebx
801035cd:	5e                   	pop    %esi
801035ce:	5f                   	pop    %edi
801035cf:	5d                   	pop    %ebp
801035d0:	c3                   	ret    
801035d1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  if(*f0)
801035d8:	8b 03                	mov    (%ebx),%eax
801035da:	85 c0                	test   %eax,%eax
801035dc:	75 c8                	jne    801035a6 <pipealloc+0xc6>
801035de:	eb d2                	jmp    801035b2 <pipealloc+0xd2>

801035e0 <pipeclose>:

void
pipeclose(struct pipe *p, int writable)
{
801035e0:	55                   	push   %ebp
801035e1:	89 e5                	mov    %esp,%ebp
801035e3:	56                   	push   %esi
801035e4:	53                   	push   %ebx
801035e5:	8b 5d 08             	mov    0x8(%ebp),%ebx
801035e8:	8b 75 0c             	mov    0xc(%ebp),%esi
  acquire(&p->lock);
801035eb:	83 ec 0c             	sub    $0xc,%esp
801035ee:	53                   	push   %ebx
801035ef:	e8 ec 1b 00 00       	call   801051e0 <acquire>
  if(writable){
801035f4:	83 c4 10             	add    $0x10,%esp
801035f7:	85 f6                	test   %esi,%esi
801035f9:	74 65                	je     80103660 <pipeclose+0x80>
    p->writeopen = 0;
    wakeup(&p->nread);
801035fb:	83 ec 0c             	sub    $0xc,%esp
801035fe:	8d 83 34 02 00 00    	lea    0x234(%ebx),%eax
    p->writeopen = 0;
80103604:	c7 83 40 02 00 00 00 	movl   $0x0,0x240(%ebx)
8010360b:	00 00 00 
    wakeup(&p->nread);
8010360e:	50                   	push   %eax
8010360f:	e8 cc 0e 00 00       	call   801044e0 <wakeup>
80103614:	83 c4 10             	add    $0x10,%esp
  } else {
    p->readopen = 0;
    wakeup(&p->nwrite);
  }
  if(p->readopen == 0 && p->writeopen == 0){
80103617:	8b 93 3c 02 00 00    	mov    0x23c(%ebx),%edx
8010361d:	85 d2                	test   %edx,%edx
8010361f:	75 0a                	jne    8010362b <pipeclose+0x4b>
80103621:	8b 83 40 02 00 00    	mov    0x240(%ebx),%eax
80103627:	85 c0                	test   %eax,%eax
80103629:	74 15                	je     80103640 <pipeclose+0x60>
    release(&p->lock);
    kfree((char*)p);
  } else
    release(&p->lock);
8010362b:	89 5d 08             	mov    %ebx,0x8(%ebp)
}
8010362e:	8d 65 f8             	lea    -0x8(%ebp),%esp
80103631:	5b                   	pop    %ebx
80103632:	5e                   	pop    %esi
80103633:	5d                   	pop    %ebp
    release(&p->lock);
80103634:	e9 47 1b 00 00       	jmp    80105180 <release>
80103639:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    release(&p->lock);
80103640:	83 ec 0c             	sub    $0xc,%esp
80103643:	53                   	push   %ebx
80103644:	e8 37 1b 00 00       	call   80105180 <release>
    kfree((char*)p);
80103649:	89 5d 08             	mov    %ebx,0x8(%ebp)
8010364c:	83 c4 10             	add    $0x10,%esp
}
8010364f:	8d 65 f8             	lea    -0x8(%ebp),%esp
80103652:	5b                   	pop    %ebx
80103653:	5e                   	pop    %esi
80103654:	5d                   	pop    %ebp
    kfree((char*)p);
80103655:	e9 16 ef ff ff       	jmp    80102570 <kfree>
8010365a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    wakeup(&p->nwrite);
80103660:	83 ec 0c             	sub    $0xc,%esp
80103663:	8d 83 38 02 00 00    	lea    0x238(%ebx),%eax
    p->readopen = 0;
80103669:	c7 83 3c 02 00 00 00 	movl   $0x0,0x23c(%ebx)
80103670:	00 00 00 
    wakeup(&p->nwrite);
80103673:	50                   	push   %eax
80103674:	e8 67 0e 00 00       	call   801044e0 <wakeup>
80103679:	83 c4 10             	add    $0x10,%esp
8010367c:	eb 99                	jmp    80103617 <pipeclose+0x37>
8010367e:	66 90                	xchg   %ax,%ax

80103680 <pipewrite>:

//PAGEBREAK: 40
int
pipewrite(struct pipe *p, char *addr, int n)
{
80103680:	55                   	push   %ebp
80103681:	89 e5                	mov    %esp,%ebp
80103683:	57                   	push   %edi
80103684:	56                   	push   %esi
80103685:	53                   	push   %ebx
80103686:	83 ec 28             	sub    $0x28,%esp
80103689:	8b 5d 08             	mov    0x8(%ebp),%ebx
  int i;

  acquire(&p->lock);
8010368c:	53                   	push   %ebx
8010368d:	e8 4e 1b 00 00       	call   801051e0 <acquire>
  for(i = 0; i < n; i++){
80103692:	8b 45 10             	mov    0x10(%ebp),%eax
80103695:	83 c4 10             	add    $0x10,%esp
80103698:	85 c0                	test   %eax,%eax
8010369a:	0f 8e c0 00 00 00    	jle    80103760 <pipewrite+0xe0>
801036a0:	8b 45 0c             	mov    0xc(%ebp),%eax
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
801036a3:	8b 8b 38 02 00 00    	mov    0x238(%ebx),%ecx
      if(p->readopen == 0 || myproc()->killed){
        release(&p->lock);
        return -1;
      }
      wakeup(&p->nread);
801036a9:	8d bb 34 02 00 00    	lea    0x234(%ebx),%edi
801036af:	89 45 e4             	mov    %eax,-0x1c(%ebp)
801036b2:	03 45 10             	add    0x10(%ebp),%eax
801036b5:	89 45 e0             	mov    %eax,-0x20(%ebp)
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
801036b8:	8b 83 34 02 00 00    	mov    0x234(%ebx),%eax
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
801036be:	8d b3 38 02 00 00    	lea    0x238(%ebx),%esi
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
801036c4:	89 ca                	mov    %ecx,%edx
801036c6:	05 00 02 00 00       	add    $0x200,%eax
801036cb:	39 c1                	cmp    %eax,%ecx
801036cd:	74 3f                	je     8010370e <pipewrite+0x8e>
801036cf:	eb 67                	jmp    80103738 <pipewrite+0xb8>
801036d1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
      if(p->readopen == 0 || myproc()->killed){
801036d8:	e8 93 03 00 00       	call   80103a70 <myproc>
801036dd:	8b 48 24             	mov    0x24(%eax),%ecx
801036e0:	85 c9                	test   %ecx,%ecx
801036e2:	75 34                	jne    80103718 <pipewrite+0x98>
      wakeup(&p->nread);
801036e4:	83 ec 0c             	sub    $0xc,%esp
801036e7:	57                   	push   %edi
801036e8:	e8 f3 0d 00 00       	call   801044e0 <wakeup>
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
801036ed:	58                   	pop    %eax
801036ee:	5a                   	pop    %edx
801036ef:	53                   	push   %ebx
801036f0:	56                   	push   %esi
801036f1:	e8 2a 0d 00 00       	call   80104420 <sleep>
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
801036f6:	8b 83 34 02 00 00    	mov    0x234(%ebx),%eax
801036fc:	8b 93 38 02 00 00    	mov    0x238(%ebx),%edx
80103702:	83 c4 10             	add    $0x10,%esp
80103705:	05 00 02 00 00       	add    $0x200,%eax
8010370a:	39 c2                	cmp    %eax,%edx
8010370c:	75 2a                	jne    80103738 <pipewrite+0xb8>
      if(p->readopen == 0 || myproc()->killed){
8010370e:	8b 83 3c 02 00 00    	mov    0x23c(%ebx),%eax
80103714:	85 c0                	test   %eax,%eax
80103716:	75 c0                	jne    801036d8 <pipewrite+0x58>
        release(&p->lock);
80103718:	83 ec 0c             	sub    $0xc,%esp
8010371b:	53                   	push   %ebx
8010371c:	e8 5f 1a 00 00       	call   80105180 <release>
        return -1;
80103721:	83 c4 10             	add    $0x10,%esp
80103724:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
  }
  wakeup(&p->nread);  //DOC: pipewrite-wakeup1
  release(&p->lock);
  return n;
}
80103729:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010372c:	5b                   	pop    %ebx
8010372d:	5e                   	pop    %esi
8010372e:	5f                   	pop    %edi
8010372f:	5d                   	pop    %ebp
80103730:	c3                   	ret    
80103731:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
80103738:	8b 75 e4             	mov    -0x1c(%ebp),%esi
8010373b:	8d 4a 01             	lea    0x1(%edx),%ecx
8010373e:	81 e2 ff 01 00 00    	and    $0x1ff,%edx
80103744:	89 8b 38 02 00 00    	mov    %ecx,0x238(%ebx)
8010374a:	0f b6 06             	movzbl (%esi),%eax
  for(i = 0; i < n; i++){
8010374d:	83 c6 01             	add    $0x1,%esi
80103750:	89 75 e4             	mov    %esi,-0x1c(%ebp)
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
80103753:	88 44 13 34          	mov    %al,0x34(%ebx,%edx,1)
  for(i = 0; i < n; i++){
80103757:	3b 75 e0             	cmp    -0x20(%ebp),%esi
8010375a:	0f 85 58 ff ff ff    	jne    801036b8 <pipewrite+0x38>
  wakeup(&p->nread);  //DOC: pipewrite-wakeup1
80103760:	83 ec 0c             	sub    $0xc,%esp
80103763:	8d 83 34 02 00 00    	lea    0x234(%ebx),%eax
80103769:	50                   	push   %eax
8010376a:	e8 71 0d 00 00       	call   801044e0 <wakeup>
  release(&p->lock);
8010376f:	89 1c 24             	mov    %ebx,(%esp)
80103772:	e8 09 1a 00 00       	call   80105180 <release>
  return n;
80103777:	8b 45 10             	mov    0x10(%ebp),%eax
8010377a:	83 c4 10             	add    $0x10,%esp
8010377d:	eb aa                	jmp    80103729 <pipewrite+0xa9>
8010377f:	90                   	nop

80103780 <piperead>:

int
piperead(struct pipe *p, char *addr, int n)
{
80103780:	55                   	push   %ebp
80103781:	89 e5                	mov    %esp,%ebp
80103783:	57                   	push   %edi
80103784:	56                   	push   %esi
80103785:	53                   	push   %ebx
80103786:	83 ec 18             	sub    $0x18,%esp
80103789:	8b 75 08             	mov    0x8(%ebp),%esi
8010378c:	8b 7d 0c             	mov    0xc(%ebp),%edi
  int i;

  acquire(&p->lock);
8010378f:	56                   	push   %esi
80103790:	8d 9e 34 02 00 00    	lea    0x234(%esi),%ebx
80103796:	e8 45 1a 00 00       	call   801051e0 <acquire>
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
8010379b:	8b 86 34 02 00 00    	mov    0x234(%esi),%eax
801037a1:	83 c4 10             	add    $0x10,%esp
801037a4:	39 86 38 02 00 00    	cmp    %eax,0x238(%esi)
801037aa:	74 2f                	je     801037db <piperead+0x5b>
801037ac:	eb 37                	jmp    801037e5 <piperead+0x65>
801037ae:	66 90                	xchg   %ax,%ax
    if(myproc()->killed){
801037b0:	e8 bb 02 00 00       	call   80103a70 <myproc>
801037b5:	8b 48 24             	mov    0x24(%eax),%ecx
801037b8:	85 c9                	test   %ecx,%ecx
801037ba:	0f 85 80 00 00 00    	jne    80103840 <piperead+0xc0>
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
801037c0:	83 ec 08             	sub    $0x8,%esp
801037c3:	56                   	push   %esi
801037c4:	53                   	push   %ebx
801037c5:	e8 56 0c 00 00       	call   80104420 <sleep>
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
801037ca:	8b 86 38 02 00 00    	mov    0x238(%esi),%eax
801037d0:	83 c4 10             	add    $0x10,%esp
801037d3:	39 86 34 02 00 00    	cmp    %eax,0x234(%esi)
801037d9:	75 0a                	jne    801037e5 <piperead+0x65>
801037db:	8b 86 40 02 00 00    	mov    0x240(%esi),%eax
801037e1:	85 c0                	test   %eax,%eax
801037e3:	75 cb                	jne    801037b0 <piperead+0x30>
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
801037e5:	8b 55 10             	mov    0x10(%ebp),%edx
801037e8:	31 db                	xor    %ebx,%ebx
801037ea:	85 d2                	test   %edx,%edx
801037ec:	7f 20                	jg     8010380e <piperead+0x8e>
801037ee:	eb 2c                	jmp    8010381c <piperead+0x9c>
    if(p->nread == p->nwrite)
      break;
    addr[i] = p->data[p->nread++ % PIPESIZE];
801037f0:	8d 48 01             	lea    0x1(%eax),%ecx
801037f3:	25 ff 01 00 00       	and    $0x1ff,%eax
801037f8:	89 8e 34 02 00 00    	mov    %ecx,0x234(%esi)
801037fe:	0f b6 44 06 34       	movzbl 0x34(%esi,%eax,1),%eax
80103803:	88 04 1f             	mov    %al,(%edi,%ebx,1)
  for(i = 0; i < n; i++){  //DOC: piperead-copy
80103806:	83 c3 01             	add    $0x1,%ebx
80103809:	39 5d 10             	cmp    %ebx,0x10(%ebp)
8010380c:	74 0e                	je     8010381c <piperead+0x9c>
    if(p->nread == p->nwrite)
8010380e:	8b 86 34 02 00 00    	mov    0x234(%esi),%eax
80103814:	3b 86 38 02 00 00    	cmp    0x238(%esi),%eax
8010381a:	75 d4                	jne    801037f0 <piperead+0x70>
  }
  wakeup(&p->nwrite);  //DOC: piperead-wakeup
8010381c:	83 ec 0c             	sub    $0xc,%esp
8010381f:	8d 86 38 02 00 00    	lea    0x238(%esi),%eax
80103825:	50                   	push   %eax
80103826:	e8 b5 0c 00 00       	call   801044e0 <wakeup>
  release(&p->lock);
8010382b:	89 34 24             	mov    %esi,(%esp)
8010382e:	e8 4d 19 00 00       	call   80105180 <release>
  return i;
80103833:	83 c4 10             	add    $0x10,%esp
}
80103836:	8d 65 f4             	lea    -0xc(%ebp),%esp
80103839:	89 d8                	mov    %ebx,%eax
8010383b:	5b                   	pop    %ebx
8010383c:	5e                   	pop    %esi
8010383d:	5f                   	pop    %edi
8010383e:	5d                   	pop    %ebp
8010383f:	c3                   	ret    
      release(&p->lock);
80103840:	83 ec 0c             	sub    $0xc,%esp
      return -1;
80103843:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
      release(&p->lock);
80103848:	56                   	push   %esi
80103849:	e8 32 19 00 00       	call   80105180 <release>
      return -1;
8010384e:	83 c4 10             	add    $0x10,%esp
}
80103851:	8d 65 f4             	lea    -0xc(%ebp),%esp
80103854:	89 d8                	mov    %ebx,%eax
80103856:	5b                   	pop    %ebx
80103857:	5e                   	pop    %esi
80103858:	5f                   	pop    %edi
80103859:	5d                   	pop    %ebp
8010385a:	c3                   	ret    
8010385b:	66 90                	xchg   %ax,%ax
8010385d:	66 90                	xchg   %ax,%ax
8010385f:	90                   	nop

80103860 <allocproc>:
// If found, change state to EMBRYO and initialize
// state required to run in the kernel.
// Otherwise return 0.
static struct proc*
allocproc(void)
{
80103860:	55                   	push   %ebp
80103861:	89 e5                	mov    %esp,%ebp
80103863:	53                   	push   %ebx
  struct proc *p;
  char *sp;

  acquire(&ptable.lock);

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80103864:	bb 54 2d 11 80       	mov    $0x80112d54,%ebx
{
80103869:	83 ec 10             	sub    $0x10,%esp
  acquire(&ptable.lock);
8010386c:	68 20 2d 11 80       	push   $0x80112d20
80103871:	e8 6a 19 00 00       	call   801051e0 <acquire>
80103876:	83 c4 10             	add    $0x10,%esp
80103879:	eb 13                	jmp    8010388e <allocproc+0x2e>
8010387b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010387f:	90                   	nop
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80103880:	81 c3 b8 00 00 00    	add    $0xb8,%ebx
80103886:	81 fb 54 5b 11 80    	cmp    $0x80115b54,%ebx
8010388c:	74 7a                	je     80103908 <allocproc+0xa8>
    if(p->state == UNUSED)
8010388e:	8b 43 0c             	mov    0xc(%ebx),%eax
80103891:	85 c0                	test   %eax,%eax
80103893:	75 eb                	jne    80103880 <allocproc+0x20>
  release(&ptable.lock);
  return 0;

found:
  p->state = EMBRYO;
  p->pid = nextpid++;
80103895:	a1 04 b0 10 80       	mov    0x8010b004,%eax

  release(&ptable.lock);
8010389a:	83 ec 0c             	sub    $0xc,%esp
  p->state = EMBRYO;
8010389d:	c7 43 0c 01 00 00 00 	movl   $0x1,0xc(%ebx)
  p->pid = nextpid++;
801038a4:	89 43 10             	mov    %eax,0x10(%ebx)
801038a7:	8d 50 01             	lea    0x1(%eax),%edx
  release(&ptable.lock);
801038aa:	68 20 2d 11 80       	push   $0x80112d20
  p->pid = nextpid++;
801038af:	89 15 04 b0 10 80    	mov    %edx,0x8010b004
  release(&ptable.lock);
801038b5:	e8 c6 18 00 00       	call   80105180 <release>

  // Allocate kernel stack.
  if((p->kstack = kalloc()) == 0){
801038ba:	e8 71 ee ff ff       	call   80102730 <kalloc>
801038bf:	83 c4 10             	add    $0x10,%esp
801038c2:	89 43 08             	mov    %eax,0x8(%ebx)
801038c5:	85 c0                	test   %eax,%eax
801038c7:	74 58                	je     80103921 <allocproc+0xc1>
    return 0;
  }
  sp = p->kstack + KSTACKSIZE;

  // Leave room for trap frame.
  sp -= sizeof *p->tf;
801038c9:	8d 90 b4 0f 00 00    	lea    0xfb4(%eax),%edx
  sp -= 4;
  *(uint*)sp = (uint)trapret;

  sp -= sizeof *p->context;
  p->context = (struct context*)sp;
  memset(p->context, 0, sizeof *p->context);
801038cf:	83 ec 04             	sub    $0x4,%esp
  sp -= sizeof *p->context;
801038d2:	05 9c 0f 00 00       	add    $0xf9c,%eax
  sp -= sizeof *p->tf;
801038d7:	89 53 18             	mov    %edx,0x18(%ebx)
  *(uint*)sp = (uint)trapret;
801038da:	c7 40 14 a7 64 10 80 	movl   $0x801064a7,0x14(%eax)
  p->context = (struct context*)sp;
801038e1:	89 43 1c             	mov    %eax,0x1c(%ebx)
  memset(p->context, 0, sizeof *p->context);
801038e4:	6a 14                	push   $0x14
801038e6:	6a 00                	push   $0x0
801038e8:	50                   	push   %eax
801038e9:	e8 b2 19 00 00       	call   801052a0 <memset>
  p->context->eip = (uint)forkret;
801038ee:	8b 43 1c             	mov    0x1c(%ebx),%eax

  return p;
801038f1:	83 c4 10             	add    $0x10,%esp
  p->context->eip = (uint)forkret;
801038f4:	c7 40 10 40 39 10 80 	movl   $0x80103940,0x10(%eax)
}
801038fb:	89 d8                	mov    %ebx,%eax
801038fd:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80103900:	c9                   	leave  
80103901:	c3                   	ret    
80103902:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  release(&ptable.lock);
80103908:	83 ec 0c             	sub    $0xc,%esp
  return 0;
8010390b:	31 db                	xor    %ebx,%ebx
  release(&ptable.lock);
8010390d:	68 20 2d 11 80       	push   $0x80112d20
80103912:	e8 69 18 00 00       	call   80105180 <release>
}
80103917:	89 d8                	mov    %ebx,%eax
  return 0;
80103919:	83 c4 10             	add    $0x10,%esp
}
8010391c:	8b 5d fc             	mov    -0x4(%ebp),%ebx
8010391f:	c9                   	leave  
80103920:	c3                   	ret    
    p->state = UNUSED;
80103921:	c7 43 0c 00 00 00 00 	movl   $0x0,0xc(%ebx)
    return 0;
80103928:	31 db                	xor    %ebx,%ebx
}
8010392a:	89 d8                	mov    %ebx,%eax
8010392c:	8b 5d fc             	mov    -0x4(%ebp),%ebx
8010392f:	c9                   	leave  
80103930:	c3                   	ret    
80103931:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80103938:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010393f:	90                   	nop

80103940 <forkret>:

// A fork child's very first scheduling by scheduler()
// will swtch here.  "Return" to user space.
void
forkret(void)
{
80103940:	55                   	push   %ebp
80103941:	89 e5                	mov    %esp,%ebp
80103943:	83 ec 14             	sub    $0x14,%esp
  static int first = 1;
  // Still holding ptable.lock from scheduler.
  release(&ptable.lock);
80103946:	68 20 2d 11 80       	push   $0x80112d20
8010394b:	e8 30 18 00 00       	call   80105180 <release>

  if (first) {
80103950:	a1 00 b0 10 80       	mov    0x8010b000,%eax
80103955:	83 c4 10             	add    $0x10,%esp
80103958:	85 c0                	test   %eax,%eax
8010395a:	75 04                	jne    80103960 <forkret+0x20>
    iinit(ROOTDEV);
    initlog(ROOTDEV);
  }

  // Return to "caller", actually trapret (see allocproc).
}
8010395c:	c9                   	leave  
8010395d:	c3                   	ret    
8010395e:	66 90                	xchg   %ax,%ax
    first = 0;
80103960:	c7 05 00 b0 10 80 00 	movl   $0x0,0x8010b000
80103967:	00 00 00 
    iinit(ROOTDEV);
8010396a:	83 ec 0c             	sub    $0xc,%esp
8010396d:	6a 01                	push   $0x1
8010396f:	e8 9c dc ff ff       	call   80101610 <iinit>
    initlog(ROOTDEV);
80103974:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
8010397b:	e8 f0 f3 ff ff       	call   80102d70 <initlog>
}
80103980:	83 c4 10             	add    $0x10,%esp
80103983:	c9                   	leave  
80103984:	c3                   	ret    
80103985:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010398c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80103990 <change_num>:
change_num(struct proc* p, int inc_or_dec){
80103990:	55                   	push   %ebp
80103991:	89 e5                	mov    %esp,%ebp
  if(p->queue==CLASS1){
80103993:	8b 45 08             	mov    0x8(%ebp),%eax
change_num(struct proc* p, int inc_or_dec){
80103996:	8b 55 0c             	mov    0xc(%ebp),%edx
  if(p->queue==CLASS1){
80103999:	8b 40 7c             	mov    0x7c(%eax),%eax
8010399c:	85 c0                	test   %eax,%eax
8010399e:	75 10                	jne    801039b0 <change_num+0x20>
    ptable.class1_num = ptable.class1_num + inc_or_dec;
801039a0:	01 15 54 5b 11 80    	add    %edx,0x80115b54
}
801039a6:	5d                   	pop    %ebp
801039a7:	c3                   	ret    
801039a8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801039af:	90                   	nop
    if(p->queue==CLASS2_RR){
801039b0:	83 f8 01             	cmp    $0x1,%eax
801039b3:	74 0b                	je     801039c0 <change_num+0x30>
      ptable.fcfs_num = ptable.fcfs_num + inc_or_dec;
801039b5:	01 15 5c 5b 11 80    	add    %edx,0x80115b5c
}
801039bb:	5d                   	pop    %ebp
801039bc:	c3                   	ret    
801039bd:	8d 76 00             	lea    0x0(%esi),%esi
      ptable.rr_num = ptable.rr_num + inc_or_dec;
801039c0:	01 15 58 5b 11 80    	add    %edx,0x80115b58
}
801039c6:	5d                   	pop    %ebp
801039c7:	c3                   	ret    
801039c8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801039cf:	90                   	nop

801039d0 <pinit>:
{
801039d0:	55                   	push   %ebp
801039d1:	89 e5                	mov    %esp,%ebp
801039d3:	83 ec 10             	sub    $0x10,%esp
  initlock(&ptable.lock, "ptable");
801039d6:	68 80 83 10 80       	push   $0x80108380
801039db:	68 20 2d 11 80       	push   $0x80112d20
801039e0:	e8 2b 16 00 00       	call   80105010 <initlock>
}
801039e5:	83 c4 10             	add    $0x10,%esp
801039e8:	c9                   	leave  
801039e9:	c3                   	ret    
801039ea:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

801039f0 <mycpu>:
{
801039f0:	55                   	push   %ebp
801039f1:	89 e5                	mov    %esp,%ebp
801039f3:	56                   	push   %esi
801039f4:	53                   	push   %ebx
  asm volatile("pushfl; popl %0" : "=r" (eflags));
801039f5:	9c                   	pushf  
801039f6:	58                   	pop    %eax
  if(readeflags()&FL_IF)
801039f7:	f6 c4 02             	test   $0x2,%ah
801039fa:	75 46                	jne    80103a42 <mycpu+0x52>
  apicid = lapicid();
801039fc:	e8 9f ef ff ff       	call   801029a0 <lapicid>
  for (i = 0; i < ncpu; ++i) {
80103a01:	8b 35 84 27 11 80    	mov    0x80112784,%esi
80103a07:	85 f6                	test   %esi,%esi
80103a09:	7e 2a                	jle    80103a35 <mycpu+0x45>
80103a0b:	31 d2                	xor    %edx,%edx
80103a0d:	eb 08                	jmp    80103a17 <mycpu+0x27>
80103a0f:	90                   	nop
80103a10:	83 c2 01             	add    $0x1,%edx
80103a13:	39 f2                	cmp    %esi,%edx
80103a15:	74 1e                	je     80103a35 <mycpu+0x45>
    if (cpus[i].apicid == apicid)
80103a17:	69 ca b0 00 00 00    	imul   $0xb0,%edx,%ecx
80103a1d:	0f b6 99 a0 27 11 80 	movzbl -0x7feed860(%ecx),%ebx
80103a24:	39 c3                	cmp    %eax,%ebx
80103a26:	75 e8                	jne    80103a10 <mycpu+0x20>
}
80103a28:	8d 65 f8             	lea    -0x8(%ebp),%esp
      return &cpus[i];
80103a2b:	8d 81 a0 27 11 80    	lea    -0x7feed860(%ecx),%eax
}
80103a31:	5b                   	pop    %ebx
80103a32:	5e                   	pop    %esi
80103a33:	5d                   	pop    %ebp
80103a34:	c3                   	ret    
  panic("unknown apicid\n");
80103a35:	83 ec 0c             	sub    $0xc,%esp
80103a38:	68 87 83 10 80       	push   $0x80108387
80103a3d:	e8 3e c9 ff ff       	call   80100380 <panic>
    panic("mycpu called with interrupts enabled\n");
80103a42:	83 ec 0c             	sub    $0xc,%esp
80103a45:	68 e8 84 10 80       	push   $0x801084e8
80103a4a:	e8 31 c9 ff ff       	call   80100380 <panic>
80103a4f:	90                   	nop

80103a50 <cpuid>:
cpuid() {
80103a50:	55                   	push   %ebp
80103a51:	89 e5                	mov    %esp,%ebp
80103a53:	83 ec 08             	sub    $0x8,%esp
  return mycpu()-cpus;
80103a56:	e8 95 ff ff ff       	call   801039f0 <mycpu>
}
80103a5b:	c9                   	leave  
  return mycpu()-cpus;
80103a5c:	2d a0 27 11 80       	sub    $0x801127a0,%eax
80103a61:	c1 f8 04             	sar    $0x4,%eax
80103a64:	69 c0 a3 8b 2e ba    	imul   $0xba2e8ba3,%eax,%eax
}
80103a6a:	c3                   	ret    
80103a6b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80103a6f:	90                   	nop

80103a70 <myproc>:
myproc(void) {
80103a70:	55                   	push   %ebp
80103a71:	89 e5                	mov    %esp,%ebp
80103a73:	53                   	push   %ebx
80103a74:	83 ec 04             	sub    $0x4,%esp
  pushcli();
80103a77:	e8 14 16 00 00       	call   80105090 <pushcli>
  c = mycpu();
80103a7c:	e8 6f ff ff ff       	call   801039f0 <mycpu>
  p = c->proc;
80103a81:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
  popcli();
80103a87:	e8 54 16 00 00       	call   801050e0 <popcli>
}
80103a8c:	89 d8                	mov    %ebx,%eax
80103a8e:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80103a91:	c9                   	leave  
80103a92:	c3                   	ret    
80103a93:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80103a9a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80103aa0 <userinit>:
{
80103aa0:	55                   	push   %ebp
80103aa1:	89 e5                	mov    %esp,%ebp
80103aa3:	53                   	push   %ebx
80103aa4:	83 ec 04             	sub    $0x4,%esp
  p = allocproc();
80103aa7:	e8 b4 fd ff ff       	call   80103860 <allocproc>
80103aac:	89 c3                	mov    %eax,%ebx
  initproc = p;
80103aae:	a3 60 5b 11 80       	mov    %eax,0x80115b60
  if((p->pgdir = setupkvm()) == 0)
80103ab3:	e8 e8 3f 00 00       	call   80107aa0 <setupkvm>
80103ab8:	89 43 04             	mov    %eax,0x4(%ebx)
80103abb:	85 c0                	test   %eax,%eax
80103abd:	0f 84 ea 00 00 00    	je     80103bad <userinit+0x10d>
  inituvm(p->pgdir, _binary_initcode_start, (int)_binary_initcode_size);
80103ac3:	83 ec 04             	sub    $0x4,%esp
80103ac6:	68 2c 00 00 00       	push   $0x2c
80103acb:	68 60 b4 10 80       	push   $0x8010b460
80103ad0:	50                   	push   %eax
80103ad1:	e8 7a 3c 00 00       	call   80107750 <inituvm>
  memset(p->tf, 0, sizeof(*p->tf));
80103ad6:	83 c4 0c             	add    $0xc,%esp
  p->sz = PGSIZE;
80103ad9:	c7 03 00 10 00 00    	movl   $0x1000,(%ebx)
  memset(p->tf, 0, sizeof(*p->tf));
80103adf:	6a 4c                	push   $0x4c
80103ae1:	6a 00                	push   $0x0
80103ae3:	ff 73 18             	push   0x18(%ebx)
80103ae6:	e8 b5 17 00 00       	call   801052a0 <memset>
  p->tf->cs = (SEG_UCODE << 3) | DPL_USER;
80103aeb:	8b 43 18             	mov    0x18(%ebx),%eax
80103aee:	ba 1b 00 00 00       	mov    $0x1b,%edx
  safestrcpy(p->name, "initcode", sizeof(p->name));
80103af3:	83 c4 0c             	add    $0xc,%esp
  p->tf->ds = (SEG_UDATA << 3) | DPL_USER;
80103af6:	b9 23 00 00 00       	mov    $0x23,%ecx
  p->tf->cs = (SEG_UCODE << 3) | DPL_USER;
80103afb:	66 89 50 3c          	mov    %dx,0x3c(%eax)
  p->tf->ds = (SEG_UDATA << 3) | DPL_USER;
80103aff:	8b 43 18             	mov    0x18(%ebx),%eax
80103b02:	66 89 48 2c          	mov    %cx,0x2c(%eax)
  p->tf->es = p->tf->ds;
80103b06:	8b 43 18             	mov    0x18(%ebx),%eax
80103b09:	0f b7 50 2c          	movzwl 0x2c(%eax),%edx
80103b0d:	66 89 50 28          	mov    %dx,0x28(%eax)
  p->tf->ss = p->tf->ds;
80103b11:	8b 43 18             	mov    0x18(%ebx),%eax
80103b14:	0f b7 50 2c          	movzwl 0x2c(%eax),%edx
80103b18:	66 89 50 48          	mov    %dx,0x48(%eax)
  p->tf->eflags = FL_IF;
80103b1c:	8b 43 18             	mov    0x18(%ebx),%eax
80103b1f:	c7 40 40 00 02 00 00 	movl   $0x200,0x40(%eax)
  p->tf->esp = PGSIZE;
80103b26:	8b 43 18             	mov    0x18(%ebx),%eax
80103b29:	c7 40 44 00 10 00 00 	movl   $0x1000,0x44(%eax)
  p->tf->eip = 0;  // beginning of initcode.S
80103b30:	8b 43 18             	mov    0x18(%ebx),%eax
80103b33:	c7 40 38 00 00 00 00 	movl   $0x0,0x38(%eax)
  safestrcpy(p->name, "initcode", sizeof(p->name));
80103b3a:	8d 43 6c             	lea    0x6c(%ebx),%eax
80103b3d:	6a 10                	push   $0x10
80103b3f:	68 b0 83 10 80       	push   $0x801083b0
80103b44:	50                   	push   %eax
80103b45:	e8 16 19 00 00       	call   80105460 <safestrcpy>
  p->cwd = namei("/");
80103b4a:	c7 04 24 b9 83 10 80 	movl   $0x801083b9,(%esp)
80103b51:	e8 fa e5 ff ff       	call   80102150 <namei>
80103b56:	89 43 68             	mov    %eax,0x68(%ebx)
  acquire(&ptable.lock);
80103b59:	c7 04 24 20 2d 11 80 	movl   $0x80112d20,(%esp)
80103b60:	e8 7b 16 00 00       	call   801051e0 <acquire>
  p->arrival_time = ticks;
80103b65:	a1 80 5b 11 80       	mov    0x80115b80,%eax
  p->state = RUNNABLE;
80103b6a:	c7 43 0c 03 00 00 00 	movl   $0x3,0xc(%ebx)
  p->deadline = 0;
80103b71:	c7 83 84 00 00 00 00 	movl   $0x0,0x84(%ebx)
80103b78:	00 00 00 
  p->arrival_time = ticks;
80103b7b:	89 83 80 00 00 00    	mov    %eax,0x80(%ebx)
  p->queue = CLASS2_RR;
80103b81:	c7 43 7c 01 00 00 00 	movl   $0x1,0x7c(%ebx)
  p->age = 0;
80103b88:	c7 83 88 00 00 00 00 	movl   $0x0,0x88(%ebx)
80103b8f:	00 00 00 
  release(&ptable.lock);
80103b92:	c7 04 24 20 2d 11 80 	movl   $0x80112d20,(%esp)
      ptable.rr_num = ptable.rr_num + inc_or_dec;
80103b99:	83 05 58 5b 11 80 01 	addl   $0x1,0x80115b58
  release(&ptable.lock);
80103ba0:	e8 db 15 00 00       	call   80105180 <release>
}
80103ba5:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80103ba8:	83 c4 10             	add    $0x10,%esp
80103bab:	c9                   	leave  
80103bac:	c3                   	ret    
    panic("userinit: out of memory?");
80103bad:	83 ec 0c             	sub    $0xc,%esp
80103bb0:	68 97 83 10 80       	push   $0x80108397
80103bb5:	e8 c6 c7 ff ff       	call   80100380 <panic>
80103bba:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80103bc0 <growproc>:
{
80103bc0:	55                   	push   %ebp
80103bc1:	89 e5                	mov    %esp,%ebp
80103bc3:	56                   	push   %esi
80103bc4:	53                   	push   %ebx
80103bc5:	8b 75 08             	mov    0x8(%ebp),%esi
  pushcli();
80103bc8:	e8 c3 14 00 00       	call   80105090 <pushcli>
  c = mycpu();
80103bcd:	e8 1e fe ff ff       	call   801039f0 <mycpu>
  p = c->proc;
80103bd2:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
  popcli();
80103bd8:	e8 03 15 00 00       	call   801050e0 <popcli>
  sz = curproc->sz;
80103bdd:	8b 03                	mov    (%ebx),%eax
  if(n > 0){
80103bdf:	85 f6                	test   %esi,%esi
80103be1:	7f 1d                	jg     80103c00 <growproc+0x40>
  } else if(n < 0){
80103be3:	75 3b                	jne    80103c20 <growproc+0x60>
  switchuvm(curproc);
80103be5:	83 ec 0c             	sub    $0xc,%esp
  curproc->sz = sz;
80103be8:	89 03                	mov    %eax,(%ebx)
  switchuvm(curproc);
80103bea:	53                   	push   %ebx
80103beb:	e8 50 3a 00 00       	call   80107640 <switchuvm>
  return 0;
80103bf0:	83 c4 10             	add    $0x10,%esp
80103bf3:	31 c0                	xor    %eax,%eax
}
80103bf5:	8d 65 f8             	lea    -0x8(%ebp),%esp
80103bf8:	5b                   	pop    %ebx
80103bf9:	5e                   	pop    %esi
80103bfa:	5d                   	pop    %ebp
80103bfb:	c3                   	ret    
80103bfc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    if((sz = allocuvm(curproc->pgdir, sz, sz + n)) == 0)
80103c00:	83 ec 04             	sub    $0x4,%esp
80103c03:	01 c6                	add    %eax,%esi
80103c05:	56                   	push   %esi
80103c06:	50                   	push   %eax
80103c07:	ff 73 04             	push   0x4(%ebx)
80103c0a:	e8 b1 3c 00 00       	call   801078c0 <allocuvm>
80103c0f:	83 c4 10             	add    $0x10,%esp
80103c12:	85 c0                	test   %eax,%eax
80103c14:	75 cf                	jne    80103be5 <growproc+0x25>
      return -1;
80103c16:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80103c1b:	eb d8                	jmp    80103bf5 <growproc+0x35>
80103c1d:	8d 76 00             	lea    0x0(%esi),%esi
    if((sz = deallocuvm(curproc->pgdir, sz, sz + n)) == 0)
80103c20:	83 ec 04             	sub    $0x4,%esp
80103c23:	01 c6                	add    %eax,%esi
80103c25:	56                   	push   %esi
80103c26:	50                   	push   %eax
80103c27:	ff 73 04             	push   0x4(%ebx)
80103c2a:	e8 c1 3d 00 00       	call   801079f0 <deallocuvm>
80103c2f:	83 c4 10             	add    $0x10,%esp
80103c32:	85 c0                	test   %eax,%eax
80103c34:	75 af                	jne    80103be5 <growproc+0x25>
80103c36:	eb de                	jmp    80103c16 <growproc+0x56>
80103c38:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80103c3f:	90                   	nop

80103c40 <fork>:
{
80103c40:	55                   	push   %ebp
80103c41:	89 e5                	mov    %esp,%ebp
80103c43:	57                   	push   %edi
80103c44:	56                   	push   %esi
80103c45:	53                   	push   %ebx
80103c46:	83 ec 1c             	sub    $0x1c,%esp
  pushcli();
80103c49:	e8 42 14 00 00       	call   80105090 <pushcli>
  c = mycpu();
80103c4e:	e8 9d fd ff ff       	call   801039f0 <mycpu>
  p = c->proc;
80103c53:	8b 90 ac 00 00 00    	mov    0xac(%eax),%edx
80103c59:	89 55 e4             	mov    %edx,-0x1c(%ebp)
  popcli();
80103c5c:	e8 7f 14 00 00       	call   801050e0 <popcli>
  if((np = allocproc()) == 0){
80103c61:	e8 fa fb ff ff       	call   80103860 <allocproc>
80103c66:	85 c0                	test   %eax,%eax
80103c68:	0f 84 3f 01 00 00    	je     80103dad <fork+0x16d>
  if((np->pgdir = copyuvm(curproc->pgdir, curproc->sz)) == 0){
80103c6e:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80103c71:	83 ec 08             	sub    $0x8,%esp
80103c74:	89 c3                	mov    %eax,%ebx
80103c76:	ff 32                	push   (%edx)
80103c78:	ff 72 04             	push   0x4(%edx)
80103c7b:	e8 10 3f 00 00       	call   80107b90 <copyuvm>
80103c80:	83 c4 10             	add    $0x10,%esp
80103c83:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80103c86:	85 c0                	test   %eax,%eax
80103c88:	89 43 04             	mov    %eax,0x4(%ebx)
80103c8b:	0f 84 23 01 00 00    	je     80103db4 <fork+0x174>
  np->sz = curproc->sz;
80103c91:	8b 02                	mov    (%edx),%eax
  *np->tf = *curproc->tf;
80103c93:	8b 7b 18             	mov    0x18(%ebx),%edi
  np->parent = curproc;
80103c96:	89 53 14             	mov    %edx,0x14(%ebx)
  *np->tf = *curproc->tf;
80103c99:	b9 13 00 00 00       	mov    $0x13,%ecx
  np->sz = curproc->sz;
80103c9e:	89 03                	mov    %eax,(%ebx)
  *np->tf = *curproc->tf;
80103ca0:	8b 72 18             	mov    0x18(%edx),%esi
80103ca3:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)
  for(i = 0; i < NOFILE; i++)
80103ca5:	31 f6                	xor    %esi,%esi
  np->tf->eax = 0;
80103ca7:	8b 43 18             	mov    0x18(%ebx),%eax
80103caa:	c7 40 1c 00 00 00 00 	movl   $0x0,0x1c(%eax)
  for(i = 0; i < NOFILE; i++)
80103cb1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    if(curproc->ofile[i])
80103cb8:	8b 44 b2 28          	mov    0x28(%edx,%esi,4),%eax
80103cbc:	85 c0                	test   %eax,%eax
80103cbe:	74 16                	je     80103cd6 <fork+0x96>
      np->ofile[i] = filedup(curproc->ofile[i]);
80103cc0:	83 ec 0c             	sub    $0xc,%esp
80103cc3:	89 55 e4             	mov    %edx,-0x1c(%ebp)
80103cc6:	50                   	push   %eax
80103cc7:	e8 84 d2 ff ff       	call   80100f50 <filedup>
80103ccc:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80103ccf:	83 c4 10             	add    $0x10,%esp
80103cd2:	89 44 b3 28          	mov    %eax,0x28(%ebx,%esi,4)
  for(i = 0; i < NOFILE; i++)
80103cd6:	83 c6 01             	add    $0x1,%esi
80103cd9:	83 fe 10             	cmp    $0x10,%esi
80103cdc:	75 da                	jne    80103cb8 <fork+0x78>
  np->cwd = idup(curproc->cwd);
80103cde:	83 ec 0c             	sub    $0xc,%esp
80103ce1:	ff 72 68             	push   0x68(%edx)
  safestrcpy(np->name, curproc->name, sizeof(curproc->name));
80103ce4:	8d 73 6c             	lea    0x6c(%ebx),%esi
  np->cwd = idup(curproc->cwd);
80103ce7:	89 55 e4             	mov    %edx,-0x1c(%ebp)
80103cea:	e8 11 db ff ff       	call   80101800 <idup>
  safestrcpy(np->name, curproc->name, sizeof(curproc->name));
80103cef:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80103cf2:	83 c4 0c             	add    $0xc,%esp
  np->cwd = idup(curproc->cwd);
80103cf5:	89 43 68             	mov    %eax,0x68(%ebx)
  safestrcpy(np->name, curproc->name, sizeof(curproc->name));
80103cf8:	83 c2 6c             	add    $0x6c,%edx
80103cfb:	6a 10                	push   $0x10
80103cfd:	52                   	push   %edx
80103cfe:	56                   	push   %esi
80103cff:	e8 5c 17 00 00       	call   80105460 <safestrcpy>
  pid = np->pid;
80103d04:	8b 7b 10             	mov    0x10(%ebx),%edi
  acquire(&ptable.lock);
80103d07:	c7 04 24 20 2d 11 80 	movl   $0x80112d20,(%esp)
80103d0e:	e8 cd 14 00 00       	call   801051e0 <acquire>
  np->arrival_time = ticks;
80103d13:	a1 80 5b 11 80       	mov    0x80115b80,%eax
  if (strncmp(np->name,"sh",3)||strncmp(np->parent->name,"sh",3)){
80103d18:	83 c4 0c             	add    $0xc,%esp
  np->state = RUNNABLE;
80103d1b:	c7 43 0c 03 00 00 00 	movl   $0x3,0xc(%ebx)
  np->deadline = 0;
80103d22:	c7 83 84 00 00 00 00 	movl   $0x0,0x84(%ebx)
80103d29:	00 00 00 
  np->arrival_time = ticks;
80103d2c:	89 83 80 00 00 00    	mov    %eax,0x80(%ebx)
  np->age=0;
80103d32:	c7 83 88 00 00 00 00 	movl   $0x0,0x88(%ebx)
80103d39:	00 00 00 
  np->queue = CLASS2_FCFS;
80103d3c:	c7 43 7c 02 00 00 00 	movl   $0x2,0x7c(%ebx)
  if (strncmp(np->name,"sh",3)||strncmp(np->parent->name,"sh",3)){
80103d43:	6a 03                	push   $0x3
80103d45:	68 8d 7e 10 80       	push   $0x80107e8d
80103d4a:	56                   	push   %esi
80103d4b:	e8 60 16 00 00       	call   801053b0 <strncmp>
80103d50:	83 c4 10             	add    $0x10,%esp
80103d53:	85 c0                	test   %eax,%eax
80103d55:	74 29                	je     80103d80 <fork+0x140>
80103d57:	c7 43 7c 01 00 00 00 	movl   $0x1,0x7c(%ebx)
      ptable.rr_num = ptable.rr_num + inc_or_dec;
80103d5e:	83 05 58 5b 11 80 01 	addl   $0x1,0x80115b58
  release(&ptable.lock);
80103d65:	83 ec 0c             	sub    $0xc,%esp
80103d68:	68 20 2d 11 80       	push   $0x80112d20
80103d6d:	e8 0e 14 00 00       	call   80105180 <release>
  return pid;
80103d72:	83 c4 10             	add    $0x10,%esp
}
80103d75:	8d 65 f4             	lea    -0xc(%ebp),%esp
80103d78:	89 f8                	mov    %edi,%eax
80103d7a:	5b                   	pop    %ebx
80103d7b:	5e                   	pop    %esi
80103d7c:	5f                   	pop    %edi
80103d7d:	5d                   	pop    %ebp
80103d7e:	c3                   	ret    
80103d7f:	90                   	nop
  if (strncmp(np->name,"sh",3)||strncmp(np->parent->name,"sh",3)){
80103d80:	83 ec 04             	sub    $0x4,%esp
80103d83:	6a 03                	push   $0x3
80103d85:	68 8d 7e 10 80       	push   $0x80107e8d
80103d8a:	8b 43 14             	mov    0x14(%ebx),%eax
80103d8d:	83 c0 6c             	add    $0x6c,%eax
80103d90:	50                   	push   %eax
80103d91:	e8 1a 16 00 00       	call   801053b0 <strncmp>
80103d96:	83 c4 10             	add    $0x10,%esp
80103d99:	85 c0                	test   %eax,%eax
80103d9b:	75 ba                	jne    80103d57 <fork+0x117>
80103d9d:	c7 43 7c 02 00 00 00 	movl   $0x2,0x7c(%ebx)
      ptable.fcfs_num = ptable.fcfs_num + inc_or_dec;
80103da4:	83 05 5c 5b 11 80 01 	addl   $0x1,0x80115b5c
80103dab:	eb b8                	jmp    80103d65 <fork+0x125>
    return -1;
80103dad:	bf ff ff ff ff       	mov    $0xffffffff,%edi
80103db2:	eb c1                	jmp    80103d75 <fork+0x135>
    kfree(np->kstack);
80103db4:	83 ec 0c             	sub    $0xc,%esp
80103db7:	ff 73 08             	push   0x8(%ebx)
    return -1;
80103dba:	bf ff ff ff ff       	mov    $0xffffffff,%edi
    kfree(np->kstack);
80103dbf:	e8 ac e7 ff ff       	call   80102570 <kfree>
    np->kstack = 0;
80103dc4:	c7 43 08 00 00 00 00 	movl   $0x0,0x8(%ebx)
    return -1;
80103dcb:	83 c4 10             	add    $0x10,%esp
    np->state = UNUSED;
80103dce:	c7 43 0c 00 00 00 00 	movl   $0x0,0xc(%ebx)
    return -1;
80103dd5:	eb 9e                	jmp    80103d75 <fork+0x135>
80103dd7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80103dde:	66 90                	xchg   %ax,%ax

80103de0 <scheduler>:
{
80103de0:	55                   	push   %ebp
80103de1:	89 e5                	mov    %esp,%ebp
80103de3:	57                   	push   %edi
80103de4:	56                   	push   %esi
80103de5:	53                   	push   %ebx
80103de6:	83 ec 1c             	sub    $0x1c,%esp
  struct cpu *c = mycpu();
80103de9:	e8 02 fc ff ff       	call   801039f0 <mycpu>
  c->proc = 0;
80103dee:	c7 80 ac 00 00 00 00 	movl   $0x0,0xac(%eax)
80103df5:	00 00 00 
  struct cpu *c = mycpu();
80103df8:	89 c6                	mov    %eax,%esi
  c->proc = 0;
80103dfa:	8d 40 04             	lea    0x4(%eax),%eax
80103dfd:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  asm volatile("sti");
80103e00:	fb                   	sti    
    acquire(&ptable.lock);
80103e01:	83 ec 0c             	sub    $0xc,%esp
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80103e04:	bb 54 2d 11 80       	mov    $0x80112d54,%ebx
    acquire(&ptable.lock);
80103e09:	68 20 2d 11 80       	push   $0x80112d20
80103e0e:	e8 cd 13 00 00       	call   801051e0 <acquire>
80103e13:	83 c4 10             	add    $0x10,%esp
80103e16:	eb 16                	jmp    80103e2e <scheduler+0x4e>
80103e18:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80103e1f:	90                   	nop
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80103e20:	81 c3 b8 00 00 00    	add    $0xb8,%ebx
80103e26:	81 fb 54 5b 11 80    	cmp    $0x80115b54,%ebx
80103e2c:	74 5a                	je     80103e88 <scheduler+0xa8>
        if(p->state==RUNNABLE && p->queue==CLASS2_FCFS){
80103e2e:	83 7b 0c 03          	cmpl   $0x3,0xc(%ebx)
80103e32:	75 ec                	jne    80103e20 <scheduler+0x40>
80103e34:	83 7b 7c 02          	cmpl   $0x2,0x7c(%ebx)
80103e38:	75 e6                	jne    80103e20 <scheduler+0x40>
          p->age++;    
80103e3a:	8b 83 88 00 00 00    	mov    0x88(%ebx),%eax
80103e40:	83 c0 01             	add    $0x1,%eax
80103e43:	89 83 88 00 00 00    	mov    %eax,0x88(%ebx)
          if(p->age>=800){
80103e49:	3d 1f 03 00 00       	cmp    $0x31f,%eax
80103e4e:	76 d0                	jbe    80103e20 <scheduler+0x40>
            cprintf("PID %d: got increased priority to RR due to growing old!\n",p->pid);
80103e50:	83 ec 08             	sub    $0x8,%esp
            p->queue = CLASS2_RR ;
80103e53:	c7 43 7c 01 00 00 00 	movl   $0x1,0x7c(%ebx)
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80103e5a:	81 c3 b8 00 00 00    	add    $0xb8,%ebx
            p->age = 0;
80103e60:	c7 43 d0 00 00 00 00 	movl   $0x0,-0x30(%ebx)
            cprintf("PID %d: got increased priority to RR due to growing old!\n",p->pid);
80103e67:	ff b3 58 ff ff ff    	push   -0xa8(%ebx)
80103e6d:	68 10 85 10 80       	push   $0x80108510
80103e72:	e8 29 c8 ff ff       	call   801006a0 <cprintf>
80103e77:	83 c4 10             	add    $0x10,%esp
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80103e7a:	81 fb 54 5b 11 80    	cmp    $0x80115b54,%ebx
80103e80:	75 ac                	jne    80103e2e <scheduler+0x4e>
80103e82:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    if(ptable.class1_num>0){
80103e88:	8b 0d 54 5b 11 80    	mov    0x80115b54,%ecx
80103e8e:	85 c9                	test   %ecx,%ecx
80103e90:	7f 33                	jg     80103ec5 <scheduler+0xe5>
      if(!runable_p && ptable.rr_num>0){
80103e92:	8b 1d 58 5b 11 80    	mov    0x80115b58,%ebx
80103e98:	85 db                	test   %ebx,%ebx
80103e9a:	0f 8f e7 00 00 00    	jg     80103f87 <scheduler+0x1a7>
      if(!runable_p && ptable.fcfs_num>0){
80103ea0:	8b 0d 5c 5b 11 80    	mov    0x80115b5c,%ecx
80103ea6:	85 c9                	test   %ecx,%ecx
80103ea8:	0f 8f 11 01 00 00    	jg     80103fbf <scheduler+0x1df>
80103eae:	66 90                	xchg   %ax,%ax
    release(&ptable.lock);
80103eb0:	83 ec 0c             	sub    $0xc,%esp
80103eb3:	68 20 2d 11 80       	push   $0x80112d20
80103eb8:	e8 c3 12 00 00       	call   80105180 <release>
  for(;;){
80103ebd:	83 c4 10             	add    $0x10,%esp
80103ec0:	e9 3b ff ff ff       	jmp    80103e00 <scheduler+0x20>
          int argMin = ticks - p->deadline ;
80103ec5:	8b 3d 80 5b 11 80    	mov    0x80115b80,%edi
      int min_dead = 1000000;
80103ecb:	b9 40 42 0f 00       	mov    $0xf4240,%ecx
      struct proc *runable_p = 0;
80103ed0:	31 db                	xor    %ebx,%ebx
      for(p=ptable.proc; p <&ptable.proc[NPROC]; p++){
80103ed2:	b8 54 2d 11 80       	mov    $0x80112d54,%eax
80103ed7:	eb 13                	jmp    80103eec <scheduler+0x10c>
80103ed9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80103ee0:	05 b8 00 00 00       	add    $0xb8,%eax
80103ee5:	3d 54 5b 11 80       	cmp    $0x80115b54,%eax
80103eea:	74 2d                	je     80103f19 <scheduler+0x139>
        if(p->state==RUNNABLE && p->queue==CLASS1){
80103eec:	83 78 0c 03          	cmpl   $0x3,0xc(%eax)
80103ef0:	75 ee                	jne    80103ee0 <scheduler+0x100>
80103ef2:	8b 50 7c             	mov    0x7c(%eax),%edx
80103ef5:	85 d2                	test   %edx,%edx
80103ef7:	75 e7                	jne    80103ee0 <scheduler+0x100>
          int argMin = ticks - p->deadline ;
80103ef9:	89 fa                	mov    %edi,%edx
80103efb:	2b 90 84 00 00 00    	sub    0x84(%eax),%edx
          if(!runable_p || argMin<min_dead){
80103f01:	85 db                	test   %ebx,%ebx
80103f03:	74 04                	je     80103f09 <scheduler+0x129>
80103f05:	39 ca                	cmp    %ecx,%edx
80103f07:	7d d7                	jge    80103ee0 <scheduler+0x100>
80103f09:	89 c3                	mov    %eax,%ebx
      for(p=ptable.proc; p <&ptable.proc[NPROC]; p++){
80103f0b:	05 b8 00 00 00       	add    $0xb8,%eax
          int argMin = ticks - p->deadline ;
80103f10:	89 d1                	mov    %edx,%ecx
      for(p=ptable.proc; p <&ptable.proc[NPROC]; p++){
80103f12:	3d 54 5b 11 80       	cmp    $0x80115b54,%eax
80103f17:	75 d3                	jne    80103eec <scheduler+0x10c>
      if(!runable_p && ptable.rr_num>0){
80103f19:	85 db                	test   %ebx,%ebx
80103f1b:	0f 84 71 ff ff ff    	je     80103e92 <scheduler+0xb2>
        c->proc = runable_p;
80103f21:	89 9e ac 00 00 00    	mov    %ebx,0xac(%esi)
  if(p->queue==CLASS1){
80103f27:	8b 43 7c             	mov    0x7c(%ebx),%eax
80103f2a:	85 c0                	test   %eax,%eax
80103f2c:	75 4b                	jne    80103f79 <scheduler+0x199>
    ptable.class1_num = ptable.class1_num + inc_or_dec;
80103f2e:	83 2d 54 5b 11 80 01 	subl   $0x1,0x80115b54
        if(runable_p->queue==CLASS2_FCFS){
80103f35:	83 7b 7c 02          	cmpl   $0x2,0x7c(%ebx)
80103f39:	75 0a                	jne    80103f45 <scheduler+0x165>
          runable_p->age = 0;
80103f3b:	c7 83 88 00 00 00 00 	movl   $0x0,0x88(%ebx)
80103f42:	00 00 00 
        switchuvm(runable_p);
80103f45:	83 ec 0c             	sub    $0xc,%esp
80103f48:	53                   	push   %ebx
80103f49:	e8 f2 36 00 00       	call   80107640 <switchuvm>
        runable_p->state = RUNNING;
80103f4e:	c7 43 0c 04 00 00 00 	movl   $0x4,0xc(%ebx)
        swtch(&(c->scheduler),runable_p->context);
80103f55:	58                   	pop    %eax
80103f56:	5a                   	pop    %edx
80103f57:	ff 73 1c             	push   0x1c(%ebx)
80103f5a:	ff 75 e4             	push   -0x1c(%ebp)
80103f5d:	e8 59 15 00 00       	call   801054bb <swtch>
      switchkvm();
80103f62:	e8 c9 36 00 00       	call   80107630 <switchkvm>
      c->proc = 0;
80103f67:	83 c4 10             	add    $0x10,%esp
80103f6a:	c7 86 ac 00 00 00 00 	movl   $0x0,0xac(%esi)
80103f71:	00 00 00 
80103f74:	e9 37 ff ff ff       	jmp    80103eb0 <scheduler+0xd0>
    if(p->queue==CLASS2_RR){
80103f79:	83 f8 01             	cmp    $0x1,%eax
80103f7c:	74 35                	je     80103fb3 <scheduler+0x1d3>
      ptable.fcfs_num = ptable.fcfs_num + inc_or_dec;
80103f7e:	83 2d 5c 5b 11 80 01 	subl   $0x1,0x80115b5c
80103f85:	eb ae                	jmp    80103f35 <scheduler+0x155>
        for(p=ptable.proc; p <&ptable.proc[NPROC]; p++){
80103f87:	bb 54 2d 11 80       	mov    $0x80112d54,%ebx
80103f8c:	eb 14                	jmp    80103fa2 <scheduler+0x1c2>
80103f8e:	66 90                	xchg   %ax,%ax
80103f90:	81 c3 b8 00 00 00    	add    $0xb8,%ebx
80103f96:	81 fb 54 5b 11 80    	cmp    $0x80115b54,%ebx
80103f9c:	0f 84 fe fe ff ff    	je     80103ea0 <scheduler+0xc0>
          if(p->state==RUNNABLE && p->queue==CLASS2_RR){
80103fa2:	83 7b 0c 03          	cmpl   $0x3,0xc(%ebx)
80103fa6:	75 e8                	jne    80103f90 <scheduler+0x1b0>
80103fa8:	83 7b 7c 01          	cmpl   $0x1,0x7c(%ebx)
80103fac:	75 e2                	jne    80103f90 <scheduler+0x1b0>
80103fae:	e9 6e ff ff ff       	jmp    80103f21 <scheduler+0x141>
      ptable.rr_num = ptable.rr_num + inc_or_dec;
80103fb3:	83 2d 58 5b 11 80 01 	subl   $0x1,0x80115b58
80103fba:	e9 76 ff ff ff       	jmp    80103f35 <scheduler+0x155>
80103fbf:	31 db                	xor    %ebx,%ebx
        for(p=ptable.proc; p <&ptable.proc[NPROC]; p++){
80103fc1:	b8 54 2d 11 80       	mov    $0x80112d54,%eax
80103fc6:	eb 14                	jmp    80103fdc <scheduler+0x1fc>
80103fc8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80103fcf:	90                   	nop
80103fd0:	05 b8 00 00 00       	add    $0xb8,%eax
80103fd5:	3d 54 5b 11 80       	cmp    $0x80115b54,%eax
80103fda:	74 2b                	je     80104007 <scheduler+0x227>
          if(p->state==RUNNABLE && p->queue==CLASS2_FCFS){
80103fdc:	83 78 0c 03          	cmpl   $0x3,0xc(%eax)
80103fe0:	75 ee                	jne    80103fd0 <scheduler+0x1f0>
80103fe2:	83 78 7c 02          	cmpl   $0x2,0x7c(%eax)
80103fe6:	75 e8                	jne    80103fd0 <scheduler+0x1f0>
            if(!runable_p || p->arrival_time < runable_p->arrival_time){
80103fe8:	85 db                	test   %ebx,%ebx
80103fea:	74 28                	je     80104014 <scheduler+0x234>
80103fec:	8b bb 80 00 00 00    	mov    0x80(%ebx),%edi
80103ff2:	39 b8 80 00 00 00    	cmp    %edi,0x80(%eax)
80103ff8:	0f 42 d8             	cmovb  %eax,%ebx
        for(p=ptable.proc; p <&ptable.proc[NPROC]; p++){
80103ffb:	05 b8 00 00 00       	add    $0xb8,%eax
80104000:	3d 54 5b 11 80       	cmp    $0x80115b54,%eax
80104005:	75 d5                	jne    80103fdc <scheduler+0x1fc>
      if(runable_p){
80104007:	85 db                	test   %ebx,%ebx
80104009:	0f 85 12 ff ff ff    	jne    80103f21 <scheduler+0x141>
8010400f:	e9 9c fe ff ff       	jmp    80103eb0 <scheduler+0xd0>
80104014:	89 c3                	mov    %eax,%ebx
80104016:	eb b8                	jmp    80103fd0 <scheduler+0x1f0>
80104018:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010401f:	90                   	nop

80104020 <sched>:
{
80104020:	55                   	push   %ebp
80104021:	89 e5                	mov    %esp,%ebp
80104023:	56                   	push   %esi
80104024:	53                   	push   %ebx
  pushcli();
80104025:	e8 66 10 00 00       	call   80105090 <pushcli>
  c = mycpu();
8010402a:	e8 c1 f9 ff ff       	call   801039f0 <mycpu>
  p = c->proc;
8010402f:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
  popcli();
80104035:	e8 a6 10 00 00       	call   801050e0 <popcli>
  if(!holding(&ptable.lock))
8010403a:	83 ec 0c             	sub    $0xc,%esp
8010403d:	68 20 2d 11 80       	push   $0x80112d20
80104042:	e8 f9 10 00 00       	call   80105140 <holding>
80104047:	83 c4 10             	add    $0x10,%esp
8010404a:	85 c0                	test   %eax,%eax
8010404c:	74 4f                	je     8010409d <sched+0x7d>
  if(mycpu()->ncli != 1)
8010404e:	e8 9d f9 ff ff       	call   801039f0 <mycpu>
80104053:	83 b8 a4 00 00 00 01 	cmpl   $0x1,0xa4(%eax)
8010405a:	75 68                	jne    801040c4 <sched+0xa4>
  if(p->state == RUNNING)
8010405c:	83 7b 0c 04          	cmpl   $0x4,0xc(%ebx)
80104060:	74 55                	je     801040b7 <sched+0x97>
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80104062:	9c                   	pushf  
80104063:	58                   	pop    %eax
  if(readeflags()&FL_IF)
80104064:	f6 c4 02             	test   $0x2,%ah
80104067:	75 41                	jne    801040aa <sched+0x8a>
  intena = mycpu()->intena;
80104069:	e8 82 f9 ff ff       	call   801039f0 <mycpu>
  swtch(&p->context, mycpu()->scheduler);
8010406e:	83 c3 1c             	add    $0x1c,%ebx
  intena = mycpu()->intena;
80104071:	8b b0 a8 00 00 00    	mov    0xa8(%eax),%esi
  swtch(&p->context, mycpu()->scheduler);
80104077:	e8 74 f9 ff ff       	call   801039f0 <mycpu>
8010407c:	83 ec 08             	sub    $0x8,%esp
8010407f:	ff 70 04             	push   0x4(%eax)
80104082:	53                   	push   %ebx
80104083:	e8 33 14 00 00       	call   801054bb <swtch>
  mycpu()->intena = intena;
80104088:	e8 63 f9 ff ff       	call   801039f0 <mycpu>
}
8010408d:	83 c4 10             	add    $0x10,%esp
  mycpu()->intena = intena;
80104090:	89 b0 a8 00 00 00    	mov    %esi,0xa8(%eax)
}
80104096:	8d 65 f8             	lea    -0x8(%ebp),%esp
80104099:	5b                   	pop    %ebx
8010409a:	5e                   	pop    %esi
8010409b:	5d                   	pop    %ebp
8010409c:	c3                   	ret    
    panic("sched ptable.lock");
8010409d:	83 ec 0c             	sub    $0xc,%esp
801040a0:	68 bb 83 10 80       	push   $0x801083bb
801040a5:	e8 d6 c2 ff ff       	call   80100380 <panic>
    panic("sched interruptible");
801040aa:	83 ec 0c             	sub    $0xc,%esp
801040ad:	68 e7 83 10 80       	push   $0x801083e7
801040b2:	e8 c9 c2 ff ff       	call   80100380 <panic>
    panic("sched running");
801040b7:	83 ec 0c             	sub    $0xc,%esp
801040ba:	68 d9 83 10 80       	push   $0x801083d9
801040bf:	e8 bc c2 ff ff       	call   80100380 <panic>
    panic("sched locks");
801040c4:	83 ec 0c             	sub    $0xc,%esp
801040c7:	68 cd 83 10 80       	push   $0x801083cd
801040cc:	e8 af c2 ff ff       	call   80100380 <panic>
801040d1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801040d8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801040df:	90                   	nop

801040e0 <exit>:
{
801040e0:	55                   	push   %ebp
801040e1:	89 e5                	mov    %esp,%ebp
801040e3:	57                   	push   %edi
801040e4:	56                   	push   %esi
801040e5:	53                   	push   %ebx
801040e6:	83 ec 0c             	sub    $0xc,%esp
  struct proc *curproc = myproc();
801040e9:	e8 82 f9 ff ff       	call   80103a70 <myproc>
  if(curproc == initproc)
801040ee:	39 05 60 5b 11 80    	cmp    %eax,0x80115b60
801040f4:	0f 84 52 01 00 00    	je     8010424c <exit+0x16c>
801040fa:	89 c3                	mov    %eax,%ebx
801040fc:	8d 70 28             	lea    0x28(%eax),%esi
801040ff:	8d 78 68             	lea    0x68(%eax),%edi
80104102:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    if(curproc->ofile[fd]){
80104108:	8b 06                	mov    (%esi),%eax
8010410a:	85 c0                	test   %eax,%eax
8010410c:	74 12                	je     80104120 <exit+0x40>
      fileclose(curproc->ofile[fd]);
8010410e:	83 ec 0c             	sub    $0xc,%esp
80104111:	50                   	push   %eax
80104112:	e8 89 ce ff ff       	call   80100fa0 <fileclose>
      curproc->ofile[fd] = 0;
80104117:	c7 06 00 00 00 00    	movl   $0x0,(%esi)
8010411d:	83 c4 10             	add    $0x10,%esp
  for(fd = 0; fd < NOFILE; fd++){
80104120:	83 c6 04             	add    $0x4,%esi
80104123:	39 f7                	cmp    %esi,%edi
80104125:	75 e1                	jne    80104108 <exit+0x28>
  begin_op();
80104127:	e8 e4 ec ff ff       	call   80102e10 <begin_op>
  iput(curproc->cwd);
8010412c:	83 ec 0c             	sub    $0xc,%esp
8010412f:	ff 73 68             	push   0x68(%ebx)
80104132:	e8 29 d8 ff ff       	call   80101960 <iput>
  end_op();
80104137:	e8 44 ed ff ff       	call   80102e80 <end_op>
  curproc->cwd = 0;
8010413c:	c7 43 68 00 00 00 00 	movl   $0x0,0x68(%ebx)
  acquire(&ptable.lock);
80104143:	c7 04 24 20 2d 11 80 	movl   $0x80112d20,(%esp)
8010414a:	e8 91 10 00 00       	call   801051e0 <acquire>
  wakeup1(curproc->parent);
8010414f:	8b 53 14             	mov    0x14(%ebx),%edx
80104152:	83 c4 10             	add    $0x10,%esp
static void
wakeup1(void *chan)
{
  struct proc *p;

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104155:	b8 54 2d 11 80       	mov    $0x80112d54,%eax
8010415a:	eb 10                	jmp    8010416c <exit+0x8c>
8010415c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80104160:	05 b8 00 00 00       	add    $0xb8,%eax
80104165:	3d 54 5b 11 80       	cmp    $0x80115b54,%eax
8010416a:	74 2c                	je     80104198 <exit+0xb8>
    if(p->state == SLEEPING && p->chan == chan){
8010416c:	83 78 0c 02          	cmpl   $0x2,0xc(%eax)
80104170:	75 ee                	jne    80104160 <exit+0x80>
80104172:	3b 50 20             	cmp    0x20(%eax),%edx
80104175:	75 e9                	jne    80104160 <exit+0x80>
  if(p->queue==CLASS1){
80104177:	8b 48 7c             	mov    0x7c(%eax),%ecx
      p->state = RUNNABLE;
8010417a:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  if(p->queue==CLASS1){
80104181:	85 c9                	test   %ecx,%ecx
80104183:	75 79                	jne    801041fe <exit+0x11e>
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104185:	05 b8 00 00 00       	add    $0xb8,%eax
    ptable.class1_num = ptable.class1_num + inc_or_dec;
8010418a:	83 05 54 5b 11 80 01 	addl   $0x1,0x80115b54
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104191:	3d 54 5b 11 80       	cmp    $0x80115b54,%eax
80104196:	75 d4                	jne    8010416c <exit+0x8c>
      p->parent = initproc;
80104198:	8b 0d 60 5b 11 80    	mov    0x80115b60,%ecx
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
8010419e:	ba 54 2d 11 80       	mov    $0x80112d54,%edx
801041a3:	eb 11                	jmp    801041b6 <exit+0xd6>
801041a5:	8d 76 00             	lea    0x0(%esi),%esi
801041a8:	81 c2 b8 00 00 00    	add    $0xb8,%edx
801041ae:	81 fa 54 5b 11 80    	cmp    $0x80115b54,%edx
801041b4:	74 71                	je     80104227 <exit+0x147>
    if(p->parent == curproc){
801041b6:	39 5a 14             	cmp    %ebx,0x14(%edx)
801041b9:	75 ed                	jne    801041a8 <exit+0xc8>
      if(p->state == ZOMBIE)
801041bb:	83 7a 0c 05          	cmpl   $0x5,0xc(%edx)
      p->parent = initproc;
801041bf:	89 4a 14             	mov    %ecx,0x14(%edx)
      if(p->state == ZOMBIE)
801041c2:	75 e4                	jne    801041a8 <exit+0xc8>
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
801041c4:	b8 54 2d 11 80       	mov    $0x80112d54,%eax
801041c9:	eb 11                	jmp    801041dc <exit+0xfc>
801041cb:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801041cf:	90                   	nop
801041d0:	05 b8 00 00 00       	add    $0xb8,%eax
801041d5:	3d 54 5b 11 80       	cmp    $0x80115b54,%eax
801041da:	74 cc                	je     801041a8 <exit+0xc8>
    if(p->state == SLEEPING && p->chan == chan){
801041dc:	83 78 0c 02          	cmpl   $0x2,0xc(%eax)
801041e0:	75 ee                	jne    801041d0 <exit+0xf0>
801041e2:	3b 48 20             	cmp    0x20(%eax),%ecx
801041e5:	75 e9                	jne    801041d0 <exit+0xf0>
  if(p->queue==CLASS1){
801041e7:	8b 70 7c             	mov    0x7c(%eax),%esi
      p->state = RUNNABLE;
801041ea:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  if(p->queue==CLASS1){
801041f1:	85 f6                	test   %esi,%esi
801041f3:	75 1b                	jne    80104210 <exit+0x130>
    ptable.class1_num = ptable.class1_num + inc_or_dec;
801041f5:	83 05 54 5b 11 80 01 	addl   $0x1,0x80115b54
801041fc:	eb d2                	jmp    801041d0 <exit+0xf0>
    if(p->queue==CLASS2_RR){
801041fe:	83 f9 01             	cmp    $0x1,%ecx
80104201:	74 3d                	je     80104240 <exit+0x160>
      ptable.fcfs_num = ptable.fcfs_num + inc_or_dec;
80104203:	83 05 5c 5b 11 80 01 	addl   $0x1,0x80115b5c
8010420a:	e9 51 ff ff ff       	jmp    80104160 <exit+0x80>
8010420f:	90                   	nop
    if(p->queue==CLASS2_RR){
80104210:	83 fe 01             	cmp    $0x1,%esi
80104213:	74 09                	je     8010421e <exit+0x13e>
      ptable.fcfs_num = ptable.fcfs_num + inc_or_dec;
80104215:	83 05 5c 5b 11 80 01 	addl   $0x1,0x80115b5c
8010421c:	eb b2                	jmp    801041d0 <exit+0xf0>
      ptable.rr_num = ptable.rr_num + inc_or_dec;
8010421e:	83 05 58 5b 11 80 01 	addl   $0x1,0x80115b58
80104225:	eb a9                	jmp    801041d0 <exit+0xf0>
  curproc->state = ZOMBIE;
80104227:	c7 43 0c 05 00 00 00 	movl   $0x5,0xc(%ebx)
  sched();
8010422e:	e8 ed fd ff ff       	call   80104020 <sched>
  panic("zombie exit");
80104233:	83 ec 0c             	sub    $0xc,%esp
80104236:	68 08 84 10 80       	push   $0x80108408
8010423b:	e8 40 c1 ff ff       	call   80100380 <panic>
      ptable.rr_num = ptable.rr_num + inc_or_dec;
80104240:	83 05 58 5b 11 80 01 	addl   $0x1,0x80115b58
80104247:	e9 14 ff ff ff       	jmp    80104160 <exit+0x80>
    panic("init exiting");
8010424c:	83 ec 0c             	sub    $0xc,%esp
8010424f:	68 fb 83 10 80       	push   $0x801083fb
80104254:	e8 27 c1 ff ff       	call   80100380 <panic>
80104259:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80104260 <wait>:
{
80104260:	55                   	push   %ebp
80104261:	89 e5                	mov    %esp,%ebp
80104263:	56                   	push   %esi
80104264:	53                   	push   %ebx
  pushcli();
80104265:	e8 26 0e 00 00       	call   80105090 <pushcli>
  c = mycpu();
8010426a:	e8 81 f7 ff ff       	call   801039f0 <mycpu>
  p = c->proc;
8010426f:	8b b0 ac 00 00 00    	mov    0xac(%eax),%esi
  popcli();
80104275:	e8 66 0e 00 00       	call   801050e0 <popcli>
  acquire(&ptable.lock);
8010427a:	83 ec 0c             	sub    $0xc,%esp
8010427d:	68 20 2d 11 80       	push   $0x80112d20
80104282:	e8 59 0f 00 00       	call   801051e0 <acquire>
80104287:	83 c4 10             	add    $0x10,%esp
    havekids = 0;
8010428a:	31 c0                	xor    %eax,%eax
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
8010428c:	bb 54 2d 11 80       	mov    $0x80112d54,%ebx
80104291:	eb 13                	jmp    801042a6 <wait+0x46>
80104293:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80104297:	90                   	nop
80104298:	81 c3 b8 00 00 00    	add    $0xb8,%ebx
8010429e:	81 fb 54 5b 11 80    	cmp    $0x80115b54,%ebx
801042a4:	74 1e                	je     801042c4 <wait+0x64>
      if(p->parent != curproc)
801042a6:	39 73 14             	cmp    %esi,0x14(%ebx)
801042a9:	75 ed                	jne    80104298 <wait+0x38>
      if(p->state == ZOMBIE){
801042ab:	83 7b 0c 05          	cmpl   $0x5,0xc(%ebx)
801042af:	74 5f                	je     80104310 <wait+0xb0>
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801042b1:	81 c3 b8 00 00 00    	add    $0xb8,%ebx
      havekids = 1;
801042b7:	b8 01 00 00 00       	mov    $0x1,%eax
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801042bc:	81 fb 54 5b 11 80    	cmp    $0x80115b54,%ebx
801042c2:	75 e2                	jne    801042a6 <wait+0x46>
    if(!havekids || curproc->killed){
801042c4:	85 c0                	test   %eax,%eax
801042c6:	0f 84 9a 00 00 00    	je     80104366 <wait+0x106>
801042cc:	8b 46 24             	mov    0x24(%esi),%eax
801042cf:	85 c0                	test   %eax,%eax
801042d1:	0f 85 8f 00 00 00    	jne    80104366 <wait+0x106>
  pushcli();
801042d7:	e8 b4 0d 00 00       	call   80105090 <pushcli>
  c = mycpu();
801042dc:	e8 0f f7 ff ff       	call   801039f0 <mycpu>
  p = c->proc;
801042e1:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
  popcli();
801042e7:	e8 f4 0d 00 00       	call   801050e0 <popcli>
  if(p == 0)
801042ec:	85 db                	test   %ebx,%ebx
801042ee:	0f 84 89 00 00 00    	je     8010437d <wait+0x11d>
  p->chan = chan;
801042f4:	89 73 20             	mov    %esi,0x20(%ebx)
  p->state = SLEEPING;
801042f7:	c7 43 0c 02 00 00 00 	movl   $0x2,0xc(%ebx)
  sched();
801042fe:	e8 1d fd ff ff       	call   80104020 <sched>
  p->chan = 0;
80104303:	c7 43 20 00 00 00 00 	movl   $0x0,0x20(%ebx)
}
8010430a:	e9 7b ff ff ff       	jmp    8010428a <wait+0x2a>
8010430f:	90                   	nop
        kfree(p->kstack);
80104310:	83 ec 0c             	sub    $0xc,%esp
        pid = p->pid;
80104313:	8b 73 10             	mov    0x10(%ebx),%esi
        kfree(p->kstack);
80104316:	ff 73 08             	push   0x8(%ebx)
80104319:	e8 52 e2 ff ff       	call   80102570 <kfree>
        p->kstack = 0;
8010431e:	c7 43 08 00 00 00 00 	movl   $0x0,0x8(%ebx)
        freevm(p->pgdir);
80104325:	5a                   	pop    %edx
80104326:	ff 73 04             	push   0x4(%ebx)
80104329:	e8 f2 36 00 00       	call   80107a20 <freevm>
        p->pid = 0;
8010432e:	c7 43 10 00 00 00 00 	movl   $0x0,0x10(%ebx)
        p->parent = 0;
80104335:	c7 43 14 00 00 00 00 	movl   $0x0,0x14(%ebx)
        p->name[0] = 0;
8010433c:	c6 43 6c 00          	movb   $0x0,0x6c(%ebx)
        p->killed = 0;
80104340:	c7 43 24 00 00 00 00 	movl   $0x0,0x24(%ebx)
        p->state = UNUSED;
80104347:	c7 43 0c 00 00 00 00 	movl   $0x0,0xc(%ebx)
        release(&ptable.lock);
8010434e:	c7 04 24 20 2d 11 80 	movl   $0x80112d20,(%esp)
80104355:	e8 26 0e 00 00       	call   80105180 <release>
        return pid;
8010435a:	83 c4 10             	add    $0x10,%esp
}
8010435d:	8d 65 f8             	lea    -0x8(%ebp),%esp
80104360:	89 f0                	mov    %esi,%eax
80104362:	5b                   	pop    %ebx
80104363:	5e                   	pop    %esi
80104364:	5d                   	pop    %ebp
80104365:	c3                   	ret    
      release(&ptable.lock);
80104366:	83 ec 0c             	sub    $0xc,%esp
      return -1;
80104369:	be ff ff ff ff       	mov    $0xffffffff,%esi
      release(&ptable.lock);
8010436e:	68 20 2d 11 80       	push   $0x80112d20
80104373:	e8 08 0e 00 00       	call   80105180 <release>
      return -1;
80104378:	83 c4 10             	add    $0x10,%esp
8010437b:	eb e0                	jmp    8010435d <wait+0xfd>
    panic("sleep");
8010437d:	83 ec 0c             	sub    $0xc,%esp
80104380:	68 14 84 10 80       	push   $0x80108414
80104385:	e8 f6 bf ff ff       	call   80100380 <panic>
8010438a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80104390 <yield>:
{
80104390:	55                   	push   %ebp
80104391:	89 e5                	mov    %esp,%ebp
80104393:	53                   	push   %ebx
80104394:	83 ec 10             	sub    $0x10,%esp
  acquire(&ptable.lock);  //DOC: yieldlock
80104397:	68 20 2d 11 80       	push   $0x80112d20
8010439c:	e8 3f 0e 00 00       	call   801051e0 <acquire>
  pushcli();
801043a1:	e8 ea 0c 00 00       	call   80105090 <pushcli>
  c = mycpu();
801043a6:	e8 45 f6 ff ff       	call   801039f0 <mycpu>
  p = c->proc;
801043ab:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
  popcli();
801043b1:	e8 2a 0d 00 00       	call   801050e0 <popcli>
  myproc()->state = RUNNABLE;
801043b6:	c7 43 0c 03 00 00 00 	movl   $0x3,0xc(%ebx)
  pushcli();
801043bd:	e8 ce 0c 00 00       	call   80105090 <pushcli>
  c = mycpu();
801043c2:	e8 29 f6 ff ff       	call   801039f0 <mycpu>
  p = c->proc;
801043c7:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
  popcli();
801043cd:	e8 0e 0d 00 00       	call   801050e0 <popcli>
  if(p->queue==CLASS1){
801043d2:	83 c4 10             	add    $0x10,%esp
801043d5:	8b 43 7c             	mov    0x7c(%ebx),%eax
801043d8:	85 c0                	test   %eax,%eax
801043da:	75 24                	jne    80104400 <yield+0x70>
    ptable.class1_num = ptable.class1_num + inc_or_dec;
801043dc:	83 05 54 5b 11 80 01 	addl   $0x1,0x80115b54
  sched();
801043e3:	e8 38 fc ff ff       	call   80104020 <sched>
  release(&ptable.lock);
801043e8:	83 ec 0c             	sub    $0xc,%esp
801043eb:	68 20 2d 11 80       	push   $0x80112d20
801043f0:	e8 8b 0d 00 00       	call   80105180 <release>
}
801043f5:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801043f8:	83 c4 10             	add    $0x10,%esp
801043fb:	c9                   	leave  
801043fc:	c3                   	ret    
801043fd:	8d 76 00             	lea    0x0(%esi),%esi
    if(p->queue==CLASS2_RR){
80104400:	83 f8 01             	cmp    $0x1,%eax
80104403:	74 0b                	je     80104410 <yield+0x80>
      ptable.fcfs_num = ptable.fcfs_num + inc_or_dec;
80104405:	83 05 5c 5b 11 80 01 	addl   $0x1,0x80115b5c
8010440c:	eb d5                	jmp    801043e3 <yield+0x53>
8010440e:	66 90                	xchg   %ax,%ax
      ptable.rr_num = ptable.rr_num + inc_or_dec;
80104410:	83 05 58 5b 11 80 01 	addl   $0x1,0x80115b58
80104417:	eb ca                	jmp    801043e3 <yield+0x53>
80104419:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80104420 <sleep>:
{
80104420:	55                   	push   %ebp
80104421:	89 e5                	mov    %esp,%ebp
80104423:	57                   	push   %edi
80104424:	56                   	push   %esi
80104425:	53                   	push   %ebx
80104426:	83 ec 0c             	sub    $0xc,%esp
80104429:	8b 7d 08             	mov    0x8(%ebp),%edi
8010442c:	8b 75 0c             	mov    0xc(%ebp),%esi
  pushcli();
8010442f:	e8 5c 0c 00 00       	call   80105090 <pushcli>
  c = mycpu();
80104434:	e8 b7 f5 ff ff       	call   801039f0 <mycpu>
  p = c->proc;
80104439:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
  popcli();
8010443f:	e8 9c 0c 00 00       	call   801050e0 <popcli>
  if(p == 0)
80104444:	85 db                	test   %ebx,%ebx
80104446:	0f 84 87 00 00 00    	je     801044d3 <sleep+0xb3>
  if(lk == 0)
8010444c:	85 f6                	test   %esi,%esi
8010444e:	74 76                	je     801044c6 <sleep+0xa6>
  if(lk != &ptable.lock){  //DOC: sleeplock0
80104450:	81 fe 20 2d 11 80    	cmp    $0x80112d20,%esi
80104456:	74 50                	je     801044a8 <sleep+0x88>
    acquire(&ptable.lock);  //DOC: sleeplock1
80104458:	83 ec 0c             	sub    $0xc,%esp
8010445b:	68 20 2d 11 80       	push   $0x80112d20
80104460:	e8 7b 0d 00 00       	call   801051e0 <acquire>
    release(lk);
80104465:	89 34 24             	mov    %esi,(%esp)
80104468:	e8 13 0d 00 00       	call   80105180 <release>
  p->chan = chan;
8010446d:	89 7b 20             	mov    %edi,0x20(%ebx)
  p->state = SLEEPING;
80104470:	c7 43 0c 02 00 00 00 	movl   $0x2,0xc(%ebx)
  sched();
80104477:	e8 a4 fb ff ff       	call   80104020 <sched>
  p->chan = 0;
8010447c:	c7 43 20 00 00 00 00 	movl   $0x0,0x20(%ebx)
    release(&ptable.lock);
80104483:	c7 04 24 20 2d 11 80 	movl   $0x80112d20,(%esp)
8010448a:	e8 f1 0c 00 00       	call   80105180 <release>
    acquire(lk);
8010448f:	89 75 08             	mov    %esi,0x8(%ebp)
80104492:	83 c4 10             	add    $0x10,%esp
}
80104495:	8d 65 f4             	lea    -0xc(%ebp),%esp
80104498:	5b                   	pop    %ebx
80104499:	5e                   	pop    %esi
8010449a:	5f                   	pop    %edi
8010449b:	5d                   	pop    %ebp
    acquire(lk);
8010449c:	e9 3f 0d 00 00       	jmp    801051e0 <acquire>
801044a1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  p->chan = chan;
801044a8:	89 7b 20             	mov    %edi,0x20(%ebx)
  p->state = SLEEPING;
801044ab:	c7 43 0c 02 00 00 00 	movl   $0x2,0xc(%ebx)
  sched();
801044b2:	e8 69 fb ff ff       	call   80104020 <sched>
  p->chan = 0;
801044b7:	c7 43 20 00 00 00 00 	movl   $0x0,0x20(%ebx)
}
801044be:	8d 65 f4             	lea    -0xc(%ebp),%esp
801044c1:	5b                   	pop    %ebx
801044c2:	5e                   	pop    %esi
801044c3:	5f                   	pop    %edi
801044c4:	5d                   	pop    %ebp
801044c5:	c3                   	ret    
    panic("sleep without lk");
801044c6:	83 ec 0c             	sub    $0xc,%esp
801044c9:	68 1a 84 10 80       	push   $0x8010841a
801044ce:	e8 ad be ff ff       	call   80100380 <panic>
    panic("sleep");
801044d3:	83 ec 0c             	sub    $0xc,%esp
801044d6:	68 14 84 10 80       	push   $0x80108414
801044db:	e8 a0 be ff ff       	call   80100380 <panic>

801044e0 <wakeup>:
}

// Wake up all processes sleeping on chan.
void
wakeup(void *chan)
{
801044e0:	55                   	push   %ebp
801044e1:	89 e5                	mov    %esp,%ebp
801044e3:	53                   	push   %ebx
801044e4:	83 ec 10             	sub    $0x10,%esp
801044e7:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquire(&ptable.lock);
801044ea:	68 20 2d 11 80       	push   $0x80112d20
801044ef:	e8 ec 0c 00 00       	call   801051e0 <acquire>
801044f4:	83 c4 10             	add    $0x10,%esp
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
801044f7:	b8 54 2d 11 80       	mov    $0x80112d54,%eax
801044fc:	eb 0e                	jmp    8010450c <wakeup+0x2c>
801044fe:	66 90                	xchg   %ax,%ax
80104500:	05 b8 00 00 00       	add    $0xb8,%eax
80104505:	3d 54 5b 11 80       	cmp    $0x80115b54,%eax
8010450a:	74 2c                	je     80104538 <wakeup+0x58>
    if(p->state == SLEEPING && p->chan == chan){
8010450c:	83 78 0c 02          	cmpl   $0x2,0xc(%eax)
80104510:	75 ee                	jne    80104500 <wakeup+0x20>
80104512:	3b 58 20             	cmp    0x20(%eax),%ebx
80104515:	75 e9                	jne    80104500 <wakeup+0x20>
  if(p->queue==CLASS1){
80104517:	8b 50 7c             	mov    0x7c(%eax),%edx
      p->state = RUNNABLE;
8010451a:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  if(p->queue==CLASS1){
80104521:	85 d2                	test   %edx,%edx
80104523:	75 2b                	jne    80104550 <wakeup+0x70>
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104525:	05 b8 00 00 00       	add    $0xb8,%eax
    ptable.class1_num = ptable.class1_num + inc_or_dec;
8010452a:	83 05 54 5b 11 80 01 	addl   $0x1,0x80115b54
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104531:	3d 54 5b 11 80       	cmp    $0x80115b54,%eax
80104536:	75 d4                	jne    8010450c <wakeup+0x2c>
  wakeup1(chan);
  release(&ptable.lock);
80104538:	c7 45 08 20 2d 11 80 	movl   $0x80112d20,0x8(%ebp)
}
8010453f:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80104542:	c9                   	leave  
  release(&ptable.lock);
80104543:	e9 38 0c 00 00       	jmp    80105180 <release>
80104548:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010454f:	90                   	nop
    if(p->queue==CLASS2_RR){
80104550:	83 fa 01             	cmp    $0x1,%edx
80104553:	74 09                	je     8010455e <wakeup+0x7e>
      ptable.fcfs_num = ptable.fcfs_num + inc_or_dec;
80104555:	83 05 5c 5b 11 80 01 	addl   $0x1,0x80115b5c
8010455c:	eb a2                	jmp    80104500 <wakeup+0x20>
      ptable.rr_num = ptable.rr_num + inc_or_dec;
8010455e:	83 05 58 5b 11 80 01 	addl   $0x1,0x80115b58
80104565:	eb 99                	jmp    80104500 <wakeup+0x20>
80104567:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010456e:	66 90                	xchg   %ax,%ax

80104570 <kill>:
// Kill the process with the given pid.
// Process won't exit until it returns
// to user space (see trap in trap.c).
int
kill(int pid)
{
80104570:	55                   	push   %ebp
80104571:	89 e5                	mov    %esp,%ebp
80104573:	53                   	push   %ebx
80104574:	83 ec 10             	sub    $0x10,%esp
80104577:	8b 5d 08             	mov    0x8(%ebp),%ebx
  struct proc *p;

  acquire(&ptable.lock);
8010457a:	68 20 2d 11 80       	push   $0x80112d20
8010457f:	e8 5c 0c 00 00       	call   801051e0 <acquire>
80104584:	83 c4 10             	add    $0x10,%esp
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104587:	b8 54 2d 11 80       	mov    $0x80112d54,%eax
8010458c:	eb 0e                	jmp    8010459c <kill+0x2c>
8010458e:	66 90                	xchg   %ax,%ax
80104590:	05 b8 00 00 00       	add    $0xb8,%eax
80104595:	3d 54 5b 11 80       	cmp    $0x80115b54,%eax
8010459a:	74 2c                	je     801045c8 <kill+0x58>
    if(p->pid == pid){
8010459c:	39 58 10             	cmp    %ebx,0x10(%eax)
8010459f:	75 ef                	jne    80104590 <kill+0x20>
      p->killed = 1;
      // Wake process from sleep if necessary.
      if(p->state == SLEEPING){
801045a1:	83 78 0c 02          	cmpl   $0x2,0xc(%eax)
      p->killed = 1;
801045a5:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
      if(p->state == SLEEPING){
801045ac:	74 3a                	je     801045e8 <kill+0x78>
        change_num(p, 1);
        p->state = RUNNABLE;
      }  
      release(&ptable.lock);
801045ae:	83 ec 0c             	sub    $0xc,%esp
801045b1:	68 20 2d 11 80       	push   $0x80112d20
801045b6:	e8 c5 0b 00 00       	call   80105180 <release>
      return 0;
    }
  }
  release(&ptable.lock);
  return -1;
}
801045bb:	8b 5d fc             	mov    -0x4(%ebp),%ebx
      return 0;
801045be:	83 c4 10             	add    $0x10,%esp
801045c1:	31 c0                	xor    %eax,%eax
}
801045c3:	c9                   	leave  
801045c4:	c3                   	ret    
801045c5:	8d 76 00             	lea    0x0(%esi),%esi
  release(&ptable.lock);
801045c8:	83 ec 0c             	sub    $0xc,%esp
801045cb:	68 20 2d 11 80       	push   $0x80112d20
801045d0:	e8 ab 0b 00 00       	call   80105180 <release>
}
801045d5:	8b 5d fc             	mov    -0x4(%ebp),%ebx
  return -1;
801045d8:	83 c4 10             	add    $0x10,%esp
801045db:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801045e0:	c9                   	leave  
801045e1:	c3                   	ret    
801045e2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  if(p->queue==CLASS1){
801045e8:	8b 50 7c             	mov    0x7c(%eax),%edx
801045eb:	85 d2                	test   %edx,%edx
801045ed:	75 10                	jne    801045ff <kill+0x8f>
    ptable.class1_num = ptable.class1_num + inc_or_dec;
801045ef:	83 05 54 5b 11 80 01 	addl   $0x1,0x80115b54
        p->state = RUNNABLE;
801045f6:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
801045fd:	eb af                	jmp    801045ae <kill+0x3e>
    if(p->queue==CLASS2_RR){
801045ff:	83 fa 01             	cmp    $0x1,%edx
80104602:	74 09                	je     8010460d <kill+0x9d>
      ptable.fcfs_num = ptable.fcfs_num + inc_or_dec;
80104604:	83 05 5c 5b 11 80 01 	addl   $0x1,0x80115b5c
8010460b:	eb e9                	jmp    801045f6 <kill+0x86>
      ptable.rr_num = ptable.rr_num + inc_or_dec;
8010460d:	83 05 58 5b 11 80 01 	addl   $0x1,0x80115b58
80104614:	eb e0                	jmp    801045f6 <kill+0x86>
80104616:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010461d:	8d 76 00             	lea    0x0(%esi),%esi

80104620 <procdump>:
// Print a process listing to console.  For debugging.
// Runs when user types ^P on console.
// No lock to avoid wedging a stuck machine further.
void
procdump(void)
{
80104620:	55                   	push   %ebp
80104621:	89 e5                	mov    %esp,%ebp
80104623:	57                   	push   %edi
80104624:	56                   	push   %esi
80104625:	8d 75 e8             	lea    -0x18(%ebp),%esi
80104628:	53                   	push   %ebx
80104629:	bb c0 2d 11 80       	mov    $0x80112dc0,%ebx
8010462e:	83 ec 3c             	sub    $0x3c,%esp
80104631:	eb 27                	jmp    8010465a <procdump+0x3a>
80104633:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80104637:	90                   	nop
    if(p->state == SLEEPING){
      getcallerpcs((uint*)p->context->ebp+2, pc);
      for(i=0; i<10 && pc[i] != 0; i++)
        cprintf(" %p", pc[i]);
    }
    cprintf("\n");
80104638:	83 ec 0c             	sub    $0xc,%esp
8010463b:	68 4c 84 10 80       	push   $0x8010844c
80104640:	e8 5b c0 ff ff       	call   801006a0 <cprintf>
80104645:	83 c4 10             	add    $0x10,%esp
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104648:	81 c3 b8 00 00 00    	add    $0xb8,%ebx
8010464e:	81 fb c0 5b 11 80    	cmp    $0x80115bc0,%ebx
80104654:	0f 84 7e 00 00 00    	je     801046d8 <procdump+0xb8>
    if(p->state == UNUSED)
8010465a:	8b 43 a0             	mov    -0x60(%ebx),%eax
8010465d:	85 c0                	test   %eax,%eax
8010465f:	74 e7                	je     80104648 <procdump+0x28>
      state = "???";
80104661:	ba 2b 84 10 80       	mov    $0x8010842b,%edx
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
80104666:	83 f8 05             	cmp    $0x5,%eax
80104669:	77 11                	ja     8010467c <procdump+0x5c>
8010466b:	8b 14 85 7c 86 10 80 	mov    -0x7fef7984(,%eax,4),%edx
      state = "???";
80104672:	b8 2b 84 10 80       	mov    $0x8010842b,%eax
80104677:	85 d2                	test   %edx,%edx
80104679:	0f 44 d0             	cmove  %eax,%edx
    cprintf("%d %s %s", p->pid, state, p->name);
8010467c:	53                   	push   %ebx
8010467d:	52                   	push   %edx
8010467e:	ff 73 a4             	push   -0x5c(%ebx)
80104681:	68 2f 84 10 80       	push   $0x8010842f
80104686:	e8 15 c0 ff ff       	call   801006a0 <cprintf>
    if(p->state == SLEEPING){
8010468b:	83 c4 10             	add    $0x10,%esp
8010468e:	83 7b a0 02          	cmpl   $0x2,-0x60(%ebx)
80104692:	75 a4                	jne    80104638 <procdump+0x18>
      getcallerpcs((uint*)p->context->ebp+2, pc);
80104694:	83 ec 08             	sub    $0x8,%esp
80104697:	8d 45 c0             	lea    -0x40(%ebp),%eax
8010469a:	8d 7d c0             	lea    -0x40(%ebp),%edi
8010469d:	50                   	push   %eax
8010469e:	8b 43 b0             	mov    -0x50(%ebx),%eax
801046a1:	8b 40 0c             	mov    0xc(%eax),%eax
801046a4:	83 c0 08             	add    $0x8,%eax
801046a7:	50                   	push   %eax
801046a8:	e8 83 09 00 00       	call   80105030 <getcallerpcs>
      for(i=0; i<10 && pc[i] != 0; i++)
801046ad:	83 c4 10             	add    $0x10,%esp
801046b0:	8b 17                	mov    (%edi),%edx
801046b2:	85 d2                	test   %edx,%edx
801046b4:	74 82                	je     80104638 <procdump+0x18>
        cprintf(" %p", pc[i]);
801046b6:	83 ec 08             	sub    $0x8,%esp
      for(i=0; i<10 && pc[i] != 0; i++)
801046b9:	83 c7 04             	add    $0x4,%edi
        cprintf(" %p", pc[i]);
801046bc:	52                   	push   %edx
801046bd:	68 41 7e 10 80       	push   $0x80107e41
801046c2:	e8 d9 bf ff ff       	call   801006a0 <cprintf>
      for(i=0; i<10 && pc[i] != 0; i++)
801046c7:	83 c4 10             	add    $0x10,%esp
801046ca:	39 fe                	cmp    %edi,%esi
801046cc:	75 e2                	jne    801046b0 <procdump+0x90>
801046ce:	e9 65 ff ff ff       	jmp    80104638 <procdump+0x18>
801046d3:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801046d7:	90                   	nop
  }

}
801046d8:	8d 65 f4             	lea    -0xc(%ebp),%esp
801046db:	5b                   	pop    %ebx
801046dc:	5e                   	pop    %esi
801046dd:	5f                   	pop    %edi
801046de:	5d                   	pop    %ebp
801046df:	c3                   	ret    

801046e0 <sys_setlevel>:

int sys_setlevel(void){
801046e0:	55                   	push   %ebp
801046e1:	89 e5                	mov    %esp,%ebp
801046e3:	56                   	push   %esi
801046e4:	53                   	push   %ebx
    int pid, new_level;
    struct proc *p;

  
    if(argint(0, &pid) < 0)
801046e5:	8d 45 f0             	lea    -0x10(%ebp),%eax
int sys_setlevel(void){
801046e8:	83 ec 18             	sub    $0x18,%esp
    if(argint(0, &pid) < 0)
801046eb:	50                   	push   %eax
801046ec:	6a 00                	push   $0x0
801046ee:	e8 6d 0e 00 00       	call   80105560 <argint>
801046f3:	83 c4 10             	add    $0x10,%esp
801046f6:	85 c0                	test   %eax,%eax
801046f8:	0f 88 a2 00 00 00    	js     801047a0 <sys_setlevel+0xc0>
        return -1;
    if(argint(1, &new_level) < 0)
801046fe:	83 ec 08             	sub    $0x8,%esp
80104701:	8d 45 f4             	lea    -0xc(%ebp),%eax
80104704:	50                   	push   %eax
80104705:	6a 01                	push   $0x1
80104707:	e8 54 0e 00 00       	call   80105560 <argint>
8010470c:	83 c4 10             	add    $0x10,%esp
8010470f:	85 c0                	test   %eax,%eax
80104711:	0f 88 89 00 00 00    	js     801047a0 <sys_setlevel+0xc0>
        return -1;

 
    if(new_level != CLASS2_RR && new_level != CLASS2_FCFS)
80104717:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010471a:	83 e8 01             	sub    $0x1,%eax
8010471d:	83 f8 01             	cmp    $0x1,%eax
80104720:	77 7e                	ja     801047a0 <sys_setlevel+0xc0>
        return -1;

    acquire(&ptable.lock);
80104722:	83 ec 0c             	sub    $0xc,%esp
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104725:	bb 54 2d 11 80       	mov    $0x80112d54,%ebx
    acquire(&ptable.lock);
8010472a:	68 20 2d 11 80       	push   $0x80112d20
8010472f:	e8 ac 0a 00 00       	call   801051e0 <acquire>
        if(p->pid == pid){
80104734:	8b 55 f0             	mov    -0x10(%ebp),%edx
          if(p->queue == new_level){
80104737:	8b 75 f4             	mov    -0xc(%ebp),%esi
8010473a:	83 c4 10             	add    $0x10,%esp
8010473d:	eb 0f                	jmp    8010474e <sys_setlevel+0x6e>
8010473f:	90                   	nop
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104740:	81 c3 b8 00 00 00    	add    $0xb8,%ebx
80104746:	81 fb 54 5b 11 80    	cmp    $0x80115b54,%ebx
8010474c:	74 42                	je     80104790 <sys_setlevel+0xb0>
        if(p->pid == pid){
8010474e:	39 53 10             	cmp    %edx,0x10(%ebx)
80104751:	75 ed                	jne    80104740 <sys_setlevel+0x60>
          if(p->queue == new_level){
80104753:	8b 43 7c             	mov    0x7c(%ebx),%eax
80104756:	39 f0                	cmp    %esi,%eax
80104758:	74 52                	je     801047ac <sys_setlevel+0xcc>
            release(&ptable.lock);
            cprintf("the process is in this level now, can't change level!\n");
            return -1;
          }
            if(p->queue == CLASS2_FCFS || p->queue == CLASS2_RR){
8010475a:	8d 48 ff             	lea    -0x1(%eax),%ecx
8010475d:	83 f9 01             	cmp    $0x1,%ecx
80104760:	76 72                	jbe    801047d4 <sys_setlevel+0xf4>
               // p->age = 0;
                cprintf("\n",p->queue);
                release(&ptable.lock);
                cprintf("changing level done!\n");
                return 0; 
            } if(p->queue == CLASS1) {
80104762:	85 c0                	test   %eax,%eax
80104764:	75 da                	jne    80104740 <sys_setlevel+0x60>
                release(&ptable.lock);
80104766:	83 ec 0c             	sub    $0xc,%esp
80104769:	68 20 2d 11 80       	push   $0x80112d20
8010476e:	e8 0d 0a 00 00       	call   80105180 <release>
                cprintf("Not in class2, can't change level!\n");
80104773:	c7 04 24 84 85 10 80 	movl   $0x80108584,(%esp)
8010477a:	e8 21 bf ff ff       	call   801006a0 <cprintf>
                return -1; 
8010477f:	83 c4 10             	add    $0x10,%esp
            }
        }
    }
    release(&ptable.lock);
    return -1; // pid not found
}
80104782:	8d 65 f8             	lea    -0x8(%ebp),%esp
                return -1; 
80104785:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010478a:	5b                   	pop    %ebx
8010478b:	5e                   	pop    %esi
8010478c:	5d                   	pop    %ebp
8010478d:	c3                   	ret    
8010478e:	66 90                	xchg   %ax,%ax
    release(&ptable.lock);
80104790:	83 ec 0c             	sub    $0xc,%esp
80104793:	68 20 2d 11 80       	push   $0x80112d20
80104798:	e8 e3 09 00 00       	call   80105180 <release>
    return -1; // pid not found
8010479d:	83 c4 10             	add    $0x10,%esp
}
801047a0:	8d 65 f8             	lea    -0x8(%ebp),%esp
    return -1; // pid not found
801047a3:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801047a8:	5b                   	pop    %ebx
801047a9:	5e                   	pop    %esi
801047aa:	5d                   	pop    %ebp
801047ab:	c3                   	ret    
            release(&ptable.lock);
801047ac:	83 ec 0c             	sub    $0xc,%esp
801047af:	68 20 2d 11 80       	push   $0x80112d20
801047b4:	e8 c7 09 00 00       	call   80105180 <release>
            cprintf("the process is in this level now, can't change level!\n");
801047b9:	c7 04 24 4c 85 10 80 	movl   $0x8010854c,(%esp)
801047c0:	e8 db be ff ff       	call   801006a0 <cprintf>
            return -1;
801047c5:	83 c4 10             	add    $0x10,%esp
}
801047c8:	8d 65 f8             	lea    -0x8(%ebp),%esp
            return -1;
801047cb:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801047d0:	5b                   	pop    %ebx
801047d1:	5e                   	pop    %esi
801047d2:	5d                   	pop    %ebp
801047d3:	c3                   	ret    
                cprintf("\n",p->queue);
801047d4:	83 ec 08             	sub    $0x8,%esp
801047d7:	50                   	push   %eax
801047d8:	68 4c 84 10 80       	push   $0x8010844c
801047dd:	e8 be be ff ff       	call   801006a0 <cprintf>
                p->queue = new_level;
801047e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
                cprintf("\n",p->queue);
801047e5:	5a                   	pop    %edx
801047e6:	59                   	pop    %ecx
801047e7:	50                   	push   %eax
801047e8:	68 4c 84 10 80       	push   $0x8010844c
                p->queue = new_level;
801047ed:	89 43 7c             	mov    %eax,0x7c(%ebx)
                cprintf("\n",p->queue);
801047f0:	e8 ab be ff ff       	call   801006a0 <cprintf>
                release(&ptable.lock);
801047f5:	c7 04 24 20 2d 11 80 	movl   $0x80112d20,(%esp)
801047fc:	e8 7f 09 00 00       	call   80105180 <release>
                cprintf("changing level done!\n");
80104801:	c7 04 24 38 84 10 80 	movl   $0x80108438,(%esp)
80104808:	e8 93 be ff ff       	call   801006a0 <cprintf>
                return 0; 
8010480d:	83 c4 10             	add    $0x10,%esp
}
80104810:	8d 65 f8             	lea    -0x8(%ebp),%esp
                return 0; 
80104813:	31 c0                	xor    %eax,%eax
}
80104815:	5b                   	pop    %ebx
80104816:	5e                   	pop    %esi
80104817:	5d                   	pop    %ebp
80104818:	c3                   	ret    
80104819:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80104820 <getStateString>:





char* getStateString(enum procstate state) {
80104820:	55                   	push   %ebp
80104821:	b8 4e 84 10 80       	mov    $0x8010844e,%eax
80104826:	89 e5                	mov    %esp,%ebp
80104828:	8b 55 08             	mov    0x8(%ebp),%edx
8010482b:	83 fa 05             	cmp    $0x5,%edx
8010482e:	77 07                	ja     80104837 <getStateString+0x17>
80104830:	8b 04 95 64 86 10 80 	mov    -0x7fef799c(,%edx,4),%eax
    case RUNNABLE: return "RUNNABLE";
    case RUNNING: return "RUNNING";
    case ZOMBIE: return "ZOMBIE";
    default: return "UNKNOWN";
  }
}
80104837:	5d                   	pop    %ebp
80104838:	c3                   	ret    
80104839:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80104840 <getClassString>:
char* getClassString(enum priority_queue q) {
80104840:	55                   	push   %ebp
80104841:	b8 56 84 10 80       	mov    $0x80108456,%eax
80104846:	89 e5                	mov    %esp,%ebp
80104848:	8b 55 08             	mov    0x8(%ebp),%edx
8010484b:	83 fa 02             	cmp    $0x2,%edx
8010484e:	77 07                	ja     80104857 <getClassString+0x17>
80104850:	8b 04 95 58 86 10 80 	mov    -0x7fef79a8(,%edx,4),%eax
    case CLASS1: return "real-time";
    case CLASS2_RR: return "normal";
    case CLASS2_FCFS: return "normal";
    default: return "unknown";
  }
}
80104857:	5d                   	pop    %ebp
80104858:	c3                   	ret    
80104859:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80104860 <getAlgorithmString>:
char* getAlgorithmString(struct proc *p) {
80104860:	55                   	push   %ebp
80104861:	89 e5                	mov    %esp,%ebp
  switch (p->queue) {
80104863:	8b 45 08             	mov    0x8(%ebp),%eax
80104866:	8b 50 7c             	mov    0x7c(%eax),%edx
80104869:	b8 56 84 10 80       	mov    $0x80108456,%eax
8010486e:	83 fa 02             	cmp    $0x2,%edx
80104871:	77 07                	ja     8010487a <getAlgorithmString+0x1a>
80104873:	8b 04 95 4c 86 10 80 	mov    -0x7fef79b4(,%edx,4),%eax
    case CLASS1: return "EDF";
    case CLASS2_RR: return "RR";
    case CLASS2_FCFS: return "FCFS";
    default: return "unknown";
  }
}
8010487a:	5d                   	pop    %ebp
8010487b:	c3                   	ret    
8010487c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80104880 <count_digits>:


int count_digits(int number) {
80104880:	55                   	push   %ebp
80104881:	b9 01 00 00 00       	mov    $0x1,%ecx
80104886:	89 e5                	mov    %esp,%ebp
80104888:	53                   	push   %ebx
80104889:	8b 55 08             	mov    0x8(%ebp),%edx
  if (number == 0)
8010488c:	85 d2                	test   %edx,%edx
8010488e:	74 1c                	je     801048ac <count_digits+0x2c>
      return 1;  

  int count = 0;
80104890:	b9 00 00 00 00       	mov    $0x0,%ecx
  if (number < 0) {
80104895:	78 21                	js     801048b8 <count_digits+0x38>
      number = -number; 
  }

  while (number > 0) {
      count++;
      number /= 10; 
80104897:	bb cd cc cc cc       	mov    $0xcccccccd,%ebx
8010489c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801048a0:	89 d0                	mov    %edx,%eax
      count++;
801048a2:	83 c1 01             	add    $0x1,%ecx
      number /= 10; 
801048a5:	f7 e3                	mul    %ebx
  while (number > 0) {
801048a7:	c1 ea 03             	shr    $0x3,%edx
801048aa:	75 f4                	jne    801048a0 <count_digits+0x20>
  }
  return count;
}
801048ac:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801048af:	89 c8                	mov    %ecx,%eax
801048b1:	c9                   	leave  
801048b2:	c3                   	ret    
801048b3:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801048b7:	90                   	nop
      number = -number; 
801048b8:	f7 da                	neg    %edx
      count++;     
801048ba:	b9 01 00 00 00       	mov    $0x1,%ecx
801048bf:	eb d6                	jmp    80104897 <count_digits+0x17>
801048c1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801048c8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801048cf:	90                   	nop

801048d0 <printspaces>:

void printspaces(int count) { 
801048d0:	55                   	push   %ebp
801048d1:	89 e5                	mov    %esp,%ebp
801048d3:	56                   	push   %esi
801048d4:	8b 75 08             	mov    0x8(%ebp),%esi
801048d7:	53                   	push   %ebx
  for (int i = 0; i < count; i++) {
801048d8:	85 f6                	test   %esi,%esi
801048da:	7e 1b                	jle    801048f7 <printspaces+0x27>
801048dc:	31 db                	xor    %ebx,%ebx
801048de:	66 90                	xchg   %ax,%ax
    cprintf(" ");
801048e0:	83 ec 0c             	sub    $0xc,%esp
  for (int i = 0; i < count; i++) {
801048e3:	83 c3 01             	add    $0x1,%ebx
    cprintf(" ");
801048e6:	68 de 84 10 80       	push   $0x801084de
801048eb:	e8 b0 bd ff ff       	call   801006a0 <cprintf>
  for (int i = 0; i < count; i++) {
801048f0:	83 c4 10             	add    $0x10,%esp
801048f3:	39 de                	cmp    %ebx,%esi
801048f5:	75 e9                	jne    801048e0 <printspaces+0x10>
  }
}
801048f7:	8d 65 f8             	lea    -0x8(%ebp),%esp
801048fa:	5b                   	pop    %ebx
801048fb:	5e                   	pop    %esi
801048fc:	5d                   	pop    %ebp
801048fd:	c3                   	ret    
801048fe:	66 90                	xchg   %ax,%ax

80104900 <printprocinfo>:

void printprocinfo(void) {
80104900:	55                   	push   %ebp
80104901:	89 e5                	mov    %esp,%ebp
80104903:	57                   	push   %edi
80104904:	56                   	push   %esi
80104905:	53                   	push   %ebx
80104906:	bb c0 2d 11 80       	mov    $0x80112dc0,%ebx
8010490b:	83 ec 18             	sub    $0x18,%esp
  struct proc *p;

  acquire(&ptable.lock);
8010490e:	68 20 2d 11 80       	push   $0x80112d20
80104913:	e8 c8 08 00 00       	call   801051e0 <acquire>

  cprintf("name            pid  state class   algorithm wait_time deadline cons_run arrival\n"
80104918:	c7 04 24 a8 85 10 80 	movl   $0x801085a8,(%esp)
8010491f:	e8 7c bd ff ff       	call   801006a0 <cprintf>
    "------------------------------------------------------------------------------\n");

  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
80104924:	83 c4 10             	add    $0x10,%esp
80104927:	eb 19                	jmp    80104942 <printprocinfo+0x42>
80104929:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104930:	81 c3 b8 00 00 00    	add    $0xb8,%ebx
80104936:	81 fb c0 5b 11 80    	cmp    $0x80115bc0,%ebx
8010493c:	0f 84 97 03 00 00    	je     80104cd9 <printprocinfo+0x3d9>
    if (p->state == UNUSED)
80104942:	8b 43 a0             	mov    -0x60(%ebx),%eax
80104945:	85 c0                	test   %eax,%eax
80104947:	74 e7                	je     80104930 <printprocinfo+0x30>
      continue;

    static int columns[] = { 16, 5, 10, 9, 10, 8, 8, 8, 8 };

    cprintf("%s", p->name);
80104949:	83 ec 08             	sub    $0x8,%esp
    printspaces(columns[0] - strlen(p->name));
8010494c:	be 10 00 00 00       	mov    $0x10,%esi
    cprintf("%s", p->name);
80104951:	53                   	push   %ebx
80104952:	68 35 84 10 80       	push   $0x80108435
80104957:	e8 44 bd ff ff       	call   801006a0 <cprintf>
    printspaces(columns[0] - strlen(p->name));
8010495c:	89 1c 24             	mov    %ebx,(%esp)
8010495f:	e8 3c 0b 00 00       	call   801054a0 <strlen>
  for (int i = 0; i < count; i++) {
80104964:	83 c4 10             	add    $0x10,%esp
    printspaces(columns[0] - strlen(p->name));
80104967:	29 c6                	sub    %eax,%esi
  for (int i = 0; i < count; i++) {
80104969:	85 f6                	test   %esi,%esi
8010496b:	7e 1a                	jle    80104987 <printprocinfo+0x87>
8010496d:	31 ff                	xor    %edi,%edi
8010496f:	90                   	nop
    cprintf(" ");
80104970:	83 ec 0c             	sub    $0xc,%esp
  for (int i = 0; i < count; i++) {
80104973:	83 c7 01             	add    $0x1,%edi
    cprintf(" ");
80104976:	68 de 84 10 80       	push   $0x801084de
8010497b:	e8 20 bd ff ff       	call   801006a0 <cprintf>
  for (int i = 0; i < count; i++) {
80104980:	83 c4 10             	add    $0x10,%esp
80104983:	39 fe                	cmp    %edi,%esi
80104985:	75 e9                	jne    80104970 <printprocinfo+0x70>

    cprintf("%d", p->pid);
80104987:	83 ec 08             	sub    $0x8,%esp
8010498a:	ff 73 a4             	push   -0x5c(%ebx)
8010498d:	68 5e 84 10 80       	push   $0x8010845e
80104992:	e8 09 bd ff ff       	call   801006a0 <cprintf>
    printspaces(columns[1] - count_digits(p->pid));
80104997:	8b 53 a4             	mov    -0x5c(%ebx),%edx
  if (number == 0)
8010499a:	83 c4 10             	add    $0x10,%esp
8010499d:	85 d2                	test   %edx,%edx
8010499f:	0f 84 fb 03 00 00    	je     80104da0 <printprocinfo+0x4a0>
  int count = 0;
801049a5:	b9 00 00 00 00       	mov    $0x0,%ecx
  if (number < 0) {
801049aa:	79 07                	jns    801049b3 <printprocinfo+0xb3>
      number = -number; 
801049ac:	f7 da                	neg    %edx
      count++;     
801049ae:	b9 01 00 00 00       	mov    $0x1,%ecx
      number /= 10; 
801049b3:	be cd cc cc cc       	mov    $0xcccccccd,%esi
801049b8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801049bf:	90                   	nop
801049c0:	89 d0                	mov    %edx,%eax
      count++;
801049c2:	83 c1 01             	add    $0x1,%ecx
      number /= 10; 
801049c5:	f7 e6                	mul    %esi
  while (number > 0) {
801049c7:	c1 ea 03             	shr    $0x3,%edx
801049ca:	75 f4                	jne    801049c0 <printprocinfo+0xc0>
    printspaces(columns[1] - count_digits(p->pid));
801049cc:	be 05 00 00 00       	mov    $0x5,%esi
801049d1:	29 ce                	sub    %ecx,%esi
  for (int i = 0; i < count; i++) {
801049d3:	85 f6                	test   %esi,%esi
801049d5:	7e 20                	jle    801049f7 <printprocinfo+0xf7>
    printspaces(columns[1] - count_digits(p->pid));
801049d7:	31 ff                	xor    %edi,%edi
801049d9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    cprintf(" ");
801049e0:	83 ec 0c             	sub    $0xc,%esp
  for (int i = 0; i < count; i++) {
801049e3:	83 c7 01             	add    $0x1,%edi
    cprintf(" ");
801049e6:	68 de 84 10 80       	push   $0x801084de
801049eb:	e8 b0 bc ff ff       	call   801006a0 <cprintf>
  for (int i = 0; i < count; i++) {
801049f0:	83 c4 10             	add    $0x10,%esp
801049f3:	39 f7                	cmp    %esi,%edi
801049f5:	7c e9                	jl     801049e0 <printprocinfo+0xe0>

    char *stateStr = getStateString(p->state);
801049f7:	8b 43 a0             	mov    -0x60(%ebx),%eax
801049fa:	be 4e 84 10 80       	mov    $0x8010844e,%esi
801049ff:	83 f8 05             	cmp    $0x5,%eax
80104a02:	77 07                	ja     80104a0b <printprocinfo+0x10b>
80104a04:	8b 34 85 64 86 10 80 	mov    -0x7fef799c(,%eax,4),%esi
    cprintf("%s", stateStr);
80104a0b:	83 ec 08             	sub    $0x8,%esp
80104a0e:	56                   	push   %esi
80104a0f:	68 35 84 10 80       	push   $0x80108435
80104a14:	e8 87 bc ff ff       	call   801006a0 <cprintf>
    printspaces(columns[2] - strlen(stateStr));
80104a19:	89 34 24             	mov    %esi,(%esp)
80104a1c:	be 0a 00 00 00       	mov    $0xa,%esi
80104a21:	e8 7a 0a 00 00       	call   801054a0 <strlen>
  for (int i = 0; i < count; i++) {
80104a26:	83 c4 10             	add    $0x10,%esp
    printspaces(columns[2] - strlen(stateStr));
80104a29:	29 c6                	sub    %eax,%esi
  for (int i = 0; i < count; i++) {
80104a2b:	85 f6                	test   %esi,%esi
80104a2d:	7e 20                	jle    80104a4f <printprocinfo+0x14f>
80104a2f:	31 ff                	xor    %edi,%edi
80104a31:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    cprintf(" ");
80104a38:	83 ec 0c             	sub    $0xc,%esp
  for (int i = 0; i < count; i++) {
80104a3b:	83 c7 01             	add    $0x1,%edi
    cprintf(" ");
80104a3e:	68 de 84 10 80       	push   $0x801084de
80104a43:	e8 58 bc ff ff       	call   801006a0 <cprintf>
  for (int i = 0; i < count; i++) {
80104a48:	83 c4 10             	add    $0x10,%esp
80104a4b:	39 fe                	cmp    %edi,%esi
80104a4d:	75 e9                	jne    80104a38 <printprocinfo+0x138>

    char *classStr = getClassString(p->queue);
80104a4f:	8b 43 10             	mov    0x10(%ebx),%eax
80104a52:	be 56 84 10 80       	mov    $0x80108456,%esi
80104a57:	83 f8 02             	cmp    $0x2,%eax
80104a5a:	77 07                	ja     80104a63 <printprocinfo+0x163>
80104a5c:	8b 34 85 58 86 10 80 	mov    -0x7fef79a8(,%eax,4),%esi
    cprintf("%s", classStr);
80104a63:	83 ec 08             	sub    $0x8,%esp
80104a66:	56                   	push   %esi
80104a67:	68 35 84 10 80       	push   $0x80108435
80104a6c:	e8 2f bc ff ff       	call   801006a0 <cprintf>
    printspaces(columns[3] - strlen(classStr));
80104a71:	89 34 24             	mov    %esi,(%esp)
80104a74:	be 09 00 00 00       	mov    $0x9,%esi
80104a79:	e8 22 0a 00 00       	call   801054a0 <strlen>
  for (int i = 0; i < count; i++) {
80104a7e:	83 c4 10             	add    $0x10,%esp
    printspaces(columns[3] - strlen(classStr));
80104a81:	29 c6                	sub    %eax,%esi
  for (int i = 0; i < count; i++) {
80104a83:	85 f6                	test   %esi,%esi
80104a85:	7e 20                	jle    80104aa7 <printprocinfo+0x1a7>
80104a87:	31 ff                	xor    %edi,%edi
80104a89:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    cprintf(" ");
80104a90:	83 ec 0c             	sub    $0xc,%esp
  for (int i = 0; i < count; i++) {
80104a93:	83 c7 01             	add    $0x1,%edi
    cprintf(" ");
80104a96:	68 de 84 10 80       	push   $0x801084de
80104a9b:	e8 00 bc ff ff       	call   801006a0 <cprintf>
  for (int i = 0; i < count; i++) {
80104aa0:	83 c4 10             	add    $0x10,%esp
80104aa3:	39 fe                	cmp    %edi,%esi
80104aa5:	75 e9                	jne    80104a90 <printprocinfo+0x190>
  switch (p->queue) {
80104aa7:	8b 43 10             	mov    0x10(%ebx),%eax
80104aaa:	be 56 84 10 80       	mov    $0x80108456,%esi
80104aaf:	83 f8 02             	cmp    $0x2,%eax
80104ab2:	77 07                	ja     80104abb <printprocinfo+0x1bb>
80104ab4:	8b 34 85 4c 86 10 80 	mov    -0x7fef79b4(,%eax,4),%esi

    char *algoStr = getAlgorithmString(p);
    cprintf("%s", algoStr);
80104abb:	83 ec 08             	sub    $0x8,%esp
80104abe:	56                   	push   %esi
80104abf:	68 35 84 10 80       	push   $0x80108435
80104ac4:	e8 d7 bb ff ff       	call   801006a0 <cprintf>
    printspaces(columns[4] - strlen(algoStr));
80104ac9:	89 34 24             	mov    %esi,(%esp)
80104acc:	be 0a 00 00 00       	mov    $0xa,%esi
80104ad1:	e8 ca 09 00 00       	call   801054a0 <strlen>
  for (int i = 0; i < count; i++) {
80104ad6:	83 c4 10             	add    $0x10,%esp
    printspaces(columns[4] - strlen(algoStr));
80104ad9:	29 c6                	sub    %eax,%esi
  for (int i = 0; i < count; i++) {
80104adb:	85 f6                	test   %esi,%esi
80104add:	7e 20                	jle    80104aff <printprocinfo+0x1ff>
80104adf:	31 ff                	xor    %edi,%edi
80104ae1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    cprintf(" ");
80104ae8:	83 ec 0c             	sub    $0xc,%esp
  for (int i = 0; i < count; i++) {
80104aeb:	83 c7 01             	add    $0x1,%edi
    cprintf(" ");
80104aee:	68 de 84 10 80       	push   $0x801084de
80104af3:	e8 a8 bb ff ff       	call   801006a0 <cprintf>
  for (int i = 0; i < count; i++) {
80104af8:	83 c4 10             	add    $0x10,%esp
80104afb:	39 fe                	cmp    %edi,%esi
80104afd:	75 e9                	jne    80104ae8 <printprocinfo+0x1e8>

    cprintf("%d", p->wait_time);
80104aff:	83 ec 08             	sub    $0x8,%esp
80104b02:	ff 73 40             	push   0x40(%ebx)
80104b05:	68 5e 84 10 80       	push   $0x8010845e
80104b0a:	e8 91 bb ff ff       	call   801006a0 <cprintf>
    printspaces(columns[5] - count_digits(p->wait_time));
80104b0f:	8b 53 40             	mov    0x40(%ebx),%edx
  if (number == 0)
80104b12:	83 c4 10             	add    $0x10,%esp
80104b15:	85 d2                	test   %edx,%edx
80104b17:	0f 84 73 02 00 00    	je     80104d90 <printprocinfo+0x490>
  int count = 0;
80104b1d:	b9 00 00 00 00       	mov    $0x0,%ecx
  if (number < 0) {
80104b22:	79 07                	jns    80104b2b <printprocinfo+0x22b>
      number = -number; 
80104b24:	f7 da                	neg    %edx
      count++;     
80104b26:	b9 01 00 00 00       	mov    $0x1,%ecx
      number /= 10; 
80104b2b:	be cd cc cc cc       	mov    $0xcccccccd,%esi
80104b30:	89 d0                	mov    %edx,%eax
      count++;
80104b32:	83 c1 01             	add    $0x1,%ecx
      number /= 10; 
80104b35:	f7 e6                	mul    %esi
  while (number > 0) {
80104b37:	c1 ea 03             	shr    $0x3,%edx
80104b3a:	75 f4                	jne    80104b30 <printprocinfo+0x230>
    printspaces(columns[5] - count_digits(p->wait_time));
80104b3c:	be 08 00 00 00       	mov    $0x8,%esi
80104b41:	29 ce                	sub    %ecx,%esi
  for (int i = 0; i < count; i++) {
80104b43:	85 f6                	test   %esi,%esi
80104b45:	7e 20                	jle    80104b67 <printprocinfo+0x267>
    printspaces(columns[5] - count_digits(p->wait_time));
80104b47:	31 ff                	xor    %edi,%edi
80104b49:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    cprintf(" ");
80104b50:	83 ec 0c             	sub    $0xc,%esp
  for (int i = 0; i < count; i++) {
80104b53:	83 c7 01             	add    $0x1,%edi
    cprintf(" ");
80104b56:	68 de 84 10 80       	push   $0x801084de
80104b5b:	e8 40 bb ff ff       	call   801006a0 <cprintf>
  for (int i = 0; i < count; i++) {
80104b60:	83 c4 10             	add    $0x10,%esp
80104b63:	39 f7                	cmp    %esi,%edi
80104b65:	7c e9                	jl     80104b50 <printprocinfo+0x250>

    cprintf("%d", p->deadline);
80104b67:	83 ec 08             	sub    $0x8,%esp
80104b6a:	ff 73 18             	push   0x18(%ebx)
80104b6d:	68 5e 84 10 80       	push   $0x8010845e
80104b72:	e8 29 bb ff ff       	call   801006a0 <cprintf>
    printspaces(columns[6] - count_digits(p->deadline));
80104b77:	8b 53 18             	mov    0x18(%ebx),%edx
  if (number == 0)
80104b7a:	83 c4 10             	add    $0x10,%esp
80104b7d:	85 d2                	test   %edx,%edx
80104b7f:	0f 84 fb 01 00 00    	je     80104d80 <printprocinfo+0x480>
  int count = 0;
80104b85:	b9 00 00 00 00       	mov    $0x0,%ecx
  if (number < 0) {
80104b8a:	79 07                	jns    80104b93 <printprocinfo+0x293>
      number = -number; 
80104b8c:	f7 da                	neg    %edx
      count++;     
80104b8e:	b9 01 00 00 00       	mov    $0x1,%ecx
      number /= 10; 
80104b93:	be cd cc cc cc       	mov    $0xcccccccd,%esi
80104b98:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104b9f:	90                   	nop
80104ba0:	89 d0                	mov    %edx,%eax
      count++;
80104ba2:	83 c1 01             	add    $0x1,%ecx
      number /= 10; 
80104ba5:	f7 e6                	mul    %esi
  while (number > 0) {
80104ba7:	c1 ea 03             	shr    $0x3,%edx
80104baa:	75 f4                	jne    80104ba0 <printprocinfo+0x2a0>
    printspaces(columns[6] - count_digits(p->deadline));
80104bac:	be 08 00 00 00       	mov    $0x8,%esi
80104bb1:	29 ce                	sub    %ecx,%esi
  for (int i = 0; i < count; i++) {
80104bb3:	85 f6                	test   %esi,%esi
80104bb5:	7e 20                	jle    80104bd7 <printprocinfo+0x2d7>
    printspaces(columns[6] - count_digits(p->deadline));
80104bb7:	31 ff                	xor    %edi,%edi
80104bb9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    cprintf(" ");
80104bc0:	83 ec 0c             	sub    $0xc,%esp
  for (int i = 0; i < count; i++) {
80104bc3:	83 c7 01             	add    $0x1,%edi
    cprintf(" ");
80104bc6:	68 de 84 10 80       	push   $0x801084de
80104bcb:	e8 d0 ba ff ff       	call   801006a0 <cprintf>
  for (int i = 0; i < count; i++) {
80104bd0:	83 c4 10             	add    $0x10,%esp
80104bd3:	39 f7                	cmp    %esi,%edi
80104bd5:	7c e9                	jl     80104bc0 <printprocinfo+0x2c0>

    cprintf("%d", p->consecutive_run_ticks);
80104bd7:	83 ec 08             	sub    $0x8,%esp
80104bda:	ff 73 44             	push   0x44(%ebx)
80104bdd:	68 5e 84 10 80       	push   $0x8010845e
80104be2:	e8 b9 ba ff ff       	call   801006a0 <cprintf>
    printspaces(columns[7] - count_digits(p->consecutive_run_ticks));
80104be7:	8b 53 44             	mov    0x44(%ebx),%edx
  if (number == 0)
80104bea:	83 c4 10             	add    $0x10,%esp
80104bed:	85 d2                	test   %edx,%edx
80104bef:	0f 84 7b 01 00 00    	je     80104d70 <printprocinfo+0x470>
  int count = 0;
80104bf5:	b9 00 00 00 00       	mov    $0x0,%ecx
  if (number < 0) {
80104bfa:	79 07                	jns    80104c03 <printprocinfo+0x303>
      number = -number; 
80104bfc:	f7 da                	neg    %edx
      count++;     
80104bfe:	b9 01 00 00 00       	mov    $0x1,%ecx
      number /= 10; 
80104c03:	be cd cc cc cc       	mov    $0xcccccccd,%esi
80104c08:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104c0f:	90                   	nop
80104c10:	89 d0                	mov    %edx,%eax
      count++;
80104c12:	83 c1 01             	add    $0x1,%ecx
      number /= 10; 
80104c15:	f7 e6                	mul    %esi
  while (number > 0) {
80104c17:	c1 ea 03             	shr    $0x3,%edx
80104c1a:	75 f4                	jne    80104c10 <printprocinfo+0x310>
    printspaces(columns[7] - count_digits(p->consecutive_run_ticks));
80104c1c:	be 08 00 00 00       	mov    $0x8,%esi
80104c21:	29 ce                	sub    %ecx,%esi
  for (int i = 0; i < count; i++) {
80104c23:	85 f6                	test   %esi,%esi
80104c25:	7e 20                	jle    80104c47 <printprocinfo+0x347>
    printspaces(columns[7] - count_digits(p->consecutive_run_ticks));
80104c27:	31 ff                	xor    %edi,%edi
80104c29:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    cprintf(" ");
80104c30:	83 ec 0c             	sub    $0xc,%esp
  for (int i = 0; i < count; i++) {
80104c33:	83 c7 01             	add    $0x1,%edi
    cprintf(" ");
80104c36:	68 de 84 10 80       	push   $0x801084de
80104c3b:	e8 60 ba ff ff       	call   801006a0 <cprintf>
  for (int i = 0; i < count; i++) {
80104c40:	83 c4 10             	add    $0x10,%esp
80104c43:	39 f7                	cmp    %esi,%edi
80104c45:	7c e9                	jl     80104c30 <printprocinfo+0x330>

    cprintf("%d", p->queue_arrival_time);
80104c47:	83 ec 08             	sub    $0x8,%esp
80104c4a:	ff 73 48             	push   0x48(%ebx)
80104c4d:	68 5e 84 10 80       	push   $0x8010845e
80104c52:	e8 49 ba ff ff       	call   801006a0 <cprintf>
    printspaces(columns[8] - count_digits(p->queue_arrival_time));
80104c57:	8b 53 48             	mov    0x48(%ebx),%edx
  if (number == 0)
80104c5a:	83 c4 10             	add    $0x10,%esp
80104c5d:	85 d2                	test   %edx,%edx
80104c5f:	0f 84 fb 00 00 00    	je     80104d60 <printprocinfo+0x460>
  int count = 0;
80104c65:	b9 00 00 00 00       	mov    $0x0,%ecx
  if (number < 0) {
80104c6a:	79 07                	jns    80104c73 <printprocinfo+0x373>
      number = -number; 
80104c6c:	f7 da                	neg    %edx
      count++;     
80104c6e:	b9 01 00 00 00       	mov    $0x1,%ecx
      number /= 10; 
80104c73:	be cd cc cc cc       	mov    $0xcccccccd,%esi
80104c78:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104c7f:	90                   	nop
80104c80:	89 d0                	mov    %edx,%eax
      count++;
80104c82:	83 c1 01             	add    $0x1,%ecx
      number /= 10; 
80104c85:	f7 e6                	mul    %esi
  while (number > 0) {
80104c87:	c1 ea 03             	shr    $0x3,%edx
80104c8a:	75 f4                	jne    80104c80 <printprocinfo+0x380>
    printspaces(columns[8] - count_digits(p->queue_arrival_time));
80104c8c:	be 08 00 00 00       	mov    $0x8,%esi
80104c91:	29 ce                	sub    %ecx,%esi
  for (int i = 0; i < count; i++) {
80104c93:	85 f6                	test   %esi,%esi
80104c95:	7e 20                	jle    80104cb7 <printprocinfo+0x3b7>
    printspaces(columns[8] - count_digits(p->queue_arrival_time));
80104c97:	31 ff                	xor    %edi,%edi
80104c99:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    cprintf(" ");
80104ca0:	83 ec 0c             	sub    $0xc,%esp
  for (int i = 0; i < count; i++) {
80104ca3:	83 c7 01             	add    $0x1,%edi
    cprintf(" ");
80104ca6:	68 de 84 10 80       	push   $0x801084de
80104cab:	e8 f0 b9 ff ff       	call   801006a0 <cprintf>
  for (int i = 0; i < count; i++) {
80104cb0:	83 c4 10             	add    $0x10,%esp
80104cb3:	39 f7                	cmp    %esi,%edi
80104cb5:	7c e9                	jl     80104ca0 <printprocinfo+0x3a0>

    cprintf("\n");
80104cb7:	83 ec 0c             	sub    $0xc,%esp
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
80104cba:	81 c3 b8 00 00 00    	add    $0xb8,%ebx
    cprintf("\n");
80104cc0:	68 4c 84 10 80       	push   $0x8010844c
80104cc5:	e8 d6 b9 ff ff       	call   801006a0 <cprintf>
80104cca:	83 c4 10             	add    $0x10,%esp
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
80104ccd:	81 fb c0 5b 11 80    	cmp    $0x80115bc0,%ebx
80104cd3:	0f 85 69 fc ff ff    	jne    80104942 <printprocinfo+0x42>
  }
  cprintf("\n");
80104cd9:	83 ec 0c             	sub    $0xc,%esp
80104cdc:	68 4c 84 10 80       	push   $0x8010844c
80104ce1:	e8 ba b9 ff ff       	call   801006a0 <cprintf>
  cprintf("%d", ptable.class1_num);
80104ce6:	58                   	pop    %eax
80104ce7:	5a                   	pop    %edx
80104ce8:	ff 35 54 5b 11 80    	push   0x80115b54
80104cee:	68 5e 84 10 80       	push   $0x8010845e
80104cf3:	e8 a8 b9 ff ff       	call   801006a0 <cprintf>
  cprintf("\n");
80104cf8:	c7 04 24 4c 84 10 80 	movl   $0x8010844c,(%esp)
80104cff:	e8 9c b9 ff ff       	call   801006a0 <cprintf>
  cprintf("%d", ptable.rr_num);
80104d04:	59                   	pop    %ecx
80104d05:	5b                   	pop    %ebx
80104d06:	ff 35 58 5b 11 80    	push   0x80115b58
80104d0c:	68 5e 84 10 80       	push   $0x8010845e
80104d11:	e8 8a b9 ff ff       	call   801006a0 <cprintf>
  cprintf("\n");
80104d16:	c7 04 24 4c 84 10 80 	movl   $0x8010844c,(%esp)
80104d1d:	e8 7e b9 ff ff       	call   801006a0 <cprintf>
  cprintf("%d", ptable.fcfs_num);
80104d22:	5e                   	pop    %esi
80104d23:	5f                   	pop    %edi
80104d24:	ff 35 5c 5b 11 80    	push   0x80115b5c
80104d2a:	68 5e 84 10 80       	push   $0x8010845e
80104d2f:	e8 6c b9 ff ff       	call   801006a0 <cprintf>
  cprintf("\n");
80104d34:	c7 04 24 4c 84 10 80 	movl   $0x8010844c,(%esp)
80104d3b:	e8 60 b9 ff ff       	call   801006a0 <cprintf>


  release(&ptable.lock);
80104d40:	c7 04 24 20 2d 11 80 	movl   $0x80112d20,(%esp)
80104d47:	e8 34 04 00 00       	call   80105180 <release>
}
80104d4c:	83 c4 10             	add    $0x10,%esp
80104d4f:	8d 65 f4             	lea    -0xc(%ebp),%esp
80104d52:	5b                   	pop    %ebx
80104d53:	5e                   	pop    %esi
80104d54:	5f                   	pop    %edi
80104d55:	5d                   	pop    %ebp
80104d56:	c3                   	ret    
80104d57:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104d5e:	66 90                	xchg   %ax,%ax
    printspaces(columns[8] - count_digits(p->queue_arrival_time));
80104d60:	be 07 00 00 00       	mov    $0x7,%esi
80104d65:	e9 2d ff ff ff       	jmp    80104c97 <printprocinfo+0x397>
80104d6a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    printspaces(columns[7] - count_digits(p->consecutive_run_ticks));
80104d70:	be 07 00 00 00       	mov    $0x7,%esi
80104d75:	e9 ad fe ff ff       	jmp    80104c27 <printprocinfo+0x327>
80104d7a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    printspaces(columns[6] - count_digits(p->deadline));
80104d80:	be 07 00 00 00       	mov    $0x7,%esi
80104d85:	e9 2d fe ff ff       	jmp    80104bb7 <printprocinfo+0x2b7>
80104d8a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    printspaces(columns[5] - count_digits(p->wait_time));
80104d90:	be 07 00 00 00       	mov    $0x7,%esi
80104d95:	e9 ad fd ff ff       	jmp    80104b47 <printprocinfo+0x247>
80104d9a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    printspaces(columns[1] - count_digits(p->pid));
80104da0:	be 04 00 00 00       	mov    $0x4,%esi
80104da5:	e9 2d fc ff ff       	jmp    801049d7 <printprocinfo+0xd7>
80104daa:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80104db0 <sys_dl_proc>:

int sys_dl_proc(void){
80104db0:	55                   	push   %ebp
80104db1:	89 e5                	mov    %esp,%ebp
80104db3:	83 ec 20             	sub    $0x20,%esp
  int pid, dead_line;
  
    if(argint(0, &pid) < 0)
80104db6:	8d 45 f0             	lea    -0x10(%ebp),%eax
80104db9:	50                   	push   %eax
80104dba:	6a 00                	push   $0x0
80104dbc:	e8 9f 07 00 00       	call   80105560 <argint>
80104dc1:	83 c4 10             	add    $0x10,%esp
80104dc4:	85 c0                	test   %eax,%eax
80104dc6:	0f 88 8c 00 00 00    	js     80104e58 <sys_dl_proc+0xa8>
        return -1;
    if(argint(1, &dead_line) < 0)
80104dcc:	83 ec 08             	sub    $0x8,%esp
80104dcf:	8d 45 f4             	lea    -0xc(%ebp),%eax
80104dd2:	50                   	push   %eax
80104dd3:	6a 01                	push   $0x1
80104dd5:	e8 86 07 00 00       	call   80105560 <argint>
80104dda:	83 c4 10             	add    $0x10,%esp
80104ddd:	85 c0                	test   %eax,%eax
80104ddf:	78 77                	js     80104e58 <sys_dl_proc+0xa8>
        return -1;
    int flag = 0;
    acquire(&ptable.lock);
80104de1:	83 ec 0c             	sub    $0xc,%esp
80104de4:	68 20 2d 11 80       	push   $0x80112d20
80104de9:	e8 f2 03 00 00       	call   801051e0 <acquire>
    for(int index = 0; index < NPROC; index++) {
      if(ptable.proc[index].pid == pid) {
80104dee:	8b 4d f0             	mov    -0x10(%ebp),%ecx
80104df1:	ba 64 2d 11 80       	mov    $0x80112d64,%edx
80104df6:	83 c4 10             	add    $0x10,%esp
    for(int index = 0; index < NPROC; index++) {
80104df9:	31 c0                	xor    %eax,%eax
80104dfb:	eb 11                	jmp    80104e0e <sys_dl_proc+0x5e>
80104dfd:	8d 76 00             	lea    0x0(%esi),%esi
80104e00:	83 c0 01             	add    $0x1,%eax
80104e03:	81 c2 b8 00 00 00    	add    $0xb8,%edx
80104e09:	83 f8 40             	cmp    $0x40,%eax
80104e0c:	74 52                	je     80104e60 <sys_dl_proc+0xb0>
      if(ptable.proc[index].pid == pid) {
80104e0e:	39 0a                	cmp    %ecx,(%edx)
80104e10:	75 ee                	jne    80104e00 <sys_dl_proc+0x50>
        ptable.proc[index].deadline = dead_line;
80104e12:	69 d0 b8 00 00 00    	imul   $0xb8,%eax,%edx
80104e18:	8b 4d f4             	mov    -0xc(%ebp),%ecx
80104e1b:	81 c2 20 2d 11 80    	add    $0x80112d20,%edx
        if(ptable.proc[index].state == RUNNABLE) {
80104e21:	83 7a 40 03          	cmpl   $0x3,0x40(%edx)
        ptable.proc[index].deadline = dead_line;
80104e25:	89 8a b8 00 00 00    	mov    %ecx,0xb8(%edx)
        if(ptable.proc[index].state == RUNNABLE) {
80104e2b:	74 5b                	je     80104e88 <sys_dl_proc+0xd8>
          change_num(&ptable.proc[index], -1);
          ptable.proc[index].queue = CLASS1;
          change_num(&ptable.proc[index], 1);
        }
        else {
          ptable.proc[index].queue = CLASS1;
80104e2d:	c7 82 b0 00 00 00 00 	movl   $0x0,0xb0(%edx)
80104e34:	00 00 00 
        }
        flag = 1;
        break;
      }
    }
    release(&ptable.lock);
80104e37:	83 ec 0c             	sub    $0xc,%esp
80104e3a:	68 20 2d 11 80       	push   $0x80112d20
80104e3f:	e8 3c 03 00 00       	call   80105180 <release>
80104e44:	83 c4 10             	add    $0x10,%esp
    if(flag == 1){
      printprocinfo();
80104e47:	e8 b4 fa ff ff       	call   80104900 <printprocinfo>
    }
    else {
      cprintf("proc not found!");

    }
    return 1;
80104e4c:	b8 01 00 00 00       	mov    $0x1,%eax
80104e51:	c9                   	leave  
80104e52:	c3                   	ret    
80104e53:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80104e57:	90                   	nop
80104e58:	c9                   	leave  
        return -1;
80104e59:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104e5e:	c3                   	ret    
80104e5f:	90                   	nop
    release(&ptable.lock);
80104e60:	83 ec 0c             	sub    $0xc,%esp
80104e63:	68 20 2d 11 80       	push   $0x80112d20
80104e68:	e8 13 03 00 00       	call   80105180 <release>
      cprintf("proc not found!");
80104e6d:	c7 04 24 61 84 10 80 	movl   $0x80108461,(%esp)
80104e74:	e8 27 b8 ff ff       	call   801006a0 <cprintf>
80104e79:	83 c4 10             	add    $0x10,%esp
    return 1;
80104e7c:	b8 01 00 00 00       	mov    $0x1,%eax
80104e81:	c9                   	leave  
80104e82:	c3                   	ret    
80104e83:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80104e87:	90                   	nop
  if(p->queue==CLASS1){
80104e88:	8b 8a b0 00 00 00    	mov    0xb0(%edx),%ecx
    ptable.class1_num = ptable.class1_num + inc_or_dec;
80104e8e:	8b 15 54 5b 11 80    	mov    0x80115b54,%edx
  if(p->queue==CLASS1){
80104e94:	85 c9                	test   %ecx,%ecx
80104e96:	74 0f                	je     80104ea7 <sys_dl_proc+0xf7>
    ptable.class1_num = ptable.class1_num + inc_or_dec;
80104e98:	83 c2 01             	add    $0x1,%edx
    if(p->queue==CLASS2_RR){
80104e9b:	83 f9 01             	cmp    $0x1,%ecx
80104e9e:	74 32                	je     80104ed2 <sys_dl_proc+0x122>
      ptable.fcfs_num = ptable.fcfs_num + inc_or_dec;
80104ea0:	83 2d 5c 5b 11 80 01 	subl   $0x1,0x80115b5c
    release(&ptable.lock);
80104ea7:	83 ec 0c             	sub    $0xc,%esp
          ptable.proc[index].queue = CLASS1;
80104eaa:	69 c0 b8 00 00 00    	imul   $0xb8,%eax,%eax
    ptable.class1_num = ptable.class1_num + inc_or_dec;
80104eb0:	89 15 54 5b 11 80    	mov    %edx,0x80115b54
    release(&ptable.lock);
80104eb6:	68 20 2d 11 80       	push   $0x80112d20
          ptable.proc[index].queue = CLASS1;
80104ebb:	c7 80 d0 2d 11 80 00 	movl   $0x0,-0x7feed230(%eax)
80104ec2:	00 00 00 
    release(&ptable.lock);
80104ec5:	e8 b6 02 00 00       	call   80105180 <release>
80104eca:	83 c4 10             	add    $0x10,%esp
80104ecd:	e9 75 ff ff ff       	jmp    80104e47 <sys_dl_proc+0x97>
      ptable.rr_num = ptable.rr_num + inc_or_dec;
80104ed2:	83 2d 58 5b 11 80 01 	subl   $0x1,0x80115b58
80104ed9:	eb cc                	jmp    80104ea7 <sys_dl_proc+0xf7>
80104edb:	66 90                	xchg   %ax,%ax
80104edd:	66 90                	xchg   %ax,%ax
80104edf:	90                   	nop

80104ee0 <initsleeplock>:
#include "spinlock.h"
#include "sleeplock.h"

void
initsleeplock(struct sleeplock *lk, char *name)
{
80104ee0:	55                   	push   %ebp
80104ee1:	89 e5                	mov    %esp,%ebp
80104ee3:	53                   	push   %ebx
80104ee4:	83 ec 0c             	sub    $0xc,%esp
80104ee7:	8b 5d 08             	mov    0x8(%ebp),%ebx
  initlock(&lk->lk, "sleep lock");
80104eea:	68 94 86 10 80       	push   $0x80108694
80104eef:	8d 43 04             	lea    0x4(%ebx),%eax
80104ef2:	50                   	push   %eax
80104ef3:	e8 18 01 00 00       	call   80105010 <initlock>
  lk->name = name;
80104ef8:	8b 45 0c             	mov    0xc(%ebp),%eax
  lk->locked = 0;
80104efb:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
  lk->pid = 0;
}
80104f01:	83 c4 10             	add    $0x10,%esp
  lk->pid = 0;
80104f04:	c7 43 3c 00 00 00 00 	movl   $0x0,0x3c(%ebx)
  lk->name = name;
80104f0b:	89 43 38             	mov    %eax,0x38(%ebx)
}
80104f0e:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80104f11:	c9                   	leave  
80104f12:	c3                   	ret    
80104f13:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104f1a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80104f20 <acquiresleep>:

void
acquiresleep(struct sleeplock *lk)
{
80104f20:	55                   	push   %ebp
80104f21:	89 e5                	mov    %esp,%ebp
80104f23:	56                   	push   %esi
80104f24:	53                   	push   %ebx
80104f25:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquire(&lk->lk);
80104f28:	8d 73 04             	lea    0x4(%ebx),%esi
80104f2b:	83 ec 0c             	sub    $0xc,%esp
80104f2e:	56                   	push   %esi
80104f2f:	e8 ac 02 00 00       	call   801051e0 <acquire>
  while (lk->locked) {
80104f34:	8b 13                	mov    (%ebx),%edx
80104f36:	83 c4 10             	add    $0x10,%esp
80104f39:	85 d2                	test   %edx,%edx
80104f3b:	74 16                	je     80104f53 <acquiresleep+0x33>
80104f3d:	8d 76 00             	lea    0x0(%esi),%esi
    sleep(lk, &lk->lk);
80104f40:	83 ec 08             	sub    $0x8,%esp
80104f43:	56                   	push   %esi
80104f44:	53                   	push   %ebx
80104f45:	e8 d6 f4 ff ff       	call   80104420 <sleep>
  while (lk->locked) {
80104f4a:	8b 03                	mov    (%ebx),%eax
80104f4c:	83 c4 10             	add    $0x10,%esp
80104f4f:	85 c0                	test   %eax,%eax
80104f51:	75 ed                	jne    80104f40 <acquiresleep+0x20>
  }
  lk->locked = 1;
80104f53:	c7 03 01 00 00 00    	movl   $0x1,(%ebx)
  lk->pid = myproc()->pid;
80104f59:	e8 12 eb ff ff       	call   80103a70 <myproc>
80104f5e:	8b 40 10             	mov    0x10(%eax),%eax
80104f61:	89 43 3c             	mov    %eax,0x3c(%ebx)
  release(&lk->lk);
80104f64:	89 75 08             	mov    %esi,0x8(%ebp)
}
80104f67:	8d 65 f8             	lea    -0x8(%ebp),%esp
80104f6a:	5b                   	pop    %ebx
80104f6b:	5e                   	pop    %esi
80104f6c:	5d                   	pop    %ebp
  release(&lk->lk);
80104f6d:	e9 0e 02 00 00       	jmp    80105180 <release>
80104f72:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104f79:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80104f80 <releasesleep>:

void
releasesleep(struct sleeplock *lk)
{
80104f80:	55                   	push   %ebp
80104f81:	89 e5                	mov    %esp,%ebp
80104f83:	56                   	push   %esi
80104f84:	53                   	push   %ebx
80104f85:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquire(&lk->lk);
80104f88:	8d 73 04             	lea    0x4(%ebx),%esi
80104f8b:	83 ec 0c             	sub    $0xc,%esp
80104f8e:	56                   	push   %esi
80104f8f:	e8 4c 02 00 00       	call   801051e0 <acquire>
  lk->locked = 0;
80104f94:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
  lk->pid = 0;
80104f9a:	c7 43 3c 00 00 00 00 	movl   $0x0,0x3c(%ebx)
  wakeup(lk);
80104fa1:	89 1c 24             	mov    %ebx,(%esp)
80104fa4:	e8 37 f5 ff ff       	call   801044e0 <wakeup>
  release(&lk->lk);
80104fa9:	89 75 08             	mov    %esi,0x8(%ebp)
80104fac:	83 c4 10             	add    $0x10,%esp
}
80104faf:	8d 65 f8             	lea    -0x8(%ebp),%esp
80104fb2:	5b                   	pop    %ebx
80104fb3:	5e                   	pop    %esi
80104fb4:	5d                   	pop    %ebp
  release(&lk->lk);
80104fb5:	e9 c6 01 00 00       	jmp    80105180 <release>
80104fba:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80104fc0 <holdingsleep>:

int
holdingsleep(struct sleeplock *lk)
{
80104fc0:	55                   	push   %ebp
80104fc1:	89 e5                	mov    %esp,%ebp
80104fc3:	57                   	push   %edi
80104fc4:	31 ff                	xor    %edi,%edi
80104fc6:	56                   	push   %esi
80104fc7:	53                   	push   %ebx
80104fc8:	83 ec 18             	sub    $0x18,%esp
80104fcb:	8b 5d 08             	mov    0x8(%ebp),%ebx
  int r;
  
  acquire(&lk->lk);
80104fce:	8d 73 04             	lea    0x4(%ebx),%esi
80104fd1:	56                   	push   %esi
80104fd2:	e8 09 02 00 00       	call   801051e0 <acquire>
  r = lk->locked && (lk->pid == myproc()->pid);
80104fd7:	8b 03                	mov    (%ebx),%eax
80104fd9:	83 c4 10             	add    $0x10,%esp
80104fdc:	85 c0                	test   %eax,%eax
80104fde:	75 18                	jne    80104ff8 <holdingsleep+0x38>
  release(&lk->lk);
80104fe0:	83 ec 0c             	sub    $0xc,%esp
80104fe3:	56                   	push   %esi
80104fe4:	e8 97 01 00 00       	call   80105180 <release>
  return r;
}
80104fe9:	8d 65 f4             	lea    -0xc(%ebp),%esp
80104fec:	89 f8                	mov    %edi,%eax
80104fee:	5b                   	pop    %ebx
80104fef:	5e                   	pop    %esi
80104ff0:	5f                   	pop    %edi
80104ff1:	5d                   	pop    %ebp
80104ff2:	c3                   	ret    
80104ff3:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80104ff7:	90                   	nop
  r = lk->locked && (lk->pid == myproc()->pid);
80104ff8:	8b 5b 3c             	mov    0x3c(%ebx),%ebx
80104ffb:	e8 70 ea ff ff       	call   80103a70 <myproc>
80105000:	39 58 10             	cmp    %ebx,0x10(%eax)
80105003:	0f 94 c0             	sete   %al
80105006:	0f b6 c0             	movzbl %al,%eax
80105009:	89 c7                	mov    %eax,%edi
8010500b:	eb d3                	jmp    80104fe0 <holdingsleep+0x20>
8010500d:	66 90                	xchg   %ax,%ax
8010500f:	90                   	nop

80105010 <initlock>:
#include "proc.h"
#include "spinlock.h"

void
initlock(struct spinlock *lk, char *name)
{
80105010:	55                   	push   %ebp
80105011:	89 e5                	mov    %esp,%ebp
80105013:	8b 45 08             	mov    0x8(%ebp),%eax
  lk->name = name;
80105016:	8b 55 0c             	mov    0xc(%ebp),%edx
  lk->locked = 0;
80105019:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  lk->name = name;
8010501f:	89 50 04             	mov    %edx,0x4(%eax)
  lk->cpu = 0;
80105022:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
}
80105029:	5d                   	pop    %ebp
8010502a:	c3                   	ret    
8010502b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010502f:	90                   	nop

80105030 <getcallerpcs>:
}

// Record the current call stack in pcs[] by following the %ebp chain.
void
getcallerpcs(void *v, uint pcs[])
{
80105030:	55                   	push   %ebp
  uint *ebp;
  int i;

  ebp = (uint*)v - 2;
  for(i = 0; i < 10; i++){
80105031:	31 d2                	xor    %edx,%edx
{
80105033:	89 e5                	mov    %esp,%ebp
80105035:	53                   	push   %ebx
  ebp = (uint*)v - 2;
80105036:	8b 45 08             	mov    0x8(%ebp),%eax
{
80105039:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  ebp = (uint*)v - 2;
8010503c:	83 e8 08             	sub    $0x8,%eax
  for(i = 0; i < 10; i++){
8010503f:	90                   	nop
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
80105040:	8d 98 00 00 00 80    	lea    -0x80000000(%eax),%ebx
80105046:	81 fb fe ff ff 7f    	cmp    $0x7ffffffe,%ebx
8010504c:	77 1a                	ja     80105068 <getcallerpcs+0x38>
      break;
    pcs[i] = ebp[1];     // saved %eip
8010504e:	8b 58 04             	mov    0x4(%eax),%ebx
80105051:	89 1c 91             	mov    %ebx,(%ecx,%edx,4)
  for(i = 0; i < 10; i++){
80105054:	83 c2 01             	add    $0x1,%edx
    ebp = (uint*)ebp[0]; // saved %ebp
80105057:	8b 00                	mov    (%eax),%eax
  for(i = 0; i < 10; i++){
80105059:	83 fa 0a             	cmp    $0xa,%edx
8010505c:	75 e2                	jne    80105040 <getcallerpcs+0x10>
  }
  for(; i < 10; i++)
    pcs[i] = 0;
}
8010505e:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80105061:	c9                   	leave  
80105062:	c3                   	ret    
80105063:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80105067:	90                   	nop
  for(; i < 10; i++)
80105068:	8d 04 91             	lea    (%ecx,%edx,4),%eax
8010506b:	8d 51 28             	lea    0x28(%ecx),%edx
8010506e:	66 90                	xchg   %ax,%ax
    pcs[i] = 0;
80105070:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  for(; i < 10; i++)
80105076:	83 c0 04             	add    $0x4,%eax
80105079:	39 d0                	cmp    %edx,%eax
8010507b:	75 f3                	jne    80105070 <getcallerpcs+0x40>
}
8010507d:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80105080:	c9                   	leave  
80105081:	c3                   	ret    
80105082:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105089:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80105090 <pushcli>:
// it takes two popcli to undo two pushcli.  Also, if interrupts
// are off, then pushcli, popcli leaves them off.

void
pushcli(void)
{
80105090:	55                   	push   %ebp
80105091:	89 e5                	mov    %esp,%ebp
80105093:	53                   	push   %ebx
80105094:	83 ec 04             	sub    $0x4,%esp
80105097:	9c                   	pushf  
80105098:	5b                   	pop    %ebx
  asm volatile("cli");
80105099:	fa                   	cli    
  int eflags;

  eflags = readeflags();
  cli();
  if(mycpu()->ncli == 0)
8010509a:	e8 51 e9 ff ff       	call   801039f0 <mycpu>
8010509f:	8b 80 a4 00 00 00    	mov    0xa4(%eax),%eax
801050a5:	85 c0                	test   %eax,%eax
801050a7:	74 17                	je     801050c0 <pushcli+0x30>
    mycpu()->intena = eflags & FL_IF;
  mycpu()->ncli += 1;
801050a9:	e8 42 e9 ff ff       	call   801039f0 <mycpu>
801050ae:	83 80 a4 00 00 00 01 	addl   $0x1,0xa4(%eax)
}
801050b5:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801050b8:	c9                   	leave  
801050b9:	c3                   	ret    
801050ba:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    mycpu()->intena = eflags & FL_IF;
801050c0:	e8 2b e9 ff ff       	call   801039f0 <mycpu>
801050c5:	81 e3 00 02 00 00    	and    $0x200,%ebx
801050cb:	89 98 a8 00 00 00    	mov    %ebx,0xa8(%eax)
801050d1:	eb d6                	jmp    801050a9 <pushcli+0x19>
801050d3:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801050da:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

801050e0 <popcli>:

void
popcli(void)
{
801050e0:	55                   	push   %ebp
801050e1:	89 e5                	mov    %esp,%ebp
801050e3:	83 ec 08             	sub    $0x8,%esp
  asm volatile("pushfl; popl %0" : "=r" (eflags));
801050e6:	9c                   	pushf  
801050e7:	58                   	pop    %eax
  if(readeflags()&FL_IF)
801050e8:	f6 c4 02             	test   $0x2,%ah
801050eb:	75 35                	jne    80105122 <popcli+0x42>
    panic("popcli - interruptible");
  if(--mycpu()->ncli < 0)
801050ed:	e8 fe e8 ff ff       	call   801039f0 <mycpu>
801050f2:	83 a8 a4 00 00 00 01 	subl   $0x1,0xa4(%eax)
801050f9:	78 34                	js     8010512f <popcli+0x4f>
    panic("popcli");
  if(mycpu()->ncli == 0 && mycpu()->intena)
801050fb:	e8 f0 e8 ff ff       	call   801039f0 <mycpu>
80105100:	8b 90 a4 00 00 00    	mov    0xa4(%eax),%edx
80105106:	85 d2                	test   %edx,%edx
80105108:	74 06                	je     80105110 <popcli+0x30>
    sti();
}
8010510a:	c9                   	leave  
8010510b:	c3                   	ret    
8010510c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  if(mycpu()->ncli == 0 && mycpu()->intena)
80105110:	e8 db e8 ff ff       	call   801039f0 <mycpu>
80105115:	8b 80 a8 00 00 00    	mov    0xa8(%eax),%eax
8010511b:	85 c0                	test   %eax,%eax
8010511d:	74 eb                	je     8010510a <popcli+0x2a>
  asm volatile("sti");
8010511f:	fb                   	sti    
}
80105120:	c9                   	leave  
80105121:	c3                   	ret    
    panic("popcli - interruptible");
80105122:	83 ec 0c             	sub    $0xc,%esp
80105125:	68 9f 86 10 80       	push   $0x8010869f
8010512a:	e8 51 b2 ff ff       	call   80100380 <panic>
    panic("popcli");
8010512f:	83 ec 0c             	sub    $0xc,%esp
80105132:	68 b6 86 10 80       	push   $0x801086b6
80105137:	e8 44 b2 ff ff       	call   80100380 <panic>
8010513c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80105140 <holding>:
{
80105140:	55                   	push   %ebp
80105141:	89 e5                	mov    %esp,%ebp
80105143:	56                   	push   %esi
80105144:	53                   	push   %ebx
80105145:	8b 75 08             	mov    0x8(%ebp),%esi
80105148:	31 db                	xor    %ebx,%ebx
  pushcli();
8010514a:	e8 41 ff ff ff       	call   80105090 <pushcli>
  r = lock->locked && lock->cpu == mycpu();
8010514f:	8b 06                	mov    (%esi),%eax
80105151:	85 c0                	test   %eax,%eax
80105153:	75 0b                	jne    80105160 <holding+0x20>
  popcli();
80105155:	e8 86 ff ff ff       	call   801050e0 <popcli>
}
8010515a:	89 d8                	mov    %ebx,%eax
8010515c:	5b                   	pop    %ebx
8010515d:	5e                   	pop    %esi
8010515e:	5d                   	pop    %ebp
8010515f:	c3                   	ret    
  r = lock->locked && lock->cpu == mycpu();
80105160:	8b 5e 08             	mov    0x8(%esi),%ebx
80105163:	e8 88 e8 ff ff       	call   801039f0 <mycpu>
80105168:	39 c3                	cmp    %eax,%ebx
8010516a:	0f 94 c3             	sete   %bl
  popcli();
8010516d:	e8 6e ff ff ff       	call   801050e0 <popcli>
  r = lock->locked && lock->cpu == mycpu();
80105172:	0f b6 db             	movzbl %bl,%ebx
}
80105175:	89 d8                	mov    %ebx,%eax
80105177:	5b                   	pop    %ebx
80105178:	5e                   	pop    %esi
80105179:	5d                   	pop    %ebp
8010517a:	c3                   	ret    
8010517b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010517f:	90                   	nop

80105180 <release>:
{
80105180:	55                   	push   %ebp
80105181:	89 e5                	mov    %esp,%ebp
80105183:	56                   	push   %esi
80105184:	53                   	push   %ebx
80105185:	8b 5d 08             	mov    0x8(%ebp),%ebx
  pushcli();
80105188:	e8 03 ff ff ff       	call   80105090 <pushcli>
  r = lock->locked && lock->cpu == mycpu();
8010518d:	8b 03                	mov    (%ebx),%eax
8010518f:	85 c0                	test   %eax,%eax
80105191:	75 15                	jne    801051a8 <release+0x28>
  popcli();
80105193:	e8 48 ff ff ff       	call   801050e0 <popcli>
    panic("release");
80105198:	83 ec 0c             	sub    $0xc,%esp
8010519b:	68 bd 86 10 80       	push   $0x801086bd
801051a0:	e8 db b1 ff ff       	call   80100380 <panic>
801051a5:	8d 76 00             	lea    0x0(%esi),%esi
  r = lock->locked && lock->cpu == mycpu();
801051a8:	8b 73 08             	mov    0x8(%ebx),%esi
801051ab:	e8 40 e8 ff ff       	call   801039f0 <mycpu>
801051b0:	39 c6                	cmp    %eax,%esi
801051b2:	75 df                	jne    80105193 <release+0x13>
  popcli();
801051b4:	e8 27 ff ff ff       	call   801050e0 <popcli>
  lk->pcs[0] = 0;
801051b9:	c7 43 0c 00 00 00 00 	movl   $0x0,0xc(%ebx)
  lk->cpu = 0;
801051c0:	c7 43 08 00 00 00 00 	movl   $0x0,0x8(%ebx)
  __sync_synchronize();
801051c7:	f0 83 0c 24 00       	lock orl $0x0,(%esp)
  asm volatile("movl $0, %0" : "+m" (lk->locked) : );
801051cc:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
}
801051d2:	8d 65 f8             	lea    -0x8(%ebp),%esp
801051d5:	5b                   	pop    %ebx
801051d6:	5e                   	pop    %esi
801051d7:	5d                   	pop    %ebp
  popcli();
801051d8:	e9 03 ff ff ff       	jmp    801050e0 <popcli>
801051dd:	8d 76 00             	lea    0x0(%esi),%esi

801051e0 <acquire>:
{
801051e0:	55                   	push   %ebp
801051e1:	89 e5                	mov    %esp,%ebp
801051e3:	53                   	push   %ebx
801051e4:	83 ec 04             	sub    $0x4,%esp
  pushcli(); // disable interrupts to avoid deadlock.
801051e7:	e8 a4 fe ff ff       	call   80105090 <pushcli>
  if(holding(lk))
801051ec:	8b 5d 08             	mov    0x8(%ebp),%ebx
  pushcli();
801051ef:	e8 9c fe ff ff       	call   80105090 <pushcli>
  r = lock->locked && lock->cpu == mycpu();
801051f4:	8b 03                	mov    (%ebx),%eax
801051f6:	85 c0                	test   %eax,%eax
801051f8:	75 7e                	jne    80105278 <acquire+0x98>
  popcli();
801051fa:	e8 e1 fe ff ff       	call   801050e0 <popcli>
  asm volatile("lock; xchgl %0, %1" :
801051ff:	b9 01 00 00 00       	mov    $0x1,%ecx
80105204:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  while(xchg(&lk->locked, 1) != 0)
80105208:	8b 55 08             	mov    0x8(%ebp),%edx
8010520b:	89 c8                	mov    %ecx,%eax
8010520d:	f0 87 02             	lock xchg %eax,(%edx)
80105210:	85 c0                	test   %eax,%eax
80105212:	75 f4                	jne    80105208 <acquire+0x28>
  __sync_synchronize();
80105214:	f0 83 0c 24 00       	lock orl $0x0,(%esp)
  lk->cpu = mycpu();
80105219:	8b 5d 08             	mov    0x8(%ebp),%ebx
8010521c:	e8 cf e7 ff ff       	call   801039f0 <mycpu>
  getcallerpcs(&lk, lk->pcs);
80105221:	8b 4d 08             	mov    0x8(%ebp),%ecx
  ebp = (uint*)v - 2;
80105224:	89 ea                	mov    %ebp,%edx
  lk->cpu = mycpu();
80105226:	89 43 08             	mov    %eax,0x8(%ebx)
  for(i = 0; i < 10; i++){
80105229:	31 c0                	xor    %eax,%eax
8010522b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010522f:	90                   	nop
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
80105230:	8d 9a 00 00 00 80    	lea    -0x80000000(%edx),%ebx
80105236:	81 fb fe ff ff 7f    	cmp    $0x7ffffffe,%ebx
8010523c:	77 1a                	ja     80105258 <acquire+0x78>
    pcs[i] = ebp[1];     // saved %eip
8010523e:	8b 5a 04             	mov    0x4(%edx),%ebx
80105241:	89 5c 81 0c          	mov    %ebx,0xc(%ecx,%eax,4)
  for(i = 0; i < 10; i++){
80105245:	83 c0 01             	add    $0x1,%eax
    ebp = (uint*)ebp[0]; // saved %ebp
80105248:	8b 12                	mov    (%edx),%edx
  for(i = 0; i < 10; i++){
8010524a:	83 f8 0a             	cmp    $0xa,%eax
8010524d:	75 e1                	jne    80105230 <acquire+0x50>
}
8010524f:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80105252:	c9                   	leave  
80105253:	c3                   	ret    
80105254:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  for(; i < 10; i++)
80105258:	8d 44 81 0c          	lea    0xc(%ecx,%eax,4),%eax
8010525c:	8d 51 34             	lea    0x34(%ecx),%edx
8010525f:	90                   	nop
    pcs[i] = 0;
80105260:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  for(; i < 10; i++)
80105266:	83 c0 04             	add    $0x4,%eax
80105269:	39 c2                	cmp    %eax,%edx
8010526b:	75 f3                	jne    80105260 <acquire+0x80>
}
8010526d:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80105270:	c9                   	leave  
80105271:	c3                   	ret    
80105272:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  r = lock->locked && lock->cpu == mycpu();
80105278:	8b 5b 08             	mov    0x8(%ebx),%ebx
8010527b:	e8 70 e7 ff ff       	call   801039f0 <mycpu>
80105280:	39 c3                	cmp    %eax,%ebx
80105282:	0f 85 72 ff ff ff    	jne    801051fa <acquire+0x1a>
  popcli();
80105288:	e8 53 fe ff ff       	call   801050e0 <popcli>
    panic("acquire");
8010528d:	83 ec 0c             	sub    $0xc,%esp
80105290:	68 c5 86 10 80       	push   $0x801086c5
80105295:	e8 e6 b0 ff ff       	call   80100380 <panic>
8010529a:	66 90                	xchg   %ax,%ax
8010529c:	66 90                	xchg   %ax,%ax
8010529e:	66 90                	xchg   %ax,%ax

801052a0 <memset>:
#include "types.h"
#include "x86.h"

void*
memset(void *dst, int c, uint n)
{
801052a0:	55                   	push   %ebp
801052a1:	89 e5                	mov    %esp,%ebp
801052a3:	57                   	push   %edi
801052a4:	8b 55 08             	mov    0x8(%ebp),%edx
801052a7:	8b 4d 10             	mov    0x10(%ebp),%ecx
801052aa:	53                   	push   %ebx
801052ab:	8b 45 0c             	mov    0xc(%ebp),%eax
  if ((int)dst%4 == 0 && n%4 == 0){
801052ae:	89 d7                	mov    %edx,%edi
801052b0:	09 cf                	or     %ecx,%edi
801052b2:	83 e7 03             	and    $0x3,%edi
801052b5:	75 29                	jne    801052e0 <memset+0x40>
    c &= 0xFF;
801052b7:	0f b6 f8             	movzbl %al,%edi
    stosl(dst, (c<<24)|(c<<16)|(c<<8)|c, n/4);
801052ba:	c1 e0 18             	shl    $0x18,%eax
801052bd:	89 fb                	mov    %edi,%ebx
801052bf:	c1 e9 02             	shr    $0x2,%ecx
801052c2:	c1 e3 10             	shl    $0x10,%ebx
801052c5:	09 d8                	or     %ebx,%eax
801052c7:	09 f8                	or     %edi,%eax
801052c9:	c1 e7 08             	shl    $0x8,%edi
801052cc:	09 f8                	or     %edi,%eax
  asm volatile("cld; rep stosl" :
801052ce:	89 d7                	mov    %edx,%edi
801052d0:	fc                   	cld    
801052d1:	f3 ab                	rep stos %eax,%es:(%edi)
  } else
    stosb(dst, c, n);
  return dst;
}
801052d3:	5b                   	pop    %ebx
801052d4:	89 d0                	mov    %edx,%eax
801052d6:	5f                   	pop    %edi
801052d7:	5d                   	pop    %ebp
801052d8:	c3                   	ret    
801052d9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  asm volatile("cld; rep stosb" :
801052e0:	89 d7                	mov    %edx,%edi
801052e2:	fc                   	cld    
801052e3:	f3 aa                	rep stos %al,%es:(%edi)
801052e5:	5b                   	pop    %ebx
801052e6:	89 d0                	mov    %edx,%eax
801052e8:	5f                   	pop    %edi
801052e9:	5d                   	pop    %ebp
801052ea:	c3                   	ret    
801052eb:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801052ef:	90                   	nop

801052f0 <memcmp>:

int
memcmp(const void *v1, const void *v2, uint n)
{
801052f0:	55                   	push   %ebp
801052f1:	89 e5                	mov    %esp,%ebp
801052f3:	56                   	push   %esi
801052f4:	8b 75 10             	mov    0x10(%ebp),%esi
801052f7:	8b 55 08             	mov    0x8(%ebp),%edx
801052fa:	53                   	push   %ebx
801052fb:	8b 45 0c             	mov    0xc(%ebp),%eax
  const uchar *s1, *s2;

  s1 = v1;
  s2 = v2;
  while(n-- > 0){
801052fe:	85 f6                	test   %esi,%esi
80105300:	74 2e                	je     80105330 <memcmp+0x40>
80105302:	01 c6                	add    %eax,%esi
80105304:	eb 14                	jmp    8010531a <memcmp+0x2a>
80105306:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010530d:	8d 76 00             	lea    0x0(%esi),%esi
    if(*s1 != *s2)
      return *s1 - *s2;
    s1++, s2++;
80105310:	83 c0 01             	add    $0x1,%eax
80105313:	83 c2 01             	add    $0x1,%edx
  while(n-- > 0){
80105316:	39 f0                	cmp    %esi,%eax
80105318:	74 16                	je     80105330 <memcmp+0x40>
    if(*s1 != *s2)
8010531a:	0f b6 0a             	movzbl (%edx),%ecx
8010531d:	0f b6 18             	movzbl (%eax),%ebx
80105320:	38 d9                	cmp    %bl,%cl
80105322:	74 ec                	je     80105310 <memcmp+0x20>
      return *s1 - *s2;
80105324:	0f b6 c1             	movzbl %cl,%eax
80105327:	29 d8                	sub    %ebx,%eax
  }

  return 0;
}
80105329:	5b                   	pop    %ebx
8010532a:	5e                   	pop    %esi
8010532b:	5d                   	pop    %ebp
8010532c:	c3                   	ret    
8010532d:	8d 76 00             	lea    0x0(%esi),%esi
80105330:	5b                   	pop    %ebx
  return 0;
80105331:	31 c0                	xor    %eax,%eax
}
80105333:	5e                   	pop    %esi
80105334:	5d                   	pop    %ebp
80105335:	c3                   	ret    
80105336:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010533d:	8d 76 00             	lea    0x0(%esi),%esi

80105340 <memmove>:

void*
memmove(void *dst, const void *src, uint n)
{
80105340:	55                   	push   %ebp
80105341:	89 e5                	mov    %esp,%ebp
80105343:	57                   	push   %edi
80105344:	8b 55 08             	mov    0x8(%ebp),%edx
80105347:	8b 4d 10             	mov    0x10(%ebp),%ecx
8010534a:	56                   	push   %esi
8010534b:	8b 75 0c             	mov    0xc(%ebp),%esi
  const char *s;
  char *d;

  s = src;
  d = dst;
  if(s < d && s + n > d){
8010534e:	39 d6                	cmp    %edx,%esi
80105350:	73 26                	jae    80105378 <memmove+0x38>
80105352:	8d 3c 0e             	lea    (%esi,%ecx,1),%edi
80105355:	39 fa                	cmp    %edi,%edx
80105357:	73 1f                	jae    80105378 <memmove+0x38>
80105359:	8d 41 ff             	lea    -0x1(%ecx),%eax
    s += n;
    d += n;
    while(n-- > 0)
8010535c:	85 c9                	test   %ecx,%ecx
8010535e:	74 0c                	je     8010536c <memmove+0x2c>
      *--d = *--s;
80105360:	0f b6 0c 06          	movzbl (%esi,%eax,1),%ecx
80105364:	88 0c 02             	mov    %cl,(%edx,%eax,1)
    while(n-- > 0)
80105367:	83 e8 01             	sub    $0x1,%eax
8010536a:	73 f4                	jae    80105360 <memmove+0x20>
  } else
    while(n-- > 0)
      *d++ = *s++;

  return dst;
}
8010536c:	5e                   	pop    %esi
8010536d:	89 d0                	mov    %edx,%eax
8010536f:	5f                   	pop    %edi
80105370:	5d                   	pop    %ebp
80105371:	c3                   	ret    
80105372:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    while(n-- > 0)
80105378:	8d 04 0e             	lea    (%esi,%ecx,1),%eax
8010537b:	89 d7                	mov    %edx,%edi
8010537d:	85 c9                	test   %ecx,%ecx
8010537f:	74 eb                	je     8010536c <memmove+0x2c>
80105381:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
      *d++ = *s++;
80105388:	a4                   	movsb  %ds:(%esi),%es:(%edi)
    while(n-- > 0)
80105389:	39 c6                	cmp    %eax,%esi
8010538b:	75 fb                	jne    80105388 <memmove+0x48>
}
8010538d:	5e                   	pop    %esi
8010538e:	89 d0                	mov    %edx,%eax
80105390:	5f                   	pop    %edi
80105391:	5d                   	pop    %ebp
80105392:	c3                   	ret    
80105393:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010539a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

801053a0 <memcpy>:

// memcpy exists to placate GCC.  Use memmove.
void*
memcpy(void *dst, const void *src, uint n)
{
  return memmove(dst, src, n);
801053a0:	eb 9e                	jmp    80105340 <memmove>
801053a2:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801053a9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

801053b0 <strncmp>:
}

int
strncmp(const char *p, const char *q, uint n)
{
801053b0:	55                   	push   %ebp
801053b1:	89 e5                	mov    %esp,%ebp
801053b3:	56                   	push   %esi
801053b4:	8b 75 10             	mov    0x10(%ebp),%esi
801053b7:	8b 4d 08             	mov    0x8(%ebp),%ecx
801053ba:	53                   	push   %ebx
801053bb:	8b 55 0c             	mov    0xc(%ebp),%edx
  while(n > 0 && *p && *p == *q)
801053be:	85 f6                	test   %esi,%esi
801053c0:	74 2e                	je     801053f0 <strncmp+0x40>
801053c2:	01 d6                	add    %edx,%esi
801053c4:	eb 18                	jmp    801053de <strncmp+0x2e>
801053c6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801053cd:	8d 76 00             	lea    0x0(%esi),%esi
801053d0:	38 d8                	cmp    %bl,%al
801053d2:	75 14                	jne    801053e8 <strncmp+0x38>
    n--, p++, q++;
801053d4:	83 c2 01             	add    $0x1,%edx
801053d7:	83 c1 01             	add    $0x1,%ecx
  while(n > 0 && *p && *p == *q)
801053da:	39 f2                	cmp    %esi,%edx
801053dc:	74 12                	je     801053f0 <strncmp+0x40>
801053de:	0f b6 01             	movzbl (%ecx),%eax
801053e1:	0f b6 1a             	movzbl (%edx),%ebx
801053e4:	84 c0                	test   %al,%al
801053e6:	75 e8                	jne    801053d0 <strncmp+0x20>
  if(n == 0)
    return 0;
  return (uchar)*p - (uchar)*q;
801053e8:	29 d8                	sub    %ebx,%eax
}
801053ea:	5b                   	pop    %ebx
801053eb:	5e                   	pop    %esi
801053ec:	5d                   	pop    %ebp
801053ed:	c3                   	ret    
801053ee:	66 90                	xchg   %ax,%ax
801053f0:	5b                   	pop    %ebx
    return 0;
801053f1:	31 c0                	xor    %eax,%eax
}
801053f3:	5e                   	pop    %esi
801053f4:	5d                   	pop    %ebp
801053f5:	c3                   	ret    
801053f6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801053fd:	8d 76 00             	lea    0x0(%esi),%esi

80105400 <strncpy>:

char*
strncpy(char *s, const char *t, int n)
{
80105400:	55                   	push   %ebp
80105401:	89 e5                	mov    %esp,%ebp
80105403:	57                   	push   %edi
80105404:	56                   	push   %esi
80105405:	8b 75 08             	mov    0x8(%ebp),%esi
80105408:	53                   	push   %ebx
80105409:	8b 4d 10             	mov    0x10(%ebp),%ecx
  char *os;

  os = s;
  while(n-- > 0 && (*s++ = *t++) != 0)
8010540c:	89 f0                	mov    %esi,%eax
8010540e:	eb 15                	jmp    80105425 <strncpy+0x25>
80105410:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
80105414:	8b 7d 0c             	mov    0xc(%ebp),%edi
80105417:	83 c0 01             	add    $0x1,%eax
8010541a:	0f b6 57 ff          	movzbl -0x1(%edi),%edx
8010541e:	88 50 ff             	mov    %dl,-0x1(%eax)
80105421:	84 d2                	test   %dl,%dl
80105423:	74 09                	je     8010542e <strncpy+0x2e>
80105425:	89 cb                	mov    %ecx,%ebx
80105427:	83 e9 01             	sub    $0x1,%ecx
8010542a:	85 db                	test   %ebx,%ebx
8010542c:	7f e2                	jg     80105410 <strncpy+0x10>
    ;
  while(n-- > 0)
8010542e:	89 c2                	mov    %eax,%edx
80105430:	85 c9                	test   %ecx,%ecx
80105432:	7e 17                	jle    8010544b <strncpy+0x4b>
80105434:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    *s++ = 0;
80105438:	83 c2 01             	add    $0x1,%edx
8010543b:	89 c1                	mov    %eax,%ecx
8010543d:	c6 42 ff 00          	movb   $0x0,-0x1(%edx)
  while(n-- > 0)
80105441:	29 d1                	sub    %edx,%ecx
80105443:	8d 4c 0b ff          	lea    -0x1(%ebx,%ecx,1),%ecx
80105447:	85 c9                	test   %ecx,%ecx
80105449:	7f ed                	jg     80105438 <strncpy+0x38>
  return os;
}
8010544b:	5b                   	pop    %ebx
8010544c:	89 f0                	mov    %esi,%eax
8010544e:	5e                   	pop    %esi
8010544f:	5f                   	pop    %edi
80105450:	5d                   	pop    %ebp
80105451:	c3                   	ret    
80105452:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105459:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80105460 <safestrcpy>:

// Like strncpy but guaranteed to NUL-terminate.
char*
safestrcpy(char *s, const char *t, int n)
{
80105460:	55                   	push   %ebp
80105461:	89 e5                	mov    %esp,%ebp
80105463:	56                   	push   %esi
80105464:	8b 55 10             	mov    0x10(%ebp),%edx
80105467:	8b 75 08             	mov    0x8(%ebp),%esi
8010546a:	53                   	push   %ebx
8010546b:	8b 45 0c             	mov    0xc(%ebp),%eax
  char *os;

  os = s;
  if(n <= 0)
8010546e:	85 d2                	test   %edx,%edx
80105470:	7e 25                	jle    80105497 <safestrcpy+0x37>
80105472:	8d 5c 10 ff          	lea    -0x1(%eax,%edx,1),%ebx
80105476:	89 f2                	mov    %esi,%edx
80105478:	eb 16                	jmp    80105490 <safestrcpy+0x30>
8010547a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    return os;
  while(--n > 0 && (*s++ = *t++) != 0)
80105480:	0f b6 08             	movzbl (%eax),%ecx
80105483:	83 c0 01             	add    $0x1,%eax
80105486:	83 c2 01             	add    $0x1,%edx
80105489:	88 4a ff             	mov    %cl,-0x1(%edx)
8010548c:	84 c9                	test   %cl,%cl
8010548e:	74 04                	je     80105494 <safestrcpy+0x34>
80105490:	39 d8                	cmp    %ebx,%eax
80105492:	75 ec                	jne    80105480 <safestrcpy+0x20>
    ;
  *s = 0;
80105494:	c6 02 00             	movb   $0x0,(%edx)
  return os;
}
80105497:	89 f0                	mov    %esi,%eax
80105499:	5b                   	pop    %ebx
8010549a:	5e                   	pop    %esi
8010549b:	5d                   	pop    %ebp
8010549c:	c3                   	ret    
8010549d:	8d 76 00             	lea    0x0(%esi),%esi

801054a0 <strlen>:

int
strlen(const char *s)
{
801054a0:	55                   	push   %ebp
  int n;

  for(n = 0; s[n]; n++)
801054a1:	31 c0                	xor    %eax,%eax
{
801054a3:	89 e5                	mov    %esp,%ebp
801054a5:	8b 55 08             	mov    0x8(%ebp),%edx
  for(n = 0; s[n]; n++)
801054a8:	80 3a 00             	cmpb   $0x0,(%edx)
801054ab:	74 0c                	je     801054b9 <strlen+0x19>
801054ad:	8d 76 00             	lea    0x0(%esi),%esi
801054b0:	83 c0 01             	add    $0x1,%eax
801054b3:	80 3c 02 00          	cmpb   $0x0,(%edx,%eax,1)
801054b7:	75 f7                	jne    801054b0 <strlen+0x10>
    ;
  return n;
}
801054b9:	5d                   	pop    %ebp
801054ba:	c3                   	ret    

801054bb <swtch>:
# a struct context, and save its address in *old.
# Switch stacks to new and pop previously-saved registers.

.globl swtch
swtch:
  movl 4(%esp), %eax
801054bb:	8b 44 24 04          	mov    0x4(%esp),%eax
  movl 8(%esp), %edx
801054bf:	8b 54 24 08          	mov    0x8(%esp),%edx

  # Save old callee-saved registers
  pushl %ebp
801054c3:	55                   	push   %ebp
  pushl %ebx
801054c4:	53                   	push   %ebx
  pushl %esi
801054c5:	56                   	push   %esi
  pushl %edi
801054c6:	57                   	push   %edi

  # Switch stacks
  movl %esp, (%eax)
801054c7:	89 20                	mov    %esp,(%eax)
  movl %edx, %esp
801054c9:	89 d4                	mov    %edx,%esp

  # Load new callee-saved registers
  popl %edi
801054cb:	5f                   	pop    %edi
  popl %esi
801054cc:	5e                   	pop    %esi
  popl %ebx
801054cd:	5b                   	pop    %ebx
  popl %ebp
801054ce:	5d                   	pop    %ebp
  ret
801054cf:	c3                   	ret    

801054d0 <fetchint>:
// to a saved program counter, and then the first argument.

// Fetch the int at addr from the current process.
int
fetchint(uint addr, int *ip)
{
801054d0:	55                   	push   %ebp
801054d1:	89 e5                	mov    %esp,%ebp
801054d3:	53                   	push   %ebx
801054d4:	83 ec 04             	sub    $0x4,%esp
801054d7:	8b 5d 08             	mov    0x8(%ebp),%ebx
  struct proc *curproc = myproc();
801054da:	e8 91 e5 ff ff       	call   80103a70 <myproc>

  if(addr >= curproc->sz || addr+4 > curproc->sz)
801054df:	8b 00                	mov    (%eax),%eax
801054e1:	39 d8                	cmp    %ebx,%eax
801054e3:	76 1b                	jbe    80105500 <fetchint+0x30>
801054e5:	8d 53 04             	lea    0x4(%ebx),%edx
801054e8:	39 d0                	cmp    %edx,%eax
801054ea:	72 14                	jb     80105500 <fetchint+0x30>
    return -1;
  *ip = *(int*)(addr);
801054ec:	8b 45 0c             	mov    0xc(%ebp),%eax
801054ef:	8b 13                	mov    (%ebx),%edx
801054f1:	89 10                	mov    %edx,(%eax)
  return 0;
801054f3:	31 c0                	xor    %eax,%eax
}
801054f5:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801054f8:	c9                   	leave  
801054f9:	c3                   	ret    
801054fa:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    return -1;
80105500:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105505:	eb ee                	jmp    801054f5 <fetchint+0x25>
80105507:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010550e:	66 90                	xchg   %ax,%ax

80105510 <fetchstr>:
// Fetch the nul-terminated string at addr from the current process.
// Doesn't actually copy the string - just sets *pp to point at it.
// Returns length of string, not including nul.
int
fetchstr(uint addr, char **pp)
{
80105510:	55                   	push   %ebp
80105511:	89 e5                	mov    %esp,%ebp
80105513:	53                   	push   %ebx
80105514:	83 ec 04             	sub    $0x4,%esp
80105517:	8b 5d 08             	mov    0x8(%ebp),%ebx
  char *s, *ep;
  struct proc *curproc = myproc();
8010551a:	e8 51 e5 ff ff       	call   80103a70 <myproc>

  if(addr >= curproc->sz)
8010551f:	39 18                	cmp    %ebx,(%eax)
80105521:	76 2d                	jbe    80105550 <fetchstr+0x40>
    return -1;
  *pp = (char*)addr;
80105523:	8b 55 0c             	mov    0xc(%ebp),%edx
80105526:	89 1a                	mov    %ebx,(%edx)
  ep = (char*)curproc->sz;
80105528:	8b 10                	mov    (%eax),%edx
  for(s = *pp; s < ep; s++){
8010552a:	39 d3                	cmp    %edx,%ebx
8010552c:	73 22                	jae    80105550 <fetchstr+0x40>
8010552e:	89 d8                	mov    %ebx,%eax
80105530:	eb 0d                	jmp    8010553f <fetchstr+0x2f>
80105532:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80105538:	83 c0 01             	add    $0x1,%eax
8010553b:	39 c2                	cmp    %eax,%edx
8010553d:	76 11                	jbe    80105550 <fetchstr+0x40>
    if(*s == 0)
8010553f:	80 38 00             	cmpb   $0x0,(%eax)
80105542:	75 f4                	jne    80105538 <fetchstr+0x28>
      return s - *pp;
80105544:	29 d8                	sub    %ebx,%eax
  }
  return -1;
}
80105546:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80105549:	c9                   	leave  
8010554a:	c3                   	ret    
8010554b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010554f:	90                   	nop
80105550:	8b 5d fc             	mov    -0x4(%ebp),%ebx
    return -1;
80105553:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105558:	c9                   	leave  
80105559:	c3                   	ret    
8010555a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80105560 <argint>:

// Fetch the nth 32-bit system call argument.
int
argint(int n, int *ip)
{
80105560:	55                   	push   %ebp
80105561:	89 e5                	mov    %esp,%ebp
80105563:	56                   	push   %esi
80105564:	53                   	push   %ebx
  return fetchint((myproc()->tf->esp) + 4 + 4*n, ip);
80105565:	e8 06 e5 ff ff       	call   80103a70 <myproc>
8010556a:	8b 55 08             	mov    0x8(%ebp),%edx
8010556d:	8b 40 18             	mov    0x18(%eax),%eax
80105570:	8b 40 44             	mov    0x44(%eax),%eax
80105573:	8d 1c 90             	lea    (%eax,%edx,4),%ebx
  struct proc *curproc = myproc();
80105576:	e8 f5 e4 ff ff       	call   80103a70 <myproc>
  return fetchint((myproc()->tf->esp) + 4 + 4*n, ip);
8010557b:	8d 73 04             	lea    0x4(%ebx),%esi
  if(addr >= curproc->sz || addr+4 > curproc->sz)
8010557e:	8b 00                	mov    (%eax),%eax
80105580:	39 c6                	cmp    %eax,%esi
80105582:	73 1c                	jae    801055a0 <argint+0x40>
80105584:	8d 53 08             	lea    0x8(%ebx),%edx
80105587:	39 d0                	cmp    %edx,%eax
80105589:	72 15                	jb     801055a0 <argint+0x40>
  *ip = *(int*)(addr);
8010558b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010558e:	8b 53 04             	mov    0x4(%ebx),%edx
80105591:	89 10                	mov    %edx,(%eax)
  return 0;
80105593:	31 c0                	xor    %eax,%eax
}
80105595:	5b                   	pop    %ebx
80105596:	5e                   	pop    %esi
80105597:	5d                   	pop    %ebp
80105598:	c3                   	ret    
80105599:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    return -1;
801055a0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
  return fetchint((myproc()->tf->esp) + 4 + 4*n, ip);
801055a5:	eb ee                	jmp    80105595 <argint+0x35>
801055a7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801055ae:	66 90                	xchg   %ax,%ax

801055b0 <argptr>:
// Fetch the nth word-sized system call argument as a pointer
// to a block of memory of size bytes.  Check that the pointer
// lies within the process address space.
int
argptr(int n, char **pp, int size)
{
801055b0:	55                   	push   %ebp
801055b1:	89 e5                	mov    %esp,%ebp
801055b3:	57                   	push   %edi
801055b4:	56                   	push   %esi
801055b5:	53                   	push   %ebx
801055b6:	83 ec 0c             	sub    $0xc,%esp
  int i;
  struct proc *curproc = myproc();
801055b9:	e8 b2 e4 ff ff       	call   80103a70 <myproc>
801055be:	89 c6                	mov    %eax,%esi
  return fetchint((myproc()->tf->esp) + 4 + 4*n, ip);
801055c0:	e8 ab e4 ff ff       	call   80103a70 <myproc>
801055c5:	8b 55 08             	mov    0x8(%ebp),%edx
801055c8:	8b 40 18             	mov    0x18(%eax),%eax
801055cb:	8b 40 44             	mov    0x44(%eax),%eax
801055ce:	8d 1c 90             	lea    (%eax,%edx,4),%ebx
  struct proc *curproc = myproc();
801055d1:	e8 9a e4 ff ff       	call   80103a70 <myproc>
  return fetchint((myproc()->tf->esp) + 4 + 4*n, ip);
801055d6:	8d 7b 04             	lea    0x4(%ebx),%edi
  if(addr >= curproc->sz || addr+4 > curproc->sz)
801055d9:	8b 00                	mov    (%eax),%eax
801055db:	39 c7                	cmp    %eax,%edi
801055dd:	73 31                	jae    80105610 <argptr+0x60>
801055df:	8d 4b 08             	lea    0x8(%ebx),%ecx
801055e2:	39 c8                	cmp    %ecx,%eax
801055e4:	72 2a                	jb     80105610 <argptr+0x60>
 
  if(argint(n, &i) < 0)
    return -1;
  if(size < 0 || (uint)i >= curproc->sz || (uint)i+size > curproc->sz)
801055e6:	8b 55 10             	mov    0x10(%ebp),%edx
  *ip = *(int*)(addr);
801055e9:	8b 43 04             	mov    0x4(%ebx),%eax
  if(size < 0 || (uint)i >= curproc->sz || (uint)i+size > curproc->sz)
801055ec:	85 d2                	test   %edx,%edx
801055ee:	78 20                	js     80105610 <argptr+0x60>
801055f0:	8b 16                	mov    (%esi),%edx
801055f2:	39 c2                	cmp    %eax,%edx
801055f4:	76 1a                	jbe    80105610 <argptr+0x60>
801055f6:	8b 5d 10             	mov    0x10(%ebp),%ebx
801055f9:	01 c3                	add    %eax,%ebx
801055fb:	39 da                	cmp    %ebx,%edx
801055fd:	72 11                	jb     80105610 <argptr+0x60>
    return -1;
  *pp = (char*)i;
801055ff:	8b 55 0c             	mov    0xc(%ebp),%edx
80105602:	89 02                	mov    %eax,(%edx)
  return 0;
80105604:	31 c0                	xor    %eax,%eax
}
80105606:	83 c4 0c             	add    $0xc,%esp
80105609:	5b                   	pop    %ebx
8010560a:	5e                   	pop    %esi
8010560b:	5f                   	pop    %edi
8010560c:	5d                   	pop    %ebp
8010560d:	c3                   	ret    
8010560e:	66 90                	xchg   %ax,%ax
    return -1;
80105610:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105615:	eb ef                	jmp    80105606 <argptr+0x56>
80105617:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010561e:	66 90                	xchg   %ax,%ax

80105620 <argstr>:
// Check that the pointer is valid and the string is nul-terminated.
// (There is no shared writable memory, so the string can't change
// between this check and being used by the kernel.)
int
argstr(int n, char **pp)
{
80105620:	55                   	push   %ebp
80105621:	89 e5                	mov    %esp,%ebp
80105623:	56                   	push   %esi
80105624:	53                   	push   %ebx
  return fetchint((myproc()->tf->esp) + 4 + 4*n, ip);
80105625:	e8 46 e4 ff ff       	call   80103a70 <myproc>
8010562a:	8b 55 08             	mov    0x8(%ebp),%edx
8010562d:	8b 40 18             	mov    0x18(%eax),%eax
80105630:	8b 40 44             	mov    0x44(%eax),%eax
80105633:	8d 1c 90             	lea    (%eax,%edx,4),%ebx
  struct proc *curproc = myproc();
80105636:	e8 35 e4 ff ff       	call   80103a70 <myproc>
  return fetchint((myproc()->tf->esp) + 4 + 4*n, ip);
8010563b:	8d 73 04             	lea    0x4(%ebx),%esi
  if(addr >= curproc->sz || addr+4 > curproc->sz)
8010563e:	8b 00                	mov    (%eax),%eax
80105640:	39 c6                	cmp    %eax,%esi
80105642:	73 44                	jae    80105688 <argstr+0x68>
80105644:	8d 53 08             	lea    0x8(%ebx),%edx
80105647:	39 d0                	cmp    %edx,%eax
80105649:	72 3d                	jb     80105688 <argstr+0x68>
  *ip = *(int*)(addr);
8010564b:	8b 5b 04             	mov    0x4(%ebx),%ebx
  struct proc *curproc = myproc();
8010564e:	e8 1d e4 ff ff       	call   80103a70 <myproc>
  if(addr >= curproc->sz)
80105653:	3b 18                	cmp    (%eax),%ebx
80105655:	73 31                	jae    80105688 <argstr+0x68>
  *pp = (char*)addr;
80105657:	8b 55 0c             	mov    0xc(%ebp),%edx
8010565a:	89 1a                	mov    %ebx,(%edx)
  ep = (char*)curproc->sz;
8010565c:	8b 10                	mov    (%eax),%edx
  for(s = *pp; s < ep; s++){
8010565e:	39 d3                	cmp    %edx,%ebx
80105660:	73 26                	jae    80105688 <argstr+0x68>
80105662:	89 d8                	mov    %ebx,%eax
80105664:	eb 11                	jmp    80105677 <argstr+0x57>
80105666:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010566d:	8d 76 00             	lea    0x0(%esi),%esi
80105670:	83 c0 01             	add    $0x1,%eax
80105673:	39 c2                	cmp    %eax,%edx
80105675:	76 11                	jbe    80105688 <argstr+0x68>
    if(*s == 0)
80105677:	80 38 00             	cmpb   $0x0,(%eax)
8010567a:	75 f4                	jne    80105670 <argstr+0x50>
      return s - *pp;
8010567c:	29 d8                	sub    %ebx,%eax
  int addr;
  if(argint(n, &addr) < 0)
    return -1;
  return fetchstr(addr, pp);
}
8010567e:	5b                   	pop    %ebx
8010567f:	5e                   	pop    %esi
80105680:	5d                   	pop    %ebp
80105681:	c3                   	ret    
80105682:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80105688:	5b                   	pop    %ebx
    return -1;
80105689:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010568e:	5e                   	pop    %esi
8010568f:	5d                   	pop    %ebp
80105690:	c3                   	ret    
80105691:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105698:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010569f:	90                   	nop

801056a0 <syscall>:
[SYS_dl_proc]   sys_dl_proc, 
};

void
syscall(void)
{
801056a0:	55                   	push   %ebp
801056a1:	89 e5                	mov    %esp,%ebp
801056a3:	53                   	push   %ebx
801056a4:	83 ec 04             	sub    $0x4,%esp
  int num;
  struct proc *curproc = myproc();
801056a7:	e8 c4 e3 ff ff       	call   80103a70 <myproc>
801056ac:	89 c3                	mov    %eax,%ebx

  num = curproc->tf->eax;
801056ae:	8b 40 18             	mov    0x18(%eax),%eax
801056b1:	8b 40 1c             	mov    0x1c(%eax),%eax
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
801056b4:	8d 50 ff             	lea    -0x1(%eax),%edx
801056b7:	83 fa 17             	cmp    $0x17,%edx
801056ba:	77 24                	ja     801056e0 <syscall+0x40>
801056bc:	8b 14 85 00 87 10 80 	mov    -0x7fef7900(,%eax,4),%edx
801056c3:	85 d2                	test   %edx,%edx
801056c5:	74 19                	je     801056e0 <syscall+0x40>
    curproc->tf->eax = syscalls[num]();
801056c7:	ff d2                	call   *%edx
801056c9:	89 c2                	mov    %eax,%edx
801056cb:	8b 43 18             	mov    0x18(%ebx),%eax
801056ce:	89 50 1c             	mov    %edx,0x1c(%eax)
  } else {
    cprintf("%d %s: unknown sys call %d\n",
            curproc->pid, curproc->name, num);
    curproc->tf->eax = -1;
  }
}
801056d1:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801056d4:	c9                   	leave  
801056d5:	c3                   	ret    
801056d6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801056dd:	8d 76 00             	lea    0x0(%esi),%esi
    cprintf("%d %s: unknown sys call %d\n",
801056e0:	50                   	push   %eax
            curproc->pid, curproc->name, num);
801056e1:	8d 43 6c             	lea    0x6c(%ebx),%eax
    cprintf("%d %s: unknown sys call %d\n",
801056e4:	50                   	push   %eax
801056e5:	ff 73 10             	push   0x10(%ebx)
801056e8:	68 cd 86 10 80       	push   $0x801086cd
801056ed:	e8 ae af ff ff       	call   801006a0 <cprintf>
    curproc->tf->eax = -1;
801056f2:	8b 43 18             	mov    0x18(%ebx),%eax
801056f5:	83 c4 10             	add    $0x10,%esp
801056f8:	c7 40 1c ff ff ff ff 	movl   $0xffffffff,0x1c(%eax)
}
801056ff:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80105702:	c9                   	leave  
80105703:	c3                   	ret    
80105704:	66 90                	xchg   %ax,%ax
80105706:	66 90                	xchg   %ax,%ax
80105708:	66 90                	xchg   %ax,%ax
8010570a:	66 90                	xchg   %ax,%ax
8010570c:	66 90                	xchg   %ax,%ax
8010570e:	66 90                	xchg   %ax,%ax

80105710 <create>:
  return -1;
}

static struct inode*
create(char *path, short type, short major, short minor)
{
80105710:	55                   	push   %ebp
80105711:	89 e5                	mov    %esp,%ebp
80105713:	57                   	push   %edi
80105714:	56                   	push   %esi
  struct inode *ip, *dp;
  char name[DIRSIZ];

  if((dp = nameiparent(path, name)) == 0)
80105715:	8d 7d da             	lea    -0x26(%ebp),%edi
{
80105718:	53                   	push   %ebx
80105719:	83 ec 34             	sub    $0x34,%esp
8010571c:	89 4d d0             	mov    %ecx,-0x30(%ebp)
8010571f:	8b 4d 08             	mov    0x8(%ebp),%ecx
  if((dp = nameiparent(path, name)) == 0)
80105722:	57                   	push   %edi
80105723:	50                   	push   %eax
{
80105724:	89 55 d4             	mov    %edx,-0x2c(%ebp)
80105727:	89 4d cc             	mov    %ecx,-0x34(%ebp)
  if((dp = nameiparent(path, name)) == 0)
8010572a:	e8 41 ca ff ff       	call   80102170 <nameiparent>
8010572f:	83 c4 10             	add    $0x10,%esp
80105732:	85 c0                	test   %eax,%eax
80105734:	0f 84 46 01 00 00    	je     80105880 <create+0x170>
    return 0;
  ilock(dp);
8010573a:	83 ec 0c             	sub    $0xc,%esp
8010573d:	89 c3                	mov    %eax,%ebx
8010573f:	50                   	push   %eax
80105740:	e8 eb c0 ff ff       	call   80101830 <ilock>

  if((ip = dirlookup(dp, name, 0)) != 0){
80105745:	83 c4 0c             	add    $0xc,%esp
80105748:	6a 00                	push   $0x0
8010574a:	57                   	push   %edi
8010574b:	53                   	push   %ebx
8010574c:	e8 3f c6 ff ff       	call   80101d90 <dirlookup>
80105751:	83 c4 10             	add    $0x10,%esp
80105754:	89 c6                	mov    %eax,%esi
80105756:	85 c0                	test   %eax,%eax
80105758:	74 56                	je     801057b0 <create+0xa0>
    iunlockput(dp);
8010575a:	83 ec 0c             	sub    $0xc,%esp
8010575d:	53                   	push   %ebx
8010575e:	e8 5d c3 ff ff       	call   80101ac0 <iunlockput>
    ilock(ip);
80105763:	89 34 24             	mov    %esi,(%esp)
80105766:	e8 c5 c0 ff ff       	call   80101830 <ilock>
    if(type == T_FILE && ip->type == T_FILE)
8010576b:	83 c4 10             	add    $0x10,%esp
8010576e:	66 83 7d d4 02       	cmpw   $0x2,-0x2c(%ebp)
80105773:	75 1b                	jne    80105790 <create+0x80>
80105775:	66 83 7e 50 02       	cmpw   $0x2,0x50(%esi)
8010577a:	75 14                	jne    80105790 <create+0x80>
    panic("create: dirlink");

  iunlockput(dp);

  return ip;
}
8010577c:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010577f:	89 f0                	mov    %esi,%eax
80105781:	5b                   	pop    %ebx
80105782:	5e                   	pop    %esi
80105783:	5f                   	pop    %edi
80105784:	5d                   	pop    %ebp
80105785:	c3                   	ret    
80105786:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010578d:	8d 76 00             	lea    0x0(%esi),%esi
    iunlockput(ip);
80105790:	83 ec 0c             	sub    $0xc,%esp
80105793:	56                   	push   %esi
    return 0;
80105794:	31 f6                	xor    %esi,%esi
    iunlockput(ip);
80105796:	e8 25 c3 ff ff       	call   80101ac0 <iunlockput>
    return 0;
8010579b:	83 c4 10             	add    $0x10,%esp
}
8010579e:	8d 65 f4             	lea    -0xc(%ebp),%esp
801057a1:	89 f0                	mov    %esi,%eax
801057a3:	5b                   	pop    %ebx
801057a4:	5e                   	pop    %esi
801057a5:	5f                   	pop    %edi
801057a6:	5d                   	pop    %ebp
801057a7:	c3                   	ret    
801057a8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801057af:	90                   	nop
  if((ip = ialloc(dp->dev, type)) == 0)
801057b0:	0f bf 45 d4          	movswl -0x2c(%ebp),%eax
801057b4:	83 ec 08             	sub    $0x8,%esp
801057b7:	50                   	push   %eax
801057b8:	ff 33                	push   (%ebx)
801057ba:	e8 01 bf ff ff       	call   801016c0 <ialloc>
801057bf:	83 c4 10             	add    $0x10,%esp
801057c2:	89 c6                	mov    %eax,%esi
801057c4:	85 c0                	test   %eax,%eax
801057c6:	0f 84 cd 00 00 00    	je     80105899 <create+0x189>
  ilock(ip);
801057cc:	83 ec 0c             	sub    $0xc,%esp
801057cf:	50                   	push   %eax
801057d0:	e8 5b c0 ff ff       	call   80101830 <ilock>
  ip->major = major;
801057d5:	0f b7 45 d0          	movzwl -0x30(%ebp),%eax
801057d9:	66 89 46 52          	mov    %ax,0x52(%esi)
  ip->minor = minor;
801057dd:	0f b7 45 cc          	movzwl -0x34(%ebp),%eax
801057e1:	66 89 46 54          	mov    %ax,0x54(%esi)
  ip->nlink = 1;
801057e5:	b8 01 00 00 00       	mov    $0x1,%eax
801057ea:	66 89 46 56          	mov    %ax,0x56(%esi)
  iupdate(ip);
801057ee:	89 34 24             	mov    %esi,(%esp)
801057f1:	e8 8a bf ff ff       	call   80101780 <iupdate>
  if(type == T_DIR){  // Create . and .. entries.
801057f6:	83 c4 10             	add    $0x10,%esp
801057f9:	66 83 7d d4 01       	cmpw   $0x1,-0x2c(%ebp)
801057fe:	74 30                	je     80105830 <create+0x120>
  if(dirlink(dp, name, ip->inum) < 0)
80105800:	83 ec 04             	sub    $0x4,%esp
80105803:	ff 76 04             	push   0x4(%esi)
80105806:	57                   	push   %edi
80105807:	53                   	push   %ebx
80105808:	e8 83 c8 ff ff       	call   80102090 <dirlink>
8010580d:	83 c4 10             	add    $0x10,%esp
80105810:	85 c0                	test   %eax,%eax
80105812:	78 78                	js     8010588c <create+0x17c>
  iunlockput(dp);
80105814:	83 ec 0c             	sub    $0xc,%esp
80105817:	53                   	push   %ebx
80105818:	e8 a3 c2 ff ff       	call   80101ac0 <iunlockput>
  return ip;
8010581d:	83 c4 10             	add    $0x10,%esp
}
80105820:	8d 65 f4             	lea    -0xc(%ebp),%esp
80105823:	89 f0                	mov    %esi,%eax
80105825:	5b                   	pop    %ebx
80105826:	5e                   	pop    %esi
80105827:	5f                   	pop    %edi
80105828:	5d                   	pop    %ebp
80105829:	c3                   	ret    
8010582a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    iupdate(dp);
80105830:	83 ec 0c             	sub    $0xc,%esp
    dp->nlink++;  // for ".."
80105833:	66 83 43 56 01       	addw   $0x1,0x56(%ebx)
    iupdate(dp);
80105838:	53                   	push   %ebx
80105839:	e8 42 bf ff ff       	call   80101780 <iupdate>
    if(dirlink(ip, ".", ip->inum) < 0 || dirlink(ip, "..", dp->inum) < 0)
8010583e:	83 c4 0c             	add    $0xc,%esp
80105841:	ff 76 04             	push   0x4(%esi)
80105844:	68 80 87 10 80       	push   $0x80108780
80105849:	56                   	push   %esi
8010584a:	e8 41 c8 ff ff       	call   80102090 <dirlink>
8010584f:	83 c4 10             	add    $0x10,%esp
80105852:	85 c0                	test   %eax,%eax
80105854:	78 18                	js     8010586e <create+0x15e>
80105856:	83 ec 04             	sub    $0x4,%esp
80105859:	ff 73 04             	push   0x4(%ebx)
8010585c:	68 7f 87 10 80       	push   $0x8010877f
80105861:	56                   	push   %esi
80105862:	e8 29 c8 ff ff       	call   80102090 <dirlink>
80105867:	83 c4 10             	add    $0x10,%esp
8010586a:	85 c0                	test   %eax,%eax
8010586c:	79 92                	jns    80105800 <create+0xf0>
      panic("create dots");
8010586e:	83 ec 0c             	sub    $0xc,%esp
80105871:	68 73 87 10 80       	push   $0x80108773
80105876:	e8 05 ab ff ff       	call   80100380 <panic>
8010587b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010587f:	90                   	nop
}
80105880:	8d 65 f4             	lea    -0xc(%ebp),%esp
    return 0;
80105883:	31 f6                	xor    %esi,%esi
}
80105885:	5b                   	pop    %ebx
80105886:	89 f0                	mov    %esi,%eax
80105888:	5e                   	pop    %esi
80105889:	5f                   	pop    %edi
8010588a:	5d                   	pop    %ebp
8010588b:	c3                   	ret    
    panic("create: dirlink");
8010588c:	83 ec 0c             	sub    $0xc,%esp
8010588f:	68 82 87 10 80       	push   $0x80108782
80105894:	e8 e7 aa ff ff       	call   80100380 <panic>
    panic("create: ialloc");
80105899:	83 ec 0c             	sub    $0xc,%esp
8010589c:	68 64 87 10 80       	push   $0x80108764
801058a1:	e8 da aa ff ff       	call   80100380 <panic>
801058a6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801058ad:	8d 76 00             	lea    0x0(%esi),%esi

801058b0 <sys_dup>:
{
801058b0:	55                   	push   %ebp
801058b1:	89 e5                	mov    %esp,%ebp
801058b3:	56                   	push   %esi
801058b4:	53                   	push   %ebx
  if(argint(n, &fd) < 0)
801058b5:	8d 45 f4             	lea    -0xc(%ebp),%eax
{
801058b8:	83 ec 18             	sub    $0x18,%esp
  if(argint(n, &fd) < 0)
801058bb:	50                   	push   %eax
801058bc:	6a 00                	push   $0x0
801058be:	e8 9d fc ff ff       	call   80105560 <argint>
801058c3:	83 c4 10             	add    $0x10,%esp
801058c6:	85 c0                	test   %eax,%eax
801058c8:	78 36                	js     80105900 <sys_dup+0x50>
  if(fd < 0 || fd >= NOFILE || (f=myproc()->ofile[fd]) == 0)
801058ca:	83 7d f4 0f          	cmpl   $0xf,-0xc(%ebp)
801058ce:	77 30                	ja     80105900 <sys_dup+0x50>
801058d0:	e8 9b e1 ff ff       	call   80103a70 <myproc>
801058d5:	8b 55 f4             	mov    -0xc(%ebp),%edx
801058d8:	8b 74 90 28          	mov    0x28(%eax,%edx,4),%esi
801058dc:	85 f6                	test   %esi,%esi
801058de:	74 20                	je     80105900 <sys_dup+0x50>
  struct proc *curproc = myproc();
801058e0:	e8 8b e1 ff ff       	call   80103a70 <myproc>
  for(fd = 0; fd < NOFILE; fd++){
801058e5:	31 db                	xor    %ebx,%ebx
801058e7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801058ee:	66 90                	xchg   %ax,%ax
    if(curproc->ofile[fd] == 0){
801058f0:	8b 54 98 28          	mov    0x28(%eax,%ebx,4),%edx
801058f4:	85 d2                	test   %edx,%edx
801058f6:	74 18                	je     80105910 <sys_dup+0x60>
  for(fd = 0; fd < NOFILE; fd++){
801058f8:	83 c3 01             	add    $0x1,%ebx
801058fb:	83 fb 10             	cmp    $0x10,%ebx
801058fe:	75 f0                	jne    801058f0 <sys_dup+0x40>
}
80105900:	8d 65 f8             	lea    -0x8(%ebp),%esp
    return -1;
80105903:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
}
80105908:	89 d8                	mov    %ebx,%eax
8010590a:	5b                   	pop    %ebx
8010590b:	5e                   	pop    %esi
8010590c:	5d                   	pop    %ebp
8010590d:	c3                   	ret    
8010590e:	66 90                	xchg   %ax,%ax
  filedup(f);
80105910:	83 ec 0c             	sub    $0xc,%esp
      curproc->ofile[fd] = f;
80105913:	89 74 98 28          	mov    %esi,0x28(%eax,%ebx,4)
  filedup(f);
80105917:	56                   	push   %esi
80105918:	e8 33 b6 ff ff       	call   80100f50 <filedup>
  return fd;
8010591d:	83 c4 10             	add    $0x10,%esp
}
80105920:	8d 65 f8             	lea    -0x8(%ebp),%esp
80105923:	89 d8                	mov    %ebx,%eax
80105925:	5b                   	pop    %ebx
80105926:	5e                   	pop    %esi
80105927:	5d                   	pop    %ebp
80105928:	c3                   	ret    
80105929:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80105930 <sys_read>:
{
80105930:	55                   	push   %ebp
80105931:	89 e5                	mov    %esp,%ebp
80105933:	56                   	push   %esi
80105934:	53                   	push   %ebx
  if(argint(n, &fd) < 0)
80105935:	8d 5d f4             	lea    -0xc(%ebp),%ebx
{
80105938:	83 ec 18             	sub    $0x18,%esp
  if(argint(n, &fd) < 0)
8010593b:	53                   	push   %ebx
8010593c:	6a 00                	push   $0x0
8010593e:	e8 1d fc ff ff       	call   80105560 <argint>
80105943:	83 c4 10             	add    $0x10,%esp
80105946:	85 c0                	test   %eax,%eax
80105948:	78 5e                	js     801059a8 <sys_read+0x78>
  if(fd < 0 || fd >= NOFILE || (f=myproc()->ofile[fd]) == 0)
8010594a:	83 7d f4 0f          	cmpl   $0xf,-0xc(%ebp)
8010594e:	77 58                	ja     801059a8 <sys_read+0x78>
80105950:	e8 1b e1 ff ff       	call   80103a70 <myproc>
80105955:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105958:	8b 74 90 28          	mov    0x28(%eax,%edx,4),%esi
8010595c:	85 f6                	test   %esi,%esi
8010595e:	74 48                	je     801059a8 <sys_read+0x78>
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
80105960:	83 ec 08             	sub    $0x8,%esp
80105963:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105966:	50                   	push   %eax
80105967:	6a 02                	push   $0x2
80105969:	e8 f2 fb ff ff       	call   80105560 <argint>
8010596e:	83 c4 10             	add    $0x10,%esp
80105971:	85 c0                	test   %eax,%eax
80105973:	78 33                	js     801059a8 <sys_read+0x78>
80105975:	83 ec 04             	sub    $0x4,%esp
80105978:	ff 75 f0             	push   -0x10(%ebp)
8010597b:	53                   	push   %ebx
8010597c:	6a 01                	push   $0x1
8010597e:	e8 2d fc ff ff       	call   801055b0 <argptr>
80105983:	83 c4 10             	add    $0x10,%esp
80105986:	85 c0                	test   %eax,%eax
80105988:	78 1e                	js     801059a8 <sys_read+0x78>
  return fileread(f, p, n);
8010598a:	83 ec 04             	sub    $0x4,%esp
8010598d:	ff 75 f0             	push   -0x10(%ebp)
80105990:	ff 75 f4             	push   -0xc(%ebp)
80105993:	56                   	push   %esi
80105994:	e8 37 b7 ff ff       	call   801010d0 <fileread>
80105999:	83 c4 10             	add    $0x10,%esp
}
8010599c:	8d 65 f8             	lea    -0x8(%ebp),%esp
8010599f:	5b                   	pop    %ebx
801059a0:	5e                   	pop    %esi
801059a1:	5d                   	pop    %ebp
801059a2:	c3                   	ret    
801059a3:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801059a7:	90                   	nop
    return -1;
801059a8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801059ad:	eb ed                	jmp    8010599c <sys_read+0x6c>
801059af:	90                   	nop

801059b0 <sys_write>:
{
801059b0:	55                   	push   %ebp
801059b1:	89 e5                	mov    %esp,%ebp
801059b3:	56                   	push   %esi
801059b4:	53                   	push   %ebx
  if(argint(n, &fd) < 0)
801059b5:	8d 5d f4             	lea    -0xc(%ebp),%ebx
{
801059b8:	83 ec 18             	sub    $0x18,%esp
  if(argint(n, &fd) < 0)
801059bb:	53                   	push   %ebx
801059bc:	6a 00                	push   $0x0
801059be:	e8 9d fb ff ff       	call   80105560 <argint>
801059c3:	83 c4 10             	add    $0x10,%esp
801059c6:	85 c0                	test   %eax,%eax
801059c8:	78 5e                	js     80105a28 <sys_write+0x78>
  if(fd < 0 || fd >= NOFILE || (f=myproc()->ofile[fd]) == 0)
801059ca:	83 7d f4 0f          	cmpl   $0xf,-0xc(%ebp)
801059ce:	77 58                	ja     80105a28 <sys_write+0x78>
801059d0:	e8 9b e0 ff ff       	call   80103a70 <myproc>
801059d5:	8b 55 f4             	mov    -0xc(%ebp),%edx
801059d8:	8b 74 90 28          	mov    0x28(%eax,%edx,4),%esi
801059dc:	85 f6                	test   %esi,%esi
801059de:	74 48                	je     80105a28 <sys_write+0x78>
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
801059e0:	83 ec 08             	sub    $0x8,%esp
801059e3:	8d 45 f0             	lea    -0x10(%ebp),%eax
801059e6:	50                   	push   %eax
801059e7:	6a 02                	push   $0x2
801059e9:	e8 72 fb ff ff       	call   80105560 <argint>
801059ee:	83 c4 10             	add    $0x10,%esp
801059f1:	85 c0                	test   %eax,%eax
801059f3:	78 33                	js     80105a28 <sys_write+0x78>
801059f5:	83 ec 04             	sub    $0x4,%esp
801059f8:	ff 75 f0             	push   -0x10(%ebp)
801059fb:	53                   	push   %ebx
801059fc:	6a 01                	push   $0x1
801059fe:	e8 ad fb ff ff       	call   801055b0 <argptr>
80105a03:	83 c4 10             	add    $0x10,%esp
80105a06:	85 c0                	test   %eax,%eax
80105a08:	78 1e                	js     80105a28 <sys_write+0x78>
  return filewrite(f, p, n);
80105a0a:	83 ec 04             	sub    $0x4,%esp
80105a0d:	ff 75 f0             	push   -0x10(%ebp)
80105a10:	ff 75 f4             	push   -0xc(%ebp)
80105a13:	56                   	push   %esi
80105a14:	e8 47 b7 ff ff       	call   80101160 <filewrite>
80105a19:	83 c4 10             	add    $0x10,%esp
}
80105a1c:	8d 65 f8             	lea    -0x8(%ebp),%esp
80105a1f:	5b                   	pop    %ebx
80105a20:	5e                   	pop    %esi
80105a21:	5d                   	pop    %ebp
80105a22:	c3                   	ret    
80105a23:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80105a27:	90                   	nop
    return -1;
80105a28:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105a2d:	eb ed                	jmp    80105a1c <sys_write+0x6c>
80105a2f:	90                   	nop

80105a30 <sys_close>:
{
80105a30:	55                   	push   %ebp
80105a31:	89 e5                	mov    %esp,%ebp
80105a33:	56                   	push   %esi
80105a34:	53                   	push   %ebx
  if(argint(n, &fd) < 0)
80105a35:	8d 45 f4             	lea    -0xc(%ebp),%eax
{
80105a38:	83 ec 18             	sub    $0x18,%esp
  if(argint(n, &fd) < 0)
80105a3b:	50                   	push   %eax
80105a3c:	6a 00                	push   $0x0
80105a3e:	e8 1d fb ff ff       	call   80105560 <argint>
80105a43:	83 c4 10             	add    $0x10,%esp
80105a46:	85 c0                	test   %eax,%eax
80105a48:	78 3e                	js     80105a88 <sys_close+0x58>
  if(fd < 0 || fd >= NOFILE || (f=myproc()->ofile[fd]) == 0)
80105a4a:	83 7d f4 0f          	cmpl   $0xf,-0xc(%ebp)
80105a4e:	77 38                	ja     80105a88 <sys_close+0x58>
80105a50:	e8 1b e0 ff ff       	call   80103a70 <myproc>
80105a55:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105a58:	8d 5a 08             	lea    0x8(%edx),%ebx
80105a5b:	8b 74 98 08          	mov    0x8(%eax,%ebx,4),%esi
80105a5f:	85 f6                	test   %esi,%esi
80105a61:	74 25                	je     80105a88 <sys_close+0x58>
  myproc()->ofile[fd] = 0;
80105a63:	e8 08 e0 ff ff       	call   80103a70 <myproc>
  fileclose(f);
80105a68:	83 ec 0c             	sub    $0xc,%esp
  myproc()->ofile[fd] = 0;
80105a6b:	c7 44 98 08 00 00 00 	movl   $0x0,0x8(%eax,%ebx,4)
80105a72:	00 
  fileclose(f);
80105a73:	56                   	push   %esi
80105a74:	e8 27 b5 ff ff       	call   80100fa0 <fileclose>
  return 0;
80105a79:	83 c4 10             	add    $0x10,%esp
80105a7c:	31 c0                	xor    %eax,%eax
}
80105a7e:	8d 65 f8             	lea    -0x8(%ebp),%esp
80105a81:	5b                   	pop    %ebx
80105a82:	5e                   	pop    %esi
80105a83:	5d                   	pop    %ebp
80105a84:	c3                   	ret    
80105a85:	8d 76 00             	lea    0x0(%esi),%esi
    return -1;
80105a88:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105a8d:	eb ef                	jmp    80105a7e <sys_close+0x4e>
80105a8f:	90                   	nop

80105a90 <sys_fstat>:
{
80105a90:	55                   	push   %ebp
80105a91:	89 e5                	mov    %esp,%ebp
80105a93:	56                   	push   %esi
80105a94:	53                   	push   %ebx
  if(argint(n, &fd) < 0)
80105a95:	8d 5d f4             	lea    -0xc(%ebp),%ebx
{
80105a98:	83 ec 18             	sub    $0x18,%esp
  if(argint(n, &fd) < 0)
80105a9b:	53                   	push   %ebx
80105a9c:	6a 00                	push   $0x0
80105a9e:	e8 bd fa ff ff       	call   80105560 <argint>
80105aa3:	83 c4 10             	add    $0x10,%esp
80105aa6:	85 c0                	test   %eax,%eax
80105aa8:	78 46                	js     80105af0 <sys_fstat+0x60>
  if(fd < 0 || fd >= NOFILE || (f=myproc()->ofile[fd]) == 0)
80105aaa:	83 7d f4 0f          	cmpl   $0xf,-0xc(%ebp)
80105aae:	77 40                	ja     80105af0 <sys_fstat+0x60>
80105ab0:	e8 bb df ff ff       	call   80103a70 <myproc>
80105ab5:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105ab8:	8b 74 90 28          	mov    0x28(%eax,%edx,4),%esi
80105abc:	85 f6                	test   %esi,%esi
80105abe:	74 30                	je     80105af0 <sys_fstat+0x60>
  if(argfd(0, 0, &f) < 0 || argptr(1, (void*)&st, sizeof(*st)) < 0)
80105ac0:	83 ec 04             	sub    $0x4,%esp
80105ac3:	6a 14                	push   $0x14
80105ac5:	53                   	push   %ebx
80105ac6:	6a 01                	push   $0x1
80105ac8:	e8 e3 fa ff ff       	call   801055b0 <argptr>
80105acd:	83 c4 10             	add    $0x10,%esp
80105ad0:	85 c0                	test   %eax,%eax
80105ad2:	78 1c                	js     80105af0 <sys_fstat+0x60>
  return filestat(f, st);
80105ad4:	83 ec 08             	sub    $0x8,%esp
80105ad7:	ff 75 f4             	push   -0xc(%ebp)
80105ada:	56                   	push   %esi
80105adb:	e8 a0 b5 ff ff       	call   80101080 <filestat>
80105ae0:	83 c4 10             	add    $0x10,%esp
}
80105ae3:	8d 65 f8             	lea    -0x8(%ebp),%esp
80105ae6:	5b                   	pop    %ebx
80105ae7:	5e                   	pop    %esi
80105ae8:	5d                   	pop    %ebp
80105ae9:	c3                   	ret    
80105aea:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    return -1;
80105af0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105af5:	eb ec                	jmp    80105ae3 <sys_fstat+0x53>
80105af7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105afe:	66 90                	xchg   %ax,%ax

80105b00 <sys_link>:
{
80105b00:	55                   	push   %ebp
80105b01:	89 e5                	mov    %esp,%ebp
80105b03:	57                   	push   %edi
80105b04:	56                   	push   %esi
  if(argstr(0, &old) < 0 || argstr(1, &new) < 0)
80105b05:	8d 45 d4             	lea    -0x2c(%ebp),%eax
{
80105b08:	53                   	push   %ebx
80105b09:	83 ec 34             	sub    $0x34,%esp
  if(argstr(0, &old) < 0 || argstr(1, &new) < 0)
80105b0c:	50                   	push   %eax
80105b0d:	6a 00                	push   $0x0
80105b0f:	e8 0c fb ff ff       	call   80105620 <argstr>
80105b14:	83 c4 10             	add    $0x10,%esp
80105b17:	85 c0                	test   %eax,%eax
80105b19:	0f 88 fb 00 00 00    	js     80105c1a <sys_link+0x11a>
80105b1f:	83 ec 08             	sub    $0x8,%esp
80105b22:	8d 45 d0             	lea    -0x30(%ebp),%eax
80105b25:	50                   	push   %eax
80105b26:	6a 01                	push   $0x1
80105b28:	e8 f3 fa ff ff       	call   80105620 <argstr>
80105b2d:	83 c4 10             	add    $0x10,%esp
80105b30:	85 c0                	test   %eax,%eax
80105b32:	0f 88 e2 00 00 00    	js     80105c1a <sys_link+0x11a>
  begin_op();
80105b38:	e8 d3 d2 ff ff       	call   80102e10 <begin_op>
  if((ip = namei(old)) == 0){
80105b3d:	83 ec 0c             	sub    $0xc,%esp
80105b40:	ff 75 d4             	push   -0x2c(%ebp)
80105b43:	e8 08 c6 ff ff       	call   80102150 <namei>
80105b48:	83 c4 10             	add    $0x10,%esp
80105b4b:	89 c3                	mov    %eax,%ebx
80105b4d:	85 c0                	test   %eax,%eax
80105b4f:	0f 84 e4 00 00 00    	je     80105c39 <sys_link+0x139>
  ilock(ip);
80105b55:	83 ec 0c             	sub    $0xc,%esp
80105b58:	50                   	push   %eax
80105b59:	e8 d2 bc ff ff       	call   80101830 <ilock>
  if(ip->type == T_DIR){
80105b5e:	83 c4 10             	add    $0x10,%esp
80105b61:	66 83 7b 50 01       	cmpw   $0x1,0x50(%ebx)
80105b66:	0f 84 b5 00 00 00    	je     80105c21 <sys_link+0x121>
  iupdate(ip);
80105b6c:	83 ec 0c             	sub    $0xc,%esp
  ip->nlink++;
80105b6f:	66 83 43 56 01       	addw   $0x1,0x56(%ebx)
  if((dp = nameiparent(new, name)) == 0)
80105b74:	8d 7d da             	lea    -0x26(%ebp),%edi
  iupdate(ip);
80105b77:	53                   	push   %ebx
80105b78:	e8 03 bc ff ff       	call   80101780 <iupdate>
  iunlock(ip);
80105b7d:	89 1c 24             	mov    %ebx,(%esp)
80105b80:	e8 8b bd ff ff       	call   80101910 <iunlock>
  if((dp = nameiparent(new, name)) == 0)
80105b85:	58                   	pop    %eax
80105b86:	5a                   	pop    %edx
80105b87:	57                   	push   %edi
80105b88:	ff 75 d0             	push   -0x30(%ebp)
80105b8b:	e8 e0 c5 ff ff       	call   80102170 <nameiparent>
80105b90:	83 c4 10             	add    $0x10,%esp
80105b93:	89 c6                	mov    %eax,%esi
80105b95:	85 c0                	test   %eax,%eax
80105b97:	74 5b                	je     80105bf4 <sys_link+0xf4>
  ilock(dp);
80105b99:	83 ec 0c             	sub    $0xc,%esp
80105b9c:	50                   	push   %eax
80105b9d:	e8 8e bc ff ff       	call   80101830 <ilock>
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
80105ba2:	8b 03                	mov    (%ebx),%eax
80105ba4:	83 c4 10             	add    $0x10,%esp
80105ba7:	39 06                	cmp    %eax,(%esi)
80105ba9:	75 3d                	jne    80105be8 <sys_link+0xe8>
80105bab:	83 ec 04             	sub    $0x4,%esp
80105bae:	ff 73 04             	push   0x4(%ebx)
80105bb1:	57                   	push   %edi
80105bb2:	56                   	push   %esi
80105bb3:	e8 d8 c4 ff ff       	call   80102090 <dirlink>
80105bb8:	83 c4 10             	add    $0x10,%esp
80105bbb:	85 c0                	test   %eax,%eax
80105bbd:	78 29                	js     80105be8 <sys_link+0xe8>
  iunlockput(dp);
80105bbf:	83 ec 0c             	sub    $0xc,%esp
80105bc2:	56                   	push   %esi
80105bc3:	e8 f8 be ff ff       	call   80101ac0 <iunlockput>
  iput(ip);
80105bc8:	89 1c 24             	mov    %ebx,(%esp)
80105bcb:	e8 90 bd ff ff       	call   80101960 <iput>
  end_op();
80105bd0:	e8 ab d2 ff ff       	call   80102e80 <end_op>
  return 0;
80105bd5:	83 c4 10             	add    $0x10,%esp
80105bd8:	31 c0                	xor    %eax,%eax
}
80105bda:	8d 65 f4             	lea    -0xc(%ebp),%esp
80105bdd:	5b                   	pop    %ebx
80105bde:	5e                   	pop    %esi
80105bdf:	5f                   	pop    %edi
80105be0:	5d                   	pop    %ebp
80105be1:	c3                   	ret    
80105be2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    iunlockput(dp);
80105be8:	83 ec 0c             	sub    $0xc,%esp
80105beb:	56                   	push   %esi
80105bec:	e8 cf be ff ff       	call   80101ac0 <iunlockput>
    goto bad;
80105bf1:	83 c4 10             	add    $0x10,%esp
  ilock(ip);
80105bf4:	83 ec 0c             	sub    $0xc,%esp
80105bf7:	53                   	push   %ebx
80105bf8:	e8 33 bc ff ff       	call   80101830 <ilock>
  ip->nlink--;
80105bfd:	66 83 6b 56 01       	subw   $0x1,0x56(%ebx)
  iupdate(ip);
80105c02:	89 1c 24             	mov    %ebx,(%esp)
80105c05:	e8 76 bb ff ff       	call   80101780 <iupdate>
  iunlockput(ip);
80105c0a:	89 1c 24             	mov    %ebx,(%esp)
80105c0d:	e8 ae be ff ff       	call   80101ac0 <iunlockput>
  end_op();
80105c12:	e8 69 d2 ff ff       	call   80102e80 <end_op>
  return -1;
80105c17:	83 c4 10             	add    $0x10,%esp
80105c1a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105c1f:	eb b9                	jmp    80105bda <sys_link+0xda>
    iunlockput(ip);
80105c21:	83 ec 0c             	sub    $0xc,%esp
80105c24:	53                   	push   %ebx
80105c25:	e8 96 be ff ff       	call   80101ac0 <iunlockput>
    end_op();
80105c2a:	e8 51 d2 ff ff       	call   80102e80 <end_op>
    return -1;
80105c2f:	83 c4 10             	add    $0x10,%esp
80105c32:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105c37:	eb a1                	jmp    80105bda <sys_link+0xda>
    end_op();
80105c39:	e8 42 d2 ff ff       	call   80102e80 <end_op>
    return -1;
80105c3e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105c43:	eb 95                	jmp    80105bda <sys_link+0xda>
80105c45:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80105c4c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80105c50 <sys_unlink>:
{
80105c50:	55                   	push   %ebp
80105c51:	89 e5                	mov    %esp,%ebp
80105c53:	57                   	push   %edi
80105c54:	56                   	push   %esi
  if(argstr(0, &path) < 0)
80105c55:	8d 45 c0             	lea    -0x40(%ebp),%eax
{
80105c58:	53                   	push   %ebx
80105c59:	83 ec 54             	sub    $0x54,%esp
  if(argstr(0, &path) < 0)
80105c5c:	50                   	push   %eax
80105c5d:	6a 00                	push   $0x0
80105c5f:	e8 bc f9 ff ff       	call   80105620 <argstr>
80105c64:	83 c4 10             	add    $0x10,%esp
80105c67:	85 c0                	test   %eax,%eax
80105c69:	0f 88 7a 01 00 00    	js     80105de9 <sys_unlink+0x199>
  begin_op();
80105c6f:	e8 9c d1 ff ff       	call   80102e10 <begin_op>
  if((dp = nameiparent(path, name)) == 0){
80105c74:	8d 5d ca             	lea    -0x36(%ebp),%ebx
80105c77:	83 ec 08             	sub    $0x8,%esp
80105c7a:	53                   	push   %ebx
80105c7b:	ff 75 c0             	push   -0x40(%ebp)
80105c7e:	e8 ed c4 ff ff       	call   80102170 <nameiparent>
80105c83:	83 c4 10             	add    $0x10,%esp
80105c86:	89 45 b4             	mov    %eax,-0x4c(%ebp)
80105c89:	85 c0                	test   %eax,%eax
80105c8b:	0f 84 62 01 00 00    	je     80105df3 <sys_unlink+0x1a3>
  ilock(dp);
80105c91:	8b 7d b4             	mov    -0x4c(%ebp),%edi
80105c94:	83 ec 0c             	sub    $0xc,%esp
80105c97:	57                   	push   %edi
80105c98:	e8 93 bb ff ff       	call   80101830 <ilock>
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
80105c9d:	58                   	pop    %eax
80105c9e:	5a                   	pop    %edx
80105c9f:	68 80 87 10 80       	push   $0x80108780
80105ca4:	53                   	push   %ebx
80105ca5:	e8 c6 c0 ff ff       	call   80101d70 <namecmp>
80105caa:	83 c4 10             	add    $0x10,%esp
80105cad:	85 c0                	test   %eax,%eax
80105caf:	0f 84 fb 00 00 00    	je     80105db0 <sys_unlink+0x160>
80105cb5:	83 ec 08             	sub    $0x8,%esp
80105cb8:	68 7f 87 10 80       	push   $0x8010877f
80105cbd:	53                   	push   %ebx
80105cbe:	e8 ad c0 ff ff       	call   80101d70 <namecmp>
80105cc3:	83 c4 10             	add    $0x10,%esp
80105cc6:	85 c0                	test   %eax,%eax
80105cc8:	0f 84 e2 00 00 00    	je     80105db0 <sys_unlink+0x160>
  if((ip = dirlookup(dp, name, &off)) == 0)
80105cce:	83 ec 04             	sub    $0x4,%esp
80105cd1:	8d 45 c4             	lea    -0x3c(%ebp),%eax
80105cd4:	50                   	push   %eax
80105cd5:	53                   	push   %ebx
80105cd6:	57                   	push   %edi
80105cd7:	e8 b4 c0 ff ff       	call   80101d90 <dirlookup>
80105cdc:	83 c4 10             	add    $0x10,%esp
80105cdf:	89 c3                	mov    %eax,%ebx
80105ce1:	85 c0                	test   %eax,%eax
80105ce3:	0f 84 c7 00 00 00    	je     80105db0 <sys_unlink+0x160>
  ilock(ip);
80105ce9:	83 ec 0c             	sub    $0xc,%esp
80105cec:	50                   	push   %eax
80105ced:	e8 3e bb ff ff       	call   80101830 <ilock>
  if(ip->nlink < 1)
80105cf2:	83 c4 10             	add    $0x10,%esp
80105cf5:	66 83 7b 56 00       	cmpw   $0x0,0x56(%ebx)
80105cfa:	0f 8e 1c 01 00 00    	jle    80105e1c <sys_unlink+0x1cc>
  if(ip->type == T_DIR && !isdirempty(ip)){
80105d00:	66 83 7b 50 01       	cmpw   $0x1,0x50(%ebx)
80105d05:	8d 7d d8             	lea    -0x28(%ebp),%edi
80105d08:	74 66                	je     80105d70 <sys_unlink+0x120>
  memset(&de, 0, sizeof(de));
80105d0a:	83 ec 04             	sub    $0x4,%esp
80105d0d:	6a 10                	push   $0x10
80105d0f:	6a 00                	push   $0x0
80105d11:	57                   	push   %edi
80105d12:	e8 89 f5 ff ff       	call   801052a0 <memset>
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80105d17:	6a 10                	push   $0x10
80105d19:	ff 75 c4             	push   -0x3c(%ebp)
80105d1c:	57                   	push   %edi
80105d1d:	ff 75 b4             	push   -0x4c(%ebp)
80105d20:	e8 1b bf ff ff       	call   80101c40 <writei>
80105d25:	83 c4 20             	add    $0x20,%esp
80105d28:	83 f8 10             	cmp    $0x10,%eax
80105d2b:	0f 85 de 00 00 00    	jne    80105e0f <sys_unlink+0x1bf>
  if(ip->type == T_DIR){
80105d31:	66 83 7b 50 01       	cmpw   $0x1,0x50(%ebx)
80105d36:	0f 84 94 00 00 00    	je     80105dd0 <sys_unlink+0x180>
  iunlockput(dp);
80105d3c:	83 ec 0c             	sub    $0xc,%esp
80105d3f:	ff 75 b4             	push   -0x4c(%ebp)
80105d42:	e8 79 bd ff ff       	call   80101ac0 <iunlockput>
  ip->nlink--;
80105d47:	66 83 6b 56 01       	subw   $0x1,0x56(%ebx)
  iupdate(ip);
80105d4c:	89 1c 24             	mov    %ebx,(%esp)
80105d4f:	e8 2c ba ff ff       	call   80101780 <iupdate>
  iunlockput(ip);
80105d54:	89 1c 24             	mov    %ebx,(%esp)
80105d57:	e8 64 bd ff ff       	call   80101ac0 <iunlockput>
  end_op();
80105d5c:	e8 1f d1 ff ff       	call   80102e80 <end_op>
  return 0;
80105d61:	83 c4 10             	add    $0x10,%esp
80105d64:	31 c0                	xor    %eax,%eax
}
80105d66:	8d 65 f4             	lea    -0xc(%ebp),%esp
80105d69:	5b                   	pop    %ebx
80105d6a:	5e                   	pop    %esi
80105d6b:	5f                   	pop    %edi
80105d6c:	5d                   	pop    %ebp
80105d6d:	c3                   	ret    
80105d6e:	66 90                	xchg   %ax,%ax
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
80105d70:	83 7b 58 20          	cmpl   $0x20,0x58(%ebx)
80105d74:	76 94                	jbe    80105d0a <sys_unlink+0xba>
80105d76:	be 20 00 00 00       	mov    $0x20,%esi
80105d7b:	eb 0b                	jmp    80105d88 <sys_unlink+0x138>
80105d7d:	8d 76 00             	lea    0x0(%esi),%esi
80105d80:	83 c6 10             	add    $0x10,%esi
80105d83:	3b 73 58             	cmp    0x58(%ebx),%esi
80105d86:	73 82                	jae    80105d0a <sys_unlink+0xba>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80105d88:	6a 10                	push   $0x10
80105d8a:	56                   	push   %esi
80105d8b:	57                   	push   %edi
80105d8c:	53                   	push   %ebx
80105d8d:	e8 ae bd ff ff       	call   80101b40 <readi>
80105d92:	83 c4 10             	add    $0x10,%esp
80105d95:	83 f8 10             	cmp    $0x10,%eax
80105d98:	75 68                	jne    80105e02 <sys_unlink+0x1b2>
    if(de.inum != 0)
80105d9a:	66 83 7d d8 00       	cmpw   $0x0,-0x28(%ebp)
80105d9f:	74 df                	je     80105d80 <sys_unlink+0x130>
    iunlockput(ip);
80105da1:	83 ec 0c             	sub    $0xc,%esp
80105da4:	53                   	push   %ebx
80105da5:	e8 16 bd ff ff       	call   80101ac0 <iunlockput>
    goto bad;
80105daa:	83 c4 10             	add    $0x10,%esp
80105dad:	8d 76 00             	lea    0x0(%esi),%esi
  iunlockput(dp);
80105db0:	83 ec 0c             	sub    $0xc,%esp
80105db3:	ff 75 b4             	push   -0x4c(%ebp)
80105db6:	e8 05 bd ff ff       	call   80101ac0 <iunlockput>
  end_op();
80105dbb:	e8 c0 d0 ff ff       	call   80102e80 <end_op>
  return -1;
80105dc0:	83 c4 10             	add    $0x10,%esp
80105dc3:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105dc8:	eb 9c                	jmp    80105d66 <sys_unlink+0x116>
80105dca:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    dp->nlink--;
80105dd0:	8b 45 b4             	mov    -0x4c(%ebp),%eax
    iupdate(dp);
80105dd3:	83 ec 0c             	sub    $0xc,%esp
    dp->nlink--;
80105dd6:	66 83 68 56 01       	subw   $0x1,0x56(%eax)
    iupdate(dp);
80105ddb:	50                   	push   %eax
80105ddc:	e8 9f b9 ff ff       	call   80101780 <iupdate>
80105de1:	83 c4 10             	add    $0x10,%esp
80105de4:	e9 53 ff ff ff       	jmp    80105d3c <sys_unlink+0xec>
    return -1;
80105de9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105dee:	e9 73 ff ff ff       	jmp    80105d66 <sys_unlink+0x116>
    end_op();
80105df3:	e8 88 d0 ff ff       	call   80102e80 <end_op>
    return -1;
80105df8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105dfd:	e9 64 ff ff ff       	jmp    80105d66 <sys_unlink+0x116>
      panic("isdirempty: readi");
80105e02:	83 ec 0c             	sub    $0xc,%esp
80105e05:	68 a4 87 10 80       	push   $0x801087a4
80105e0a:	e8 71 a5 ff ff       	call   80100380 <panic>
    panic("unlink: writei");
80105e0f:	83 ec 0c             	sub    $0xc,%esp
80105e12:	68 b6 87 10 80       	push   $0x801087b6
80105e17:	e8 64 a5 ff ff       	call   80100380 <panic>
    panic("unlink: nlink < 1");
80105e1c:	83 ec 0c             	sub    $0xc,%esp
80105e1f:	68 92 87 10 80       	push   $0x80108792
80105e24:	e8 57 a5 ff ff       	call   80100380 <panic>
80105e29:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80105e30 <sys_open>:

int
sys_open(void)
{
80105e30:	55                   	push   %ebp
80105e31:	89 e5                	mov    %esp,%ebp
80105e33:	57                   	push   %edi
80105e34:	56                   	push   %esi
  char *path;
  int fd, omode;
  struct file *f;
  struct inode *ip;

  if(argstr(0, &path) < 0 || argint(1, &omode) < 0)
80105e35:	8d 45 e0             	lea    -0x20(%ebp),%eax
{
80105e38:	53                   	push   %ebx
80105e39:	83 ec 24             	sub    $0x24,%esp
  if(argstr(0, &path) < 0 || argint(1, &omode) < 0)
80105e3c:	50                   	push   %eax
80105e3d:	6a 00                	push   $0x0
80105e3f:	e8 dc f7 ff ff       	call   80105620 <argstr>
80105e44:	83 c4 10             	add    $0x10,%esp
80105e47:	85 c0                	test   %eax,%eax
80105e49:	0f 88 8e 00 00 00    	js     80105edd <sys_open+0xad>
80105e4f:	83 ec 08             	sub    $0x8,%esp
80105e52:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80105e55:	50                   	push   %eax
80105e56:	6a 01                	push   $0x1
80105e58:	e8 03 f7 ff ff       	call   80105560 <argint>
80105e5d:	83 c4 10             	add    $0x10,%esp
80105e60:	85 c0                	test   %eax,%eax
80105e62:	78 79                	js     80105edd <sys_open+0xad>
    return -1;

  begin_op();
80105e64:	e8 a7 cf ff ff       	call   80102e10 <begin_op>

  if(omode & O_CREATE){
80105e69:	f6 45 e5 02          	testb  $0x2,-0x1b(%ebp)
80105e6d:	75 79                	jne    80105ee8 <sys_open+0xb8>
    if(ip == 0){
      end_op();
      return -1;
    }
  } else {
    if((ip = namei(path)) == 0){
80105e6f:	83 ec 0c             	sub    $0xc,%esp
80105e72:	ff 75 e0             	push   -0x20(%ebp)
80105e75:	e8 d6 c2 ff ff       	call   80102150 <namei>
80105e7a:	83 c4 10             	add    $0x10,%esp
80105e7d:	89 c6                	mov    %eax,%esi
80105e7f:	85 c0                	test   %eax,%eax
80105e81:	0f 84 7e 00 00 00    	je     80105f05 <sys_open+0xd5>
      end_op();
      return -1;
    }
    ilock(ip);
80105e87:	83 ec 0c             	sub    $0xc,%esp
80105e8a:	50                   	push   %eax
80105e8b:	e8 a0 b9 ff ff       	call   80101830 <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
80105e90:	83 c4 10             	add    $0x10,%esp
80105e93:	66 83 7e 50 01       	cmpw   $0x1,0x50(%esi)
80105e98:	0f 84 c2 00 00 00    	je     80105f60 <sys_open+0x130>
      end_op();
      return -1;
    }
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
80105e9e:	e8 3d b0 ff ff       	call   80100ee0 <filealloc>
80105ea3:	89 c7                	mov    %eax,%edi
80105ea5:	85 c0                	test   %eax,%eax
80105ea7:	74 23                	je     80105ecc <sys_open+0x9c>
  struct proc *curproc = myproc();
80105ea9:	e8 c2 db ff ff       	call   80103a70 <myproc>
  for(fd = 0; fd < NOFILE; fd++){
80105eae:	31 db                	xor    %ebx,%ebx
    if(curproc->ofile[fd] == 0){
80105eb0:	8b 54 98 28          	mov    0x28(%eax,%ebx,4),%edx
80105eb4:	85 d2                	test   %edx,%edx
80105eb6:	74 60                	je     80105f18 <sys_open+0xe8>
  for(fd = 0; fd < NOFILE; fd++){
80105eb8:	83 c3 01             	add    $0x1,%ebx
80105ebb:	83 fb 10             	cmp    $0x10,%ebx
80105ebe:	75 f0                	jne    80105eb0 <sys_open+0x80>
    if(f)
      fileclose(f);
80105ec0:	83 ec 0c             	sub    $0xc,%esp
80105ec3:	57                   	push   %edi
80105ec4:	e8 d7 b0 ff ff       	call   80100fa0 <fileclose>
80105ec9:	83 c4 10             	add    $0x10,%esp
    iunlockput(ip);
80105ecc:	83 ec 0c             	sub    $0xc,%esp
80105ecf:	56                   	push   %esi
80105ed0:	e8 eb bb ff ff       	call   80101ac0 <iunlockput>
    end_op();
80105ed5:	e8 a6 cf ff ff       	call   80102e80 <end_op>
    return -1;
80105eda:	83 c4 10             	add    $0x10,%esp
80105edd:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
80105ee2:	eb 6d                	jmp    80105f51 <sys_open+0x121>
80105ee4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    ip = create(path, T_FILE, 0, 0);
80105ee8:	83 ec 0c             	sub    $0xc,%esp
80105eeb:	8b 45 e0             	mov    -0x20(%ebp),%eax
80105eee:	31 c9                	xor    %ecx,%ecx
80105ef0:	ba 02 00 00 00       	mov    $0x2,%edx
80105ef5:	6a 00                	push   $0x0
80105ef7:	e8 14 f8 ff ff       	call   80105710 <create>
    if(ip == 0){
80105efc:	83 c4 10             	add    $0x10,%esp
    ip = create(path, T_FILE, 0, 0);
80105eff:	89 c6                	mov    %eax,%esi
    if(ip == 0){
80105f01:	85 c0                	test   %eax,%eax
80105f03:	75 99                	jne    80105e9e <sys_open+0x6e>
      end_op();
80105f05:	e8 76 cf ff ff       	call   80102e80 <end_op>
      return -1;
80105f0a:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
80105f0f:	eb 40                	jmp    80105f51 <sys_open+0x121>
80105f11:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  }
  iunlock(ip);
80105f18:	83 ec 0c             	sub    $0xc,%esp
      curproc->ofile[fd] = f;
80105f1b:	89 7c 98 28          	mov    %edi,0x28(%eax,%ebx,4)
  iunlock(ip);
80105f1f:	56                   	push   %esi
80105f20:	e8 eb b9 ff ff       	call   80101910 <iunlock>
  end_op();
80105f25:	e8 56 cf ff ff       	call   80102e80 <end_op>

  f->type = FD_INODE;
80105f2a:	c7 07 02 00 00 00    	movl   $0x2,(%edi)
  f->ip = ip;
  f->off = 0;
  f->readable = !(omode & O_WRONLY);
80105f30:	8b 55 e4             	mov    -0x1c(%ebp),%edx
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
80105f33:	83 c4 10             	add    $0x10,%esp
  f->ip = ip;
80105f36:	89 77 10             	mov    %esi,0x10(%edi)
  f->readable = !(omode & O_WRONLY);
80105f39:	89 d0                	mov    %edx,%eax
  f->off = 0;
80105f3b:	c7 47 14 00 00 00 00 	movl   $0x0,0x14(%edi)
  f->readable = !(omode & O_WRONLY);
80105f42:	f7 d0                	not    %eax
80105f44:	83 e0 01             	and    $0x1,%eax
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
80105f47:	83 e2 03             	and    $0x3,%edx
  f->readable = !(omode & O_WRONLY);
80105f4a:	88 47 08             	mov    %al,0x8(%edi)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
80105f4d:	0f 95 47 09          	setne  0x9(%edi)
  return fd;
}
80105f51:	8d 65 f4             	lea    -0xc(%ebp),%esp
80105f54:	89 d8                	mov    %ebx,%eax
80105f56:	5b                   	pop    %ebx
80105f57:	5e                   	pop    %esi
80105f58:	5f                   	pop    %edi
80105f59:	5d                   	pop    %ebp
80105f5a:	c3                   	ret    
80105f5b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80105f5f:	90                   	nop
    if(ip->type == T_DIR && omode != O_RDONLY){
80105f60:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
80105f63:	85 c9                	test   %ecx,%ecx
80105f65:	0f 84 33 ff ff ff    	je     80105e9e <sys_open+0x6e>
80105f6b:	e9 5c ff ff ff       	jmp    80105ecc <sys_open+0x9c>

80105f70 <sys_mkdir>:

int
sys_mkdir(void)
{
80105f70:	55                   	push   %ebp
80105f71:	89 e5                	mov    %esp,%ebp
80105f73:	83 ec 18             	sub    $0x18,%esp
  char *path;
  struct inode *ip;

  begin_op();
80105f76:	e8 95 ce ff ff       	call   80102e10 <begin_op>
  if(argstr(0, &path) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
80105f7b:	83 ec 08             	sub    $0x8,%esp
80105f7e:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105f81:	50                   	push   %eax
80105f82:	6a 00                	push   $0x0
80105f84:	e8 97 f6 ff ff       	call   80105620 <argstr>
80105f89:	83 c4 10             	add    $0x10,%esp
80105f8c:	85 c0                	test   %eax,%eax
80105f8e:	78 30                	js     80105fc0 <sys_mkdir+0x50>
80105f90:	83 ec 0c             	sub    $0xc,%esp
80105f93:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105f96:	31 c9                	xor    %ecx,%ecx
80105f98:	ba 01 00 00 00       	mov    $0x1,%edx
80105f9d:	6a 00                	push   $0x0
80105f9f:	e8 6c f7 ff ff       	call   80105710 <create>
80105fa4:	83 c4 10             	add    $0x10,%esp
80105fa7:	85 c0                	test   %eax,%eax
80105fa9:	74 15                	je     80105fc0 <sys_mkdir+0x50>
    end_op();
    return -1;
  }
  iunlockput(ip);
80105fab:	83 ec 0c             	sub    $0xc,%esp
80105fae:	50                   	push   %eax
80105faf:	e8 0c bb ff ff       	call   80101ac0 <iunlockput>
  end_op();
80105fb4:	e8 c7 ce ff ff       	call   80102e80 <end_op>
  return 0;
80105fb9:	83 c4 10             	add    $0x10,%esp
80105fbc:	31 c0                	xor    %eax,%eax
}
80105fbe:	c9                   	leave  
80105fbf:	c3                   	ret    
    end_op();
80105fc0:	e8 bb ce ff ff       	call   80102e80 <end_op>
    return -1;
80105fc5:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105fca:	c9                   	leave  
80105fcb:	c3                   	ret    
80105fcc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80105fd0 <sys_mknod>:

int
sys_mknod(void)
{
80105fd0:	55                   	push   %ebp
80105fd1:	89 e5                	mov    %esp,%ebp
80105fd3:	83 ec 18             	sub    $0x18,%esp
  struct inode *ip;
  char *path;
  int major, minor;

  begin_op();
80105fd6:	e8 35 ce ff ff       	call   80102e10 <begin_op>
  if((argstr(0, &path)) < 0 ||
80105fdb:	83 ec 08             	sub    $0x8,%esp
80105fde:	8d 45 ec             	lea    -0x14(%ebp),%eax
80105fe1:	50                   	push   %eax
80105fe2:	6a 00                	push   $0x0
80105fe4:	e8 37 f6 ff ff       	call   80105620 <argstr>
80105fe9:	83 c4 10             	add    $0x10,%esp
80105fec:	85 c0                	test   %eax,%eax
80105fee:	78 60                	js     80106050 <sys_mknod+0x80>
     argint(1, &major) < 0 ||
80105ff0:	83 ec 08             	sub    $0x8,%esp
80105ff3:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105ff6:	50                   	push   %eax
80105ff7:	6a 01                	push   $0x1
80105ff9:	e8 62 f5 ff ff       	call   80105560 <argint>
  if((argstr(0, &path)) < 0 ||
80105ffe:	83 c4 10             	add    $0x10,%esp
80106001:	85 c0                	test   %eax,%eax
80106003:	78 4b                	js     80106050 <sys_mknod+0x80>
     argint(2, &minor) < 0 ||
80106005:	83 ec 08             	sub    $0x8,%esp
80106008:	8d 45 f4             	lea    -0xc(%ebp),%eax
8010600b:	50                   	push   %eax
8010600c:	6a 02                	push   $0x2
8010600e:	e8 4d f5 ff ff       	call   80105560 <argint>
     argint(1, &major) < 0 ||
80106013:	83 c4 10             	add    $0x10,%esp
80106016:	85 c0                	test   %eax,%eax
80106018:	78 36                	js     80106050 <sys_mknod+0x80>
     (ip = create(path, T_DEV, major, minor)) == 0){
8010601a:	0f bf 45 f4          	movswl -0xc(%ebp),%eax
8010601e:	83 ec 0c             	sub    $0xc,%esp
80106021:	0f bf 4d f0          	movswl -0x10(%ebp),%ecx
80106025:	ba 03 00 00 00       	mov    $0x3,%edx
8010602a:	50                   	push   %eax
8010602b:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010602e:	e8 dd f6 ff ff       	call   80105710 <create>
     argint(2, &minor) < 0 ||
80106033:	83 c4 10             	add    $0x10,%esp
80106036:	85 c0                	test   %eax,%eax
80106038:	74 16                	je     80106050 <sys_mknod+0x80>
    end_op();
    return -1;
  }
  iunlockput(ip);
8010603a:	83 ec 0c             	sub    $0xc,%esp
8010603d:	50                   	push   %eax
8010603e:	e8 7d ba ff ff       	call   80101ac0 <iunlockput>
  end_op();
80106043:	e8 38 ce ff ff       	call   80102e80 <end_op>
  return 0;
80106048:	83 c4 10             	add    $0x10,%esp
8010604b:	31 c0                	xor    %eax,%eax
}
8010604d:	c9                   	leave  
8010604e:	c3                   	ret    
8010604f:	90                   	nop
    end_op();
80106050:	e8 2b ce ff ff       	call   80102e80 <end_op>
    return -1;
80106055:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010605a:	c9                   	leave  
8010605b:	c3                   	ret    
8010605c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80106060 <sys_chdir>:

int
sys_chdir(void)
{
80106060:	55                   	push   %ebp
80106061:	89 e5                	mov    %esp,%ebp
80106063:	56                   	push   %esi
80106064:	53                   	push   %ebx
80106065:	83 ec 10             	sub    $0x10,%esp
  char *path;
  struct inode *ip;
  struct proc *curproc = myproc();
80106068:	e8 03 da ff ff       	call   80103a70 <myproc>
8010606d:	89 c6                	mov    %eax,%esi
  
  begin_op();
8010606f:	e8 9c cd ff ff       	call   80102e10 <begin_op>
  if(argstr(0, &path) < 0 || (ip = namei(path)) == 0){
80106074:	83 ec 08             	sub    $0x8,%esp
80106077:	8d 45 f4             	lea    -0xc(%ebp),%eax
8010607a:	50                   	push   %eax
8010607b:	6a 00                	push   $0x0
8010607d:	e8 9e f5 ff ff       	call   80105620 <argstr>
80106082:	83 c4 10             	add    $0x10,%esp
80106085:	85 c0                	test   %eax,%eax
80106087:	78 77                	js     80106100 <sys_chdir+0xa0>
80106089:	83 ec 0c             	sub    $0xc,%esp
8010608c:	ff 75 f4             	push   -0xc(%ebp)
8010608f:	e8 bc c0 ff ff       	call   80102150 <namei>
80106094:	83 c4 10             	add    $0x10,%esp
80106097:	89 c3                	mov    %eax,%ebx
80106099:	85 c0                	test   %eax,%eax
8010609b:	74 63                	je     80106100 <sys_chdir+0xa0>
    end_op();
    return -1;
  }
  ilock(ip);
8010609d:	83 ec 0c             	sub    $0xc,%esp
801060a0:	50                   	push   %eax
801060a1:	e8 8a b7 ff ff       	call   80101830 <ilock>
  if(ip->type != T_DIR){
801060a6:	83 c4 10             	add    $0x10,%esp
801060a9:	66 83 7b 50 01       	cmpw   $0x1,0x50(%ebx)
801060ae:	75 30                	jne    801060e0 <sys_chdir+0x80>
    iunlockput(ip);
    end_op();
    return -1;
  }
  iunlock(ip);
801060b0:	83 ec 0c             	sub    $0xc,%esp
801060b3:	53                   	push   %ebx
801060b4:	e8 57 b8 ff ff       	call   80101910 <iunlock>
  iput(curproc->cwd);
801060b9:	58                   	pop    %eax
801060ba:	ff 76 68             	push   0x68(%esi)
801060bd:	e8 9e b8 ff ff       	call   80101960 <iput>
  end_op();
801060c2:	e8 b9 cd ff ff       	call   80102e80 <end_op>
  curproc->cwd = ip;
801060c7:	89 5e 68             	mov    %ebx,0x68(%esi)
  return 0;
801060ca:	83 c4 10             	add    $0x10,%esp
801060cd:	31 c0                	xor    %eax,%eax
}
801060cf:	8d 65 f8             	lea    -0x8(%ebp),%esp
801060d2:	5b                   	pop    %ebx
801060d3:	5e                   	pop    %esi
801060d4:	5d                   	pop    %ebp
801060d5:	c3                   	ret    
801060d6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801060dd:	8d 76 00             	lea    0x0(%esi),%esi
    iunlockput(ip);
801060e0:	83 ec 0c             	sub    $0xc,%esp
801060e3:	53                   	push   %ebx
801060e4:	e8 d7 b9 ff ff       	call   80101ac0 <iunlockput>
    end_op();
801060e9:	e8 92 cd ff ff       	call   80102e80 <end_op>
    return -1;
801060ee:	83 c4 10             	add    $0x10,%esp
801060f1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801060f6:	eb d7                	jmp    801060cf <sys_chdir+0x6f>
801060f8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801060ff:	90                   	nop
    end_op();
80106100:	e8 7b cd ff ff       	call   80102e80 <end_op>
    return -1;
80106105:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010610a:	eb c3                	jmp    801060cf <sys_chdir+0x6f>
8010610c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80106110 <sys_exec>:

int
sys_exec(void)
{
80106110:	55                   	push   %ebp
80106111:	89 e5                	mov    %esp,%ebp
80106113:	57                   	push   %edi
80106114:	56                   	push   %esi
  char *path, *argv[MAXARG];
  int i;
  uint uargv, uarg;

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
80106115:	8d 85 5c ff ff ff    	lea    -0xa4(%ebp),%eax
{
8010611b:	53                   	push   %ebx
8010611c:	81 ec a4 00 00 00    	sub    $0xa4,%esp
  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
80106122:	50                   	push   %eax
80106123:	6a 00                	push   $0x0
80106125:	e8 f6 f4 ff ff       	call   80105620 <argstr>
8010612a:	83 c4 10             	add    $0x10,%esp
8010612d:	85 c0                	test   %eax,%eax
8010612f:	0f 88 87 00 00 00    	js     801061bc <sys_exec+0xac>
80106135:	83 ec 08             	sub    $0x8,%esp
80106138:	8d 85 60 ff ff ff    	lea    -0xa0(%ebp),%eax
8010613e:	50                   	push   %eax
8010613f:	6a 01                	push   $0x1
80106141:	e8 1a f4 ff ff       	call   80105560 <argint>
80106146:	83 c4 10             	add    $0x10,%esp
80106149:	85 c0                	test   %eax,%eax
8010614b:	78 6f                	js     801061bc <sys_exec+0xac>
    return -1;
  }
  memset(argv, 0, sizeof(argv));
8010614d:	83 ec 04             	sub    $0x4,%esp
80106150:	8d b5 68 ff ff ff    	lea    -0x98(%ebp),%esi
  for(i=0;; i++){
80106156:	31 db                	xor    %ebx,%ebx
  memset(argv, 0, sizeof(argv));
80106158:	68 80 00 00 00       	push   $0x80
8010615d:	6a 00                	push   $0x0
8010615f:	56                   	push   %esi
80106160:	e8 3b f1 ff ff       	call   801052a0 <memset>
80106165:	83 c4 10             	add    $0x10,%esp
80106168:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010616f:	90                   	nop
    if(i >= NELEM(argv))
      return -1;
    if(fetchint(uargv+4*i, (int*)&uarg) < 0)
80106170:	83 ec 08             	sub    $0x8,%esp
80106173:	8d 85 64 ff ff ff    	lea    -0x9c(%ebp),%eax
80106179:	8d 3c 9d 00 00 00 00 	lea    0x0(,%ebx,4),%edi
80106180:	50                   	push   %eax
80106181:	8b 85 60 ff ff ff    	mov    -0xa0(%ebp),%eax
80106187:	01 f8                	add    %edi,%eax
80106189:	50                   	push   %eax
8010618a:	e8 41 f3 ff ff       	call   801054d0 <fetchint>
8010618f:	83 c4 10             	add    $0x10,%esp
80106192:	85 c0                	test   %eax,%eax
80106194:	78 26                	js     801061bc <sys_exec+0xac>
      return -1;
    if(uarg == 0){
80106196:	8b 85 64 ff ff ff    	mov    -0x9c(%ebp),%eax
8010619c:	85 c0                	test   %eax,%eax
8010619e:	74 30                	je     801061d0 <sys_exec+0xc0>
      argv[i] = 0;
      break;
    }
    if(fetchstr(uarg, &argv[i]) < 0)
801061a0:	83 ec 08             	sub    $0x8,%esp
801061a3:	8d 14 3e             	lea    (%esi,%edi,1),%edx
801061a6:	52                   	push   %edx
801061a7:	50                   	push   %eax
801061a8:	e8 63 f3 ff ff       	call   80105510 <fetchstr>
801061ad:	83 c4 10             	add    $0x10,%esp
801061b0:	85 c0                	test   %eax,%eax
801061b2:	78 08                	js     801061bc <sys_exec+0xac>
  for(i=0;; i++){
801061b4:	83 c3 01             	add    $0x1,%ebx
    if(i >= NELEM(argv))
801061b7:	83 fb 20             	cmp    $0x20,%ebx
801061ba:	75 b4                	jne    80106170 <sys_exec+0x60>
      return -1;
  }
  return exec(path, argv);
}
801061bc:	8d 65 f4             	lea    -0xc(%ebp),%esp
    return -1;
801061bf:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801061c4:	5b                   	pop    %ebx
801061c5:	5e                   	pop    %esi
801061c6:	5f                   	pop    %edi
801061c7:	5d                   	pop    %ebp
801061c8:	c3                   	ret    
801061c9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
      argv[i] = 0;
801061d0:	c7 84 9d 68 ff ff ff 	movl   $0x0,-0x98(%ebp,%ebx,4)
801061d7:	00 00 00 00 
  return exec(path, argv);
801061db:	83 ec 08             	sub    $0x8,%esp
801061de:	56                   	push   %esi
801061df:	ff b5 5c ff ff ff    	push   -0xa4(%ebp)
801061e5:	e8 c6 a8 ff ff       	call   80100ab0 <exec>
801061ea:	83 c4 10             	add    $0x10,%esp
}
801061ed:	8d 65 f4             	lea    -0xc(%ebp),%esp
801061f0:	5b                   	pop    %ebx
801061f1:	5e                   	pop    %esi
801061f2:	5f                   	pop    %edi
801061f3:	5d                   	pop    %ebp
801061f4:	c3                   	ret    
801061f5:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801061fc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80106200 <sys_pipe>:

int
sys_pipe(void)
{
80106200:	55                   	push   %ebp
80106201:	89 e5                	mov    %esp,%ebp
80106203:	57                   	push   %edi
80106204:	56                   	push   %esi
  int *fd;
  struct file *rf, *wf;
  int fd0, fd1;

  if(argptr(0, (void*)&fd, 2*sizeof(fd[0])) < 0)
80106205:	8d 45 dc             	lea    -0x24(%ebp),%eax
{
80106208:	53                   	push   %ebx
80106209:	83 ec 20             	sub    $0x20,%esp
  if(argptr(0, (void*)&fd, 2*sizeof(fd[0])) < 0)
8010620c:	6a 08                	push   $0x8
8010620e:	50                   	push   %eax
8010620f:	6a 00                	push   $0x0
80106211:	e8 9a f3 ff ff       	call   801055b0 <argptr>
80106216:	83 c4 10             	add    $0x10,%esp
80106219:	85 c0                	test   %eax,%eax
8010621b:	78 4a                	js     80106267 <sys_pipe+0x67>
    return -1;
  if(pipealloc(&rf, &wf) < 0)
8010621d:	83 ec 08             	sub    $0x8,%esp
80106220:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80106223:	50                   	push   %eax
80106224:	8d 45 e0             	lea    -0x20(%ebp),%eax
80106227:	50                   	push   %eax
80106228:	e8 b3 d2 ff ff       	call   801034e0 <pipealloc>
8010622d:	83 c4 10             	add    $0x10,%esp
80106230:	85 c0                	test   %eax,%eax
80106232:	78 33                	js     80106267 <sys_pipe+0x67>
    return -1;
  fd0 = -1;
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
80106234:	8b 7d e0             	mov    -0x20(%ebp),%edi
  for(fd = 0; fd < NOFILE; fd++){
80106237:	31 db                	xor    %ebx,%ebx
  struct proc *curproc = myproc();
80106239:	e8 32 d8 ff ff       	call   80103a70 <myproc>
  for(fd = 0; fd < NOFILE; fd++){
8010623e:	66 90                	xchg   %ax,%ax
    if(curproc->ofile[fd] == 0){
80106240:	8b 74 98 28          	mov    0x28(%eax,%ebx,4),%esi
80106244:	85 f6                	test   %esi,%esi
80106246:	74 28                	je     80106270 <sys_pipe+0x70>
  for(fd = 0; fd < NOFILE; fd++){
80106248:	83 c3 01             	add    $0x1,%ebx
8010624b:	83 fb 10             	cmp    $0x10,%ebx
8010624e:	75 f0                	jne    80106240 <sys_pipe+0x40>
    if(fd0 >= 0)
      myproc()->ofile[fd0] = 0;
    fileclose(rf);
80106250:	83 ec 0c             	sub    $0xc,%esp
80106253:	ff 75 e0             	push   -0x20(%ebp)
80106256:	e8 45 ad ff ff       	call   80100fa0 <fileclose>
    fileclose(wf);
8010625b:	58                   	pop    %eax
8010625c:	ff 75 e4             	push   -0x1c(%ebp)
8010625f:	e8 3c ad ff ff       	call   80100fa0 <fileclose>
    return -1;
80106264:	83 c4 10             	add    $0x10,%esp
80106267:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010626c:	eb 53                	jmp    801062c1 <sys_pipe+0xc1>
8010626e:	66 90                	xchg   %ax,%ax
      curproc->ofile[fd] = f;
80106270:	8d 73 08             	lea    0x8(%ebx),%esi
80106273:	89 7c b0 08          	mov    %edi,0x8(%eax,%esi,4)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
80106277:	8b 7d e4             	mov    -0x1c(%ebp),%edi
  struct proc *curproc = myproc();
8010627a:	e8 f1 d7 ff ff       	call   80103a70 <myproc>
  for(fd = 0; fd < NOFILE; fd++){
8010627f:	31 d2                	xor    %edx,%edx
80106281:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    if(curproc->ofile[fd] == 0){
80106288:	8b 4c 90 28          	mov    0x28(%eax,%edx,4),%ecx
8010628c:	85 c9                	test   %ecx,%ecx
8010628e:	74 20                	je     801062b0 <sys_pipe+0xb0>
  for(fd = 0; fd < NOFILE; fd++){
80106290:	83 c2 01             	add    $0x1,%edx
80106293:	83 fa 10             	cmp    $0x10,%edx
80106296:	75 f0                	jne    80106288 <sys_pipe+0x88>
      myproc()->ofile[fd0] = 0;
80106298:	e8 d3 d7 ff ff       	call   80103a70 <myproc>
8010629d:	c7 44 b0 08 00 00 00 	movl   $0x0,0x8(%eax,%esi,4)
801062a4:	00 
801062a5:	eb a9                	jmp    80106250 <sys_pipe+0x50>
801062a7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801062ae:	66 90                	xchg   %ax,%ax
      curproc->ofile[fd] = f;
801062b0:	89 7c 90 28          	mov    %edi,0x28(%eax,%edx,4)
  }
  fd[0] = fd0;
801062b4:	8b 45 dc             	mov    -0x24(%ebp),%eax
801062b7:	89 18                	mov    %ebx,(%eax)
  fd[1] = fd1;
801062b9:	8b 45 dc             	mov    -0x24(%ebp),%eax
801062bc:	89 50 04             	mov    %edx,0x4(%eax)
  return 0;
801062bf:	31 c0                	xor    %eax,%eax
}
801062c1:	8d 65 f4             	lea    -0xc(%ebp),%esp
801062c4:	5b                   	pop    %ebx
801062c5:	5e                   	pop    %esi
801062c6:	5f                   	pop    %edi
801062c7:	5d                   	pop    %ebp
801062c8:	c3                   	ret    
801062c9:	66 90                	xchg   %ax,%ax
801062cb:	66 90                	xchg   %ax,%ax
801062cd:	66 90                	xchg   %ax,%ax
801062cf:	90                   	nop

801062d0 <sys_fork>:
#include "proc.h"

int
sys_fork(void)
{
  return fork();
801062d0:	e9 6b d9 ff ff       	jmp    80103c40 <fork>
801062d5:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801062dc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

801062e0 <sys_exit>:
}

int
sys_exit(void)
{
801062e0:	55                   	push   %ebp
801062e1:	89 e5                	mov    %esp,%ebp
801062e3:	83 ec 08             	sub    $0x8,%esp
  exit();
801062e6:	e8 f5 dd ff ff       	call   801040e0 <exit>
  return 0;  // not reached
}
801062eb:	31 c0                	xor    %eax,%eax
801062ed:	c9                   	leave  
801062ee:	c3                   	ret    
801062ef:	90                   	nop

801062f0 <sys_wait>:

int
sys_wait(void)
{
  return wait();
801062f0:	e9 6b df ff ff       	jmp    80104260 <wait>
801062f5:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801062fc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80106300 <sys_kill>:
}

int
sys_kill(void)
{
80106300:	55                   	push   %ebp
80106301:	89 e5                	mov    %esp,%ebp
80106303:	83 ec 20             	sub    $0x20,%esp
  int pid;

  if(argint(0, &pid) < 0)
80106306:	8d 45 f4             	lea    -0xc(%ebp),%eax
80106309:	50                   	push   %eax
8010630a:	6a 00                	push   $0x0
8010630c:	e8 4f f2 ff ff       	call   80105560 <argint>
80106311:	83 c4 10             	add    $0x10,%esp
80106314:	85 c0                	test   %eax,%eax
80106316:	78 18                	js     80106330 <sys_kill+0x30>
    return -1;
  return kill(pid);
80106318:	83 ec 0c             	sub    $0xc,%esp
8010631b:	ff 75 f4             	push   -0xc(%ebp)
8010631e:	e8 4d e2 ff ff       	call   80104570 <kill>
80106323:	83 c4 10             	add    $0x10,%esp
}
80106326:	c9                   	leave  
80106327:	c3                   	ret    
80106328:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010632f:	90                   	nop
80106330:	c9                   	leave  
    return -1;
80106331:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80106336:	c3                   	ret    
80106337:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010633e:	66 90                	xchg   %ax,%ax

80106340 <sys_getpid>:

int
sys_getpid(void)
{
80106340:	55                   	push   %ebp
80106341:	89 e5                	mov    %esp,%ebp
80106343:	83 ec 08             	sub    $0x8,%esp
  return myproc()->pid;
80106346:	e8 25 d7 ff ff       	call   80103a70 <myproc>
8010634b:	8b 40 10             	mov    0x10(%eax),%eax
}
8010634e:	c9                   	leave  
8010634f:	c3                   	ret    

80106350 <sys_sbrk>:

int
sys_sbrk(void)
{
80106350:	55                   	push   %ebp
80106351:	89 e5                	mov    %esp,%ebp
80106353:	53                   	push   %ebx
  int addr;
  int n;

  if(argint(0, &n) < 0)
80106354:	8d 45 f4             	lea    -0xc(%ebp),%eax
{
80106357:	83 ec 1c             	sub    $0x1c,%esp
  if(argint(0, &n) < 0)
8010635a:	50                   	push   %eax
8010635b:	6a 00                	push   $0x0
8010635d:	e8 fe f1 ff ff       	call   80105560 <argint>
80106362:	83 c4 10             	add    $0x10,%esp
80106365:	85 c0                	test   %eax,%eax
80106367:	78 27                	js     80106390 <sys_sbrk+0x40>
    return -1;
  addr = myproc()->sz;
80106369:	e8 02 d7 ff ff       	call   80103a70 <myproc>
  if(growproc(n) < 0)
8010636e:	83 ec 0c             	sub    $0xc,%esp
  addr = myproc()->sz;
80106371:	8b 18                	mov    (%eax),%ebx
  if(growproc(n) < 0)
80106373:	ff 75 f4             	push   -0xc(%ebp)
80106376:	e8 45 d8 ff ff       	call   80103bc0 <growproc>
8010637b:	83 c4 10             	add    $0x10,%esp
8010637e:	85 c0                	test   %eax,%eax
80106380:	78 0e                	js     80106390 <sys_sbrk+0x40>
    return -1;
  return addr;
}
80106382:	89 d8                	mov    %ebx,%eax
80106384:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80106387:	c9                   	leave  
80106388:	c3                   	ret    
80106389:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    return -1;
80106390:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
80106395:	eb eb                	jmp    80106382 <sys_sbrk+0x32>
80106397:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010639e:	66 90                	xchg   %ax,%ax

801063a0 <sys_sleep>:

int
sys_sleep(void)
{
801063a0:	55                   	push   %ebp
801063a1:	89 e5                	mov    %esp,%ebp
801063a3:	53                   	push   %ebx
  int n;
  uint ticks0;

  if(argint(0, &n) < 0)
801063a4:	8d 45 f4             	lea    -0xc(%ebp),%eax
{
801063a7:	83 ec 1c             	sub    $0x1c,%esp
  if(argint(0, &n) < 0)
801063aa:	50                   	push   %eax
801063ab:	6a 00                	push   $0x0
801063ad:	e8 ae f1 ff ff       	call   80105560 <argint>
801063b2:	83 c4 10             	add    $0x10,%esp
801063b5:	85 c0                	test   %eax,%eax
801063b7:	0f 88 8a 00 00 00    	js     80106447 <sys_sleep+0xa7>
    return -1;
  acquire(&tickslock);
801063bd:	83 ec 0c             	sub    $0xc,%esp
801063c0:	68 a0 5b 11 80       	push   $0x80115ba0
801063c5:	e8 16 ee ff ff       	call   801051e0 <acquire>
  ticks0 = ticks;
  while(ticks - ticks0 < n){
801063ca:	8b 55 f4             	mov    -0xc(%ebp),%edx
  ticks0 = ticks;
801063cd:	8b 1d 80 5b 11 80    	mov    0x80115b80,%ebx
  while(ticks - ticks0 < n){
801063d3:	83 c4 10             	add    $0x10,%esp
801063d6:	85 d2                	test   %edx,%edx
801063d8:	75 27                	jne    80106401 <sys_sleep+0x61>
801063da:	eb 54                	jmp    80106430 <sys_sleep+0x90>
801063dc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    if(myproc()->killed){
      release(&tickslock);
      return -1;
    }
    sleep(&ticks, &tickslock);
801063e0:	83 ec 08             	sub    $0x8,%esp
801063e3:	68 a0 5b 11 80       	push   $0x80115ba0
801063e8:	68 80 5b 11 80       	push   $0x80115b80
801063ed:	e8 2e e0 ff ff       	call   80104420 <sleep>
  while(ticks - ticks0 < n){
801063f2:	a1 80 5b 11 80       	mov    0x80115b80,%eax
801063f7:	83 c4 10             	add    $0x10,%esp
801063fa:	29 d8                	sub    %ebx,%eax
801063fc:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801063ff:	73 2f                	jae    80106430 <sys_sleep+0x90>
    if(myproc()->killed){
80106401:	e8 6a d6 ff ff       	call   80103a70 <myproc>
80106406:	8b 40 24             	mov    0x24(%eax),%eax
80106409:	85 c0                	test   %eax,%eax
8010640b:	74 d3                	je     801063e0 <sys_sleep+0x40>
      release(&tickslock);
8010640d:	83 ec 0c             	sub    $0xc,%esp
80106410:	68 a0 5b 11 80       	push   $0x80115ba0
80106415:	e8 66 ed ff ff       	call   80105180 <release>
  }
  release(&tickslock);
  return 0;
}
8010641a:	8b 5d fc             	mov    -0x4(%ebp),%ebx
      return -1;
8010641d:	83 c4 10             	add    $0x10,%esp
80106420:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80106425:	c9                   	leave  
80106426:	c3                   	ret    
80106427:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010642e:	66 90                	xchg   %ax,%ax
  release(&tickslock);
80106430:	83 ec 0c             	sub    $0xc,%esp
80106433:	68 a0 5b 11 80       	push   $0x80115ba0
80106438:	e8 43 ed ff ff       	call   80105180 <release>
  return 0;
8010643d:	83 c4 10             	add    $0x10,%esp
80106440:	31 c0                	xor    %eax,%eax
}
80106442:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80106445:	c9                   	leave  
80106446:	c3                   	ret    
    return -1;
80106447:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010644c:	eb f4                	jmp    80106442 <sys_sleep+0xa2>
8010644e:	66 90                	xchg   %ax,%ax

80106450 <sys_uptime>:

// return how many clock tick interrupts have occurred
// since start.
int
sys_uptime(void)
{
80106450:	55                   	push   %ebp
80106451:	89 e5                	mov    %esp,%ebp
80106453:	53                   	push   %ebx
80106454:	83 ec 10             	sub    $0x10,%esp
  uint xticks;

  acquire(&tickslock);
80106457:	68 a0 5b 11 80       	push   $0x80115ba0
8010645c:	e8 7f ed ff ff       	call   801051e0 <acquire>
  xticks = ticks;
80106461:	8b 1d 80 5b 11 80    	mov    0x80115b80,%ebx
  release(&tickslock);
80106467:	c7 04 24 a0 5b 11 80 	movl   $0x80115ba0,(%esp)
8010646e:	e8 0d ed ff ff       	call   80105180 <release>
  return xticks;
}
80106473:	89 d8                	mov    %ebx,%eax
80106475:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80106478:	c9                   	leave  
80106479:	c3                   	ret    
8010647a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80106480 <sys_printprocinfo>:


extern void printprocinfo(void);

int sys_printprocinfo(void){
80106480:	55                   	push   %ebp
80106481:	89 e5                	mov    %esp,%ebp
80106483:	83 ec 08             	sub    $0x8,%esp
  printprocinfo();
80106486:	e8 75 e4 ff ff       	call   80104900 <printprocinfo>
  return 0;
}
8010648b:	31 c0                	xor    %eax,%eax
8010648d:	c9                   	leave  
8010648e:	c3                   	ret    

8010648f <alltraps>:

  # vectors.S sends all traps here.
.globl alltraps
alltraps:
  # Build trap frame.
  pushl %ds
8010648f:	1e                   	push   %ds
  pushl %es
80106490:	06                   	push   %es
  pushl %fs
80106491:	0f a0                	push   %fs
  pushl %gs
80106493:	0f a8                	push   %gs
  pushal
80106495:	60                   	pusha  
  
  # Set up data segments.
  movw $(SEG_KDATA<<3), %ax
80106496:	66 b8 10 00          	mov    $0x10,%ax
  movw %ax, %ds
8010649a:	8e d8                	mov    %eax,%ds
  movw %ax, %es
8010649c:	8e c0                	mov    %eax,%es

  # Call trap(tf), where tf=%esp
  pushl %esp
8010649e:	54                   	push   %esp
  call trap
8010649f:	e8 cc 00 00 00       	call   80106570 <trap>
  addl $4, %esp
801064a4:	83 c4 04             	add    $0x4,%esp

801064a7 <trapret>:

  # Return falls through to trapret...
.globl trapret
trapret:
  popal
801064a7:	61                   	popa   
  popl %gs
801064a8:	0f a9                	pop    %gs
  popl %fs
801064aa:	0f a1                	pop    %fs
  popl %es
801064ac:	07                   	pop    %es
  popl %ds
801064ad:	1f                   	pop    %ds
  addl $0x8, %esp  # trapno and errcode
801064ae:	83 c4 08             	add    $0x8,%esp
  iret
801064b1:	cf                   	iret   
801064b2:	66 90                	xchg   %ax,%ax
801064b4:	66 90                	xchg   %ax,%ax
801064b6:	66 90                	xchg   %ax,%ax
801064b8:	66 90                	xchg   %ax,%ax
801064ba:	66 90                	xchg   %ax,%ax
801064bc:	66 90                	xchg   %ax,%ax
801064be:	66 90                	xchg   %ax,%ax

801064c0 <tvinit>:
struct spinlock tickslock;
uint ticks;

void
tvinit(void)
{
801064c0:	55                   	push   %ebp
  int i;

  for(i = 0; i < 256; i++)
801064c1:	31 c0                	xor    %eax,%eax
{
801064c3:	89 e5                	mov    %esp,%ebp
801064c5:	83 ec 08             	sub    $0x8,%esp
801064c8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801064cf:	90                   	nop
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
801064d0:	8b 14 85 08 b0 10 80 	mov    -0x7fef4ff8(,%eax,4),%edx
801064d7:	c7 04 c5 e2 5b 11 80 	movl   $0x8e000008,-0x7feea41e(,%eax,8)
801064de:	08 00 00 8e 
801064e2:	66 89 14 c5 e0 5b 11 	mov    %dx,-0x7feea420(,%eax,8)
801064e9:	80 
801064ea:	c1 ea 10             	shr    $0x10,%edx
801064ed:	66 89 14 c5 e6 5b 11 	mov    %dx,-0x7feea41a(,%eax,8)
801064f4:	80 
  for(i = 0; i < 256; i++)
801064f5:	83 c0 01             	add    $0x1,%eax
801064f8:	3d 00 01 00 00       	cmp    $0x100,%eax
801064fd:	75 d1                	jne    801064d0 <tvinit+0x10>
  SETGATE(idt[T_SYSCALL], 1, SEG_KCODE<<3, vectors[T_SYSCALL], DPL_USER);

  initlock(&tickslock, "time");
801064ff:	83 ec 08             	sub    $0x8,%esp
  SETGATE(idt[T_SYSCALL], 1, SEG_KCODE<<3, vectors[T_SYSCALL], DPL_USER);
80106502:	a1 08 b1 10 80       	mov    0x8010b108,%eax
80106507:	c7 05 e2 5d 11 80 08 	movl   $0xef000008,0x80115de2
8010650e:	00 00 ef 
  initlock(&tickslock, "time");
80106511:	68 82 84 10 80       	push   $0x80108482
80106516:	68 a0 5b 11 80       	push   $0x80115ba0
  SETGATE(idt[T_SYSCALL], 1, SEG_KCODE<<3, vectors[T_SYSCALL], DPL_USER);
8010651b:	66 a3 e0 5d 11 80    	mov    %ax,0x80115de0
80106521:	c1 e8 10             	shr    $0x10,%eax
80106524:	66 a3 e6 5d 11 80    	mov    %ax,0x80115de6
  initlock(&tickslock, "time");
8010652a:	e8 e1 ea ff ff       	call   80105010 <initlock>
}
8010652f:	83 c4 10             	add    $0x10,%esp
80106532:	c9                   	leave  
80106533:	c3                   	ret    
80106534:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010653b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010653f:	90                   	nop

80106540 <idtinit>:

void
idtinit(void)
{
80106540:	55                   	push   %ebp
  pd[0] = size-1;
80106541:	b8 ff 07 00 00       	mov    $0x7ff,%eax
80106546:	89 e5                	mov    %esp,%ebp
80106548:	83 ec 10             	sub    $0x10,%esp
8010654b:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
8010654f:	b8 e0 5b 11 80       	mov    $0x80115be0,%eax
80106554:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
80106558:	c1 e8 10             	shr    $0x10,%eax
8010655b:	66 89 45 fe          	mov    %ax,-0x2(%ebp)
  asm volatile("lidt (%0)" : : "r" (pd));
8010655f:	8d 45 fa             	lea    -0x6(%ebp),%eax
80106562:	0f 01 18             	lidtl  (%eax)
  lidt(idt, sizeof(idt));
}
80106565:	c9                   	leave  
80106566:	c3                   	ret    
80106567:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010656e:	66 90                	xchg   %ax,%ax

80106570 <trap>:

//PAGEBREAK: 41
void
trap(struct trapframe *tf)
{
80106570:	55                   	push   %ebp
80106571:	89 e5                	mov    %esp,%ebp
80106573:	57                   	push   %edi
80106574:	56                   	push   %esi
80106575:	53                   	push   %ebx
80106576:	83 ec 1c             	sub    $0x1c,%esp
80106579:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(tf->trapno == T_SYSCALL){
8010657c:	8b 43 30             	mov    0x30(%ebx),%eax
8010657f:	83 f8 40             	cmp    $0x40,%eax
80106582:	0f 84 68 01 00 00    	je     801066f0 <trap+0x180>
    if(myproc()->killed)
      exit();
    return;
  }

  switch(tf->trapno){
80106588:	83 e8 20             	sub    $0x20,%eax
8010658b:	83 f8 1f             	cmp    $0x1f,%eax
8010658e:	0f 87 8c 00 00 00    	ja     80106620 <trap+0xb0>
80106594:	ff 24 85 68 88 10 80 	jmp    *-0x7fef7798(,%eax,4)
8010659b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
8010659f:	90                   	nop
      release(&tickslock);
    }
    lapiceoi();
    break;
  case T_IRQ0 + IRQ_IDE:
    ideintr();
801065a0:	e8 4b bd ff ff       	call   801022f0 <ideintr>
    lapiceoi();
801065a5:	e8 16 c4 ff ff       	call   801029c0 <lapiceoi>
  }

  // Force process exit if it has been killed and is in user space.
  // (If it is still executing in the kernel, let it keep running
  // until it gets to the regular system call return.)
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
801065aa:	e8 c1 d4 ff ff       	call   80103a70 <myproc>
801065af:	85 c0                	test   %eax,%eax
801065b1:	74 1d                	je     801065d0 <trap+0x60>
801065b3:	e8 b8 d4 ff ff       	call   80103a70 <myproc>
801065b8:	8b 50 24             	mov    0x24(%eax),%edx
801065bb:	85 d2                	test   %edx,%edx
801065bd:	74 11                	je     801065d0 <trap+0x60>
801065bf:	0f b7 43 3c          	movzwl 0x3c(%ebx),%eax
801065c3:	83 e0 03             	and    $0x3,%eax
801065c6:	66 83 f8 03          	cmp    $0x3,%ax
801065ca:	0f 84 e8 01 00 00    	je     801067b8 <trap+0x248>
    exit();

  // Force process to give up CPU on clock tick.
  // If interrupts were on while locks held, would need to check nlock.
  if(myproc() && myproc()->state == RUNNING &&
801065d0:	e8 9b d4 ff ff       	call   80103a70 <myproc>
801065d5:	85 c0                	test   %eax,%eax
801065d7:	74 0f                	je     801065e8 <trap+0x78>
801065d9:	e8 92 d4 ff ff       	call   80103a70 <myproc>
801065de:	83 78 0c 04          	cmpl   $0x4,0xc(%eax)
801065e2:	0f 84 b8 00 00 00    	je     801066a0 <trap+0x130>
     tf->trapno == T_IRQ0+IRQ_TIMER)
    yield();

  // Check if the process has been killed since we yielded
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
801065e8:	e8 83 d4 ff ff       	call   80103a70 <myproc>
801065ed:	85 c0                	test   %eax,%eax
801065ef:	74 1d                	je     8010660e <trap+0x9e>
801065f1:	e8 7a d4 ff ff       	call   80103a70 <myproc>
801065f6:	8b 40 24             	mov    0x24(%eax),%eax
801065f9:	85 c0                	test   %eax,%eax
801065fb:	74 11                	je     8010660e <trap+0x9e>
801065fd:	0f b7 43 3c          	movzwl 0x3c(%ebx),%eax
80106601:	83 e0 03             	and    $0x3,%eax
80106604:	66 83 f8 03          	cmp    $0x3,%ax
80106608:	0f 84 0f 01 00 00    	je     8010671d <trap+0x1ad>
    exit();
}
8010660e:	8d 65 f4             	lea    -0xc(%ebp),%esp
80106611:	5b                   	pop    %ebx
80106612:	5e                   	pop    %esi
80106613:	5f                   	pop    %edi
80106614:	5d                   	pop    %ebp
80106615:	c3                   	ret    
80106616:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010661d:	8d 76 00             	lea    0x0(%esi),%esi
    if(myproc() == 0 || (tf->cs&3) == 0){
80106620:	e8 4b d4 ff ff       	call   80103a70 <myproc>
80106625:	8b 7b 38             	mov    0x38(%ebx),%edi
80106628:	85 c0                	test   %eax,%eax
8010662a:	0f 84 a2 01 00 00    	je     801067d2 <trap+0x262>
80106630:	f6 43 3c 03          	testb  $0x3,0x3c(%ebx)
80106634:	0f 84 98 01 00 00    	je     801067d2 <trap+0x262>

static inline uint
rcr2(void)
{
  uint val;
  asm volatile("movl %%cr2,%0" : "=r" (val));
8010663a:	0f 20 d1             	mov    %cr2,%ecx
8010663d:	89 4d d8             	mov    %ecx,-0x28(%ebp)
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106640:	e8 0b d4 ff ff       	call   80103a50 <cpuid>
80106645:	8b 73 30             	mov    0x30(%ebx),%esi
80106648:	89 45 dc             	mov    %eax,-0x24(%ebp)
8010664b:	8b 43 34             	mov    0x34(%ebx),%eax
8010664e:	89 45 e4             	mov    %eax,-0x1c(%ebp)
            myproc()->pid, myproc()->name, tf->trapno,
80106651:	e8 1a d4 ff ff       	call   80103a70 <myproc>
80106656:	89 45 e0             	mov    %eax,-0x20(%ebp)
80106659:	e8 12 d4 ff ff       	call   80103a70 <myproc>
    cprintf("pid %d %s: trap %d err %d on cpu %d "
8010665e:	8b 4d d8             	mov    -0x28(%ebp),%ecx
80106661:	8b 55 dc             	mov    -0x24(%ebp),%edx
80106664:	51                   	push   %ecx
80106665:	57                   	push   %edi
80106666:	52                   	push   %edx
80106667:	ff 75 e4             	push   -0x1c(%ebp)
8010666a:	56                   	push   %esi
            myproc()->pid, myproc()->name, tf->trapno,
8010666b:	8b 75 e0             	mov    -0x20(%ebp),%esi
8010666e:	83 c6 6c             	add    $0x6c,%esi
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106671:	56                   	push   %esi
80106672:	ff 70 10             	push   0x10(%eax)
80106675:	68 24 88 10 80       	push   $0x80108824
8010667a:	e8 21 a0 ff ff       	call   801006a0 <cprintf>
    myproc()->killed = 1;
8010667f:	83 c4 20             	add    $0x20,%esp
80106682:	e8 e9 d3 ff ff       	call   80103a70 <myproc>
80106687:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
8010668e:	e8 dd d3 ff ff       	call   80103a70 <myproc>
80106693:	85 c0                	test   %eax,%eax
80106695:	0f 85 18 ff ff ff    	jne    801065b3 <trap+0x43>
8010669b:	e9 30 ff ff ff       	jmp    801065d0 <trap+0x60>
  if(myproc() && myproc()->state == RUNNING &&
801066a0:	83 7b 30 20          	cmpl   $0x20,0x30(%ebx)
801066a4:	0f 85 3e ff ff ff    	jne    801065e8 <trap+0x78>
    yield();
801066aa:	e8 e1 dc ff ff       	call   80104390 <yield>
801066af:	e9 34 ff ff ff       	jmp    801065e8 <trap+0x78>
801066b4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
801066b8:	8b 7b 38             	mov    0x38(%ebx),%edi
801066bb:	0f b7 73 3c          	movzwl 0x3c(%ebx),%esi
801066bf:	e8 8c d3 ff ff       	call   80103a50 <cpuid>
801066c4:	57                   	push   %edi
801066c5:	56                   	push   %esi
801066c6:	50                   	push   %eax
801066c7:	68 cc 87 10 80       	push   $0x801087cc
801066cc:	e8 cf 9f ff ff       	call   801006a0 <cprintf>
    lapiceoi();
801066d1:	e8 ea c2 ff ff       	call   801029c0 <lapiceoi>
    break;
801066d6:	83 c4 10             	add    $0x10,%esp
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
801066d9:	e8 92 d3 ff ff       	call   80103a70 <myproc>
801066de:	85 c0                	test   %eax,%eax
801066e0:	0f 85 cd fe ff ff    	jne    801065b3 <trap+0x43>
801066e6:	e9 e5 fe ff ff       	jmp    801065d0 <trap+0x60>
801066eb:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801066ef:	90                   	nop
    if(myproc()->killed)
801066f0:	e8 7b d3 ff ff       	call   80103a70 <myproc>
801066f5:	8b 70 24             	mov    0x24(%eax),%esi
801066f8:	85 f6                	test   %esi,%esi
801066fa:	0f 85 c8 00 00 00    	jne    801067c8 <trap+0x258>
    myproc()->tf = tf;
80106700:	e8 6b d3 ff ff       	call   80103a70 <myproc>
80106705:	89 58 18             	mov    %ebx,0x18(%eax)
    syscall();
80106708:	e8 93 ef ff ff       	call   801056a0 <syscall>
    if(myproc()->killed)
8010670d:	e8 5e d3 ff ff       	call   80103a70 <myproc>
80106712:	8b 48 24             	mov    0x24(%eax),%ecx
80106715:	85 c9                	test   %ecx,%ecx
80106717:	0f 84 f1 fe ff ff    	je     8010660e <trap+0x9e>
}
8010671d:	8d 65 f4             	lea    -0xc(%ebp),%esp
80106720:	5b                   	pop    %ebx
80106721:	5e                   	pop    %esi
80106722:	5f                   	pop    %edi
80106723:	5d                   	pop    %ebp
      exit();
80106724:	e9 b7 d9 ff ff       	jmp    801040e0 <exit>
80106729:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    uartintr();
80106730:	e8 3b 02 00 00       	call   80106970 <uartintr>
    lapiceoi();
80106735:	e8 86 c2 ff ff       	call   801029c0 <lapiceoi>
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
8010673a:	e8 31 d3 ff ff       	call   80103a70 <myproc>
8010673f:	85 c0                	test   %eax,%eax
80106741:	0f 85 6c fe ff ff    	jne    801065b3 <trap+0x43>
80106747:	e9 84 fe ff ff       	jmp    801065d0 <trap+0x60>
8010674c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    kbdintr();
80106750:	e8 2b c1 ff ff       	call   80102880 <kbdintr>
    lapiceoi();
80106755:	e8 66 c2 ff ff       	call   801029c0 <lapiceoi>
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
8010675a:	e8 11 d3 ff ff       	call   80103a70 <myproc>
8010675f:	85 c0                	test   %eax,%eax
80106761:	0f 85 4c fe ff ff    	jne    801065b3 <trap+0x43>
80106767:	e9 64 fe ff ff       	jmp    801065d0 <trap+0x60>
8010676c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    if(cpuid() == 0){
80106770:	e8 db d2 ff ff       	call   80103a50 <cpuid>
80106775:	85 c0                	test   %eax,%eax
80106777:	0f 85 28 fe ff ff    	jne    801065a5 <trap+0x35>
      acquire(&tickslock);
8010677d:	83 ec 0c             	sub    $0xc,%esp
80106780:	68 a0 5b 11 80       	push   $0x80115ba0
80106785:	e8 56 ea ff ff       	call   801051e0 <acquire>
      wakeup(&ticks);
8010678a:	c7 04 24 80 5b 11 80 	movl   $0x80115b80,(%esp)
      ticks++;
80106791:	83 05 80 5b 11 80 01 	addl   $0x1,0x80115b80
      wakeup(&ticks);
80106798:	e8 43 dd ff ff       	call   801044e0 <wakeup>
      release(&tickslock);
8010679d:	c7 04 24 a0 5b 11 80 	movl   $0x80115ba0,(%esp)
801067a4:	e8 d7 e9 ff ff       	call   80105180 <release>
801067a9:	83 c4 10             	add    $0x10,%esp
    lapiceoi();
801067ac:	e9 f4 fd ff ff       	jmp    801065a5 <trap+0x35>
801067b1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    exit();
801067b8:	e8 23 d9 ff ff       	call   801040e0 <exit>
801067bd:	e9 0e fe ff ff       	jmp    801065d0 <trap+0x60>
801067c2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
      exit();
801067c8:	e8 13 d9 ff ff       	call   801040e0 <exit>
801067cd:	e9 2e ff ff ff       	jmp    80106700 <trap+0x190>
801067d2:	0f 20 d6             	mov    %cr2,%esi
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
801067d5:	e8 76 d2 ff ff       	call   80103a50 <cpuid>
801067da:	83 ec 0c             	sub    $0xc,%esp
801067dd:	56                   	push   %esi
801067de:	57                   	push   %edi
801067df:	50                   	push   %eax
801067e0:	ff 73 30             	push   0x30(%ebx)
801067e3:	68 f0 87 10 80       	push   $0x801087f0
801067e8:	e8 b3 9e ff ff       	call   801006a0 <cprintf>
      panic("trap");
801067ed:	83 c4 14             	add    $0x14,%esp
801067f0:	68 c5 87 10 80       	push   $0x801087c5
801067f5:	e8 86 9b ff ff       	call   80100380 <panic>
801067fa:	66 90                	xchg   %ax,%ax
801067fc:	66 90                	xchg   %ax,%ax
801067fe:	66 90                	xchg   %ax,%ax

80106800 <uartgetc>:
}

static int
uartgetc(void)
{
  if(!uart)
80106800:	a1 e0 63 11 80       	mov    0x801163e0,%eax
80106805:	85 c0                	test   %eax,%eax
80106807:	74 17                	je     80106820 <uartgetc+0x20>
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80106809:	ba fd 03 00 00       	mov    $0x3fd,%edx
8010680e:	ec                   	in     (%dx),%al
    return -1;
  if(!(inb(COM1+5) & 0x01))
8010680f:	a8 01                	test   $0x1,%al
80106811:	74 0d                	je     80106820 <uartgetc+0x20>
80106813:	ba f8 03 00 00       	mov    $0x3f8,%edx
80106818:	ec                   	in     (%dx),%al
    return -1;
  return inb(COM1+0);
80106819:	0f b6 c0             	movzbl %al,%eax
8010681c:	c3                   	ret    
8010681d:	8d 76 00             	lea    0x0(%esi),%esi
    return -1;
80106820:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80106825:	c3                   	ret    
80106826:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010682d:	8d 76 00             	lea    0x0(%esi),%esi

80106830 <uartinit>:
{
80106830:	55                   	push   %ebp
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80106831:	31 c9                	xor    %ecx,%ecx
80106833:	89 c8                	mov    %ecx,%eax
80106835:	89 e5                	mov    %esp,%ebp
80106837:	57                   	push   %edi
80106838:	bf fa 03 00 00       	mov    $0x3fa,%edi
8010683d:	56                   	push   %esi
8010683e:	89 fa                	mov    %edi,%edx
80106840:	53                   	push   %ebx
80106841:	83 ec 1c             	sub    $0x1c,%esp
80106844:	ee                   	out    %al,(%dx)
80106845:	be fb 03 00 00       	mov    $0x3fb,%esi
8010684a:	b8 80 ff ff ff       	mov    $0xffffff80,%eax
8010684f:	89 f2                	mov    %esi,%edx
80106851:	ee                   	out    %al,(%dx)
80106852:	b8 0c 00 00 00       	mov    $0xc,%eax
80106857:	ba f8 03 00 00       	mov    $0x3f8,%edx
8010685c:	ee                   	out    %al,(%dx)
8010685d:	bb f9 03 00 00       	mov    $0x3f9,%ebx
80106862:	89 c8                	mov    %ecx,%eax
80106864:	89 da                	mov    %ebx,%edx
80106866:	ee                   	out    %al,(%dx)
80106867:	b8 03 00 00 00       	mov    $0x3,%eax
8010686c:	89 f2                	mov    %esi,%edx
8010686e:	ee                   	out    %al,(%dx)
8010686f:	ba fc 03 00 00       	mov    $0x3fc,%edx
80106874:	89 c8                	mov    %ecx,%eax
80106876:	ee                   	out    %al,(%dx)
80106877:	b8 01 00 00 00       	mov    $0x1,%eax
8010687c:	89 da                	mov    %ebx,%edx
8010687e:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
8010687f:	ba fd 03 00 00       	mov    $0x3fd,%edx
80106884:	ec                   	in     (%dx),%al
  if(inb(COM1+5) == 0xFF)
80106885:	3c ff                	cmp    $0xff,%al
80106887:	74 78                	je     80106901 <uartinit+0xd1>
  uart = 1;
80106889:	c7 05 e0 63 11 80 01 	movl   $0x1,0x801163e0
80106890:	00 00 00 
80106893:	89 fa                	mov    %edi,%edx
80106895:	ec                   	in     (%dx),%al
80106896:	ba f8 03 00 00       	mov    $0x3f8,%edx
8010689b:	ec                   	in     (%dx),%al
  ioapicenable(IRQ_COM1, 0);
8010689c:	83 ec 08             	sub    $0x8,%esp
  for(p="xv6...\n"; *p; p++)
8010689f:	bf e8 88 10 80       	mov    $0x801088e8,%edi
801068a4:	be fd 03 00 00       	mov    $0x3fd,%esi
  ioapicenable(IRQ_COM1, 0);
801068a9:	6a 00                	push   $0x0
801068ab:	6a 04                	push   $0x4
801068ad:	e8 7e bc ff ff       	call   80102530 <ioapicenable>
  for(p="xv6...\n"; *p; p++)
801068b2:	c6 45 e7 78          	movb   $0x78,-0x19(%ebp)
  ioapicenable(IRQ_COM1, 0);
801068b6:	83 c4 10             	add    $0x10,%esp
801068b9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  if(!uart)
801068c0:	a1 e0 63 11 80       	mov    0x801163e0,%eax
801068c5:	bb 80 00 00 00       	mov    $0x80,%ebx
801068ca:	85 c0                	test   %eax,%eax
801068cc:	75 14                	jne    801068e2 <uartinit+0xb2>
801068ce:	eb 23                	jmp    801068f3 <uartinit+0xc3>
    microdelay(10);
801068d0:	83 ec 0c             	sub    $0xc,%esp
801068d3:	6a 0a                	push   $0xa
801068d5:	e8 06 c1 ff ff       	call   801029e0 <microdelay>
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
801068da:	83 c4 10             	add    $0x10,%esp
801068dd:	83 eb 01             	sub    $0x1,%ebx
801068e0:	74 07                	je     801068e9 <uartinit+0xb9>
801068e2:	89 f2                	mov    %esi,%edx
801068e4:	ec                   	in     (%dx),%al
801068e5:	a8 20                	test   $0x20,%al
801068e7:	74 e7                	je     801068d0 <uartinit+0xa0>
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801068e9:	0f b6 45 e7          	movzbl -0x19(%ebp),%eax
801068ed:	ba f8 03 00 00       	mov    $0x3f8,%edx
801068f2:	ee                   	out    %al,(%dx)
  for(p="xv6...\n"; *p; p++)
801068f3:	0f b6 47 01          	movzbl 0x1(%edi),%eax
801068f7:	83 c7 01             	add    $0x1,%edi
801068fa:	88 45 e7             	mov    %al,-0x19(%ebp)
801068fd:	84 c0                	test   %al,%al
801068ff:	75 bf                	jne    801068c0 <uartinit+0x90>
}
80106901:	8d 65 f4             	lea    -0xc(%ebp),%esp
80106904:	5b                   	pop    %ebx
80106905:	5e                   	pop    %esi
80106906:	5f                   	pop    %edi
80106907:	5d                   	pop    %ebp
80106908:	c3                   	ret    
80106909:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80106910 <uartputc>:
  if(!uart)
80106910:	a1 e0 63 11 80       	mov    0x801163e0,%eax
80106915:	85 c0                	test   %eax,%eax
80106917:	74 47                	je     80106960 <uartputc+0x50>
{
80106919:	55                   	push   %ebp
8010691a:	89 e5                	mov    %esp,%ebp
8010691c:	56                   	push   %esi
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
8010691d:	be fd 03 00 00       	mov    $0x3fd,%esi
80106922:	53                   	push   %ebx
80106923:	bb 80 00 00 00       	mov    $0x80,%ebx
80106928:	eb 18                	jmp    80106942 <uartputc+0x32>
8010692a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    microdelay(10);
80106930:	83 ec 0c             	sub    $0xc,%esp
80106933:	6a 0a                	push   $0xa
80106935:	e8 a6 c0 ff ff       	call   801029e0 <microdelay>
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
8010693a:	83 c4 10             	add    $0x10,%esp
8010693d:	83 eb 01             	sub    $0x1,%ebx
80106940:	74 07                	je     80106949 <uartputc+0x39>
80106942:	89 f2                	mov    %esi,%edx
80106944:	ec                   	in     (%dx),%al
80106945:	a8 20                	test   $0x20,%al
80106947:	74 e7                	je     80106930 <uartputc+0x20>
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80106949:	8b 45 08             	mov    0x8(%ebp),%eax
8010694c:	ba f8 03 00 00       	mov    $0x3f8,%edx
80106951:	ee                   	out    %al,(%dx)
}
80106952:	8d 65 f8             	lea    -0x8(%ebp),%esp
80106955:	5b                   	pop    %ebx
80106956:	5e                   	pop    %esi
80106957:	5d                   	pop    %ebp
80106958:	c3                   	ret    
80106959:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80106960:	c3                   	ret    
80106961:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80106968:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010696f:	90                   	nop

80106970 <uartintr>:

void
uartintr(void)
{
80106970:	55                   	push   %ebp
80106971:	89 e5                	mov    %esp,%ebp
80106973:	83 ec 14             	sub    $0x14,%esp
  consoleintr(uartgetc);
80106976:	68 00 68 10 80       	push   $0x80106800
8010697b:	e8 00 9f ff ff       	call   80100880 <consoleintr>
}
80106980:	83 c4 10             	add    $0x10,%esp
80106983:	c9                   	leave  
80106984:	c3                   	ret    

80106985 <vector0>:
# generated by vectors.pl - do not edit
# handlers
.globl alltraps
.globl vector0
vector0:
  pushl $0
80106985:	6a 00                	push   $0x0
  pushl $0
80106987:	6a 00                	push   $0x0
  jmp alltraps
80106989:	e9 01 fb ff ff       	jmp    8010648f <alltraps>

8010698e <vector1>:
.globl vector1
vector1:
  pushl $0
8010698e:	6a 00                	push   $0x0
  pushl $1
80106990:	6a 01                	push   $0x1
  jmp alltraps
80106992:	e9 f8 fa ff ff       	jmp    8010648f <alltraps>

80106997 <vector2>:
.globl vector2
vector2:
  pushl $0
80106997:	6a 00                	push   $0x0
  pushl $2
80106999:	6a 02                	push   $0x2
  jmp alltraps
8010699b:	e9 ef fa ff ff       	jmp    8010648f <alltraps>

801069a0 <vector3>:
.globl vector3
vector3:
  pushl $0
801069a0:	6a 00                	push   $0x0
  pushl $3
801069a2:	6a 03                	push   $0x3
  jmp alltraps
801069a4:	e9 e6 fa ff ff       	jmp    8010648f <alltraps>

801069a9 <vector4>:
.globl vector4
vector4:
  pushl $0
801069a9:	6a 00                	push   $0x0
  pushl $4
801069ab:	6a 04                	push   $0x4
  jmp alltraps
801069ad:	e9 dd fa ff ff       	jmp    8010648f <alltraps>

801069b2 <vector5>:
.globl vector5
vector5:
  pushl $0
801069b2:	6a 00                	push   $0x0
  pushl $5
801069b4:	6a 05                	push   $0x5
  jmp alltraps
801069b6:	e9 d4 fa ff ff       	jmp    8010648f <alltraps>

801069bb <vector6>:
.globl vector6
vector6:
  pushl $0
801069bb:	6a 00                	push   $0x0
  pushl $6
801069bd:	6a 06                	push   $0x6
  jmp alltraps
801069bf:	e9 cb fa ff ff       	jmp    8010648f <alltraps>

801069c4 <vector7>:
.globl vector7
vector7:
  pushl $0
801069c4:	6a 00                	push   $0x0
  pushl $7
801069c6:	6a 07                	push   $0x7
  jmp alltraps
801069c8:	e9 c2 fa ff ff       	jmp    8010648f <alltraps>

801069cd <vector8>:
.globl vector8
vector8:
  pushl $8
801069cd:	6a 08                	push   $0x8
  jmp alltraps
801069cf:	e9 bb fa ff ff       	jmp    8010648f <alltraps>

801069d4 <vector9>:
.globl vector9
vector9:
  pushl $0
801069d4:	6a 00                	push   $0x0
  pushl $9
801069d6:	6a 09                	push   $0x9
  jmp alltraps
801069d8:	e9 b2 fa ff ff       	jmp    8010648f <alltraps>

801069dd <vector10>:
.globl vector10
vector10:
  pushl $10
801069dd:	6a 0a                	push   $0xa
  jmp alltraps
801069df:	e9 ab fa ff ff       	jmp    8010648f <alltraps>

801069e4 <vector11>:
.globl vector11
vector11:
  pushl $11
801069e4:	6a 0b                	push   $0xb
  jmp alltraps
801069e6:	e9 a4 fa ff ff       	jmp    8010648f <alltraps>

801069eb <vector12>:
.globl vector12
vector12:
  pushl $12
801069eb:	6a 0c                	push   $0xc
  jmp alltraps
801069ed:	e9 9d fa ff ff       	jmp    8010648f <alltraps>

801069f2 <vector13>:
.globl vector13
vector13:
  pushl $13
801069f2:	6a 0d                	push   $0xd
  jmp alltraps
801069f4:	e9 96 fa ff ff       	jmp    8010648f <alltraps>

801069f9 <vector14>:
.globl vector14
vector14:
  pushl $14
801069f9:	6a 0e                	push   $0xe
  jmp alltraps
801069fb:	e9 8f fa ff ff       	jmp    8010648f <alltraps>

80106a00 <vector15>:
.globl vector15
vector15:
  pushl $0
80106a00:	6a 00                	push   $0x0
  pushl $15
80106a02:	6a 0f                	push   $0xf
  jmp alltraps
80106a04:	e9 86 fa ff ff       	jmp    8010648f <alltraps>

80106a09 <vector16>:
.globl vector16
vector16:
  pushl $0
80106a09:	6a 00                	push   $0x0
  pushl $16
80106a0b:	6a 10                	push   $0x10
  jmp alltraps
80106a0d:	e9 7d fa ff ff       	jmp    8010648f <alltraps>

80106a12 <vector17>:
.globl vector17
vector17:
  pushl $17
80106a12:	6a 11                	push   $0x11
  jmp alltraps
80106a14:	e9 76 fa ff ff       	jmp    8010648f <alltraps>

80106a19 <vector18>:
.globl vector18
vector18:
  pushl $0
80106a19:	6a 00                	push   $0x0
  pushl $18
80106a1b:	6a 12                	push   $0x12
  jmp alltraps
80106a1d:	e9 6d fa ff ff       	jmp    8010648f <alltraps>

80106a22 <vector19>:
.globl vector19
vector19:
  pushl $0
80106a22:	6a 00                	push   $0x0
  pushl $19
80106a24:	6a 13                	push   $0x13
  jmp alltraps
80106a26:	e9 64 fa ff ff       	jmp    8010648f <alltraps>

80106a2b <vector20>:
.globl vector20
vector20:
  pushl $0
80106a2b:	6a 00                	push   $0x0
  pushl $20
80106a2d:	6a 14                	push   $0x14
  jmp alltraps
80106a2f:	e9 5b fa ff ff       	jmp    8010648f <alltraps>

80106a34 <vector21>:
.globl vector21
vector21:
  pushl $0
80106a34:	6a 00                	push   $0x0
  pushl $21
80106a36:	6a 15                	push   $0x15
  jmp alltraps
80106a38:	e9 52 fa ff ff       	jmp    8010648f <alltraps>

80106a3d <vector22>:
.globl vector22
vector22:
  pushl $0
80106a3d:	6a 00                	push   $0x0
  pushl $22
80106a3f:	6a 16                	push   $0x16
  jmp alltraps
80106a41:	e9 49 fa ff ff       	jmp    8010648f <alltraps>

80106a46 <vector23>:
.globl vector23
vector23:
  pushl $0
80106a46:	6a 00                	push   $0x0
  pushl $23
80106a48:	6a 17                	push   $0x17
  jmp alltraps
80106a4a:	e9 40 fa ff ff       	jmp    8010648f <alltraps>

80106a4f <vector24>:
.globl vector24
vector24:
  pushl $0
80106a4f:	6a 00                	push   $0x0
  pushl $24
80106a51:	6a 18                	push   $0x18
  jmp alltraps
80106a53:	e9 37 fa ff ff       	jmp    8010648f <alltraps>

80106a58 <vector25>:
.globl vector25
vector25:
  pushl $0
80106a58:	6a 00                	push   $0x0
  pushl $25
80106a5a:	6a 19                	push   $0x19
  jmp alltraps
80106a5c:	e9 2e fa ff ff       	jmp    8010648f <alltraps>

80106a61 <vector26>:
.globl vector26
vector26:
  pushl $0
80106a61:	6a 00                	push   $0x0
  pushl $26
80106a63:	6a 1a                	push   $0x1a
  jmp alltraps
80106a65:	e9 25 fa ff ff       	jmp    8010648f <alltraps>

80106a6a <vector27>:
.globl vector27
vector27:
  pushl $0
80106a6a:	6a 00                	push   $0x0
  pushl $27
80106a6c:	6a 1b                	push   $0x1b
  jmp alltraps
80106a6e:	e9 1c fa ff ff       	jmp    8010648f <alltraps>

80106a73 <vector28>:
.globl vector28
vector28:
  pushl $0
80106a73:	6a 00                	push   $0x0
  pushl $28
80106a75:	6a 1c                	push   $0x1c
  jmp alltraps
80106a77:	e9 13 fa ff ff       	jmp    8010648f <alltraps>

80106a7c <vector29>:
.globl vector29
vector29:
  pushl $0
80106a7c:	6a 00                	push   $0x0
  pushl $29
80106a7e:	6a 1d                	push   $0x1d
  jmp alltraps
80106a80:	e9 0a fa ff ff       	jmp    8010648f <alltraps>

80106a85 <vector30>:
.globl vector30
vector30:
  pushl $0
80106a85:	6a 00                	push   $0x0
  pushl $30
80106a87:	6a 1e                	push   $0x1e
  jmp alltraps
80106a89:	e9 01 fa ff ff       	jmp    8010648f <alltraps>

80106a8e <vector31>:
.globl vector31
vector31:
  pushl $0
80106a8e:	6a 00                	push   $0x0
  pushl $31
80106a90:	6a 1f                	push   $0x1f
  jmp alltraps
80106a92:	e9 f8 f9 ff ff       	jmp    8010648f <alltraps>

80106a97 <vector32>:
.globl vector32
vector32:
  pushl $0
80106a97:	6a 00                	push   $0x0
  pushl $32
80106a99:	6a 20                	push   $0x20
  jmp alltraps
80106a9b:	e9 ef f9 ff ff       	jmp    8010648f <alltraps>

80106aa0 <vector33>:
.globl vector33
vector33:
  pushl $0
80106aa0:	6a 00                	push   $0x0
  pushl $33
80106aa2:	6a 21                	push   $0x21
  jmp alltraps
80106aa4:	e9 e6 f9 ff ff       	jmp    8010648f <alltraps>

80106aa9 <vector34>:
.globl vector34
vector34:
  pushl $0
80106aa9:	6a 00                	push   $0x0
  pushl $34
80106aab:	6a 22                	push   $0x22
  jmp alltraps
80106aad:	e9 dd f9 ff ff       	jmp    8010648f <alltraps>

80106ab2 <vector35>:
.globl vector35
vector35:
  pushl $0
80106ab2:	6a 00                	push   $0x0
  pushl $35
80106ab4:	6a 23                	push   $0x23
  jmp alltraps
80106ab6:	e9 d4 f9 ff ff       	jmp    8010648f <alltraps>

80106abb <vector36>:
.globl vector36
vector36:
  pushl $0
80106abb:	6a 00                	push   $0x0
  pushl $36
80106abd:	6a 24                	push   $0x24
  jmp alltraps
80106abf:	e9 cb f9 ff ff       	jmp    8010648f <alltraps>

80106ac4 <vector37>:
.globl vector37
vector37:
  pushl $0
80106ac4:	6a 00                	push   $0x0
  pushl $37
80106ac6:	6a 25                	push   $0x25
  jmp alltraps
80106ac8:	e9 c2 f9 ff ff       	jmp    8010648f <alltraps>

80106acd <vector38>:
.globl vector38
vector38:
  pushl $0
80106acd:	6a 00                	push   $0x0
  pushl $38
80106acf:	6a 26                	push   $0x26
  jmp alltraps
80106ad1:	e9 b9 f9 ff ff       	jmp    8010648f <alltraps>

80106ad6 <vector39>:
.globl vector39
vector39:
  pushl $0
80106ad6:	6a 00                	push   $0x0
  pushl $39
80106ad8:	6a 27                	push   $0x27
  jmp alltraps
80106ada:	e9 b0 f9 ff ff       	jmp    8010648f <alltraps>

80106adf <vector40>:
.globl vector40
vector40:
  pushl $0
80106adf:	6a 00                	push   $0x0
  pushl $40
80106ae1:	6a 28                	push   $0x28
  jmp alltraps
80106ae3:	e9 a7 f9 ff ff       	jmp    8010648f <alltraps>

80106ae8 <vector41>:
.globl vector41
vector41:
  pushl $0
80106ae8:	6a 00                	push   $0x0
  pushl $41
80106aea:	6a 29                	push   $0x29
  jmp alltraps
80106aec:	e9 9e f9 ff ff       	jmp    8010648f <alltraps>

80106af1 <vector42>:
.globl vector42
vector42:
  pushl $0
80106af1:	6a 00                	push   $0x0
  pushl $42
80106af3:	6a 2a                	push   $0x2a
  jmp alltraps
80106af5:	e9 95 f9 ff ff       	jmp    8010648f <alltraps>

80106afa <vector43>:
.globl vector43
vector43:
  pushl $0
80106afa:	6a 00                	push   $0x0
  pushl $43
80106afc:	6a 2b                	push   $0x2b
  jmp alltraps
80106afe:	e9 8c f9 ff ff       	jmp    8010648f <alltraps>

80106b03 <vector44>:
.globl vector44
vector44:
  pushl $0
80106b03:	6a 00                	push   $0x0
  pushl $44
80106b05:	6a 2c                	push   $0x2c
  jmp alltraps
80106b07:	e9 83 f9 ff ff       	jmp    8010648f <alltraps>

80106b0c <vector45>:
.globl vector45
vector45:
  pushl $0
80106b0c:	6a 00                	push   $0x0
  pushl $45
80106b0e:	6a 2d                	push   $0x2d
  jmp alltraps
80106b10:	e9 7a f9 ff ff       	jmp    8010648f <alltraps>

80106b15 <vector46>:
.globl vector46
vector46:
  pushl $0
80106b15:	6a 00                	push   $0x0
  pushl $46
80106b17:	6a 2e                	push   $0x2e
  jmp alltraps
80106b19:	e9 71 f9 ff ff       	jmp    8010648f <alltraps>

80106b1e <vector47>:
.globl vector47
vector47:
  pushl $0
80106b1e:	6a 00                	push   $0x0
  pushl $47
80106b20:	6a 2f                	push   $0x2f
  jmp alltraps
80106b22:	e9 68 f9 ff ff       	jmp    8010648f <alltraps>

80106b27 <vector48>:
.globl vector48
vector48:
  pushl $0
80106b27:	6a 00                	push   $0x0
  pushl $48
80106b29:	6a 30                	push   $0x30
  jmp alltraps
80106b2b:	e9 5f f9 ff ff       	jmp    8010648f <alltraps>

80106b30 <vector49>:
.globl vector49
vector49:
  pushl $0
80106b30:	6a 00                	push   $0x0
  pushl $49
80106b32:	6a 31                	push   $0x31
  jmp alltraps
80106b34:	e9 56 f9 ff ff       	jmp    8010648f <alltraps>

80106b39 <vector50>:
.globl vector50
vector50:
  pushl $0
80106b39:	6a 00                	push   $0x0
  pushl $50
80106b3b:	6a 32                	push   $0x32
  jmp alltraps
80106b3d:	e9 4d f9 ff ff       	jmp    8010648f <alltraps>

80106b42 <vector51>:
.globl vector51
vector51:
  pushl $0
80106b42:	6a 00                	push   $0x0
  pushl $51
80106b44:	6a 33                	push   $0x33
  jmp alltraps
80106b46:	e9 44 f9 ff ff       	jmp    8010648f <alltraps>

80106b4b <vector52>:
.globl vector52
vector52:
  pushl $0
80106b4b:	6a 00                	push   $0x0
  pushl $52
80106b4d:	6a 34                	push   $0x34
  jmp alltraps
80106b4f:	e9 3b f9 ff ff       	jmp    8010648f <alltraps>

80106b54 <vector53>:
.globl vector53
vector53:
  pushl $0
80106b54:	6a 00                	push   $0x0
  pushl $53
80106b56:	6a 35                	push   $0x35
  jmp alltraps
80106b58:	e9 32 f9 ff ff       	jmp    8010648f <alltraps>

80106b5d <vector54>:
.globl vector54
vector54:
  pushl $0
80106b5d:	6a 00                	push   $0x0
  pushl $54
80106b5f:	6a 36                	push   $0x36
  jmp alltraps
80106b61:	e9 29 f9 ff ff       	jmp    8010648f <alltraps>

80106b66 <vector55>:
.globl vector55
vector55:
  pushl $0
80106b66:	6a 00                	push   $0x0
  pushl $55
80106b68:	6a 37                	push   $0x37
  jmp alltraps
80106b6a:	e9 20 f9 ff ff       	jmp    8010648f <alltraps>

80106b6f <vector56>:
.globl vector56
vector56:
  pushl $0
80106b6f:	6a 00                	push   $0x0
  pushl $56
80106b71:	6a 38                	push   $0x38
  jmp alltraps
80106b73:	e9 17 f9 ff ff       	jmp    8010648f <alltraps>

80106b78 <vector57>:
.globl vector57
vector57:
  pushl $0
80106b78:	6a 00                	push   $0x0
  pushl $57
80106b7a:	6a 39                	push   $0x39
  jmp alltraps
80106b7c:	e9 0e f9 ff ff       	jmp    8010648f <alltraps>

80106b81 <vector58>:
.globl vector58
vector58:
  pushl $0
80106b81:	6a 00                	push   $0x0
  pushl $58
80106b83:	6a 3a                	push   $0x3a
  jmp alltraps
80106b85:	e9 05 f9 ff ff       	jmp    8010648f <alltraps>

80106b8a <vector59>:
.globl vector59
vector59:
  pushl $0
80106b8a:	6a 00                	push   $0x0
  pushl $59
80106b8c:	6a 3b                	push   $0x3b
  jmp alltraps
80106b8e:	e9 fc f8 ff ff       	jmp    8010648f <alltraps>

80106b93 <vector60>:
.globl vector60
vector60:
  pushl $0
80106b93:	6a 00                	push   $0x0
  pushl $60
80106b95:	6a 3c                	push   $0x3c
  jmp alltraps
80106b97:	e9 f3 f8 ff ff       	jmp    8010648f <alltraps>

80106b9c <vector61>:
.globl vector61
vector61:
  pushl $0
80106b9c:	6a 00                	push   $0x0
  pushl $61
80106b9e:	6a 3d                	push   $0x3d
  jmp alltraps
80106ba0:	e9 ea f8 ff ff       	jmp    8010648f <alltraps>

80106ba5 <vector62>:
.globl vector62
vector62:
  pushl $0
80106ba5:	6a 00                	push   $0x0
  pushl $62
80106ba7:	6a 3e                	push   $0x3e
  jmp alltraps
80106ba9:	e9 e1 f8 ff ff       	jmp    8010648f <alltraps>

80106bae <vector63>:
.globl vector63
vector63:
  pushl $0
80106bae:	6a 00                	push   $0x0
  pushl $63
80106bb0:	6a 3f                	push   $0x3f
  jmp alltraps
80106bb2:	e9 d8 f8 ff ff       	jmp    8010648f <alltraps>

80106bb7 <vector64>:
.globl vector64
vector64:
  pushl $0
80106bb7:	6a 00                	push   $0x0
  pushl $64
80106bb9:	6a 40                	push   $0x40
  jmp alltraps
80106bbb:	e9 cf f8 ff ff       	jmp    8010648f <alltraps>

80106bc0 <vector65>:
.globl vector65
vector65:
  pushl $0
80106bc0:	6a 00                	push   $0x0
  pushl $65
80106bc2:	6a 41                	push   $0x41
  jmp alltraps
80106bc4:	e9 c6 f8 ff ff       	jmp    8010648f <alltraps>

80106bc9 <vector66>:
.globl vector66
vector66:
  pushl $0
80106bc9:	6a 00                	push   $0x0
  pushl $66
80106bcb:	6a 42                	push   $0x42
  jmp alltraps
80106bcd:	e9 bd f8 ff ff       	jmp    8010648f <alltraps>

80106bd2 <vector67>:
.globl vector67
vector67:
  pushl $0
80106bd2:	6a 00                	push   $0x0
  pushl $67
80106bd4:	6a 43                	push   $0x43
  jmp alltraps
80106bd6:	e9 b4 f8 ff ff       	jmp    8010648f <alltraps>

80106bdb <vector68>:
.globl vector68
vector68:
  pushl $0
80106bdb:	6a 00                	push   $0x0
  pushl $68
80106bdd:	6a 44                	push   $0x44
  jmp alltraps
80106bdf:	e9 ab f8 ff ff       	jmp    8010648f <alltraps>

80106be4 <vector69>:
.globl vector69
vector69:
  pushl $0
80106be4:	6a 00                	push   $0x0
  pushl $69
80106be6:	6a 45                	push   $0x45
  jmp alltraps
80106be8:	e9 a2 f8 ff ff       	jmp    8010648f <alltraps>

80106bed <vector70>:
.globl vector70
vector70:
  pushl $0
80106bed:	6a 00                	push   $0x0
  pushl $70
80106bef:	6a 46                	push   $0x46
  jmp alltraps
80106bf1:	e9 99 f8 ff ff       	jmp    8010648f <alltraps>

80106bf6 <vector71>:
.globl vector71
vector71:
  pushl $0
80106bf6:	6a 00                	push   $0x0
  pushl $71
80106bf8:	6a 47                	push   $0x47
  jmp alltraps
80106bfa:	e9 90 f8 ff ff       	jmp    8010648f <alltraps>

80106bff <vector72>:
.globl vector72
vector72:
  pushl $0
80106bff:	6a 00                	push   $0x0
  pushl $72
80106c01:	6a 48                	push   $0x48
  jmp alltraps
80106c03:	e9 87 f8 ff ff       	jmp    8010648f <alltraps>

80106c08 <vector73>:
.globl vector73
vector73:
  pushl $0
80106c08:	6a 00                	push   $0x0
  pushl $73
80106c0a:	6a 49                	push   $0x49
  jmp alltraps
80106c0c:	e9 7e f8 ff ff       	jmp    8010648f <alltraps>

80106c11 <vector74>:
.globl vector74
vector74:
  pushl $0
80106c11:	6a 00                	push   $0x0
  pushl $74
80106c13:	6a 4a                	push   $0x4a
  jmp alltraps
80106c15:	e9 75 f8 ff ff       	jmp    8010648f <alltraps>

80106c1a <vector75>:
.globl vector75
vector75:
  pushl $0
80106c1a:	6a 00                	push   $0x0
  pushl $75
80106c1c:	6a 4b                	push   $0x4b
  jmp alltraps
80106c1e:	e9 6c f8 ff ff       	jmp    8010648f <alltraps>

80106c23 <vector76>:
.globl vector76
vector76:
  pushl $0
80106c23:	6a 00                	push   $0x0
  pushl $76
80106c25:	6a 4c                	push   $0x4c
  jmp alltraps
80106c27:	e9 63 f8 ff ff       	jmp    8010648f <alltraps>

80106c2c <vector77>:
.globl vector77
vector77:
  pushl $0
80106c2c:	6a 00                	push   $0x0
  pushl $77
80106c2e:	6a 4d                	push   $0x4d
  jmp alltraps
80106c30:	e9 5a f8 ff ff       	jmp    8010648f <alltraps>

80106c35 <vector78>:
.globl vector78
vector78:
  pushl $0
80106c35:	6a 00                	push   $0x0
  pushl $78
80106c37:	6a 4e                	push   $0x4e
  jmp alltraps
80106c39:	e9 51 f8 ff ff       	jmp    8010648f <alltraps>

80106c3e <vector79>:
.globl vector79
vector79:
  pushl $0
80106c3e:	6a 00                	push   $0x0
  pushl $79
80106c40:	6a 4f                	push   $0x4f
  jmp alltraps
80106c42:	e9 48 f8 ff ff       	jmp    8010648f <alltraps>

80106c47 <vector80>:
.globl vector80
vector80:
  pushl $0
80106c47:	6a 00                	push   $0x0
  pushl $80
80106c49:	6a 50                	push   $0x50
  jmp alltraps
80106c4b:	e9 3f f8 ff ff       	jmp    8010648f <alltraps>

80106c50 <vector81>:
.globl vector81
vector81:
  pushl $0
80106c50:	6a 00                	push   $0x0
  pushl $81
80106c52:	6a 51                	push   $0x51
  jmp alltraps
80106c54:	e9 36 f8 ff ff       	jmp    8010648f <alltraps>

80106c59 <vector82>:
.globl vector82
vector82:
  pushl $0
80106c59:	6a 00                	push   $0x0
  pushl $82
80106c5b:	6a 52                	push   $0x52
  jmp alltraps
80106c5d:	e9 2d f8 ff ff       	jmp    8010648f <alltraps>

80106c62 <vector83>:
.globl vector83
vector83:
  pushl $0
80106c62:	6a 00                	push   $0x0
  pushl $83
80106c64:	6a 53                	push   $0x53
  jmp alltraps
80106c66:	e9 24 f8 ff ff       	jmp    8010648f <alltraps>

80106c6b <vector84>:
.globl vector84
vector84:
  pushl $0
80106c6b:	6a 00                	push   $0x0
  pushl $84
80106c6d:	6a 54                	push   $0x54
  jmp alltraps
80106c6f:	e9 1b f8 ff ff       	jmp    8010648f <alltraps>

80106c74 <vector85>:
.globl vector85
vector85:
  pushl $0
80106c74:	6a 00                	push   $0x0
  pushl $85
80106c76:	6a 55                	push   $0x55
  jmp alltraps
80106c78:	e9 12 f8 ff ff       	jmp    8010648f <alltraps>

80106c7d <vector86>:
.globl vector86
vector86:
  pushl $0
80106c7d:	6a 00                	push   $0x0
  pushl $86
80106c7f:	6a 56                	push   $0x56
  jmp alltraps
80106c81:	e9 09 f8 ff ff       	jmp    8010648f <alltraps>

80106c86 <vector87>:
.globl vector87
vector87:
  pushl $0
80106c86:	6a 00                	push   $0x0
  pushl $87
80106c88:	6a 57                	push   $0x57
  jmp alltraps
80106c8a:	e9 00 f8 ff ff       	jmp    8010648f <alltraps>

80106c8f <vector88>:
.globl vector88
vector88:
  pushl $0
80106c8f:	6a 00                	push   $0x0
  pushl $88
80106c91:	6a 58                	push   $0x58
  jmp alltraps
80106c93:	e9 f7 f7 ff ff       	jmp    8010648f <alltraps>

80106c98 <vector89>:
.globl vector89
vector89:
  pushl $0
80106c98:	6a 00                	push   $0x0
  pushl $89
80106c9a:	6a 59                	push   $0x59
  jmp alltraps
80106c9c:	e9 ee f7 ff ff       	jmp    8010648f <alltraps>

80106ca1 <vector90>:
.globl vector90
vector90:
  pushl $0
80106ca1:	6a 00                	push   $0x0
  pushl $90
80106ca3:	6a 5a                	push   $0x5a
  jmp alltraps
80106ca5:	e9 e5 f7 ff ff       	jmp    8010648f <alltraps>

80106caa <vector91>:
.globl vector91
vector91:
  pushl $0
80106caa:	6a 00                	push   $0x0
  pushl $91
80106cac:	6a 5b                	push   $0x5b
  jmp alltraps
80106cae:	e9 dc f7 ff ff       	jmp    8010648f <alltraps>

80106cb3 <vector92>:
.globl vector92
vector92:
  pushl $0
80106cb3:	6a 00                	push   $0x0
  pushl $92
80106cb5:	6a 5c                	push   $0x5c
  jmp alltraps
80106cb7:	e9 d3 f7 ff ff       	jmp    8010648f <alltraps>

80106cbc <vector93>:
.globl vector93
vector93:
  pushl $0
80106cbc:	6a 00                	push   $0x0
  pushl $93
80106cbe:	6a 5d                	push   $0x5d
  jmp alltraps
80106cc0:	e9 ca f7 ff ff       	jmp    8010648f <alltraps>

80106cc5 <vector94>:
.globl vector94
vector94:
  pushl $0
80106cc5:	6a 00                	push   $0x0
  pushl $94
80106cc7:	6a 5e                	push   $0x5e
  jmp alltraps
80106cc9:	e9 c1 f7 ff ff       	jmp    8010648f <alltraps>

80106cce <vector95>:
.globl vector95
vector95:
  pushl $0
80106cce:	6a 00                	push   $0x0
  pushl $95
80106cd0:	6a 5f                	push   $0x5f
  jmp alltraps
80106cd2:	e9 b8 f7 ff ff       	jmp    8010648f <alltraps>

80106cd7 <vector96>:
.globl vector96
vector96:
  pushl $0
80106cd7:	6a 00                	push   $0x0
  pushl $96
80106cd9:	6a 60                	push   $0x60
  jmp alltraps
80106cdb:	e9 af f7 ff ff       	jmp    8010648f <alltraps>

80106ce0 <vector97>:
.globl vector97
vector97:
  pushl $0
80106ce0:	6a 00                	push   $0x0
  pushl $97
80106ce2:	6a 61                	push   $0x61
  jmp alltraps
80106ce4:	e9 a6 f7 ff ff       	jmp    8010648f <alltraps>

80106ce9 <vector98>:
.globl vector98
vector98:
  pushl $0
80106ce9:	6a 00                	push   $0x0
  pushl $98
80106ceb:	6a 62                	push   $0x62
  jmp alltraps
80106ced:	e9 9d f7 ff ff       	jmp    8010648f <alltraps>

80106cf2 <vector99>:
.globl vector99
vector99:
  pushl $0
80106cf2:	6a 00                	push   $0x0
  pushl $99
80106cf4:	6a 63                	push   $0x63
  jmp alltraps
80106cf6:	e9 94 f7 ff ff       	jmp    8010648f <alltraps>

80106cfb <vector100>:
.globl vector100
vector100:
  pushl $0
80106cfb:	6a 00                	push   $0x0
  pushl $100
80106cfd:	6a 64                	push   $0x64
  jmp alltraps
80106cff:	e9 8b f7 ff ff       	jmp    8010648f <alltraps>

80106d04 <vector101>:
.globl vector101
vector101:
  pushl $0
80106d04:	6a 00                	push   $0x0
  pushl $101
80106d06:	6a 65                	push   $0x65
  jmp alltraps
80106d08:	e9 82 f7 ff ff       	jmp    8010648f <alltraps>

80106d0d <vector102>:
.globl vector102
vector102:
  pushl $0
80106d0d:	6a 00                	push   $0x0
  pushl $102
80106d0f:	6a 66                	push   $0x66
  jmp alltraps
80106d11:	e9 79 f7 ff ff       	jmp    8010648f <alltraps>

80106d16 <vector103>:
.globl vector103
vector103:
  pushl $0
80106d16:	6a 00                	push   $0x0
  pushl $103
80106d18:	6a 67                	push   $0x67
  jmp alltraps
80106d1a:	e9 70 f7 ff ff       	jmp    8010648f <alltraps>

80106d1f <vector104>:
.globl vector104
vector104:
  pushl $0
80106d1f:	6a 00                	push   $0x0
  pushl $104
80106d21:	6a 68                	push   $0x68
  jmp alltraps
80106d23:	e9 67 f7 ff ff       	jmp    8010648f <alltraps>

80106d28 <vector105>:
.globl vector105
vector105:
  pushl $0
80106d28:	6a 00                	push   $0x0
  pushl $105
80106d2a:	6a 69                	push   $0x69
  jmp alltraps
80106d2c:	e9 5e f7 ff ff       	jmp    8010648f <alltraps>

80106d31 <vector106>:
.globl vector106
vector106:
  pushl $0
80106d31:	6a 00                	push   $0x0
  pushl $106
80106d33:	6a 6a                	push   $0x6a
  jmp alltraps
80106d35:	e9 55 f7 ff ff       	jmp    8010648f <alltraps>

80106d3a <vector107>:
.globl vector107
vector107:
  pushl $0
80106d3a:	6a 00                	push   $0x0
  pushl $107
80106d3c:	6a 6b                	push   $0x6b
  jmp alltraps
80106d3e:	e9 4c f7 ff ff       	jmp    8010648f <alltraps>

80106d43 <vector108>:
.globl vector108
vector108:
  pushl $0
80106d43:	6a 00                	push   $0x0
  pushl $108
80106d45:	6a 6c                	push   $0x6c
  jmp alltraps
80106d47:	e9 43 f7 ff ff       	jmp    8010648f <alltraps>

80106d4c <vector109>:
.globl vector109
vector109:
  pushl $0
80106d4c:	6a 00                	push   $0x0
  pushl $109
80106d4e:	6a 6d                	push   $0x6d
  jmp alltraps
80106d50:	e9 3a f7 ff ff       	jmp    8010648f <alltraps>

80106d55 <vector110>:
.globl vector110
vector110:
  pushl $0
80106d55:	6a 00                	push   $0x0
  pushl $110
80106d57:	6a 6e                	push   $0x6e
  jmp alltraps
80106d59:	e9 31 f7 ff ff       	jmp    8010648f <alltraps>

80106d5e <vector111>:
.globl vector111
vector111:
  pushl $0
80106d5e:	6a 00                	push   $0x0
  pushl $111
80106d60:	6a 6f                	push   $0x6f
  jmp alltraps
80106d62:	e9 28 f7 ff ff       	jmp    8010648f <alltraps>

80106d67 <vector112>:
.globl vector112
vector112:
  pushl $0
80106d67:	6a 00                	push   $0x0
  pushl $112
80106d69:	6a 70                	push   $0x70
  jmp alltraps
80106d6b:	e9 1f f7 ff ff       	jmp    8010648f <alltraps>

80106d70 <vector113>:
.globl vector113
vector113:
  pushl $0
80106d70:	6a 00                	push   $0x0
  pushl $113
80106d72:	6a 71                	push   $0x71
  jmp alltraps
80106d74:	e9 16 f7 ff ff       	jmp    8010648f <alltraps>

80106d79 <vector114>:
.globl vector114
vector114:
  pushl $0
80106d79:	6a 00                	push   $0x0
  pushl $114
80106d7b:	6a 72                	push   $0x72
  jmp alltraps
80106d7d:	e9 0d f7 ff ff       	jmp    8010648f <alltraps>

80106d82 <vector115>:
.globl vector115
vector115:
  pushl $0
80106d82:	6a 00                	push   $0x0
  pushl $115
80106d84:	6a 73                	push   $0x73
  jmp alltraps
80106d86:	e9 04 f7 ff ff       	jmp    8010648f <alltraps>

80106d8b <vector116>:
.globl vector116
vector116:
  pushl $0
80106d8b:	6a 00                	push   $0x0
  pushl $116
80106d8d:	6a 74                	push   $0x74
  jmp alltraps
80106d8f:	e9 fb f6 ff ff       	jmp    8010648f <alltraps>

80106d94 <vector117>:
.globl vector117
vector117:
  pushl $0
80106d94:	6a 00                	push   $0x0
  pushl $117
80106d96:	6a 75                	push   $0x75
  jmp alltraps
80106d98:	e9 f2 f6 ff ff       	jmp    8010648f <alltraps>

80106d9d <vector118>:
.globl vector118
vector118:
  pushl $0
80106d9d:	6a 00                	push   $0x0
  pushl $118
80106d9f:	6a 76                	push   $0x76
  jmp alltraps
80106da1:	e9 e9 f6 ff ff       	jmp    8010648f <alltraps>

80106da6 <vector119>:
.globl vector119
vector119:
  pushl $0
80106da6:	6a 00                	push   $0x0
  pushl $119
80106da8:	6a 77                	push   $0x77
  jmp alltraps
80106daa:	e9 e0 f6 ff ff       	jmp    8010648f <alltraps>

80106daf <vector120>:
.globl vector120
vector120:
  pushl $0
80106daf:	6a 00                	push   $0x0
  pushl $120
80106db1:	6a 78                	push   $0x78
  jmp alltraps
80106db3:	e9 d7 f6 ff ff       	jmp    8010648f <alltraps>

80106db8 <vector121>:
.globl vector121
vector121:
  pushl $0
80106db8:	6a 00                	push   $0x0
  pushl $121
80106dba:	6a 79                	push   $0x79
  jmp alltraps
80106dbc:	e9 ce f6 ff ff       	jmp    8010648f <alltraps>

80106dc1 <vector122>:
.globl vector122
vector122:
  pushl $0
80106dc1:	6a 00                	push   $0x0
  pushl $122
80106dc3:	6a 7a                	push   $0x7a
  jmp alltraps
80106dc5:	e9 c5 f6 ff ff       	jmp    8010648f <alltraps>

80106dca <vector123>:
.globl vector123
vector123:
  pushl $0
80106dca:	6a 00                	push   $0x0
  pushl $123
80106dcc:	6a 7b                	push   $0x7b
  jmp alltraps
80106dce:	e9 bc f6 ff ff       	jmp    8010648f <alltraps>

80106dd3 <vector124>:
.globl vector124
vector124:
  pushl $0
80106dd3:	6a 00                	push   $0x0
  pushl $124
80106dd5:	6a 7c                	push   $0x7c
  jmp alltraps
80106dd7:	e9 b3 f6 ff ff       	jmp    8010648f <alltraps>

80106ddc <vector125>:
.globl vector125
vector125:
  pushl $0
80106ddc:	6a 00                	push   $0x0
  pushl $125
80106dde:	6a 7d                	push   $0x7d
  jmp alltraps
80106de0:	e9 aa f6 ff ff       	jmp    8010648f <alltraps>

80106de5 <vector126>:
.globl vector126
vector126:
  pushl $0
80106de5:	6a 00                	push   $0x0
  pushl $126
80106de7:	6a 7e                	push   $0x7e
  jmp alltraps
80106de9:	e9 a1 f6 ff ff       	jmp    8010648f <alltraps>

80106dee <vector127>:
.globl vector127
vector127:
  pushl $0
80106dee:	6a 00                	push   $0x0
  pushl $127
80106df0:	6a 7f                	push   $0x7f
  jmp alltraps
80106df2:	e9 98 f6 ff ff       	jmp    8010648f <alltraps>

80106df7 <vector128>:
.globl vector128
vector128:
  pushl $0
80106df7:	6a 00                	push   $0x0
  pushl $128
80106df9:	68 80 00 00 00       	push   $0x80
  jmp alltraps
80106dfe:	e9 8c f6 ff ff       	jmp    8010648f <alltraps>

80106e03 <vector129>:
.globl vector129
vector129:
  pushl $0
80106e03:	6a 00                	push   $0x0
  pushl $129
80106e05:	68 81 00 00 00       	push   $0x81
  jmp alltraps
80106e0a:	e9 80 f6 ff ff       	jmp    8010648f <alltraps>

80106e0f <vector130>:
.globl vector130
vector130:
  pushl $0
80106e0f:	6a 00                	push   $0x0
  pushl $130
80106e11:	68 82 00 00 00       	push   $0x82
  jmp alltraps
80106e16:	e9 74 f6 ff ff       	jmp    8010648f <alltraps>

80106e1b <vector131>:
.globl vector131
vector131:
  pushl $0
80106e1b:	6a 00                	push   $0x0
  pushl $131
80106e1d:	68 83 00 00 00       	push   $0x83
  jmp alltraps
80106e22:	e9 68 f6 ff ff       	jmp    8010648f <alltraps>

80106e27 <vector132>:
.globl vector132
vector132:
  pushl $0
80106e27:	6a 00                	push   $0x0
  pushl $132
80106e29:	68 84 00 00 00       	push   $0x84
  jmp alltraps
80106e2e:	e9 5c f6 ff ff       	jmp    8010648f <alltraps>

80106e33 <vector133>:
.globl vector133
vector133:
  pushl $0
80106e33:	6a 00                	push   $0x0
  pushl $133
80106e35:	68 85 00 00 00       	push   $0x85
  jmp alltraps
80106e3a:	e9 50 f6 ff ff       	jmp    8010648f <alltraps>

80106e3f <vector134>:
.globl vector134
vector134:
  pushl $0
80106e3f:	6a 00                	push   $0x0
  pushl $134
80106e41:	68 86 00 00 00       	push   $0x86
  jmp alltraps
80106e46:	e9 44 f6 ff ff       	jmp    8010648f <alltraps>

80106e4b <vector135>:
.globl vector135
vector135:
  pushl $0
80106e4b:	6a 00                	push   $0x0
  pushl $135
80106e4d:	68 87 00 00 00       	push   $0x87
  jmp alltraps
80106e52:	e9 38 f6 ff ff       	jmp    8010648f <alltraps>

80106e57 <vector136>:
.globl vector136
vector136:
  pushl $0
80106e57:	6a 00                	push   $0x0
  pushl $136
80106e59:	68 88 00 00 00       	push   $0x88
  jmp alltraps
80106e5e:	e9 2c f6 ff ff       	jmp    8010648f <alltraps>

80106e63 <vector137>:
.globl vector137
vector137:
  pushl $0
80106e63:	6a 00                	push   $0x0
  pushl $137
80106e65:	68 89 00 00 00       	push   $0x89
  jmp alltraps
80106e6a:	e9 20 f6 ff ff       	jmp    8010648f <alltraps>

80106e6f <vector138>:
.globl vector138
vector138:
  pushl $0
80106e6f:	6a 00                	push   $0x0
  pushl $138
80106e71:	68 8a 00 00 00       	push   $0x8a
  jmp alltraps
80106e76:	e9 14 f6 ff ff       	jmp    8010648f <alltraps>

80106e7b <vector139>:
.globl vector139
vector139:
  pushl $0
80106e7b:	6a 00                	push   $0x0
  pushl $139
80106e7d:	68 8b 00 00 00       	push   $0x8b
  jmp alltraps
80106e82:	e9 08 f6 ff ff       	jmp    8010648f <alltraps>

80106e87 <vector140>:
.globl vector140
vector140:
  pushl $0
80106e87:	6a 00                	push   $0x0
  pushl $140
80106e89:	68 8c 00 00 00       	push   $0x8c
  jmp alltraps
80106e8e:	e9 fc f5 ff ff       	jmp    8010648f <alltraps>

80106e93 <vector141>:
.globl vector141
vector141:
  pushl $0
80106e93:	6a 00                	push   $0x0
  pushl $141
80106e95:	68 8d 00 00 00       	push   $0x8d
  jmp alltraps
80106e9a:	e9 f0 f5 ff ff       	jmp    8010648f <alltraps>

80106e9f <vector142>:
.globl vector142
vector142:
  pushl $0
80106e9f:	6a 00                	push   $0x0
  pushl $142
80106ea1:	68 8e 00 00 00       	push   $0x8e
  jmp alltraps
80106ea6:	e9 e4 f5 ff ff       	jmp    8010648f <alltraps>

80106eab <vector143>:
.globl vector143
vector143:
  pushl $0
80106eab:	6a 00                	push   $0x0
  pushl $143
80106ead:	68 8f 00 00 00       	push   $0x8f
  jmp alltraps
80106eb2:	e9 d8 f5 ff ff       	jmp    8010648f <alltraps>

80106eb7 <vector144>:
.globl vector144
vector144:
  pushl $0
80106eb7:	6a 00                	push   $0x0
  pushl $144
80106eb9:	68 90 00 00 00       	push   $0x90
  jmp alltraps
80106ebe:	e9 cc f5 ff ff       	jmp    8010648f <alltraps>

80106ec3 <vector145>:
.globl vector145
vector145:
  pushl $0
80106ec3:	6a 00                	push   $0x0
  pushl $145
80106ec5:	68 91 00 00 00       	push   $0x91
  jmp alltraps
80106eca:	e9 c0 f5 ff ff       	jmp    8010648f <alltraps>

80106ecf <vector146>:
.globl vector146
vector146:
  pushl $0
80106ecf:	6a 00                	push   $0x0
  pushl $146
80106ed1:	68 92 00 00 00       	push   $0x92
  jmp alltraps
80106ed6:	e9 b4 f5 ff ff       	jmp    8010648f <alltraps>

80106edb <vector147>:
.globl vector147
vector147:
  pushl $0
80106edb:	6a 00                	push   $0x0
  pushl $147
80106edd:	68 93 00 00 00       	push   $0x93
  jmp alltraps
80106ee2:	e9 a8 f5 ff ff       	jmp    8010648f <alltraps>

80106ee7 <vector148>:
.globl vector148
vector148:
  pushl $0
80106ee7:	6a 00                	push   $0x0
  pushl $148
80106ee9:	68 94 00 00 00       	push   $0x94
  jmp alltraps
80106eee:	e9 9c f5 ff ff       	jmp    8010648f <alltraps>

80106ef3 <vector149>:
.globl vector149
vector149:
  pushl $0
80106ef3:	6a 00                	push   $0x0
  pushl $149
80106ef5:	68 95 00 00 00       	push   $0x95
  jmp alltraps
80106efa:	e9 90 f5 ff ff       	jmp    8010648f <alltraps>

80106eff <vector150>:
.globl vector150
vector150:
  pushl $0
80106eff:	6a 00                	push   $0x0
  pushl $150
80106f01:	68 96 00 00 00       	push   $0x96
  jmp alltraps
80106f06:	e9 84 f5 ff ff       	jmp    8010648f <alltraps>

80106f0b <vector151>:
.globl vector151
vector151:
  pushl $0
80106f0b:	6a 00                	push   $0x0
  pushl $151
80106f0d:	68 97 00 00 00       	push   $0x97
  jmp alltraps
80106f12:	e9 78 f5 ff ff       	jmp    8010648f <alltraps>

80106f17 <vector152>:
.globl vector152
vector152:
  pushl $0
80106f17:	6a 00                	push   $0x0
  pushl $152
80106f19:	68 98 00 00 00       	push   $0x98
  jmp alltraps
80106f1e:	e9 6c f5 ff ff       	jmp    8010648f <alltraps>

80106f23 <vector153>:
.globl vector153
vector153:
  pushl $0
80106f23:	6a 00                	push   $0x0
  pushl $153
80106f25:	68 99 00 00 00       	push   $0x99
  jmp alltraps
80106f2a:	e9 60 f5 ff ff       	jmp    8010648f <alltraps>

80106f2f <vector154>:
.globl vector154
vector154:
  pushl $0
80106f2f:	6a 00                	push   $0x0
  pushl $154
80106f31:	68 9a 00 00 00       	push   $0x9a
  jmp alltraps
80106f36:	e9 54 f5 ff ff       	jmp    8010648f <alltraps>

80106f3b <vector155>:
.globl vector155
vector155:
  pushl $0
80106f3b:	6a 00                	push   $0x0
  pushl $155
80106f3d:	68 9b 00 00 00       	push   $0x9b
  jmp alltraps
80106f42:	e9 48 f5 ff ff       	jmp    8010648f <alltraps>

80106f47 <vector156>:
.globl vector156
vector156:
  pushl $0
80106f47:	6a 00                	push   $0x0
  pushl $156
80106f49:	68 9c 00 00 00       	push   $0x9c
  jmp alltraps
80106f4e:	e9 3c f5 ff ff       	jmp    8010648f <alltraps>

80106f53 <vector157>:
.globl vector157
vector157:
  pushl $0
80106f53:	6a 00                	push   $0x0
  pushl $157
80106f55:	68 9d 00 00 00       	push   $0x9d
  jmp alltraps
80106f5a:	e9 30 f5 ff ff       	jmp    8010648f <alltraps>

80106f5f <vector158>:
.globl vector158
vector158:
  pushl $0
80106f5f:	6a 00                	push   $0x0
  pushl $158
80106f61:	68 9e 00 00 00       	push   $0x9e
  jmp alltraps
80106f66:	e9 24 f5 ff ff       	jmp    8010648f <alltraps>

80106f6b <vector159>:
.globl vector159
vector159:
  pushl $0
80106f6b:	6a 00                	push   $0x0
  pushl $159
80106f6d:	68 9f 00 00 00       	push   $0x9f
  jmp alltraps
80106f72:	e9 18 f5 ff ff       	jmp    8010648f <alltraps>

80106f77 <vector160>:
.globl vector160
vector160:
  pushl $0
80106f77:	6a 00                	push   $0x0
  pushl $160
80106f79:	68 a0 00 00 00       	push   $0xa0
  jmp alltraps
80106f7e:	e9 0c f5 ff ff       	jmp    8010648f <alltraps>

80106f83 <vector161>:
.globl vector161
vector161:
  pushl $0
80106f83:	6a 00                	push   $0x0
  pushl $161
80106f85:	68 a1 00 00 00       	push   $0xa1
  jmp alltraps
80106f8a:	e9 00 f5 ff ff       	jmp    8010648f <alltraps>

80106f8f <vector162>:
.globl vector162
vector162:
  pushl $0
80106f8f:	6a 00                	push   $0x0
  pushl $162
80106f91:	68 a2 00 00 00       	push   $0xa2
  jmp alltraps
80106f96:	e9 f4 f4 ff ff       	jmp    8010648f <alltraps>

80106f9b <vector163>:
.globl vector163
vector163:
  pushl $0
80106f9b:	6a 00                	push   $0x0
  pushl $163
80106f9d:	68 a3 00 00 00       	push   $0xa3
  jmp alltraps
80106fa2:	e9 e8 f4 ff ff       	jmp    8010648f <alltraps>

80106fa7 <vector164>:
.globl vector164
vector164:
  pushl $0
80106fa7:	6a 00                	push   $0x0
  pushl $164
80106fa9:	68 a4 00 00 00       	push   $0xa4
  jmp alltraps
80106fae:	e9 dc f4 ff ff       	jmp    8010648f <alltraps>

80106fb3 <vector165>:
.globl vector165
vector165:
  pushl $0
80106fb3:	6a 00                	push   $0x0
  pushl $165
80106fb5:	68 a5 00 00 00       	push   $0xa5
  jmp alltraps
80106fba:	e9 d0 f4 ff ff       	jmp    8010648f <alltraps>

80106fbf <vector166>:
.globl vector166
vector166:
  pushl $0
80106fbf:	6a 00                	push   $0x0
  pushl $166
80106fc1:	68 a6 00 00 00       	push   $0xa6
  jmp alltraps
80106fc6:	e9 c4 f4 ff ff       	jmp    8010648f <alltraps>

80106fcb <vector167>:
.globl vector167
vector167:
  pushl $0
80106fcb:	6a 00                	push   $0x0
  pushl $167
80106fcd:	68 a7 00 00 00       	push   $0xa7
  jmp alltraps
80106fd2:	e9 b8 f4 ff ff       	jmp    8010648f <alltraps>

80106fd7 <vector168>:
.globl vector168
vector168:
  pushl $0
80106fd7:	6a 00                	push   $0x0
  pushl $168
80106fd9:	68 a8 00 00 00       	push   $0xa8
  jmp alltraps
80106fde:	e9 ac f4 ff ff       	jmp    8010648f <alltraps>

80106fe3 <vector169>:
.globl vector169
vector169:
  pushl $0
80106fe3:	6a 00                	push   $0x0
  pushl $169
80106fe5:	68 a9 00 00 00       	push   $0xa9
  jmp alltraps
80106fea:	e9 a0 f4 ff ff       	jmp    8010648f <alltraps>

80106fef <vector170>:
.globl vector170
vector170:
  pushl $0
80106fef:	6a 00                	push   $0x0
  pushl $170
80106ff1:	68 aa 00 00 00       	push   $0xaa
  jmp alltraps
80106ff6:	e9 94 f4 ff ff       	jmp    8010648f <alltraps>

80106ffb <vector171>:
.globl vector171
vector171:
  pushl $0
80106ffb:	6a 00                	push   $0x0
  pushl $171
80106ffd:	68 ab 00 00 00       	push   $0xab
  jmp alltraps
80107002:	e9 88 f4 ff ff       	jmp    8010648f <alltraps>

80107007 <vector172>:
.globl vector172
vector172:
  pushl $0
80107007:	6a 00                	push   $0x0
  pushl $172
80107009:	68 ac 00 00 00       	push   $0xac
  jmp alltraps
8010700e:	e9 7c f4 ff ff       	jmp    8010648f <alltraps>

80107013 <vector173>:
.globl vector173
vector173:
  pushl $0
80107013:	6a 00                	push   $0x0
  pushl $173
80107015:	68 ad 00 00 00       	push   $0xad
  jmp alltraps
8010701a:	e9 70 f4 ff ff       	jmp    8010648f <alltraps>

8010701f <vector174>:
.globl vector174
vector174:
  pushl $0
8010701f:	6a 00                	push   $0x0
  pushl $174
80107021:	68 ae 00 00 00       	push   $0xae
  jmp alltraps
80107026:	e9 64 f4 ff ff       	jmp    8010648f <alltraps>

8010702b <vector175>:
.globl vector175
vector175:
  pushl $0
8010702b:	6a 00                	push   $0x0
  pushl $175
8010702d:	68 af 00 00 00       	push   $0xaf
  jmp alltraps
80107032:	e9 58 f4 ff ff       	jmp    8010648f <alltraps>

80107037 <vector176>:
.globl vector176
vector176:
  pushl $0
80107037:	6a 00                	push   $0x0
  pushl $176
80107039:	68 b0 00 00 00       	push   $0xb0
  jmp alltraps
8010703e:	e9 4c f4 ff ff       	jmp    8010648f <alltraps>

80107043 <vector177>:
.globl vector177
vector177:
  pushl $0
80107043:	6a 00                	push   $0x0
  pushl $177
80107045:	68 b1 00 00 00       	push   $0xb1
  jmp alltraps
8010704a:	e9 40 f4 ff ff       	jmp    8010648f <alltraps>

8010704f <vector178>:
.globl vector178
vector178:
  pushl $0
8010704f:	6a 00                	push   $0x0
  pushl $178
80107051:	68 b2 00 00 00       	push   $0xb2
  jmp alltraps
80107056:	e9 34 f4 ff ff       	jmp    8010648f <alltraps>

8010705b <vector179>:
.globl vector179
vector179:
  pushl $0
8010705b:	6a 00                	push   $0x0
  pushl $179
8010705d:	68 b3 00 00 00       	push   $0xb3
  jmp alltraps
80107062:	e9 28 f4 ff ff       	jmp    8010648f <alltraps>

80107067 <vector180>:
.globl vector180
vector180:
  pushl $0
80107067:	6a 00                	push   $0x0
  pushl $180
80107069:	68 b4 00 00 00       	push   $0xb4
  jmp alltraps
8010706e:	e9 1c f4 ff ff       	jmp    8010648f <alltraps>

80107073 <vector181>:
.globl vector181
vector181:
  pushl $0
80107073:	6a 00                	push   $0x0
  pushl $181
80107075:	68 b5 00 00 00       	push   $0xb5
  jmp alltraps
8010707a:	e9 10 f4 ff ff       	jmp    8010648f <alltraps>

8010707f <vector182>:
.globl vector182
vector182:
  pushl $0
8010707f:	6a 00                	push   $0x0
  pushl $182
80107081:	68 b6 00 00 00       	push   $0xb6
  jmp alltraps
80107086:	e9 04 f4 ff ff       	jmp    8010648f <alltraps>

8010708b <vector183>:
.globl vector183
vector183:
  pushl $0
8010708b:	6a 00                	push   $0x0
  pushl $183
8010708d:	68 b7 00 00 00       	push   $0xb7
  jmp alltraps
80107092:	e9 f8 f3 ff ff       	jmp    8010648f <alltraps>

80107097 <vector184>:
.globl vector184
vector184:
  pushl $0
80107097:	6a 00                	push   $0x0
  pushl $184
80107099:	68 b8 00 00 00       	push   $0xb8
  jmp alltraps
8010709e:	e9 ec f3 ff ff       	jmp    8010648f <alltraps>

801070a3 <vector185>:
.globl vector185
vector185:
  pushl $0
801070a3:	6a 00                	push   $0x0
  pushl $185
801070a5:	68 b9 00 00 00       	push   $0xb9
  jmp alltraps
801070aa:	e9 e0 f3 ff ff       	jmp    8010648f <alltraps>

801070af <vector186>:
.globl vector186
vector186:
  pushl $0
801070af:	6a 00                	push   $0x0
  pushl $186
801070b1:	68 ba 00 00 00       	push   $0xba
  jmp alltraps
801070b6:	e9 d4 f3 ff ff       	jmp    8010648f <alltraps>

801070bb <vector187>:
.globl vector187
vector187:
  pushl $0
801070bb:	6a 00                	push   $0x0
  pushl $187
801070bd:	68 bb 00 00 00       	push   $0xbb
  jmp alltraps
801070c2:	e9 c8 f3 ff ff       	jmp    8010648f <alltraps>

801070c7 <vector188>:
.globl vector188
vector188:
  pushl $0
801070c7:	6a 00                	push   $0x0
  pushl $188
801070c9:	68 bc 00 00 00       	push   $0xbc
  jmp alltraps
801070ce:	e9 bc f3 ff ff       	jmp    8010648f <alltraps>

801070d3 <vector189>:
.globl vector189
vector189:
  pushl $0
801070d3:	6a 00                	push   $0x0
  pushl $189
801070d5:	68 bd 00 00 00       	push   $0xbd
  jmp alltraps
801070da:	e9 b0 f3 ff ff       	jmp    8010648f <alltraps>

801070df <vector190>:
.globl vector190
vector190:
  pushl $0
801070df:	6a 00                	push   $0x0
  pushl $190
801070e1:	68 be 00 00 00       	push   $0xbe
  jmp alltraps
801070e6:	e9 a4 f3 ff ff       	jmp    8010648f <alltraps>

801070eb <vector191>:
.globl vector191
vector191:
  pushl $0
801070eb:	6a 00                	push   $0x0
  pushl $191
801070ed:	68 bf 00 00 00       	push   $0xbf
  jmp alltraps
801070f2:	e9 98 f3 ff ff       	jmp    8010648f <alltraps>

801070f7 <vector192>:
.globl vector192
vector192:
  pushl $0
801070f7:	6a 00                	push   $0x0
  pushl $192
801070f9:	68 c0 00 00 00       	push   $0xc0
  jmp alltraps
801070fe:	e9 8c f3 ff ff       	jmp    8010648f <alltraps>

80107103 <vector193>:
.globl vector193
vector193:
  pushl $0
80107103:	6a 00                	push   $0x0
  pushl $193
80107105:	68 c1 00 00 00       	push   $0xc1
  jmp alltraps
8010710a:	e9 80 f3 ff ff       	jmp    8010648f <alltraps>

8010710f <vector194>:
.globl vector194
vector194:
  pushl $0
8010710f:	6a 00                	push   $0x0
  pushl $194
80107111:	68 c2 00 00 00       	push   $0xc2
  jmp alltraps
80107116:	e9 74 f3 ff ff       	jmp    8010648f <alltraps>

8010711b <vector195>:
.globl vector195
vector195:
  pushl $0
8010711b:	6a 00                	push   $0x0
  pushl $195
8010711d:	68 c3 00 00 00       	push   $0xc3
  jmp alltraps
80107122:	e9 68 f3 ff ff       	jmp    8010648f <alltraps>

80107127 <vector196>:
.globl vector196
vector196:
  pushl $0
80107127:	6a 00                	push   $0x0
  pushl $196
80107129:	68 c4 00 00 00       	push   $0xc4
  jmp alltraps
8010712e:	e9 5c f3 ff ff       	jmp    8010648f <alltraps>

80107133 <vector197>:
.globl vector197
vector197:
  pushl $0
80107133:	6a 00                	push   $0x0
  pushl $197
80107135:	68 c5 00 00 00       	push   $0xc5
  jmp alltraps
8010713a:	e9 50 f3 ff ff       	jmp    8010648f <alltraps>

8010713f <vector198>:
.globl vector198
vector198:
  pushl $0
8010713f:	6a 00                	push   $0x0
  pushl $198
80107141:	68 c6 00 00 00       	push   $0xc6
  jmp alltraps
80107146:	e9 44 f3 ff ff       	jmp    8010648f <alltraps>

8010714b <vector199>:
.globl vector199
vector199:
  pushl $0
8010714b:	6a 00                	push   $0x0
  pushl $199
8010714d:	68 c7 00 00 00       	push   $0xc7
  jmp alltraps
80107152:	e9 38 f3 ff ff       	jmp    8010648f <alltraps>

80107157 <vector200>:
.globl vector200
vector200:
  pushl $0
80107157:	6a 00                	push   $0x0
  pushl $200
80107159:	68 c8 00 00 00       	push   $0xc8
  jmp alltraps
8010715e:	e9 2c f3 ff ff       	jmp    8010648f <alltraps>

80107163 <vector201>:
.globl vector201
vector201:
  pushl $0
80107163:	6a 00                	push   $0x0
  pushl $201
80107165:	68 c9 00 00 00       	push   $0xc9
  jmp alltraps
8010716a:	e9 20 f3 ff ff       	jmp    8010648f <alltraps>

8010716f <vector202>:
.globl vector202
vector202:
  pushl $0
8010716f:	6a 00                	push   $0x0
  pushl $202
80107171:	68 ca 00 00 00       	push   $0xca
  jmp alltraps
80107176:	e9 14 f3 ff ff       	jmp    8010648f <alltraps>

8010717b <vector203>:
.globl vector203
vector203:
  pushl $0
8010717b:	6a 00                	push   $0x0
  pushl $203
8010717d:	68 cb 00 00 00       	push   $0xcb
  jmp alltraps
80107182:	e9 08 f3 ff ff       	jmp    8010648f <alltraps>

80107187 <vector204>:
.globl vector204
vector204:
  pushl $0
80107187:	6a 00                	push   $0x0
  pushl $204
80107189:	68 cc 00 00 00       	push   $0xcc
  jmp alltraps
8010718e:	e9 fc f2 ff ff       	jmp    8010648f <alltraps>

80107193 <vector205>:
.globl vector205
vector205:
  pushl $0
80107193:	6a 00                	push   $0x0
  pushl $205
80107195:	68 cd 00 00 00       	push   $0xcd
  jmp alltraps
8010719a:	e9 f0 f2 ff ff       	jmp    8010648f <alltraps>

8010719f <vector206>:
.globl vector206
vector206:
  pushl $0
8010719f:	6a 00                	push   $0x0
  pushl $206
801071a1:	68 ce 00 00 00       	push   $0xce
  jmp alltraps
801071a6:	e9 e4 f2 ff ff       	jmp    8010648f <alltraps>

801071ab <vector207>:
.globl vector207
vector207:
  pushl $0
801071ab:	6a 00                	push   $0x0
  pushl $207
801071ad:	68 cf 00 00 00       	push   $0xcf
  jmp alltraps
801071b2:	e9 d8 f2 ff ff       	jmp    8010648f <alltraps>

801071b7 <vector208>:
.globl vector208
vector208:
  pushl $0
801071b7:	6a 00                	push   $0x0
  pushl $208
801071b9:	68 d0 00 00 00       	push   $0xd0
  jmp alltraps
801071be:	e9 cc f2 ff ff       	jmp    8010648f <alltraps>

801071c3 <vector209>:
.globl vector209
vector209:
  pushl $0
801071c3:	6a 00                	push   $0x0
  pushl $209
801071c5:	68 d1 00 00 00       	push   $0xd1
  jmp alltraps
801071ca:	e9 c0 f2 ff ff       	jmp    8010648f <alltraps>

801071cf <vector210>:
.globl vector210
vector210:
  pushl $0
801071cf:	6a 00                	push   $0x0
  pushl $210
801071d1:	68 d2 00 00 00       	push   $0xd2
  jmp alltraps
801071d6:	e9 b4 f2 ff ff       	jmp    8010648f <alltraps>

801071db <vector211>:
.globl vector211
vector211:
  pushl $0
801071db:	6a 00                	push   $0x0
  pushl $211
801071dd:	68 d3 00 00 00       	push   $0xd3
  jmp alltraps
801071e2:	e9 a8 f2 ff ff       	jmp    8010648f <alltraps>

801071e7 <vector212>:
.globl vector212
vector212:
  pushl $0
801071e7:	6a 00                	push   $0x0
  pushl $212
801071e9:	68 d4 00 00 00       	push   $0xd4
  jmp alltraps
801071ee:	e9 9c f2 ff ff       	jmp    8010648f <alltraps>

801071f3 <vector213>:
.globl vector213
vector213:
  pushl $0
801071f3:	6a 00                	push   $0x0
  pushl $213
801071f5:	68 d5 00 00 00       	push   $0xd5
  jmp alltraps
801071fa:	e9 90 f2 ff ff       	jmp    8010648f <alltraps>

801071ff <vector214>:
.globl vector214
vector214:
  pushl $0
801071ff:	6a 00                	push   $0x0
  pushl $214
80107201:	68 d6 00 00 00       	push   $0xd6
  jmp alltraps
80107206:	e9 84 f2 ff ff       	jmp    8010648f <alltraps>

8010720b <vector215>:
.globl vector215
vector215:
  pushl $0
8010720b:	6a 00                	push   $0x0
  pushl $215
8010720d:	68 d7 00 00 00       	push   $0xd7
  jmp alltraps
80107212:	e9 78 f2 ff ff       	jmp    8010648f <alltraps>

80107217 <vector216>:
.globl vector216
vector216:
  pushl $0
80107217:	6a 00                	push   $0x0
  pushl $216
80107219:	68 d8 00 00 00       	push   $0xd8
  jmp alltraps
8010721e:	e9 6c f2 ff ff       	jmp    8010648f <alltraps>

80107223 <vector217>:
.globl vector217
vector217:
  pushl $0
80107223:	6a 00                	push   $0x0
  pushl $217
80107225:	68 d9 00 00 00       	push   $0xd9
  jmp alltraps
8010722a:	e9 60 f2 ff ff       	jmp    8010648f <alltraps>

8010722f <vector218>:
.globl vector218
vector218:
  pushl $0
8010722f:	6a 00                	push   $0x0
  pushl $218
80107231:	68 da 00 00 00       	push   $0xda
  jmp alltraps
80107236:	e9 54 f2 ff ff       	jmp    8010648f <alltraps>

8010723b <vector219>:
.globl vector219
vector219:
  pushl $0
8010723b:	6a 00                	push   $0x0
  pushl $219
8010723d:	68 db 00 00 00       	push   $0xdb
  jmp alltraps
80107242:	e9 48 f2 ff ff       	jmp    8010648f <alltraps>

80107247 <vector220>:
.globl vector220
vector220:
  pushl $0
80107247:	6a 00                	push   $0x0
  pushl $220
80107249:	68 dc 00 00 00       	push   $0xdc
  jmp alltraps
8010724e:	e9 3c f2 ff ff       	jmp    8010648f <alltraps>

80107253 <vector221>:
.globl vector221
vector221:
  pushl $0
80107253:	6a 00                	push   $0x0
  pushl $221
80107255:	68 dd 00 00 00       	push   $0xdd
  jmp alltraps
8010725a:	e9 30 f2 ff ff       	jmp    8010648f <alltraps>

8010725f <vector222>:
.globl vector222
vector222:
  pushl $0
8010725f:	6a 00                	push   $0x0
  pushl $222
80107261:	68 de 00 00 00       	push   $0xde
  jmp alltraps
80107266:	e9 24 f2 ff ff       	jmp    8010648f <alltraps>

8010726b <vector223>:
.globl vector223
vector223:
  pushl $0
8010726b:	6a 00                	push   $0x0
  pushl $223
8010726d:	68 df 00 00 00       	push   $0xdf
  jmp alltraps
80107272:	e9 18 f2 ff ff       	jmp    8010648f <alltraps>

80107277 <vector224>:
.globl vector224
vector224:
  pushl $0
80107277:	6a 00                	push   $0x0
  pushl $224
80107279:	68 e0 00 00 00       	push   $0xe0
  jmp alltraps
8010727e:	e9 0c f2 ff ff       	jmp    8010648f <alltraps>

80107283 <vector225>:
.globl vector225
vector225:
  pushl $0
80107283:	6a 00                	push   $0x0
  pushl $225
80107285:	68 e1 00 00 00       	push   $0xe1
  jmp alltraps
8010728a:	e9 00 f2 ff ff       	jmp    8010648f <alltraps>

8010728f <vector226>:
.globl vector226
vector226:
  pushl $0
8010728f:	6a 00                	push   $0x0
  pushl $226
80107291:	68 e2 00 00 00       	push   $0xe2
  jmp alltraps
80107296:	e9 f4 f1 ff ff       	jmp    8010648f <alltraps>

8010729b <vector227>:
.globl vector227
vector227:
  pushl $0
8010729b:	6a 00                	push   $0x0
  pushl $227
8010729d:	68 e3 00 00 00       	push   $0xe3
  jmp alltraps
801072a2:	e9 e8 f1 ff ff       	jmp    8010648f <alltraps>

801072a7 <vector228>:
.globl vector228
vector228:
  pushl $0
801072a7:	6a 00                	push   $0x0
  pushl $228
801072a9:	68 e4 00 00 00       	push   $0xe4
  jmp alltraps
801072ae:	e9 dc f1 ff ff       	jmp    8010648f <alltraps>

801072b3 <vector229>:
.globl vector229
vector229:
  pushl $0
801072b3:	6a 00                	push   $0x0
  pushl $229
801072b5:	68 e5 00 00 00       	push   $0xe5
  jmp alltraps
801072ba:	e9 d0 f1 ff ff       	jmp    8010648f <alltraps>

801072bf <vector230>:
.globl vector230
vector230:
  pushl $0
801072bf:	6a 00                	push   $0x0
  pushl $230
801072c1:	68 e6 00 00 00       	push   $0xe6
  jmp alltraps
801072c6:	e9 c4 f1 ff ff       	jmp    8010648f <alltraps>

801072cb <vector231>:
.globl vector231
vector231:
  pushl $0
801072cb:	6a 00                	push   $0x0
  pushl $231
801072cd:	68 e7 00 00 00       	push   $0xe7
  jmp alltraps
801072d2:	e9 b8 f1 ff ff       	jmp    8010648f <alltraps>

801072d7 <vector232>:
.globl vector232
vector232:
  pushl $0
801072d7:	6a 00                	push   $0x0
  pushl $232
801072d9:	68 e8 00 00 00       	push   $0xe8
  jmp alltraps
801072de:	e9 ac f1 ff ff       	jmp    8010648f <alltraps>

801072e3 <vector233>:
.globl vector233
vector233:
  pushl $0
801072e3:	6a 00                	push   $0x0
  pushl $233
801072e5:	68 e9 00 00 00       	push   $0xe9
  jmp alltraps
801072ea:	e9 a0 f1 ff ff       	jmp    8010648f <alltraps>

801072ef <vector234>:
.globl vector234
vector234:
  pushl $0
801072ef:	6a 00                	push   $0x0
  pushl $234
801072f1:	68 ea 00 00 00       	push   $0xea
  jmp alltraps
801072f6:	e9 94 f1 ff ff       	jmp    8010648f <alltraps>

801072fb <vector235>:
.globl vector235
vector235:
  pushl $0
801072fb:	6a 00                	push   $0x0
  pushl $235
801072fd:	68 eb 00 00 00       	push   $0xeb
  jmp alltraps
80107302:	e9 88 f1 ff ff       	jmp    8010648f <alltraps>

80107307 <vector236>:
.globl vector236
vector236:
  pushl $0
80107307:	6a 00                	push   $0x0
  pushl $236
80107309:	68 ec 00 00 00       	push   $0xec
  jmp alltraps
8010730e:	e9 7c f1 ff ff       	jmp    8010648f <alltraps>

80107313 <vector237>:
.globl vector237
vector237:
  pushl $0
80107313:	6a 00                	push   $0x0
  pushl $237
80107315:	68 ed 00 00 00       	push   $0xed
  jmp alltraps
8010731a:	e9 70 f1 ff ff       	jmp    8010648f <alltraps>

8010731f <vector238>:
.globl vector238
vector238:
  pushl $0
8010731f:	6a 00                	push   $0x0
  pushl $238
80107321:	68 ee 00 00 00       	push   $0xee
  jmp alltraps
80107326:	e9 64 f1 ff ff       	jmp    8010648f <alltraps>

8010732b <vector239>:
.globl vector239
vector239:
  pushl $0
8010732b:	6a 00                	push   $0x0
  pushl $239
8010732d:	68 ef 00 00 00       	push   $0xef
  jmp alltraps
80107332:	e9 58 f1 ff ff       	jmp    8010648f <alltraps>

80107337 <vector240>:
.globl vector240
vector240:
  pushl $0
80107337:	6a 00                	push   $0x0
  pushl $240
80107339:	68 f0 00 00 00       	push   $0xf0
  jmp alltraps
8010733e:	e9 4c f1 ff ff       	jmp    8010648f <alltraps>

80107343 <vector241>:
.globl vector241
vector241:
  pushl $0
80107343:	6a 00                	push   $0x0
  pushl $241
80107345:	68 f1 00 00 00       	push   $0xf1
  jmp alltraps
8010734a:	e9 40 f1 ff ff       	jmp    8010648f <alltraps>

8010734f <vector242>:
.globl vector242
vector242:
  pushl $0
8010734f:	6a 00                	push   $0x0
  pushl $242
80107351:	68 f2 00 00 00       	push   $0xf2
  jmp alltraps
80107356:	e9 34 f1 ff ff       	jmp    8010648f <alltraps>

8010735b <vector243>:
.globl vector243
vector243:
  pushl $0
8010735b:	6a 00                	push   $0x0
  pushl $243
8010735d:	68 f3 00 00 00       	push   $0xf3
  jmp alltraps
80107362:	e9 28 f1 ff ff       	jmp    8010648f <alltraps>

80107367 <vector244>:
.globl vector244
vector244:
  pushl $0
80107367:	6a 00                	push   $0x0
  pushl $244
80107369:	68 f4 00 00 00       	push   $0xf4
  jmp alltraps
8010736e:	e9 1c f1 ff ff       	jmp    8010648f <alltraps>

80107373 <vector245>:
.globl vector245
vector245:
  pushl $0
80107373:	6a 00                	push   $0x0
  pushl $245
80107375:	68 f5 00 00 00       	push   $0xf5
  jmp alltraps
8010737a:	e9 10 f1 ff ff       	jmp    8010648f <alltraps>

8010737f <vector246>:
.globl vector246
vector246:
  pushl $0
8010737f:	6a 00                	push   $0x0
  pushl $246
80107381:	68 f6 00 00 00       	push   $0xf6
  jmp alltraps
80107386:	e9 04 f1 ff ff       	jmp    8010648f <alltraps>

8010738b <vector247>:
.globl vector247
vector247:
  pushl $0
8010738b:	6a 00                	push   $0x0
  pushl $247
8010738d:	68 f7 00 00 00       	push   $0xf7
  jmp alltraps
80107392:	e9 f8 f0 ff ff       	jmp    8010648f <alltraps>

80107397 <vector248>:
.globl vector248
vector248:
  pushl $0
80107397:	6a 00                	push   $0x0
  pushl $248
80107399:	68 f8 00 00 00       	push   $0xf8
  jmp alltraps
8010739e:	e9 ec f0 ff ff       	jmp    8010648f <alltraps>

801073a3 <vector249>:
.globl vector249
vector249:
  pushl $0
801073a3:	6a 00                	push   $0x0
  pushl $249
801073a5:	68 f9 00 00 00       	push   $0xf9
  jmp alltraps
801073aa:	e9 e0 f0 ff ff       	jmp    8010648f <alltraps>

801073af <vector250>:
.globl vector250
vector250:
  pushl $0
801073af:	6a 00                	push   $0x0
  pushl $250
801073b1:	68 fa 00 00 00       	push   $0xfa
  jmp alltraps
801073b6:	e9 d4 f0 ff ff       	jmp    8010648f <alltraps>

801073bb <vector251>:
.globl vector251
vector251:
  pushl $0
801073bb:	6a 00                	push   $0x0
  pushl $251
801073bd:	68 fb 00 00 00       	push   $0xfb
  jmp alltraps
801073c2:	e9 c8 f0 ff ff       	jmp    8010648f <alltraps>

801073c7 <vector252>:
.globl vector252
vector252:
  pushl $0
801073c7:	6a 00                	push   $0x0
  pushl $252
801073c9:	68 fc 00 00 00       	push   $0xfc
  jmp alltraps
801073ce:	e9 bc f0 ff ff       	jmp    8010648f <alltraps>

801073d3 <vector253>:
.globl vector253
vector253:
  pushl $0
801073d3:	6a 00                	push   $0x0
  pushl $253
801073d5:	68 fd 00 00 00       	push   $0xfd
  jmp alltraps
801073da:	e9 b0 f0 ff ff       	jmp    8010648f <alltraps>

801073df <vector254>:
.globl vector254
vector254:
  pushl $0
801073df:	6a 00                	push   $0x0
  pushl $254
801073e1:	68 fe 00 00 00       	push   $0xfe
  jmp alltraps
801073e6:	e9 a4 f0 ff ff       	jmp    8010648f <alltraps>

801073eb <vector255>:
.globl vector255
vector255:
  pushl $0
801073eb:	6a 00                	push   $0x0
  pushl $255
801073ed:	68 ff 00 00 00       	push   $0xff
  jmp alltraps
801073f2:	e9 98 f0 ff ff       	jmp    8010648f <alltraps>
801073f7:	66 90                	xchg   %ax,%ax
801073f9:	66 90                	xchg   %ax,%ax
801073fb:	66 90                	xchg   %ax,%ax
801073fd:	66 90                	xchg   %ax,%ax
801073ff:	90                   	nop

80107400 <deallocuvm.part.0>:
// Deallocate user pages to bring the process size from oldsz to
// newsz.  oldsz and newsz need not be page-aligned, nor does newsz
// need to be less than oldsz.  oldsz can be larger than the actual
// process size.  Returns the new process size.
int
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
80107400:	55                   	push   %ebp
80107401:	89 e5                	mov    %esp,%ebp
80107403:	57                   	push   %edi
80107404:	56                   	push   %esi
80107405:	53                   	push   %ebx
  uint a, pa;

  if(newsz >= oldsz)
    return oldsz;

  a = PGROUNDUP(newsz);
80107406:	8d 99 ff 0f 00 00    	lea    0xfff(%ecx),%ebx
8010740c:	81 e3 00 f0 ff ff    	and    $0xfffff000,%ebx
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
80107412:	83 ec 1c             	sub    $0x1c,%esp
80107415:	89 4d e0             	mov    %ecx,-0x20(%ebp)
  for(; a  < oldsz; a += PGSIZE){
80107418:	39 d3                	cmp    %edx,%ebx
8010741a:	73 49                	jae    80107465 <deallocuvm.part.0+0x65>
8010741c:	89 c7                	mov    %eax,%edi
8010741e:	eb 0c                	jmp    8010742c <deallocuvm.part.0+0x2c>
    pte = walkpgdir(pgdir, (char*)a, 0);
    if(!pte)
      a = PGADDR(PDX(a) + 1, 0, 0) - PGSIZE;
80107420:	83 c0 01             	add    $0x1,%eax
80107423:	c1 e0 16             	shl    $0x16,%eax
80107426:	89 c3                	mov    %eax,%ebx
  for(; a  < oldsz; a += PGSIZE){
80107428:	39 da                	cmp    %ebx,%edx
8010742a:	76 39                	jbe    80107465 <deallocuvm.part.0+0x65>
  pde = &pgdir[PDX(va)];
8010742c:	89 d8                	mov    %ebx,%eax
8010742e:	c1 e8 16             	shr    $0x16,%eax
  if(*pde & PTE_P){
80107431:	8b 0c 87             	mov    (%edi,%eax,4),%ecx
80107434:	f6 c1 01             	test   $0x1,%cl
80107437:	74 e7                	je     80107420 <deallocuvm.part.0+0x20>
  return &pgtab[PTX(va)];
80107439:	89 de                	mov    %ebx,%esi
    pgtab = (pte_t*)P2V(PTE_ADDR(*pde));
8010743b:	81 e1 00 f0 ff ff    	and    $0xfffff000,%ecx
  return &pgtab[PTX(va)];
80107441:	c1 ee 0a             	shr    $0xa,%esi
80107444:	81 e6 fc 0f 00 00    	and    $0xffc,%esi
8010744a:	8d b4 31 00 00 00 80 	lea    -0x80000000(%ecx,%esi,1),%esi
    if(!pte)
80107451:	85 f6                	test   %esi,%esi
80107453:	74 cb                	je     80107420 <deallocuvm.part.0+0x20>
    else if((*pte & PTE_P) != 0){
80107455:	8b 06                	mov    (%esi),%eax
80107457:	a8 01                	test   $0x1,%al
80107459:	75 15                	jne    80107470 <deallocuvm.part.0+0x70>
  for(; a  < oldsz; a += PGSIZE){
8010745b:	81 c3 00 10 00 00    	add    $0x1000,%ebx
80107461:	39 da                	cmp    %ebx,%edx
80107463:	77 c7                	ja     8010742c <deallocuvm.part.0+0x2c>
      kfree(v);
      *pte = 0;
    }
  }
  return newsz;
}
80107465:	8b 45 e0             	mov    -0x20(%ebp),%eax
80107468:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010746b:	5b                   	pop    %ebx
8010746c:	5e                   	pop    %esi
8010746d:	5f                   	pop    %edi
8010746e:	5d                   	pop    %ebp
8010746f:	c3                   	ret    
      if(pa == 0)
80107470:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107475:	74 25                	je     8010749c <deallocuvm.part.0+0x9c>
      kfree(v);
80107477:	83 ec 0c             	sub    $0xc,%esp
      char *v = P2V(pa);
8010747a:	05 00 00 00 80       	add    $0x80000000,%eax
8010747f:	89 55 e4             	mov    %edx,-0x1c(%ebp)
  for(; a  < oldsz; a += PGSIZE){
80107482:	81 c3 00 10 00 00    	add    $0x1000,%ebx
      kfree(v);
80107488:	50                   	push   %eax
80107489:	e8 e2 b0 ff ff       	call   80102570 <kfree>
      *pte = 0;
8010748e:	c7 06 00 00 00 00    	movl   $0x0,(%esi)
  for(; a  < oldsz; a += PGSIZE){
80107494:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80107497:	83 c4 10             	add    $0x10,%esp
8010749a:	eb 8c                	jmp    80107428 <deallocuvm.part.0+0x28>
        panic("kfree");
8010749c:	83 ec 0c             	sub    $0xc,%esp
8010749f:	68 ae 80 10 80       	push   $0x801080ae
801074a4:	e8 d7 8e ff ff       	call   80100380 <panic>
801074a9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

801074b0 <mappages>:
{
801074b0:	55                   	push   %ebp
801074b1:	89 e5                	mov    %esp,%ebp
801074b3:	57                   	push   %edi
801074b4:	56                   	push   %esi
801074b5:	53                   	push   %ebx
  a = (char*)PGROUNDDOWN((uint)va);
801074b6:	89 d3                	mov    %edx,%ebx
801074b8:	81 e3 00 f0 ff ff    	and    $0xfffff000,%ebx
{
801074be:	83 ec 1c             	sub    $0x1c,%esp
801074c1:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  last = (char*)PGROUNDDOWN(((uint)va) + size - 1);
801074c4:	8d 44 0a ff          	lea    -0x1(%edx,%ecx,1),%eax
801074c8:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801074cd:	89 45 dc             	mov    %eax,-0x24(%ebp)
801074d0:	8b 45 08             	mov    0x8(%ebp),%eax
801074d3:	29 d8                	sub    %ebx,%eax
801074d5:	89 45 e0             	mov    %eax,-0x20(%ebp)
801074d8:	eb 3d                	jmp    80107517 <mappages+0x67>
801074da:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  return &pgtab[PTX(va)];
801074e0:	89 da                	mov    %ebx,%edx
    pgtab = (pte_t*)P2V(PTE_ADDR(*pde));
801074e2:	25 00 f0 ff ff       	and    $0xfffff000,%eax
  return &pgtab[PTX(va)];
801074e7:	c1 ea 0a             	shr    $0xa,%edx
801074ea:	81 e2 fc 0f 00 00    	and    $0xffc,%edx
801074f0:	8d 84 10 00 00 00 80 	lea    -0x80000000(%eax,%edx,1),%eax
    if((pte = walkpgdir(pgdir, a, 1)) == 0)
801074f7:	85 c0                	test   %eax,%eax
801074f9:	74 75                	je     80107570 <mappages+0xc0>
    if(*pte & PTE_P)
801074fb:	f6 00 01             	testb  $0x1,(%eax)
801074fe:	0f 85 86 00 00 00    	jne    8010758a <mappages+0xda>
    *pte = pa | perm | PTE_P;
80107504:	0b 75 0c             	or     0xc(%ebp),%esi
80107507:	83 ce 01             	or     $0x1,%esi
8010750a:	89 30                	mov    %esi,(%eax)
    if(a == last)
8010750c:	3b 5d dc             	cmp    -0x24(%ebp),%ebx
8010750f:	74 6f                	je     80107580 <mappages+0xd0>
    a += PGSIZE;
80107511:	81 c3 00 10 00 00    	add    $0x1000,%ebx
  for(;;){
80107517:	8b 45 e0             	mov    -0x20(%ebp),%eax
  pde = &pgdir[PDX(va)];
8010751a:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
8010751d:	8d 34 18             	lea    (%eax,%ebx,1),%esi
80107520:	89 d8                	mov    %ebx,%eax
80107522:	c1 e8 16             	shr    $0x16,%eax
80107525:	8d 3c 81             	lea    (%ecx,%eax,4),%edi
  if(*pde & PTE_P){
80107528:	8b 07                	mov    (%edi),%eax
8010752a:	a8 01                	test   $0x1,%al
8010752c:	75 b2                	jne    801074e0 <mappages+0x30>
    if(!alloc || (pgtab = (pte_t*)kalloc()) == 0)
8010752e:	e8 fd b1 ff ff       	call   80102730 <kalloc>
80107533:	85 c0                	test   %eax,%eax
80107535:	74 39                	je     80107570 <mappages+0xc0>
    memset(pgtab, 0, PGSIZE);
80107537:	83 ec 04             	sub    $0x4,%esp
8010753a:	89 45 d8             	mov    %eax,-0x28(%ebp)
8010753d:	68 00 10 00 00       	push   $0x1000
80107542:	6a 00                	push   $0x0
80107544:	50                   	push   %eax
80107545:	e8 56 dd ff ff       	call   801052a0 <memset>
    *pde = V2P(pgtab) | PTE_P | PTE_W | PTE_U;
8010754a:	8b 55 d8             	mov    -0x28(%ebp),%edx
  return &pgtab[PTX(va)];
8010754d:	83 c4 10             	add    $0x10,%esp
    *pde = V2P(pgtab) | PTE_P | PTE_W | PTE_U;
80107550:	8d 82 00 00 00 80    	lea    -0x80000000(%edx),%eax
80107556:	83 c8 07             	or     $0x7,%eax
80107559:	89 07                	mov    %eax,(%edi)
  return &pgtab[PTX(va)];
8010755b:	89 d8                	mov    %ebx,%eax
8010755d:	c1 e8 0a             	shr    $0xa,%eax
80107560:	25 fc 0f 00 00       	and    $0xffc,%eax
80107565:	01 d0                	add    %edx,%eax
80107567:	eb 92                	jmp    801074fb <mappages+0x4b>
80107569:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
}
80107570:	8d 65 f4             	lea    -0xc(%ebp),%esp
      return -1;
80107573:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80107578:	5b                   	pop    %ebx
80107579:	5e                   	pop    %esi
8010757a:	5f                   	pop    %edi
8010757b:	5d                   	pop    %ebp
8010757c:	c3                   	ret    
8010757d:	8d 76 00             	lea    0x0(%esi),%esi
80107580:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
80107583:	31 c0                	xor    %eax,%eax
}
80107585:	5b                   	pop    %ebx
80107586:	5e                   	pop    %esi
80107587:	5f                   	pop    %edi
80107588:	5d                   	pop    %ebp
80107589:	c3                   	ret    
      panic("remap");
8010758a:	83 ec 0c             	sub    $0xc,%esp
8010758d:	68 f0 88 10 80       	push   $0x801088f0
80107592:	e8 e9 8d ff ff       	call   80100380 <panic>
80107597:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010759e:	66 90                	xchg   %ax,%ax

801075a0 <seginit>:
{
801075a0:	55                   	push   %ebp
801075a1:	89 e5                	mov    %esp,%ebp
801075a3:	83 ec 18             	sub    $0x18,%esp
  c = &cpus[cpuid()];
801075a6:	e8 a5 c4 ff ff       	call   80103a50 <cpuid>
  pd[0] = size-1;
801075ab:	ba 2f 00 00 00       	mov    $0x2f,%edx
801075b0:	69 c0 b0 00 00 00    	imul   $0xb0,%eax,%eax
801075b6:	66 89 55 f2          	mov    %dx,-0xe(%ebp)
  c->gdt[SEG_KCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, 0);
801075ba:	c7 80 18 28 11 80 ff 	movl   $0xffff,-0x7feed7e8(%eax)
801075c1:	ff 00 00 
801075c4:	c7 80 1c 28 11 80 00 	movl   $0xcf9a00,-0x7feed7e4(%eax)
801075cb:	9a cf 00 
  c->gdt[SEG_KDATA] = SEG(STA_W, 0, 0xffffffff, 0);
801075ce:	c7 80 20 28 11 80 ff 	movl   $0xffff,-0x7feed7e0(%eax)
801075d5:	ff 00 00 
801075d8:	c7 80 24 28 11 80 00 	movl   $0xcf9200,-0x7feed7dc(%eax)
801075df:	92 cf 00 
  c->gdt[SEG_UCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, DPL_USER);
801075e2:	c7 80 28 28 11 80 ff 	movl   $0xffff,-0x7feed7d8(%eax)
801075e9:	ff 00 00 
801075ec:	c7 80 2c 28 11 80 00 	movl   $0xcffa00,-0x7feed7d4(%eax)
801075f3:	fa cf 00 
  c->gdt[SEG_UDATA] = SEG(STA_W, 0, 0xffffffff, DPL_USER);
801075f6:	c7 80 30 28 11 80 ff 	movl   $0xffff,-0x7feed7d0(%eax)
801075fd:	ff 00 00 
80107600:	c7 80 34 28 11 80 00 	movl   $0xcff200,-0x7feed7cc(%eax)
80107607:	f2 cf 00 
  lgdt(c->gdt, sizeof(c->gdt));
8010760a:	05 10 28 11 80       	add    $0x80112810,%eax
  pd[1] = (uint)p;
8010760f:	66 89 45 f4          	mov    %ax,-0xc(%ebp)
  pd[2] = (uint)p >> 16;
80107613:	c1 e8 10             	shr    $0x10,%eax
80107616:	66 89 45 f6          	mov    %ax,-0xa(%ebp)
  asm volatile("lgdt (%0)" : : "r" (pd));
8010761a:	8d 45 f2             	lea    -0xe(%ebp),%eax
8010761d:	0f 01 10             	lgdtl  (%eax)
}
80107620:	c9                   	leave  
80107621:	c3                   	ret    
80107622:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80107629:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80107630 <switchkvm>:
  lcr3(V2P(kpgdir));   // switch to the kernel page table
80107630:	a1 e4 63 11 80       	mov    0x801163e4,%eax
80107635:	05 00 00 00 80       	add    $0x80000000,%eax
}

static inline void
lcr3(uint val)
{
  asm volatile("movl %0,%%cr3" : : "r" (val));
8010763a:	0f 22 d8             	mov    %eax,%cr3
}
8010763d:	c3                   	ret    
8010763e:	66 90                	xchg   %ax,%ax

80107640 <switchuvm>:
{
80107640:	55                   	push   %ebp
80107641:	89 e5                	mov    %esp,%ebp
80107643:	57                   	push   %edi
80107644:	56                   	push   %esi
80107645:	53                   	push   %ebx
80107646:	83 ec 1c             	sub    $0x1c,%esp
80107649:	8b 75 08             	mov    0x8(%ebp),%esi
  if(p == 0)
8010764c:	85 f6                	test   %esi,%esi
8010764e:	0f 84 cb 00 00 00    	je     8010771f <switchuvm+0xdf>
  if(p->kstack == 0)
80107654:	8b 46 08             	mov    0x8(%esi),%eax
80107657:	85 c0                	test   %eax,%eax
80107659:	0f 84 da 00 00 00    	je     80107739 <switchuvm+0xf9>
  if(p->pgdir == 0)
8010765f:	8b 46 04             	mov    0x4(%esi),%eax
80107662:	85 c0                	test   %eax,%eax
80107664:	0f 84 c2 00 00 00    	je     8010772c <switchuvm+0xec>
  pushcli();
8010766a:	e8 21 da ff ff       	call   80105090 <pushcli>
  mycpu()->gdt[SEG_TSS] = SEG16(STS_T32A, &mycpu()->ts,
8010766f:	e8 7c c3 ff ff       	call   801039f0 <mycpu>
80107674:	89 c3                	mov    %eax,%ebx
80107676:	e8 75 c3 ff ff       	call   801039f0 <mycpu>
8010767b:	89 c7                	mov    %eax,%edi
8010767d:	e8 6e c3 ff ff       	call   801039f0 <mycpu>
80107682:	83 c7 08             	add    $0x8,%edi
80107685:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80107688:	e8 63 c3 ff ff       	call   801039f0 <mycpu>
8010768d:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
80107690:	ba 67 00 00 00       	mov    $0x67,%edx
80107695:	66 89 bb 9a 00 00 00 	mov    %di,0x9a(%ebx)
8010769c:	83 c0 08             	add    $0x8,%eax
8010769f:	66 89 93 98 00 00 00 	mov    %dx,0x98(%ebx)
  mycpu()->ts.iomb = (ushort) 0xFFFF;
801076a6:	bf ff ff ff ff       	mov    $0xffffffff,%edi
  mycpu()->gdt[SEG_TSS] = SEG16(STS_T32A, &mycpu()->ts,
801076ab:	83 c1 08             	add    $0x8,%ecx
801076ae:	c1 e8 18             	shr    $0x18,%eax
801076b1:	c1 e9 10             	shr    $0x10,%ecx
801076b4:	88 83 9f 00 00 00    	mov    %al,0x9f(%ebx)
801076ba:	88 8b 9c 00 00 00    	mov    %cl,0x9c(%ebx)
801076c0:	b9 99 40 00 00       	mov    $0x4099,%ecx
801076c5:	66 89 8b 9d 00 00 00 	mov    %cx,0x9d(%ebx)
  mycpu()->ts.ss0 = SEG_KDATA << 3;
801076cc:	bb 10 00 00 00       	mov    $0x10,%ebx
  mycpu()->gdt[SEG_TSS].s = 0;
801076d1:	e8 1a c3 ff ff       	call   801039f0 <mycpu>
801076d6:	80 a0 9d 00 00 00 ef 	andb   $0xef,0x9d(%eax)
  mycpu()->ts.ss0 = SEG_KDATA << 3;
801076dd:	e8 0e c3 ff ff       	call   801039f0 <mycpu>
801076e2:	66 89 58 10          	mov    %bx,0x10(%eax)
  mycpu()->ts.esp0 = (uint)p->kstack + KSTACKSIZE;
801076e6:	8b 5e 08             	mov    0x8(%esi),%ebx
801076e9:	81 c3 00 10 00 00    	add    $0x1000,%ebx
801076ef:	e8 fc c2 ff ff       	call   801039f0 <mycpu>
801076f4:	89 58 0c             	mov    %ebx,0xc(%eax)
  mycpu()->ts.iomb = (ushort) 0xFFFF;
801076f7:	e8 f4 c2 ff ff       	call   801039f0 <mycpu>
801076fc:	66 89 78 6e          	mov    %di,0x6e(%eax)
  asm volatile("ltr %0" : : "r" (sel));
80107700:	b8 28 00 00 00       	mov    $0x28,%eax
80107705:	0f 00 d8             	ltr    %ax
  lcr3(V2P(p->pgdir));  // switch to process's address space
80107708:	8b 46 04             	mov    0x4(%esi),%eax
8010770b:	05 00 00 00 80       	add    $0x80000000,%eax
  asm volatile("movl %0,%%cr3" : : "r" (val));
80107710:	0f 22 d8             	mov    %eax,%cr3
}
80107713:	8d 65 f4             	lea    -0xc(%ebp),%esp
80107716:	5b                   	pop    %ebx
80107717:	5e                   	pop    %esi
80107718:	5f                   	pop    %edi
80107719:	5d                   	pop    %ebp
  popcli();
8010771a:	e9 c1 d9 ff ff       	jmp    801050e0 <popcli>
    panic("switchuvm: no process");
8010771f:	83 ec 0c             	sub    $0xc,%esp
80107722:	68 f6 88 10 80       	push   $0x801088f6
80107727:	e8 54 8c ff ff       	call   80100380 <panic>
    panic("switchuvm: no pgdir");
8010772c:	83 ec 0c             	sub    $0xc,%esp
8010772f:	68 21 89 10 80       	push   $0x80108921
80107734:	e8 47 8c ff ff       	call   80100380 <panic>
    panic("switchuvm: no kstack");
80107739:	83 ec 0c             	sub    $0xc,%esp
8010773c:	68 0c 89 10 80       	push   $0x8010890c
80107741:	e8 3a 8c ff ff       	call   80100380 <panic>
80107746:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
8010774d:	8d 76 00             	lea    0x0(%esi),%esi

80107750 <inituvm>:
{
80107750:	55                   	push   %ebp
80107751:	89 e5                	mov    %esp,%ebp
80107753:	57                   	push   %edi
80107754:	56                   	push   %esi
80107755:	53                   	push   %ebx
80107756:	83 ec 1c             	sub    $0x1c,%esp
80107759:	8b 45 0c             	mov    0xc(%ebp),%eax
8010775c:	8b 75 10             	mov    0x10(%ebp),%esi
8010775f:	8b 7d 08             	mov    0x8(%ebp),%edi
80107762:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  if(sz >= PGSIZE)
80107765:	81 fe ff 0f 00 00    	cmp    $0xfff,%esi
8010776b:	77 4b                	ja     801077b8 <inituvm+0x68>
  mem = kalloc();
8010776d:	e8 be af ff ff       	call   80102730 <kalloc>
  memset(mem, 0, PGSIZE);
80107772:	83 ec 04             	sub    $0x4,%esp
80107775:	68 00 10 00 00       	push   $0x1000
  mem = kalloc();
8010777a:	89 c3                	mov    %eax,%ebx
  memset(mem, 0, PGSIZE);
8010777c:	6a 00                	push   $0x0
8010777e:	50                   	push   %eax
8010777f:	e8 1c db ff ff       	call   801052a0 <memset>
  mappages(pgdir, 0, PGSIZE, V2P(mem), PTE_W|PTE_U);
80107784:	58                   	pop    %eax
80107785:	8d 83 00 00 00 80    	lea    -0x80000000(%ebx),%eax
8010778b:	5a                   	pop    %edx
8010778c:	6a 06                	push   $0x6
8010778e:	b9 00 10 00 00       	mov    $0x1000,%ecx
80107793:	31 d2                	xor    %edx,%edx
80107795:	50                   	push   %eax
80107796:	89 f8                	mov    %edi,%eax
80107798:	e8 13 fd ff ff       	call   801074b0 <mappages>
  memmove(mem, init, sz);
8010779d:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801077a0:	89 75 10             	mov    %esi,0x10(%ebp)
801077a3:	83 c4 10             	add    $0x10,%esp
801077a6:	89 5d 08             	mov    %ebx,0x8(%ebp)
801077a9:	89 45 0c             	mov    %eax,0xc(%ebp)
}
801077ac:	8d 65 f4             	lea    -0xc(%ebp),%esp
801077af:	5b                   	pop    %ebx
801077b0:	5e                   	pop    %esi
801077b1:	5f                   	pop    %edi
801077b2:	5d                   	pop    %ebp
  memmove(mem, init, sz);
801077b3:	e9 88 db ff ff       	jmp    80105340 <memmove>
    panic("inituvm: more than a page");
801077b8:	83 ec 0c             	sub    $0xc,%esp
801077bb:	68 35 89 10 80       	push   $0x80108935
801077c0:	e8 bb 8b ff ff       	call   80100380 <panic>
801077c5:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801077cc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

801077d0 <loaduvm>:
{
801077d0:	55                   	push   %ebp
801077d1:	89 e5                	mov    %esp,%ebp
801077d3:	57                   	push   %edi
801077d4:	56                   	push   %esi
801077d5:	53                   	push   %ebx
801077d6:	83 ec 1c             	sub    $0x1c,%esp
801077d9:	8b 45 0c             	mov    0xc(%ebp),%eax
801077dc:	8b 75 18             	mov    0x18(%ebp),%esi
  if((uint) addr % PGSIZE != 0)
801077df:	a9 ff 0f 00 00       	test   $0xfff,%eax
801077e4:	0f 85 bb 00 00 00    	jne    801078a5 <loaduvm+0xd5>
  for(i = 0; i < sz; i += PGSIZE){
801077ea:	01 f0                	add    %esi,%eax
801077ec:	89 f3                	mov    %esi,%ebx
801077ee:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if(readi(ip, P2V(pa), offset+i, n) != n)
801077f1:	8b 45 14             	mov    0x14(%ebp),%eax
801077f4:	01 f0                	add    %esi,%eax
801077f6:	89 45 e0             	mov    %eax,-0x20(%ebp)
  for(i = 0; i < sz; i += PGSIZE){
801077f9:	85 f6                	test   %esi,%esi
801077fb:	0f 84 87 00 00 00    	je     80107888 <loaduvm+0xb8>
80107801:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  pde = &pgdir[PDX(va)];
80107808:	8b 45 e4             	mov    -0x1c(%ebp),%eax
  if(*pde & PTE_P){
8010780b:	8b 4d 08             	mov    0x8(%ebp),%ecx
8010780e:	29 d8                	sub    %ebx,%eax
  pde = &pgdir[PDX(va)];
80107810:	89 c2                	mov    %eax,%edx
80107812:	c1 ea 16             	shr    $0x16,%edx
  if(*pde & PTE_P){
80107815:	8b 14 91             	mov    (%ecx,%edx,4),%edx
80107818:	f6 c2 01             	test   $0x1,%dl
8010781b:	75 13                	jne    80107830 <loaduvm+0x60>
      panic("loaduvm: address should exist");
8010781d:	83 ec 0c             	sub    $0xc,%esp
80107820:	68 4f 89 10 80       	push   $0x8010894f
80107825:	e8 56 8b ff ff       	call   80100380 <panic>
8010782a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  return &pgtab[PTX(va)];
80107830:	c1 e8 0a             	shr    $0xa,%eax
    pgtab = (pte_t*)P2V(PTE_ADDR(*pde));
80107833:	81 e2 00 f0 ff ff    	and    $0xfffff000,%edx
  return &pgtab[PTX(va)];
80107839:	25 fc 0f 00 00       	and    $0xffc,%eax
8010783e:	8d 84 02 00 00 00 80 	lea    -0x80000000(%edx,%eax,1),%eax
    if((pte = walkpgdir(pgdir, addr+i, 0)) == 0)
80107845:	85 c0                	test   %eax,%eax
80107847:	74 d4                	je     8010781d <loaduvm+0x4d>
    pa = PTE_ADDR(*pte);
80107849:	8b 00                	mov    (%eax),%eax
    if(readi(ip, P2V(pa), offset+i, n) != n)
8010784b:	8b 4d e0             	mov    -0x20(%ebp),%ecx
    if(sz - i < PGSIZE)
8010784e:	bf 00 10 00 00       	mov    $0x1000,%edi
    pa = PTE_ADDR(*pte);
80107853:	25 00 f0 ff ff       	and    $0xfffff000,%eax
    if(sz - i < PGSIZE)
80107858:	81 fb ff 0f 00 00    	cmp    $0xfff,%ebx
8010785e:	0f 46 fb             	cmovbe %ebx,%edi
    if(readi(ip, P2V(pa), offset+i, n) != n)
80107861:	29 d9                	sub    %ebx,%ecx
80107863:	05 00 00 00 80       	add    $0x80000000,%eax
80107868:	57                   	push   %edi
80107869:	51                   	push   %ecx
8010786a:	50                   	push   %eax
8010786b:	ff 75 10             	push   0x10(%ebp)
8010786e:	e8 cd a2 ff ff       	call   80101b40 <readi>
80107873:	83 c4 10             	add    $0x10,%esp
80107876:	39 f8                	cmp    %edi,%eax
80107878:	75 1e                	jne    80107898 <loaduvm+0xc8>
  for(i = 0; i < sz; i += PGSIZE){
8010787a:	81 eb 00 10 00 00    	sub    $0x1000,%ebx
80107880:	89 f0                	mov    %esi,%eax
80107882:	29 d8                	sub    %ebx,%eax
80107884:	39 c6                	cmp    %eax,%esi
80107886:	77 80                	ja     80107808 <loaduvm+0x38>
}
80107888:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
8010788b:	31 c0                	xor    %eax,%eax
}
8010788d:	5b                   	pop    %ebx
8010788e:	5e                   	pop    %esi
8010788f:	5f                   	pop    %edi
80107890:	5d                   	pop    %ebp
80107891:	c3                   	ret    
80107892:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80107898:	8d 65 f4             	lea    -0xc(%ebp),%esp
      return -1;
8010789b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801078a0:	5b                   	pop    %ebx
801078a1:	5e                   	pop    %esi
801078a2:	5f                   	pop    %edi
801078a3:	5d                   	pop    %ebp
801078a4:	c3                   	ret    
    panic("loaduvm: addr must be page aligned");
801078a5:	83 ec 0c             	sub    $0xc,%esp
801078a8:	68 f0 89 10 80       	push   $0x801089f0
801078ad:	e8 ce 8a ff ff       	call   80100380 <panic>
801078b2:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801078b9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

801078c0 <allocuvm>:
{
801078c0:	55                   	push   %ebp
801078c1:	89 e5                	mov    %esp,%ebp
801078c3:	57                   	push   %edi
801078c4:	56                   	push   %esi
801078c5:	53                   	push   %ebx
801078c6:	83 ec 1c             	sub    $0x1c,%esp
  if(newsz >= KERNBASE)
801078c9:	8b 45 10             	mov    0x10(%ebp),%eax
{
801078cc:	8b 7d 08             	mov    0x8(%ebp),%edi
  if(newsz >= KERNBASE)
801078cf:	89 45 e4             	mov    %eax,-0x1c(%ebp)
801078d2:	85 c0                	test   %eax,%eax
801078d4:	0f 88 b6 00 00 00    	js     80107990 <allocuvm+0xd0>
  if(newsz < oldsz)
801078da:	3b 45 0c             	cmp    0xc(%ebp),%eax
    return oldsz;
801078dd:	8b 45 0c             	mov    0xc(%ebp),%eax
  if(newsz < oldsz)
801078e0:	0f 82 9a 00 00 00    	jb     80107980 <allocuvm+0xc0>
  a = PGROUNDUP(oldsz);
801078e6:	8d b0 ff 0f 00 00    	lea    0xfff(%eax),%esi
801078ec:	81 e6 00 f0 ff ff    	and    $0xfffff000,%esi
  for(; a < newsz; a += PGSIZE){
801078f2:	39 75 10             	cmp    %esi,0x10(%ebp)
801078f5:	77 44                	ja     8010793b <allocuvm+0x7b>
801078f7:	e9 87 00 00 00       	jmp    80107983 <allocuvm+0xc3>
801078fc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    memset(mem, 0, PGSIZE);
80107900:	83 ec 04             	sub    $0x4,%esp
80107903:	68 00 10 00 00       	push   $0x1000
80107908:	6a 00                	push   $0x0
8010790a:	50                   	push   %eax
8010790b:	e8 90 d9 ff ff       	call   801052a0 <memset>
    if(mappages(pgdir, (char*)a, PGSIZE, V2P(mem), PTE_W|PTE_U) < 0){
80107910:	58                   	pop    %eax
80107911:	8d 83 00 00 00 80    	lea    -0x80000000(%ebx),%eax
80107917:	5a                   	pop    %edx
80107918:	6a 06                	push   $0x6
8010791a:	b9 00 10 00 00       	mov    $0x1000,%ecx
8010791f:	89 f2                	mov    %esi,%edx
80107921:	50                   	push   %eax
80107922:	89 f8                	mov    %edi,%eax
80107924:	e8 87 fb ff ff       	call   801074b0 <mappages>
80107929:	83 c4 10             	add    $0x10,%esp
8010792c:	85 c0                	test   %eax,%eax
8010792e:	78 78                	js     801079a8 <allocuvm+0xe8>
  for(; a < newsz; a += PGSIZE){
80107930:	81 c6 00 10 00 00    	add    $0x1000,%esi
80107936:	39 75 10             	cmp    %esi,0x10(%ebp)
80107939:	76 48                	jbe    80107983 <allocuvm+0xc3>
    mem = kalloc();
8010793b:	e8 f0 ad ff ff       	call   80102730 <kalloc>
80107940:	89 c3                	mov    %eax,%ebx
    if(mem == 0){
80107942:	85 c0                	test   %eax,%eax
80107944:	75 ba                	jne    80107900 <allocuvm+0x40>
      cprintf("allocuvm out of memory\n");
80107946:	83 ec 0c             	sub    $0xc,%esp
80107949:	68 6d 89 10 80       	push   $0x8010896d
8010794e:	e8 4d 8d ff ff       	call   801006a0 <cprintf>
  if(newsz >= oldsz)
80107953:	8b 45 0c             	mov    0xc(%ebp),%eax
80107956:	83 c4 10             	add    $0x10,%esp
80107959:	39 45 10             	cmp    %eax,0x10(%ebp)
8010795c:	74 32                	je     80107990 <allocuvm+0xd0>
8010795e:	8b 55 10             	mov    0x10(%ebp),%edx
80107961:	89 c1                	mov    %eax,%ecx
80107963:	89 f8                	mov    %edi,%eax
80107965:	e8 96 fa ff ff       	call   80107400 <deallocuvm.part.0>
      return 0;
8010796a:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
}
80107971:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80107974:	8d 65 f4             	lea    -0xc(%ebp),%esp
80107977:	5b                   	pop    %ebx
80107978:	5e                   	pop    %esi
80107979:	5f                   	pop    %edi
8010797a:	5d                   	pop    %ebp
8010797b:	c3                   	ret    
8010797c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    return oldsz;
80107980:	89 45 e4             	mov    %eax,-0x1c(%ebp)
}
80107983:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80107986:	8d 65 f4             	lea    -0xc(%ebp),%esp
80107989:	5b                   	pop    %ebx
8010798a:	5e                   	pop    %esi
8010798b:	5f                   	pop    %edi
8010798c:	5d                   	pop    %ebp
8010798d:	c3                   	ret    
8010798e:	66 90                	xchg   %ax,%ax
    return 0;
80107990:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
}
80107997:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010799a:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010799d:	5b                   	pop    %ebx
8010799e:	5e                   	pop    %esi
8010799f:	5f                   	pop    %edi
801079a0:	5d                   	pop    %ebp
801079a1:	c3                   	ret    
801079a2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
      cprintf("allocuvm out of memory (2)\n");
801079a8:	83 ec 0c             	sub    $0xc,%esp
801079ab:	68 85 89 10 80       	push   $0x80108985
801079b0:	e8 eb 8c ff ff       	call   801006a0 <cprintf>
  if(newsz >= oldsz)
801079b5:	8b 45 0c             	mov    0xc(%ebp),%eax
801079b8:	83 c4 10             	add    $0x10,%esp
801079bb:	39 45 10             	cmp    %eax,0x10(%ebp)
801079be:	74 0c                	je     801079cc <allocuvm+0x10c>
801079c0:	8b 55 10             	mov    0x10(%ebp),%edx
801079c3:	89 c1                	mov    %eax,%ecx
801079c5:	89 f8                	mov    %edi,%eax
801079c7:	e8 34 fa ff ff       	call   80107400 <deallocuvm.part.0>
      kfree(mem);
801079cc:	83 ec 0c             	sub    $0xc,%esp
801079cf:	53                   	push   %ebx
801079d0:	e8 9b ab ff ff       	call   80102570 <kfree>
      return 0;
801079d5:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
801079dc:	83 c4 10             	add    $0x10,%esp
}
801079df:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801079e2:	8d 65 f4             	lea    -0xc(%ebp),%esp
801079e5:	5b                   	pop    %ebx
801079e6:	5e                   	pop    %esi
801079e7:	5f                   	pop    %edi
801079e8:	5d                   	pop    %ebp
801079e9:	c3                   	ret    
801079ea:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

801079f0 <deallocuvm>:
{
801079f0:	55                   	push   %ebp
801079f1:	89 e5                	mov    %esp,%ebp
801079f3:	8b 55 0c             	mov    0xc(%ebp),%edx
801079f6:	8b 4d 10             	mov    0x10(%ebp),%ecx
801079f9:	8b 45 08             	mov    0x8(%ebp),%eax
  if(newsz >= oldsz)
801079fc:	39 d1                	cmp    %edx,%ecx
801079fe:	73 10                	jae    80107a10 <deallocuvm+0x20>
}
80107a00:	5d                   	pop    %ebp
80107a01:	e9 fa f9 ff ff       	jmp    80107400 <deallocuvm.part.0>
80107a06:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80107a0d:	8d 76 00             	lea    0x0(%esi),%esi
80107a10:	89 d0                	mov    %edx,%eax
80107a12:	5d                   	pop    %ebp
80107a13:	c3                   	ret    
80107a14:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80107a1b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80107a1f:	90                   	nop

80107a20 <freevm>:

// Free a page table and all the physical memory pages
// in the user part.
void
freevm(pde_t *pgdir)
{
80107a20:	55                   	push   %ebp
80107a21:	89 e5                	mov    %esp,%ebp
80107a23:	57                   	push   %edi
80107a24:	56                   	push   %esi
80107a25:	53                   	push   %ebx
80107a26:	83 ec 0c             	sub    $0xc,%esp
80107a29:	8b 75 08             	mov    0x8(%ebp),%esi
  uint i;

  if(pgdir == 0)
80107a2c:	85 f6                	test   %esi,%esi
80107a2e:	74 59                	je     80107a89 <freevm+0x69>
  if(newsz >= oldsz)
80107a30:	31 c9                	xor    %ecx,%ecx
80107a32:	ba 00 00 00 80       	mov    $0x80000000,%edx
80107a37:	89 f0                	mov    %esi,%eax
80107a39:	89 f3                	mov    %esi,%ebx
80107a3b:	e8 c0 f9 ff ff       	call   80107400 <deallocuvm.part.0>
    panic("freevm: no pgdir");
  deallocuvm(pgdir, KERNBASE, 0);
  for(i = 0; i < NPDENTRIES; i++){
80107a40:	8d be 00 10 00 00    	lea    0x1000(%esi),%edi
80107a46:	eb 0f                	jmp    80107a57 <freevm+0x37>
80107a48:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80107a4f:	90                   	nop
80107a50:	83 c3 04             	add    $0x4,%ebx
80107a53:	39 df                	cmp    %ebx,%edi
80107a55:	74 23                	je     80107a7a <freevm+0x5a>
    if(pgdir[i] & PTE_P){
80107a57:	8b 03                	mov    (%ebx),%eax
80107a59:	a8 01                	test   $0x1,%al
80107a5b:	74 f3                	je     80107a50 <freevm+0x30>
      char * v = P2V(PTE_ADDR(pgdir[i]));
80107a5d:	25 00 f0 ff ff       	and    $0xfffff000,%eax
      kfree(v);
80107a62:	83 ec 0c             	sub    $0xc,%esp
  for(i = 0; i < NPDENTRIES; i++){
80107a65:	83 c3 04             	add    $0x4,%ebx
      char * v = P2V(PTE_ADDR(pgdir[i]));
80107a68:	05 00 00 00 80       	add    $0x80000000,%eax
      kfree(v);
80107a6d:	50                   	push   %eax
80107a6e:	e8 fd aa ff ff       	call   80102570 <kfree>
80107a73:	83 c4 10             	add    $0x10,%esp
  for(i = 0; i < NPDENTRIES; i++){
80107a76:	39 df                	cmp    %ebx,%edi
80107a78:	75 dd                	jne    80107a57 <freevm+0x37>
    }
  }
  kfree((char*)pgdir);
80107a7a:	89 75 08             	mov    %esi,0x8(%ebp)
}
80107a7d:	8d 65 f4             	lea    -0xc(%ebp),%esp
80107a80:	5b                   	pop    %ebx
80107a81:	5e                   	pop    %esi
80107a82:	5f                   	pop    %edi
80107a83:	5d                   	pop    %ebp
  kfree((char*)pgdir);
80107a84:	e9 e7 aa ff ff       	jmp    80102570 <kfree>
    panic("freevm: no pgdir");
80107a89:	83 ec 0c             	sub    $0xc,%esp
80107a8c:	68 a1 89 10 80       	push   $0x801089a1
80107a91:	e8 ea 88 ff ff       	call   80100380 <panic>
80107a96:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80107a9d:	8d 76 00             	lea    0x0(%esi),%esi

80107aa0 <setupkvm>:
{
80107aa0:	55                   	push   %ebp
80107aa1:	89 e5                	mov    %esp,%ebp
80107aa3:	56                   	push   %esi
80107aa4:	53                   	push   %ebx
  if((pgdir = (pde_t*)kalloc()) == 0)
80107aa5:	e8 86 ac ff ff       	call   80102730 <kalloc>
80107aaa:	89 c6                	mov    %eax,%esi
80107aac:	85 c0                	test   %eax,%eax
80107aae:	74 42                	je     80107af2 <setupkvm+0x52>
  memset(pgdir, 0, PGSIZE);
80107ab0:	83 ec 04             	sub    $0x4,%esp
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
80107ab3:	bb 20 b4 10 80       	mov    $0x8010b420,%ebx
  memset(pgdir, 0, PGSIZE);
80107ab8:	68 00 10 00 00       	push   $0x1000
80107abd:	6a 00                	push   $0x0
80107abf:	50                   	push   %eax
80107ac0:	e8 db d7 ff ff       	call   801052a0 <memset>
80107ac5:	83 c4 10             	add    $0x10,%esp
                (uint)k->phys_start, k->perm) < 0) {
80107ac8:	8b 43 04             	mov    0x4(%ebx),%eax
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start,
80107acb:	83 ec 08             	sub    $0x8,%esp
80107ace:	8b 4b 08             	mov    0x8(%ebx),%ecx
80107ad1:	ff 73 0c             	push   0xc(%ebx)
80107ad4:	8b 13                	mov    (%ebx),%edx
80107ad6:	50                   	push   %eax
80107ad7:	29 c1                	sub    %eax,%ecx
80107ad9:	89 f0                	mov    %esi,%eax
80107adb:	e8 d0 f9 ff ff       	call   801074b0 <mappages>
80107ae0:	83 c4 10             	add    $0x10,%esp
80107ae3:	85 c0                	test   %eax,%eax
80107ae5:	78 19                	js     80107b00 <setupkvm+0x60>
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
80107ae7:	83 c3 10             	add    $0x10,%ebx
80107aea:	81 fb 60 b4 10 80    	cmp    $0x8010b460,%ebx
80107af0:	75 d6                	jne    80107ac8 <setupkvm+0x28>
}
80107af2:	8d 65 f8             	lea    -0x8(%ebp),%esp
80107af5:	89 f0                	mov    %esi,%eax
80107af7:	5b                   	pop    %ebx
80107af8:	5e                   	pop    %esi
80107af9:	5d                   	pop    %ebp
80107afa:	c3                   	ret    
80107afb:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80107aff:	90                   	nop
      freevm(pgdir);
80107b00:	83 ec 0c             	sub    $0xc,%esp
80107b03:	56                   	push   %esi
      return 0;
80107b04:	31 f6                	xor    %esi,%esi
      freevm(pgdir);
80107b06:	e8 15 ff ff ff       	call   80107a20 <freevm>
      return 0;
80107b0b:	83 c4 10             	add    $0x10,%esp
}
80107b0e:	8d 65 f8             	lea    -0x8(%ebp),%esp
80107b11:	89 f0                	mov    %esi,%eax
80107b13:	5b                   	pop    %ebx
80107b14:	5e                   	pop    %esi
80107b15:	5d                   	pop    %ebp
80107b16:	c3                   	ret    
80107b17:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80107b1e:	66 90                	xchg   %ax,%ax

80107b20 <kvmalloc>:
{
80107b20:	55                   	push   %ebp
80107b21:	89 e5                	mov    %esp,%ebp
80107b23:	83 ec 08             	sub    $0x8,%esp
  kpgdir = setupkvm();
80107b26:	e8 75 ff ff ff       	call   80107aa0 <setupkvm>
80107b2b:	a3 e4 63 11 80       	mov    %eax,0x801163e4
  lcr3(V2P(kpgdir));   // switch to the kernel page table
80107b30:	05 00 00 00 80       	add    $0x80000000,%eax
80107b35:	0f 22 d8             	mov    %eax,%cr3
}
80107b38:	c9                   	leave  
80107b39:	c3                   	ret    
80107b3a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80107b40 <clearpteu>:

// Clear PTE_U on a page. Used to create an inaccessible
// page beneath the user stack.
void
clearpteu(pde_t *pgdir, char *uva)
{
80107b40:	55                   	push   %ebp
80107b41:	89 e5                	mov    %esp,%ebp
80107b43:	83 ec 08             	sub    $0x8,%esp
80107b46:	8b 45 0c             	mov    0xc(%ebp),%eax
  if(*pde & PTE_P){
80107b49:	8b 55 08             	mov    0x8(%ebp),%edx
  pde = &pgdir[PDX(va)];
80107b4c:	89 c1                	mov    %eax,%ecx
80107b4e:	c1 e9 16             	shr    $0x16,%ecx
  if(*pde & PTE_P){
80107b51:	8b 14 8a             	mov    (%edx,%ecx,4),%edx
80107b54:	f6 c2 01             	test   $0x1,%dl
80107b57:	75 17                	jne    80107b70 <clearpteu+0x30>
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
  if(pte == 0)
    panic("clearpteu");
80107b59:	83 ec 0c             	sub    $0xc,%esp
80107b5c:	68 b2 89 10 80       	push   $0x801089b2
80107b61:	e8 1a 88 ff ff       	call   80100380 <panic>
80107b66:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80107b6d:	8d 76 00             	lea    0x0(%esi),%esi
  return &pgtab[PTX(va)];
80107b70:	c1 e8 0a             	shr    $0xa,%eax
    pgtab = (pte_t*)P2V(PTE_ADDR(*pde));
80107b73:	81 e2 00 f0 ff ff    	and    $0xfffff000,%edx
  return &pgtab[PTX(va)];
80107b79:	25 fc 0f 00 00       	and    $0xffc,%eax
80107b7e:	8d 84 02 00 00 00 80 	lea    -0x80000000(%edx,%eax,1),%eax
  if(pte == 0)
80107b85:	85 c0                	test   %eax,%eax
80107b87:	74 d0                	je     80107b59 <clearpteu+0x19>
  *pte &= ~PTE_U;
80107b89:	83 20 fb             	andl   $0xfffffffb,(%eax)
}
80107b8c:	c9                   	leave  
80107b8d:	c3                   	ret    
80107b8e:	66 90                	xchg   %ax,%ax

80107b90 <copyuvm>:

// Given a parent process's page table, create a copy
// of it for a child.
pde_t*
copyuvm(pde_t *pgdir, uint sz)
{
80107b90:	55                   	push   %ebp
80107b91:	89 e5                	mov    %esp,%ebp
80107b93:	57                   	push   %edi
80107b94:	56                   	push   %esi
80107b95:	53                   	push   %ebx
80107b96:	83 ec 1c             	sub    $0x1c,%esp
  pde_t *d;
  pte_t *pte;
  uint pa, i, flags;
  char *mem;

  if((d = setupkvm()) == 0)
80107b99:	e8 02 ff ff ff       	call   80107aa0 <setupkvm>
80107b9e:	89 45 e0             	mov    %eax,-0x20(%ebp)
80107ba1:	85 c0                	test   %eax,%eax
80107ba3:	0f 84 bd 00 00 00    	je     80107c66 <copyuvm+0xd6>
    return 0;
  for(i = 0; i < sz; i += PGSIZE){
80107ba9:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80107bac:	85 c9                	test   %ecx,%ecx
80107bae:	0f 84 b2 00 00 00    	je     80107c66 <copyuvm+0xd6>
80107bb4:	31 f6                	xor    %esi,%esi
80107bb6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80107bbd:	8d 76 00             	lea    0x0(%esi),%esi
  if(*pde & PTE_P){
80107bc0:	8b 4d 08             	mov    0x8(%ebp),%ecx
  pde = &pgdir[PDX(va)];
80107bc3:	89 f0                	mov    %esi,%eax
80107bc5:	c1 e8 16             	shr    $0x16,%eax
  if(*pde & PTE_P){
80107bc8:	8b 04 81             	mov    (%ecx,%eax,4),%eax
80107bcb:	a8 01                	test   $0x1,%al
80107bcd:	75 11                	jne    80107be0 <copyuvm+0x50>
    if((pte = walkpgdir(pgdir, (void *) i, 0)) == 0)
      panic("copyuvm: pte should exist");
80107bcf:	83 ec 0c             	sub    $0xc,%esp
80107bd2:	68 bc 89 10 80       	push   $0x801089bc
80107bd7:	e8 a4 87 ff ff       	call   80100380 <panic>
80107bdc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  return &pgtab[PTX(va)];
80107be0:	89 f2                	mov    %esi,%edx
    pgtab = (pte_t*)P2V(PTE_ADDR(*pde));
80107be2:	25 00 f0 ff ff       	and    $0xfffff000,%eax
  return &pgtab[PTX(va)];
80107be7:	c1 ea 0a             	shr    $0xa,%edx
80107bea:	81 e2 fc 0f 00 00    	and    $0xffc,%edx
80107bf0:	8d 84 10 00 00 00 80 	lea    -0x80000000(%eax,%edx,1),%eax
    if((pte = walkpgdir(pgdir, (void *) i, 0)) == 0)
80107bf7:	85 c0                	test   %eax,%eax
80107bf9:	74 d4                	je     80107bcf <copyuvm+0x3f>
    if(!(*pte & PTE_P))
80107bfb:	8b 00                	mov    (%eax),%eax
80107bfd:	a8 01                	test   $0x1,%al
80107bff:	0f 84 9f 00 00 00    	je     80107ca4 <copyuvm+0x114>
      panic("copyuvm: page not present");
    pa = PTE_ADDR(*pte);
80107c05:	89 c7                	mov    %eax,%edi
    flags = PTE_FLAGS(*pte);
80107c07:	25 ff 0f 00 00       	and    $0xfff,%eax
80107c0c:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    pa = PTE_ADDR(*pte);
80107c0f:	81 e7 00 f0 ff ff    	and    $0xfffff000,%edi
    if((mem = kalloc()) == 0)
80107c15:	e8 16 ab ff ff       	call   80102730 <kalloc>
80107c1a:	89 c3                	mov    %eax,%ebx
80107c1c:	85 c0                	test   %eax,%eax
80107c1e:	74 64                	je     80107c84 <copyuvm+0xf4>
      goto bad;
    memmove(mem, (char*)P2V(pa), PGSIZE);
80107c20:	83 ec 04             	sub    $0x4,%esp
80107c23:	81 c7 00 00 00 80    	add    $0x80000000,%edi
80107c29:	68 00 10 00 00       	push   $0x1000
80107c2e:	57                   	push   %edi
80107c2f:	50                   	push   %eax
80107c30:	e8 0b d7 ff ff       	call   80105340 <memmove>
    if(mappages(d, (void*)i, PGSIZE, V2P(mem), flags) < 0) {
80107c35:	58                   	pop    %eax
80107c36:	8d 83 00 00 00 80    	lea    -0x80000000(%ebx),%eax
80107c3c:	5a                   	pop    %edx
80107c3d:	ff 75 e4             	push   -0x1c(%ebp)
80107c40:	b9 00 10 00 00       	mov    $0x1000,%ecx
80107c45:	89 f2                	mov    %esi,%edx
80107c47:	50                   	push   %eax
80107c48:	8b 45 e0             	mov    -0x20(%ebp),%eax
80107c4b:	e8 60 f8 ff ff       	call   801074b0 <mappages>
80107c50:	83 c4 10             	add    $0x10,%esp
80107c53:	85 c0                	test   %eax,%eax
80107c55:	78 21                	js     80107c78 <copyuvm+0xe8>
  for(i = 0; i < sz; i += PGSIZE){
80107c57:	81 c6 00 10 00 00    	add    $0x1000,%esi
80107c5d:	39 75 0c             	cmp    %esi,0xc(%ebp)
80107c60:	0f 87 5a ff ff ff    	ja     80107bc0 <copyuvm+0x30>
  return d;

bad:
  freevm(d);
  return 0;
}
80107c66:	8b 45 e0             	mov    -0x20(%ebp),%eax
80107c69:	8d 65 f4             	lea    -0xc(%ebp),%esp
80107c6c:	5b                   	pop    %ebx
80107c6d:	5e                   	pop    %esi
80107c6e:	5f                   	pop    %edi
80107c6f:	5d                   	pop    %ebp
80107c70:	c3                   	ret    
80107c71:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
      kfree(mem);
80107c78:	83 ec 0c             	sub    $0xc,%esp
80107c7b:	53                   	push   %ebx
80107c7c:	e8 ef a8 ff ff       	call   80102570 <kfree>
      goto bad;
80107c81:	83 c4 10             	add    $0x10,%esp
  freevm(d);
80107c84:	83 ec 0c             	sub    $0xc,%esp
80107c87:	ff 75 e0             	push   -0x20(%ebp)
80107c8a:	e8 91 fd ff ff       	call   80107a20 <freevm>
  return 0;
80107c8f:	c7 45 e0 00 00 00 00 	movl   $0x0,-0x20(%ebp)
80107c96:	83 c4 10             	add    $0x10,%esp
}
80107c99:	8b 45 e0             	mov    -0x20(%ebp),%eax
80107c9c:	8d 65 f4             	lea    -0xc(%ebp),%esp
80107c9f:	5b                   	pop    %ebx
80107ca0:	5e                   	pop    %esi
80107ca1:	5f                   	pop    %edi
80107ca2:	5d                   	pop    %ebp
80107ca3:	c3                   	ret    
      panic("copyuvm: page not present");
80107ca4:	83 ec 0c             	sub    $0xc,%esp
80107ca7:	68 d6 89 10 80       	push   $0x801089d6
80107cac:	e8 cf 86 ff ff       	call   80100380 <panic>
80107cb1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80107cb8:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80107cbf:	90                   	nop

80107cc0 <uva2ka>:

//PAGEBREAK!
// Map user virtual address to kernel address.
char*
uva2ka(pde_t *pgdir, char *uva)
{
80107cc0:	55                   	push   %ebp
80107cc1:	89 e5                	mov    %esp,%ebp
80107cc3:	8b 45 0c             	mov    0xc(%ebp),%eax
  if(*pde & PTE_P){
80107cc6:	8b 55 08             	mov    0x8(%ebp),%edx
  pde = &pgdir[PDX(va)];
80107cc9:	89 c1                	mov    %eax,%ecx
80107ccb:	c1 e9 16             	shr    $0x16,%ecx
  if(*pde & PTE_P){
80107cce:	8b 14 8a             	mov    (%edx,%ecx,4),%edx
80107cd1:	f6 c2 01             	test   $0x1,%dl
80107cd4:	0f 84 00 01 00 00    	je     80107dda <uva2ka.cold>
  return &pgtab[PTX(va)];
80107cda:	c1 e8 0c             	shr    $0xc,%eax
    pgtab = (pte_t*)P2V(PTE_ADDR(*pde));
80107cdd:	81 e2 00 f0 ff ff    	and    $0xfffff000,%edx
  if((*pte & PTE_P) == 0)
    return 0;
  if((*pte & PTE_U) == 0)
    return 0;
  return (char*)P2V(PTE_ADDR(*pte));
}
80107ce3:	5d                   	pop    %ebp
  return &pgtab[PTX(va)];
80107ce4:	25 ff 03 00 00       	and    $0x3ff,%eax
  if((*pte & PTE_P) == 0)
80107ce9:	8b 84 82 00 00 00 80 	mov    -0x80000000(%edx,%eax,4),%eax
  if((*pte & PTE_U) == 0)
80107cf0:	89 c2                	mov    %eax,%edx
  return (char*)P2V(PTE_ADDR(*pte));
80107cf2:	25 00 f0 ff ff       	and    $0xfffff000,%eax
  if((*pte & PTE_U) == 0)
80107cf7:	83 e2 05             	and    $0x5,%edx
  return (char*)P2V(PTE_ADDR(*pte));
80107cfa:	05 00 00 00 80       	add    $0x80000000,%eax
80107cff:	83 fa 05             	cmp    $0x5,%edx
80107d02:	ba 00 00 00 00       	mov    $0x0,%edx
80107d07:	0f 45 c2             	cmovne %edx,%eax
}
80107d0a:	c3                   	ret    
80107d0b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80107d0f:	90                   	nop

80107d10 <copyout>:
// Copy len bytes from p to user address va in page table pgdir.
// Most useful when pgdir is not the current page table.
// uva2ka ensures this only works for PTE_U pages.
int
copyout(pde_t *pgdir, uint va, void *p, uint len)
{
80107d10:	55                   	push   %ebp
80107d11:	89 e5                	mov    %esp,%ebp
80107d13:	57                   	push   %edi
80107d14:	56                   	push   %esi
80107d15:	53                   	push   %ebx
80107d16:	83 ec 0c             	sub    $0xc,%esp
80107d19:	8b 75 14             	mov    0x14(%ebp),%esi
80107d1c:	8b 45 0c             	mov    0xc(%ebp),%eax
80107d1f:	8b 55 10             	mov    0x10(%ebp),%edx
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
  while(len > 0){
80107d22:	85 f6                	test   %esi,%esi
80107d24:	75 51                	jne    80107d77 <copyout+0x67>
80107d26:	e9 a5 00 00 00       	jmp    80107dd0 <copyout+0xc0>
80107d2b:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80107d2f:	90                   	nop
  return (char*)P2V(PTE_ADDR(*pte));
80107d30:	81 e3 00 f0 ff ff    	and    $0xfffff000,%ebx
80107d36:	8d 8b 00 00 00 80    	lea    -0x80000000(%ebx),%ecx
    va0 = (uint)PGROUNDDOWN(va);
    pa0 = uva2ka(pgdir, (char*)va0);
    if(pa0 == 0)
80107d3c:	81 fb 00 00 00 80    	cmp    $0x80000000,%ebx
80107d42:	74 75                	je     80107db9 <copyout+0xa9>
      return -1;
    n = PGSIZE - (va - va0);
80107d44:	89 fb                	mov    %edi,%ebx
    if(n > len)
      n = len;
    memmove(pa0 + (va - va0), buf, n);
80107d46:	89 55 10             	mov    %edx,0x10(%ebp)
    n = PGSIZE - (va - va0);
80107d49:	29 c3                	sub    %eax,%ebx
80107d4b:	81 c3 00 10 00 00    	add    $0x1000,%ebx
80107d51:	39 f3                	cmp    %esi,%ebx
80107d53:	0f 47 de             	cmova  %esi,%ebx
    memmove(pa0 + (va - va0), buf, n);
80107d56:	29 f8                	sub    %edi,%eax
80107d58:	83 ec 04             	sub    $0x4,%esp
80107d5b:	01 c1                	add    %eax,%ecx
80107d5d:	53                   	push   %ebx
80107d5e:	52                   	push   %edx
80107d5f:	51                   	push   %ecx
80107d60:	e8 db d5 ff ff       	call   80105340 <memmove>
    len -= n;
    buf += n;
80107d65:	8b 55 10             	mov    0x10(%ebp),%edx
    va = va0 + PGSIZE;
80107d68:	8d 87 00 10 00 00    	lea    0x1000(%edi),%eax
  while(len > 0){
80107d6e:	83 c4 10             	add    $0x10,%esp
    buf += n;
80107d71:	01 da                	add    %ebx,%edx
  while(len > 0){
80107d73:	29 de                	sub    %ebx,%esi
80107d75:	74 59                	je     80107dd0 <copyout+0xc0>
  if(*pde & PTE_P){
80107d77:	8b 5d 08             	mov    0x8(%ebp),%ebx
  pde = &pgdir[PDX(va)];
80107d7a:	89 c1                	mov    %eax,%ecx
    va0 = (uint)PGROUNDDOWN(va);
80107d7c:	89 c7                	mov    %eax,%edi
  pde = &pgdir[PDX(va)];
80107d7e:	c1 e9 16             	shr    $0x16,%ecx
    va0 = (uint)PGROUNDDOWN(va);
80107d81:	81 e7 00 f0 ff ff    	and    $0xfffff000,%edi
  if(*pde & PTE_P){
80107d87:	8b 0c 8b             	mov    (%ebx,%ecx,4),%ecx
80107d8a:	f6 c1 01             	test   $0x1,%cl
80107d8d:	0f 84 4e 00 00 00    	je     80107de1 <copyout.cold>
  return &pgtab[PTX(va)];
80107d93:	89 fb                	mov    %edi,%ebx
    pgtab = (pte_t*)P2V(PTE_ADDR(*pde));
80107d95:	81 e1 00 f0 ff ff    	and    $0xfffff000,%ecx
  return &pgtab[PTX(va)];
80107d9b:	c1 eb 0c             	shr    $0xc,%ebx
80107d9e:	81 e3 ff 03 00 00    	and    $0x3ff,%ebx
  if((*pte & PTE_P) == 0)
80107da4:	8b 9c 99 00 00 00 80 	mov    -0x80000000(%ecx,%ebx,4),%ebx
  if((*pte & PTE_U) == 0)
80107dab:	89 d9                	mov    %ebx,%ecx
80107dad:	83 e1 05             	and    $0x5,%ecx
80107db0:	83 f9 05             	cmp    $0x5,%ecx
80107db3:	0f 84 77 ff ff ff    	je     80107d30 <copyout+0x20>
  }
  return 0;
}
80107db9:	8d 65 f4             	lea    -0xc(%ebp),%esp
      return -1;
80107dbc:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80107dc1:	5b                   	pop    %ebx
80107dc2:	5e                   	pop    %esi
80107dc3:	5f                   	pop    %edi
80107dc4:	5d                   	pop    %ebp
80107dc5:	c3                   	ret    
80107dc6:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80107dcd:	8d 76 00             	lea    0x0(%esi),%esi
80107dd0:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
80107dd3:	31 c0                	xor    %eax,%eax
}
80107dd5:	5b                   	pop    %ebx
80107dd6:	5e                   	pop    %esi
80107dd7:	5f                   	pop    %edi
80107dd8:	5d                   	pop    %ebp
80107dd9:	c3                   	ret    

80107dda <uva2ka.cold>:
  if((*pte & PTE_P) == 0)
80107dda:	a1 00 00 00 00       	mov    0x0,%eax
80107ddf:	0f 0b                	ud2    

80107de1 <copyout.cold>:
80107de1:	a1 00 00 00 00       	mov    0x0,%eax
80107de6:	0f 0b                	ud2    
